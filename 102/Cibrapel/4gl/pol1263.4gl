...
#--------------------------#
   ...
   DEFINE p_val_saldo LIKE docum.val_saldo
   ...
   DECLARE cq_docum CURSOR FOR
    ...
    SELECT docum_emis.*, docum.val_saldo 
      FROM docum_emis, docum
     WHERE docum_emis.cod_empresa = p_cod_empresa
       AND docum_emis.ies_impressao = 'N'
       AND docum_emis.num_docum = docum.num_docum
       AND docum_emis.cod_empresa = docum.cod_empresa
       AND docum_emis.ies_tip_docum = docum.ies_tip_docum
       AND docum.val_saldo > 0
       
   FOREACH cq_docum INTO p_docum.*, p_val_saldo
      ...
      LET p_docum.val_bruto = p_val_saldo
