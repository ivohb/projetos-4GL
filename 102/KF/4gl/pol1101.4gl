...
#-------------------------------#
   ...
   MESSAGE 'Aguarde !!!... carregando itens não cadastrados'
   
		DECLARE cq_carga CURSOR FOR
		SELECT UNIQUE consumo.cod_empresa,
			   consumo.cod_item,
			   consumo.cod_operac,
			...
			AND consumo.cod_item NOT IN
				(SELECT ciclo_peca_454.cod_item
					FROM ciclo_peca_454
					WHERE ciclo_peca_454.cod_empresa = consumo.cod_empresa
					AND ciclo_peca_454.cod_item = consumo.cod_item
					AND ciclo_peca_454.cod_operac = consumo.cod_operac)
...
END RECORD
   ...
   DECLARE cq_oper CURSOR FOR SELECT A.COD_OPERAC, A.NUM_SEQ_OPERAC, B.DEN_OPERAC
															...
															WHERE A.COD_ITEM = p_ciclo_peca_454.cod_item
