...
#--------------------------#
   ...
   IF NOT vdp20023_carga_inicial_exec_por_fora(p_cod_empresa,TRUE) THEN
      LET m_msg = 'A função vdp20023_carga_inicial_exec_por_fora retornou erro'
      RETURN FALSE
   END IF

  IF NOT vdp20023_insere_pedido_exec_por_fora(m_num_pedido) THEN
     LET m_msg = 'A função vdp20023_insere_pedido_exec_por_fora retornou erro'
     RETURN FALSE
  END IF

  IF NOT vdp20023_processa_consistencia_exec_por_fora('C',TRUE,FALSE,FALSE) THEN 
     LET m_msg = 'A função vdp20023_processa_consistencia_exec_por_fora retornou erro'
     RETURN FALSE
  END IF
  ...
  LET m_msg = 'Situação do pedido alterada de B para N' 
...
#-------------------------------#
 ...
 RETURN "$Archive: /Logix/Fontes_Doc/Customizacao/10R2/gps_logist_e_gerenc_de_riscos_ltda/financeiro/controle_despesa_viagem/programas/pol1396.4gl $|$Revision: 06 $|$Date: 24/08/2020 14:12 $|$Modtime: 14/08/2020 13:12 $" #Informações do controle de versão do SourceSafe - Não remover esta linha (FRAMEWORK)
