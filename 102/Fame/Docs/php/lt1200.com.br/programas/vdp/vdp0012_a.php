<?
 ...
 //RODRIGO.TROQUEI A CONDIÇÃO DE PEDIDO CANCELADO DO QUARTO SELECT PARA when  b.ies_sit_pedido =9 then '7-CANCELADOS', ANTES FILTRAVA PELO ITEM
 //ANTES O FILTRO ERA e.qtd_pecas_atend = 0 and e.qtd_pecas_cancel>0 , OU SEJA SE NÃO TIVESSE ITENS ATENDIDOS E TIVESSE UM ITEM CANCELADO ELE CONSIDERAVA O PEDIDO CANCELADO.
 $selec_pedido="
 ...
 while (is_array($mat_pedido))
 {
  ...
  //  $pdf->Cell(10,5,number_format($t_qtd_ped_cancel,0,",","."),TBLR,0,'L');
   ...
   if($vlmc > 0)
   {
    ...
    //alteração rodrigo, só incrementa variáveis de pedido cancelado se a situação do pedido for cancelado
   ...
   if($vlma > 0)
   {
    if($resumo<>"S")
    {
    $sel_fat="select b.nota_fiscal AS num_nff,CAST(b.dat_hor_emissao AS DATE) AS dat_emissao
                     from fat_nf_item a,fat_nf_mestre b
                    where a.empresa='".$empresa."'
                      and a.pedido='".$ped_ant."'
