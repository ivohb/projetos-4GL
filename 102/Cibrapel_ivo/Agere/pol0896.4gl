   ...
   INPUT ARRAY pr_lote WITHOUT DEFAULTS FROM sr_lote.*
      ...
      AFTER FIELD qtd_contagem
         IF pr_lote[p_ind].qtd_contagem IS NULL THEN
            IF (FGL_LASTKEY() = 2000 OR FGL_LASTKEY() = 2016) AND 
                pr_lote[p_ind].num_lote IS NULL THEN
            ELSE
...
#--------------------------------#
   ...
   DEFINE p_pct_umid_pad  LIKE parametros_885.pct_umid_pad,
          ...
          p_fat_conversao LIKE umd_aparas_885.fat_conversao,
   ...
   FOREACH cq_umd INTO 
      ...
      LET p_fat_conversao = 1 +((p_qtd_contagem - p_qtd_da_seq)/p_qtd_da_seq)
      
      UPDATE umd_aparas_885
         SET fat_conversao = p_fat_conversao,
