...
DEFINE l_serie		CHAR(2)
 ...
 FOREACH cq_docum INTO p_docum_pgto.num_docum
   ...
   IF p_docum.ies_serie_fat  = "SU" THEN 
   		LET l_serie = '1'
   ELSE
   		LET l_serie = p_docum.ies_serie_fat
   END IF 

   #...---#

   LET p_ies_situa = 1
   LET p_nota_dec = p_docum.num_docum_origem 
   LET p_cod_emp_nf = p_docum_pgto.num_docum[1,2] # Ivo: 04/01/2011 ...
   ...
   SELECT empresa,                           #Will - 25/10/10
    ...
    WHERE empresa           = p_cod_emp_nf   #Ivo: 04/01/2011
      AND nota_fiscal       = p_nota_dec     #Will - 25/10/10
