...
# SISTEMA.: SUPRIMENTOS                                             #
# PROGRAMA: POL0137                                                 #
# MODULOS.: POL0137 LOG0010 LOG0030 LOG0050 LOG0060 LOG0130 LOG0280 #
...
GLOBALS
 ...
 DEFINE p_nf_sup           RECORD LIKE   nf_sup.*,
        ...
        p_aviso_rec_cairu  RECORD LIKE   aviso_rec_cairu.*,
        p_nf_retorno_cairu RECORD LIKE   nf_retorno_cairu.*,
...
CALL log0180_conecta_usuario()
	LET p_versao = "pol0137-10.02.00"
LET p_num_programa = "POL0137"                       
...
DEFER INTERRUPT
 ...
 CREATE UNIQUE INDEX ix_it_rem_1 ON t_itens_remessa(cod_item, pre_unit_nf)
  ...
  IF p_status = 0  THEN
     CALL pol037_controle()
  END IF
END MAIN

#--------------------------#
 FUNCTION pol037_controle()
#--------------------------#
  ...
  INITIALIZE p_nf_sup.*,    p_aviso_rec.*, p_estrutura.*,                      
             p_aviso_rec_cp.*  
                               TO NULL
  CALL log130_procura_caminho("pol0137") RETURNING comando
  OPEN WINDOW w_pol0137 AT 2,02 WITH FORM comando
  ...
  MENU "OPCAO"
    ...
    COMMAND "Fim"        "Retorna ao Menu Anterior"
      ...
      EXIT MENU
  END MENU
  CLOSE WINDOW w_pol0137
END FUNCTION

#----------------------------#
 FUNCTION pol037_inicializa()
#----------------------------#
...
END FUNCTION
#-------------------------------#
 FUNCTION pol037_entrada_dados()
#-------------------------------#

  DEFINE p_funcao            CHAR(30)
  CALL log006_exibe_teclas("01 02 07",p_versao)

  CURRENT WINDOW IS w_pol0137
  ...
  AFTER FIELD  num_nff1     
     ...
     AND   p_tela.num_nff1     <> p_nf_retorno_cairu.num_nff1 THEN
           ...
           NEXT FIELD num_nff1
     END IF
     IF    pol037_pesq_nff(p_tela.num_nff1)   THEN 
     ELSE
           NEXT FIELD num_nff1
     END IF
  ...
  AFTER FIELD  num_nff2     
     ...
     IF p_tela.num_nff2     IS NOT NULL THEN
        IF    pol037_pesq_nff(p_tela.num_nff2)   THEN 
        ELSE
              NEXT FIELD num_nff2
        END IF
     END IF
  ...
  AFTER FIELD  num_nff3     
     ...
     IF p_tela.num_nff3 IS NOT NULL THEN
        IF    pol037_pesq_nff(p_tela.num_nff3)   THEN 
        ELSE
              NEXT FIELD num_nff3
        END IF
     END IF 
  ...
  AFTER FIELD  num_nff4     
     ...
     IF p_tela.num_nff4 IS NOT NULL THEN     
        IF    pol037_pesq_nff(p_tela.num_nff4)    THEN 
        ELSE
              NEXT FIELD num_nff4
        END IF
     END IF
  ...
  AFTER FIELD  num_nff5     
     ...
     IF p_tela.num_nff5 IS NOT NULL THEN
        IF    pol037_pesq_nff(p_tela.num_nff5)    THEN 
        ELSE
              NEXT FIELD num_nff5
        END IF
     END IF   
...
#-----------------------------------------#
 FUNCTION  pol037_pesq_nff(p_num_nff)
#-----------------------------------------#
   ...
   SELECT  b.num_cgc_cpf, a.trans_nota_fiscal  
    ...
    WHERE a.empresa = p_cod_empresa           
     ...
     IF sqlca.sqlcode <> 0           THEN
        ERROR " Nota fiscal de remessa nao foi encontada" 
        RETURN FALSE
     END IF 
...
#---------------------------#
 FUNCTION pol037_processar()
#---------------------------#

   IF pol037_entrada_dados()
   THEN
     CALL pol037_atualiza_tabelas()
...
#---------------------------------#
 FUNCTION pol037_atualiza_tabelas()
#---------------------------------#

 CALL pol037_carrega_nff()
     ...
     SELECT * FROM aviso_rec_cairu
      WHERE cod_empresa=p_cod_empresa        
       ...
       IF sqlca.sqlcode   = NOTFOUND   THEN 
          ...
          IF  pol037_trata_item()    THEN 
              CALL  pol037_grava_ar_cairu()
          ELSE
...
END FUNCTION
#------------------------------#
 FUNCTION  pol037_carrega_nff()
#------------------------------#


   IF p_tela.num_nff1   IS NOT NULL   THEN 
      CALL  pol037_processa_carga(p_tela.num_nff1)
   END IF 
   
   IF p_tela.num_nff2   IS NOT NULL   THEN 
      CALL  pol037_processa_carga(p_tela.num_nff2)
   END IF 

   IF p_tela.num_nff3   IS NOT NULL   THEN 
      CALL  pol037_processa_carga(p_tela.num_nff3)
   END IF 

   IF p_tela.num_nff4   IS NOT NULL   THEN 
      CALL  pol037_processa_carga(p_tela.num_nff4)
   END IF 

   IF p_tela.num_nff5   IS NOT NULL   THEN 
      CALL  pol037_processa_carga(p_tela.num_nff5)
   END IF 
...
END FUNCTION
#-------------------------------------------#
 FUNCTION  pol037_processa_carga(p2_num_nff)
#-------------------------------------------#
...
END FUNCTION
#--------------------------------#
 FUNCTION pol037_grava_ar_cairu()   
#--------------------------------#
