...
GLOBALS
   ...
   DEFINE pr_compon       ARRAY[200] OF RECORD
          ...
          cod_oper_p      LIKE operacao.cod_operac,
   ...
   DEFINE p_qz5010 RECORD
      qz5_filial           varchar(2,0),  
      qz5_proces           varchar(4,0),  
      qz5_peca             varchar(15,0), 
      qz5_revisa           varchar(3,0),  
      qz5_sequen           varchar(6,0),  
      qz5_compos           varchar(15,0), 
      qz5_desc             varchar(50,0), 
      qz5_qtde             float,         
      qz5_passoc           float,         
      qz5_qtdpec           float,         
      qz5_pbruto           float,         
      qz5_aprove           float,         
      qz5_obs              varchar(150,0),
      qz5_msblql           varchar(1,0),  
      d_e_l_e_t_           varchar(1,0),  
      r_e_c_n_o_           integer       
   END RECORD
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "pol1150-10.02.26"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol1150.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol1150_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol1150_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol1150") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1150 AT 2,2 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   DISPLAY p_cod_empresa TO cod_empresa
   
   CALL pol1150_carrega_letra()
   ...
   MENU "OPCAO"
      COMMAND "Incluir" "Inclui dados nas tabelas"
         ...
         MESSAGE ""
         LET INT_FLAG = 0
         CALL pol1150_incluir() RETURNING p_status
         IF p_status THEN
      ...
      COMMAND "Seguinte" "Exibe o Proximo Item Encontrado na Consulta"
         ...
         MESSAGE ""
         LET INT_FLAG = 0
         CALL pol1150_paginacao("SEGUINTE")
      COMMAND "Anterior" "Exibe o Item Anterior Encontrado na Consulta"
         HELP 006
         MESSAGE ""
         LET INT_FLAG = 0
         CALL pol1150_paginacao("ANTERIOR")
      ...
      COMMAND "Fim"       "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol1150

END FUNCTION

#------------------------------#
FUNCTION pol1150_carrega_letra()
#------------------------------#
...
#-----------------------#
FUNCTION pol1150_incluir()
#-----------------------#

   LET p_retorno = FALSE
   
   IF pol1150_aceita_chave() THEN 
      IF pol1150_carrega_compon() THEN
         IF pol1150_aceita_operacao() THEN
            IF pol1150_grava_itens() THEN
               LET p_retorno = TRUE
            END IF
         END IF
      END IF
...
#-----------------------------#
FUNCTION pol1150_aceita_chave()
#-----------------------------#

   CALL log006_exibe_teclas("01 02 07",p_versao)
   CURRENT WINDOW IS w_pol1150
   CLEAR FORM
   DISPLAY p_cod_empresa TO cod_empresa
...
#-------------------------------#
FUNCTION pol1150_carrega_compon()
#-------------------------------#

   IF NOT pol1150_cria_temps() THEN
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol1150_aceita_operacao()
#--------------------------------#
   ...
   FOREACH cq_operacao INTO 
      ...
      SELECT num_seq ||"-"|| num_sub_seq
        INTO pr_compon[p_index].cod_oper_p
			  FROM ciclo_peca_970
			 WHERE cod_empresa = p_cod_empresa
			   AND cod_item =pr_compon[p_index].cod_compon
      
      IF pr_compon[p_index].cod_oper_p = '0-0' OR 
         pr_compon[p_index].cod_oper_p IS NULL THEN
         LET pr_compon[p_index].cod_oper_p = '99-99'
      END IF
...
#-----------------------------#
   ...
   UPDATE dadosadv@prd_a:qk1010  
   SET d_e_l_e_t_ = '*', r_e_c_d_e_l_=r_e_c_n_o_
   WHERE   qk1_filial  =  p_cod_empresa  
   ...
   UPDATE dadosadv@prd_a:qkk010  
   SET d_e_l_e_t_ = '*', r_e_c_d_e_l_=r_e_c_n_o_
   WHERE   qkk_filial  =  p_cod_empresa  
   ...
   UPDATE dadosadv@prd_a:qk2010  
   SET d_e_l_e_t_ = '*', r_e_c_d_e_l_=r_e_c_n_o_
   WHERE   qk2_filial  =  p_cod_empresa  
   ...
   UPDATE dadosadv@prd_a:qz3010 
   SET d_e_l_e_t_ = '*' 
   WHERE   qz3_filial  =  p_cod_empresa  
...
#-----------------------------#
FUNCTION pol1150_grava_itens()
#-----------------------------#

   CALL log085_transacao("BEGIN") 

   if p_ja_exportou then
      IF NOT pol1150_deleta_item() then
   ...
   INITIALIZE p_qkk010, p_qk1010, p_qk2010, p_qz5010 TO NULL
   ...
   LET p_qkk010.r_e_c_d_e_l_ = 0          
      ...
      {inicializa a tab qz3010}                     

    DECLARE cq_prim CURSOR FOR
		SELECT  * FROM dadosadv@prd_a:qZ3010
	
   FOREACH cq_prim  INTO p_qz3010.* 

     IF SQLCA.SQLCODE <> 0 THEN 
        CALL log003_err_sql("DECLARE","qz3010")    
        CALL log085_transacao("ROLLBACK") 
        RETURN FALSE
      END IF   
   ...
   LET p_qz5010.qz5_filial = ' '
   LET p_qz5010.qz5_proces = ' '
   LET p_qz5010.qz5_peca   = ' '
   LET p_qz5010.qz5_revisa = ' '
   LET p_qz5010.qz5_sequen = ' '
   LET p_qz5010.qz5_compos = ' '
   LET p_qz5010.qz5_desc   = ' '
   LET p_qz5010.qz5_qtde   = 0
   LET p_qz5010.qz5_passoc = 0
   LET p_qz5010.qz5_qtdpec = 0
   LET p_qz5010.qz5_pbruto = 0
   LET p_qz5010.qz5_aprove = 0
   LET p_qz5010.qz5_obs    = ' '
   LET p_qz5010.qz5_msblql = ' '
   LET p_qz5010.d_e_l_e_t_ = ' '
   LET p_qz5010.r_e_c_n_o_ = 0
   ...
   DECLARE cq_qk1 CURSOR FOR
    SELECT r_e_c_n_o_
      FROM dadosadv@prd_a:qk1010
#     WHERE qk1_filial = p_cod_empresa
     ORDER BY 1 DESC
   ...
   DECLARE cq_qk2 CURSOR FOR
    SELECT r_e_c_n_o_
      FROM dadosadv@prd_a:qk2010
#     WHERE qk2_filial = p_cod_empresa
     ORDER BY 1 DESC
   ...
   DECLARE cq_qkk CURSOR FOR
    SELECT r_e_c_n_o_
      FROM dadosadv@prd_a:qkk010
#     WHERE qkk_filial = p_cod_empresa
     ORDER BY 1 DESC
   ...
   DECLARE cq_qz3 CURSOR FOR
    SELECT r_e_c_n_o_
      FROM dadosadv@prd_a:qz3010
#     WHERE qkk_filial = p_cod_empresa
     ORDER BY 1 DESC
   ...
   DECLARE cq_qz CURSOR FOR
    SELECT r_e_c_n_o_
      FROM dadosadv@prd_a:qz5010
#     WHERE qkk_filial = p_cod_empresa
     ORDER BY 1 DESC

   FOREACH cq_qz INTO p_qz5010.r_e_c_n_o_
      LET p_achou = TRUE
      EXIT FOREACH
   END FOREACH
   
   IF NOT p_achou THEN
      LET p_qz5010.r_e_c_n_o_ = 1
   ELSE
      LET p_qz5010.r_e_c_n_o_ = p_qz5010.r_e_c_n_o_ + 1
   END IF
...
#   LET p_qk1010.qk1_dtrevi = p_tela.dat_revisao
   ...
   LET p_qz3010.qz3_filial   = p_cod_empresa
   ...
   LET p_qz5010.qz5_filial = p_cod_empresa
   LET p_qz5010.qz5_peca   = p_tela.cod_item
   LET p_qz5010.qz5_revisa = p_tela.cod_revisao
   ...
   FOR p_ind = 1 TO ARR_COUNT()
       ...
       LET p_qkk010.qkk_item     = pol1150_formata(p_ind)
           
       INSERT INTO 
         dadosadv@prd_a:qkk010(
           qkk_filial,  
           qkk_peca,    
           qkk_rev,     
           qkk_proces,  
       ...
       LET p_qz5010.qz5_sequen = p_qkk010.qkk_nope
       LET p_qz5010.qz5_compos = pr_compon[p_ind].cod_compon
       
       SELECT den_item
         INTO p_qz5010.qz5_desc
         FROM item
        WHERE cod_empresa = p_cod_empresa
          AND cod_item    = pr_compon[p_ind].cod_compon
       ...
       SELECT qtd_neces
         INTO p_qz5010.qz5_qtde
         FROM compon_1
        WHERE cod_compon = pr_compon[p_ind].cod_compon
       ...
       END IF
{
       ...
       ELSE
          IF STATUS <> -239 THEN 
             CALL log003_err_sql("INCLUSAO","qz5010")    
             CALL log085_transacao("ROLLBACK") 
             RETURN FALSE
          END IF
       END IF
...
#-------------------------------#
FUNCTION pol1150_formata(p_item)
#-------------------------------#
...
#-----------------------------------#
   ...
   CALL pol1150_exibe_compon()
   
 END FUNCTION

#------------------------------#
 FUNCTION pol1150_exibe_compon()
#------------------------------#
...
#-----------------------------------#
 FUNCTION pol1150_paginacao(p_funcao)
#-----------------------------------#
