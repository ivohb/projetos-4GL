...
#-------------------------------#
FUNCTION pol0289_cria_temp_547()#
#-------------------------------#

   DEFINE l_ind     INTEGER
   
   CREATE TABLE pedidos_temp_547 (
      num_pedido           DECIMAL(6,0)
   );

   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','pedidos_temp_547')
      RETURN FALSE
   END IF

   CREATE UNIQUE INDEX ix_pedidos_temp_547
    ON pedidos_temp_547(num_pedido);

   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','ix_pedidos_temp_547')
      RETURN FALSE
   END IF
      
   FOR l_ind = 39606 TO 39638
   
      INSERT INTO pedidos_temp_547 VALUES(l_ind)

      IF STATUS <> 0 THEN
         CALL log003_err_sql('INSERT','pedidos_temp_547:1')
         RETURN FALSE
      END IF
      
   END FOR
   
   INSERT INTO pedidos_temp_547 VALUES(38847)

   IF STATUS <> 0 THEN
      CALL log003_err_sql('INSERT','pedidos_temp_547:2')
      RETURN FALSE
   END IF
...
#---------------------------------#
   ...
   SELECT COUNT(*) INTO l_count FROM cliente_tmp

   IF NOT log0150_verifica_se_tabela_existe("pedidos_temp_547") THEN 
      IF NOT pol0289_cria_temp_547() THEN
         RETURN FALSE
      END IF
   END IF
...
"SELECT a.cod_cliente, a.num_pedido, a.num_pedido_cli, b.num_sequencia, b.cod_item, b.prz_entrega, ",
   ...
   IF l_count > 0 THEN
      LET p_query = p_query CLIPPED, "   AND a.cod_cliente IN (SELECT cod_cliente FROM cliente_tmp) "
   END IF

   IF p_tela.dat_ini IS NOT NULL THEN
      LET p_query = p_query CLIPPED, " AND b.prz_entrega >= '",p_tela.dat_ini,"' "
   END IF

   IF p_tela.dat_fim IS NOT NULL THEN
      LET p_query = p_query CLIPPED, " AND b.prz_entrega <= '",p_tela.dat_fim,"' "
   END IF
   
   IF p_tela.tipo_item = 'K' THEN   
      LET p_query = p_query CLIPPED, ' AND a.num_pedido IN (SELECT t.num_pedido FROM pedidos_temp_547 t) '
   END IF

   IF p_tela.tipo_item = 'N' THEN   
      LET p_query = p_query CLIPPED, ' AND a.num_pedido NOT IN (SELECT t.num_pedido FROM pedidos_temp_547 t) '
   END IF
   ...
   FOREACH cq_pedidos INTO p_relat.cod_cliente,
      ...
      SELECT den_item_reduz, cod_local_estoq
         INTO p_relat.den_item_reduz, l_parametro.cod_local
      FROM item
      WHERE cod_empresa = p_cod_empresa
        AND cod_item = p_relat.cod_item 
      IF SQLCA.SQLCODE <> 0 THEN
      ...
      LET l_parametro.cod_item = p_relat.cod_item
      CALL func002_le_estoque(l_parametro) RETURNING l_msg, l_qtd_estoue
      
      IF l_msg IS NULL THEN
         LET p_relat.den_texto_1[1,10] = l_qtd_estoue
      ELSE
         LET p_relat.den_texto_1[1,10] = 'ERRO ESTOQ'
      END IF
...
#----------------------# 
   ...
   FORMAT
      ...
      FIRST PAGE HEADER  
         ...
         PRINT COLUMN 001, " PEDIDO CLIENTE  PEDIDO SEQ     ITEM            DESCRICAO       ENTREGA   COD PRODUTO      ESTOQUE     QTDE   PRE UNIT  PRE TOTAL        ORDENS DE PRODUCAO       SHIP DATE "
         PRINT COLUMN 001, "---------------- ------ --- --------------- ------------------ ---------- ---------------- ---------- ------- --------- ----------- ----------------------------- ----------"
