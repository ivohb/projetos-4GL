...
#---------------------------------------------------------------#
#--Objetivo: inserir movto de E/S de estoque normal/reversão----#
#--Obs: a rotina que a chama deve ter uma transação aberta------#
#--------------------------parâmetros---------------------------#
# Informações básicas, conforme RECORD P_ITEM logo abaixo       #
#--------------------------retorno lógico-----------------------#
...
#---espelho dos parâmetros recebidos, com visibilidade modular
...
#--variáveis modular de uso geral--#
  ...
  DEFINE p_ies_ctr_lote       CHAR(01)

  
#------------------------------------#
FUNCTION func005_insere_movto(p_item)#
#------------------------------------#
...
#---parâmetros recebidos com visibilidade local
  ...
  END RECORD
   ...
   IF p_item.cod_empresa IS NULL THEN
      LET p_msg = p_msg CLIPPED, ' - O código da empresa está nulo'
   END IF
   
   IF p_item.cod_item IS NULL THEN
      LET p_msg = p_msg CLIPPED, ' - O código do item está nulo'
   END IF

   IF p_item.cod_local IS NULL THEN
      LET p_msg = p_msg CLIPPED, ' - O código do local está nulo'
   END IF

   IF p_item.cod_operacao IS NULL THEN
      LET p_msg = p_msg CLIPPED, ' - O código da operacao está nulo'
   END IF

   IF p_item.ies_situa IS NULL THEN
      LET p_msg = p_msg CLIPPED, ' - A situação do estoq está nula'
   END IF
   ...
   IF p_item.num_lote IS NULL THEN
      LET p_item.ies_ctr_lote = 'N'
   ELSE
      LET p_item.ies_ctr_lote = 'S'
   END IF
      
   IF p_item.comprimento IS NULL THEN
      LET p_item.comprimento = 0
   END IF
   
   IF p_item.largura IS NULL THEN
      LET p_item.largura = 0
   END IF

   IF p_item.altura IS NULL THEN
      LET p_item.altura = 0
   END IF

   IF p_item.diametro IS NULL THEN
   ...
   CASE p_movto.tip_operacao
      WHEN 'E' #entrada
         IF p_movto.ies_tip_movto = 'N' THEN
            LET p_ies_estoque = 'E'
            IF NOT func005_grava_entrada() THEN
               RETURN FALSE
            END IF
         ELSE
            LET p_ies_estoque = 'S'
            IF NOT func005_reverte_entrada() THEN
               RETURN FALSE
            END IF
         END IF
      WHEN 'S' #saida
         IF p_movto.ies_tip_movto = 'N' THEN
            LET p_ies_estoque = 'S'
            IF NOT func005_grava_saida() THEN
               RETURN FALSE
            END IF
         ELSE
            LET p_ies_estoque = 'E'
            IF NOT func005_reverte_saida() THEN
               RETURN FALSE
            END IF
         END IF
   ...
   IF NOT func005_atu_estoque() THEN
      RETURN FALSE
   END IF
...
#-----------------------------#
   ...
   DELETE FROM estoque_lote 
    WHERE cod_empresa = p_movto.cod_empresa 
      AND cod_item = p_movto.cod_item AND qtd_saldo = 0
...
#-------------------------------#
FUNCTION func005_grava_entrada()#
#-------------------------------#

   IF NOT func005_gra_lote() THEN
      RETURN FALSE
   END IF

   IF NOT func005_gra_lot_ender() THEN
      RETURN FALSE
   END IF

   IF NOT func005_gra_estoque() THEN
      RETURN FALSE
   END IF

   IF NOT func005_gra_estoq_trans() THEN
      RETURN FALSE
   END IF

   IF NOT func005_gra_est_trans_end() THEN
      RETURN FALSE
   END IF

   IF NOT func005_gra_estoq_auditoria() THEN
      RETURN FALSE
   END IF
...
#--------------------------#
FUNCTION func005_gra_lote()#
#--------------------------#

   CALL func005_le_lote()
      
   IF STATUS = 100 THEN
      IF NOT func005_ins_lote() THEN
         RETURN FALSE
      END IF
   ELSE
      IF STATUS = 0 THEN
         IF NOT func005_atu_lote(p_movto.qtd_movto) THEN
            RETURN FALSE
         END IF
      ELSE
         LET p_erro = STATUS
...
#-------------------------------------#
FUNCTION func005_atu_lote(p_qtd_movto)#
#-------------------------------------#
...
#-------------------------------#
FUNCTION func005_gra_lot_ender()#
#-------------------------------#
      
   CALL func005_le_lot_ender()
      
   IF STATUS = 100 THEN
      IF NOT func005_ins_lote_ender() THEN
         RETURN FALSE
      END IF
   ELSE
      IF STATUS = 0 THEN
         LET p_num_transac = p_estoque_lote_ender.num_transac
         IF NOT func005_atu_lote_ender(p_movto.qtd_movto) THEN
            RETURN FALSE
         END IF
      ELSE
         LET p_erro = STATUS
...
#--------------------------------#
FUNCTION func005_ins_lote_ender()#
#--------------------------------#

   CALL func005_carrega_campos() 
...
#-------------------------------#
FUNCTION func005_carrega_campos()
#-------------------------------#
...
#-------------------------------------------#
FUNCTION func005_atu_lote_ender(p_qtd_movto)#
#-------------------------------------------#
...
#---------------------------------#
FUNCTION func005_gra_estoq_trans()#
#---------------------------------#
   ...
   LET p_estoque_trans.cus_unit_movto_p   = p_movto.cus_unit
   LET p_estoque_trans.cus_tot_movto_p    = p_movto.cus_tot
   ...
   IF NOT func005_ins_estoq_trans() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
FUNCTION func005_ins_estoq_trans()#
#---------------------------------#
...
#---------------------------------#
FUNCTION func005_rev_estoq_trans()#
#---------------------------------#
   ...
   IF NOT func005_ins_estoque_trans_rev() THEN
      RETURN FALSE
   END IF
...
#-----------------------------------#
FUNCTION func005_gra_est_trans_end()#
#-----------------------------------#
 ...
 #---para chamar essa rotina é necessário ter lido a estoque_lote_ender previamente---#
   ...
   IF NOT func005_ins_est_trans_end() THEN
      RETURN FALSE
   END IF
...
#-----------------------------------#
FUNCTION func005_ins_est_trans_end()#
#-----------------------------------#
...
#-------------------------------#
FUNCTION func005_rev_trans_end()#
#-------------------------------#
...
#-------------------------------------#
FUNCTION func005_gra_estoq_auditoria()#
#-------------------------------------#
...
#---------------------------------#
FUNCTION func005_reverte_entrada()#
#---------------------------------#
   ...
   IF NOT func005_rev_estoq_trans() THEN
      RETURN FALSE
   END IF

   IF NOT func005_rev_trans_end() THEN
      RETURN FALSE
   END IF
...
#---------------------------------------#
FUNCTION func005_ins_estoque_trans_rev()#
#---------------------------------------#
...
#-------------------------------#
FUNCTION func005_reverte_saida()#
#-------------------------------#
