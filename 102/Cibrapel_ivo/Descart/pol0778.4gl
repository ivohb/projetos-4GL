   ...
   MENU "OPCAO"
      ...
      COMMAND "Anterior" "Exibe o Item Anterior Encontrado na Consulta"
         ...
         ELSE
            ERROR "Informe os parãmetros previamente !!!"
         END IF
      COMMAND "Processar" "Processa o geração do conhec de complemento "
         IF p_ies_cons THEN
            CALL pol0778_conhec_processar() RETURNING p_status
            IF p_status THEN
               LET p_msg = 'Geração de conhecimento efetuada com sucesso '
               CALL log0030_mensagem(p_msg,'excla')
            ELSE
               ERROR "Operação Cancelada !!!"
            END IF
...
#-------------------------------------#
   ...
   IF p_op = 'C' THEN
      ...
      IF p_cod_transpor IS NULL THEN
         LET p_msg = 'NAO HÁ CONHECIMENTO DE FRETE P/ OS PARÂMETROS INFORMADOS!!!'
      END IF
...
#---------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET p_nom_transpor      = 'Transportador não cadastrado!!!'
...
#-----------------------------#
   ...
   IF p_edita.ies_validado = 'S' THEN
      CALL log0030_mensagem('Frete está validado. Modificação não permitida.','excla')
      RETURN FALSE
   END IF
...
#----------------------------------#
   ...
   IF p_edita.ies_validado = 'S' THEN
      CALL log0030_mensagem('Frete já está validado.','excla')
      RETURN FALSE
   END IF
...
#----------------------#
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informar nota fiscal de serviço "
         HELP 001 
         MESSAGE ""
         IF log005_seguranca(p_user,"VDP","pol0778","IN")  THEN
      ...
      COMMAND "Modificar" "Modifica relacionamento com as notas de saída"
         ...
         ELSE
            ...
            NEXT OPTION 'Informar'
         END IF 
      COMMAND "Processar" "Processa a efetivação do relacionamento "
         IF p_ies_validado = 'N' THEN
            CALL pol0778_nota_processar() RETURNING p_status
            IF p_status THEN
               ERROR 'Geração de nota efetuada com sucesso'
            ELSE
               ERROR "Operação Cancelada !!!"
            END IF
         ELSE
            LET p_msg = 'Relacionamento já efetivado!!!... Operação não permitida'
            CALL log0030_mensagem(p_msg,'exclamation')
            NEXT OPTION 'Informar'
         END IF         
      COMMAND "Excluir" "Cancela a efetivação do relacionamento "
         IF p_ies_cons THEN
            IF p_ies_validado = 'S' THEN
               ...
               END IF
            ELSE
               LET p_msg = 'Relacionamento não efetivado!!!... operação não permitida'
               CALL log0030_mensagem(p_msg,'exclamation')
               NEXT OPTION 'Informar'
            END IF
         ELSE
...
#----------------------------------#
   ...
   IF sqlca.sqlcode = 0 THEN 
      IF p_ies_nf_com_erro <> 'S' THEN
         CALL log0030_mensagem('NF já consistida. Operação não permitida','exclamation')
         RETURN TRUE
      END IF
