#-------------------------------------------------------------------#
# SISTEMA.: INTEGRAÇÃO pw1                                          #
# PROGRAMA: POL1124                                                 #
# MODULOS.: POL1124-LOG0010-LOG0030-LOG0040-LOG0050-LOG0060         #
#           LOG0090-LOG0280-LOG1200-LOG1300-LOG1400-LOG1500         #
# OBJETIVO: MANUTENCAO ITENS CRITICADOS - IMPORTAÇÃO pw1 x LOGIX    #
# AUTOR...: POLO INFORMATICA - Manuel baseado no pol0477            #
# DATA....: 13/12/2011                                              #
#-------------------------------------------------------------------#
DATABASE logix
...
GLOBALS
   DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
          ...
          p_chav_seq           LIKE apont_pw1_man912.chav_seq,
   ...
   DEFINE p_apont_pw1_man912   RECORD LIKE apont_pw1_man912.*,
          p_apont_pw1_man912a  RECORD LIKE apont_pw1_man912.*
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "POL1124-10.02.01"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("POL1124.iem") RETURNING p_nom_help
  ...
  CALL log001_acessa_usuario("ESPEC999","")
   ...
   IF p_status = 0  THEN
      CALL POL1124_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION POL1124_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("POL11241") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_POL11241 AT 2,2 WITH FORM p_nom_tela
   ...
   MENU "OPCAO"
      COMMAND 'Erros' 'Exibe os Erros que ocorreram na Importação.'
         ...
         IF POL1124_tab_livre() THEN
            CALL log085_transacao("COMMIT")
            CALL POL1124_consulta_erros()
         ELSE
      ...
      COMMAND 'Listar' 'Lista os erros que ocorreram na Importação.'
         HELP 001
         MESSAGE ''
         IF log005_seguranca(p_user,'MANUFAT','POL1124','IN') THEN
            CALL POL1124_lista()
         END IF
 ...
 #           CALL POL1124_integra_apont()
      ...
      COMMAND KEY ("B") "liBerar" 'Libera a integração de apontamentos'
         ...
         CALL POL1124_libera_apont()
         IF p_houve_erro THEN
            ERROR 'Operação cancelada!'
         ELSE
            ERROR 'Liberação efetuada com sucesso!'
         END IF
      COMMAND KEY ("D") "orDem" 'Exporta ordens para o pw1'
         MESSAGE ""
         CALL log120_procura_caminho(p_prog_exp) RETURNING comando
         LET comando = comando 
         RUN comando RETURNING p_status   
      COMMAND "Item" 'Exporta itens para o pw1'
         MESSAGE ""
   ...
   CLOSE WINDOW w_POL11241

END FUNCTION

#-----------------------#
 FUNCTION POL1124_sobre()
#-----------------------#
...
#--------------------------------#
 FUNCTION POL1124_consulta_erros()
#--------------------------------#
...
#-----------------------------#
 FUNCTION POL1124_pre_aponta()
#-----------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("POL1124") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_POL1124 AT 2,2 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   ...
   MENU "OPCAO"
      ...
      COMMAND "Seguinte" "Exibe o Proximo Item Encontrado na Consulta"
         ...
         MESSAGE ""
         LET INT_FLAG = 0
         CALL POL1124_paginacao("SEGUINTE")
      COMMAND "Anterior" "Exibe o Item Anterior Encontrado na Consulta"
         HELP 005
         MESSAGE ""
         LET INT_FLAG = 0
         CALL POL1124_paginacao("ANTERIOR")
      COMMAND "Listar" "Lista os Dados Cadastrados"
         HELP 006
         MESSAGE ""
         IF log005_seguranca(p_user,"VDP","POL1124","MO") THEN
            IF log028_saida_relat(18,35) IS NOT NULL THEN
               MESSAGE " Processando a Extracao do Relatorio..." 
                  ATTRIBUTE(REVERSE)
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT POL1124_relat TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'POL1124.tmp'
                     START REPORT POL1124_relat  TO p_caminho
                  END IF
               ELSE
                  START REPORT POL1124_relat TO p_nom_arquivo
               END IF
               CALL POL1124_emite_relatorio()   
               IF p_count = 0 THEN
                  ERROR "Nao Existem Dados para serem Listados" 
               ELSE
                  ERROR "Relatorio Processado com Sucesso" 
               END IF
               FINISH REPORT POL1124_relat   
            ELSE
               CONTINUE MENU
            END IF                                                     
            IF p_ies_impressao = "S" THEN
   ...
   CLOSE WINDOW w_POL1124

END FUNCTION

#--------------------------#
 FUNCTION POL1124_consulta()
#--------------------------#

   CLEAR FORM
   DISPLAY p_cod_empresa TO cod_empresa
   LET p_apont_pw1_man912a.* = p_apont_pw1_man912.*

   
   CONSTRUCT BY NAME where_clause ON
       apont_pw1_man912.num_op
 
   CALL log006_exibe_teclas("01",p_versao)
   CURRENT WINDOW IS w_POL1124

   IF INT_FLAG <> 0 THEN
      LET INT_FLAG = 0 
      LET p_apont_pw1_man912.* = p_apont_pw1_man912a.*
      CALL POL1124_exibe_dados()
      ERROR "Consulta Cancelada"
      RETURN
   END IF

   LET sql_stmt = "SELECT chav_seq, num_op FROM apont_pw1_man912 ",
...
#---------------------------------------#
 FUNCTION POL1124_entrada_dados(p_funcao)
#---------------------------------------#
   ...
   IF p_funcao = "INCLUSAO" THEN
      INITIALIZE p_apont_pw1_man912.* TO NULL
      CALL POL1124_exibe_dados()
   END IF

   INPUT BY NAME p_apont_pw1_man912.* 
      WITHOUT DEFAULTS  
...
#------------------------------#
 FUNCTION POL1124_exibe_dados()
#------------------------------#

   DISPLAY BY NAME p_apont_pw1_man912.*
   
END FUNCTION


#-----------------------------------#
 FUNCTION POL1124_cursor_for_update()
#-----------------------------------#

   IF POL1124_apontando() THEN
      RETURN FALSE
   END IF
   ...
   CASE SQLCA.SQLCODE
      ...
      OTHERWISE CALL log003_err_sql("LEITURA","apont_pw1_man912")
   END CASE
   CALL log085_transacao("ROLLBACK")
...
#-----------------------------#
 FUNCTION POL1124_modificacao()
#-----------------------------#

   IF POL1124_cursor_for_update() THEN
      LET p_apont_pw1_man912a.* = p_apont_pw1_man912.*
      IF POL1124_entrada_dados("MODIFICACAO") THEN
         LET p_apont_pw1_man912.num_versao = p_apont_pw1_man912.num_versao + 1
         UPDATE apont_pw1_man912 
            SET apont_pw1_man912.* = p_apont_pw1_man912.*
          WHERE CURRENT OF cm_padrao
         IF SQLCA.SQLCODE = 0 THEN
            IF POL1124_insere_hist() THEN
               CALL log085_transacao("COMMIT")
         ...
         ELSE
            CALL log085_transacao("ROLLBACK")
            CALL log003_err_sql("MODIFICACAO","apont_pw1_man912")
         END IF
      ELSE
         CALL log085_transacao("ROLLBACK")
         LET p_apont_pw1_man912.* = p_apont_pw1_man912a.*
...
#-----------------------------#
FUNCTION POL1124_insere_hist()
#-----------------------------#
   
   LET p_apont_hist_man912.chav_seq = p_apont_pw1_man912.chav_seq
   LET p_apont_hist_man912.num_versao = p_apont_pw1_man912.num_versao
   LET p_apont_hist_man912.dat_producao = p_apont_pw1_man912.dat_producao
   LET p_apont_hist_man912.cod_item = p_apont_pw1_man912.cod_item
   LET p_apont_hist_man912.num_op = p_apont_pw1_man912.num_op
   LET p_apont_hist_man912.cod_operac = p_apont_pw1_man912.cod_operac
   LET p_apont_hist_man912.cod_maquina = p_apont_pw1_man912.cod_maquina
   LET p_apont_hist_man912.qtd_refugo = p_apont_pw1_man912.qtd_refugo
   LET p_apont_hist_man912.qtd_boas = p_apont_pw1_man912.qtd_boas
   LET p_apont_hist_man912.tip_mov = p_apont_pw1_man912.tip_mov
   LET p_apont_hist_man912.mat_operador = p_apont_pw1_man912.mat_operador
   LET p_apont_hist_man912.cod_turno = p_apont_pw1_man912.cod_turno
   LET p_apont_hist_man912.hor_ini = p_apont_pw1_man912.hor_ini
   LET p_apont_hist_man912.hor_fim = p_apont_pw1_man912.hor_fim
   LET p_apont_hist_man912.cod_mov = p_apont_pw1_man912.cod_mov
   LET p_apont_hist_man912.arq_orig = p_apont_pw1_man912.arq_orig
   LET p_apont_hist_man912.situacao = NULL
   LET p_apont_hist_man912.usuario  = p_user
   LET p_apont_hist_man912.programa = 'POL1124'
...
#-----------------------------#
 FUNCTION POL1124_exportacao()
#-----------------------------#

   IF POL1124_cursor_for_update() THEN
      IF log004_confirm(18,35) THEN
         IF POL1124_atualiza_hist() THEN
            CALL log085_transacao("COMMIT")
...
#-------------------------------#
FUNCTION POL1124_atualiza_hist()
#-------------------------------#
      ...
      IF STATUS = 0 THEN
         UPDATE apont_hist_man912
            SET situacao = 'D',
                ...
                programa = 'POL1124'
          WHERE chav_seq   = p_chav_seq
            AND num_versao = p_apont_pw1_man912.num_versao
            AND cod_empresa = p_cod_empresa
   ...
   FOREACH cq_deleta INTO 
           p_apont_pw1_man912.chav_seq,
           p_apont_pw1_man912.num_versao
    
      UPDATE apont_hist_man912
         SET situacao = 'D',
             usuario  = p_user,
             programa = 'POL1124'
       WHERE chav_seq   = p_apont_pw1_man912.chav_seq
         AND num_versao = p_apont_pw1_man912.num_versao
         AND cod_empresa = p_cod_empresa
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql("DELEÇÃO","APONT_pw1_MAN912")
      RETURN FALSE
   END IF
...
#-----------------------------------#
 FUNCTION POL1124_paginacao(p_funcao)
#-----------------------------------#
...
#-----------------------------------#
 FUNCTION POL1124_emite_relatorio()
#-----------------------------------#
   LET p_count = 0
   ...
   FOREACH cq_apont INTO p_apont_pw1_man912.*
   
      OUTPUT TO REPORT POL1124_relat() 
...
#----------------------#
   ...
   FORMAT
      ...
      PAGE HEADER  
         ...
         PRINT COLUMN 001, 'POL1124',
               COLUMN 030, 'INTEGRACAO pw1 X LOGIX - REGISTROS CRITICADOS',
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 001, p_apont_pw1_man912.dat_producao,
               COLUMN 012, p_apont_pw1_man912.cod_item,
               COLUMN 029, p_apont_pw1_man912.num_op,
               COLUMN 041, p_apont_pw1_man912.cod_operac,
               COLUMN 048, p_apont_pw1_man912.cod_maquina,
               COLUMN 054, p_apont_pw1_man912.qtd_refugo,
               COLUMN 065, p_apont_pw1_man912.qtd_boas,
               COLUMN 078, p_apont_pw1_man912.tip_mov,
               COLUMN 084, p_apont_pw1_man912.mat_operador,
               COLUMN 097, p_apont_pw1_man912.cod_turno,
               COLUMN 103, p_apont_pw1_man912.hor_ini,
               COLUMN 113, p_apont_pw1_man912.hor_fim,
               COLUMN 124, p_apont_pw1_man912.cod_mov
...
#-----------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         CALL log150_procura_caminho("LST") RETURNING p_caminho
         LET p_caminho = p_caminho CLIPPED, "POL1124.tmp"
         START REPORT POL1124_imp TO p_caminho
      ELSE
         START REPORT POL1124_imp TO p_nom_arquivo
      END IF
   ELSE
      IF p_ies_impressao = "S" THEN
         START REPORT POL1124_imp TO PIPE p_nom_arquivo
      ELSE
         START REPORT POL1124_imp TO p_nom_arquivo
      END IF
   END IF

   CURRENT WINDOW IS w_POL11241
   ...
   FOREACH cq_lista INTO lr_relat.*
      OUTPUT TO REPORT POL1124_imp(lr_relat.*)
      LET l_lista = TRUE
   END FOREACH

   FINISH REPORT POL1124_imp
...
#         PRINT log500_determina_cpp(132)
         ...
         PRINT COLUMN 001, "POL1124    RELACAO DAS ORDENS DE PRODUCAO IMPORTADAS P/ LOGIX   DATA: ",
...
#-----------------------------------#
 FUNCTION POL1124_apont_final()
#-----------------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("POL11242") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_POL11242 AT 2,2 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   ...
   MENU "OPCAO"
      ...
      COMMAND "Seguinte" "Exibe o Proximo Item Encontrado na Consulta"
         ...
         MESSAGE ""
         LET INT_FLAG = 0
         CALL POL1124_paginacao_454("SEGUINTE")
      COMMAND "Anterior" "Exibe o Item Anterior Encontrado na Consulta"
         HELP 005
         MESSAGE ""
         LET INT_FLAG = 0
         CALL POL1124_paginacao_454("ANTERIOR")
      COMMAND "Listar" "Lista os Dados Cadastrados"
         HELP 006
         MESSAGE ""
         IF log005_seguranca(p_user,"VDP","POL1124","MO") THEN
            IF log028_saida_relat(18,35) IS NOT NULL THEN
               ...
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT POL1124_relat_454 TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'POL1124.tmp'
                     START REPORT POL1124_relat_454  TO p_caminho
                  END IF
               ELSE
                  START REPORT POL1124_relat_454 TO p_nom_arquivo
               END IF
               CALL POL1124_emite_relatorio_454()   
               IF p_count = 0 THEN
                  ERROR "Nao Existem Dados para serem Listados" 
               ELSE
                  ERROR "Relatorio Processado com Sucesso" 
               END IF
               FINISH REPORT POL1124_relat_454   
            ELSE
               CONTINUE MENU
            END IF                                                     
            IF p_ies_impressao = "S" THEN
   ...
   CLOSE WINDOW w_POL11242

END FUNCTION

#-------------------------------#
 FUNCTION POL1124_consulta_454()
#-------------------------------#
   ...
   CALL log006_exibe_teclas("01",p_versao)
   CURRENT WINDOW IS w_POL11242

   IF INT_FLAG <> 0 THEN
      LET INT_FLAG = 0 
      LET p_man_apont_454.* = p_man_apont_454a.*
      CALL POL1124_exibe_dados_454()
      ERROR "Consulta Cancelada"
      RETURN
   END IF
...
#--------------------------------------------#
 FUNCTION POL1124_entrada_454(p_funcao)
#--------------------------------------------#
...
#----------------------------------#
 FUNCTION POL1124_exibe_dados_454()
#----------------------------------#
...
#---------------------------------------#
 FUNCTION POL1124_cursor_for_update_454()
#---------------------------------------#
...
#----------------------------------#
 FUNCTION POL1124_modifica_454()
#----------------------------------#

   IF POL1124_cursor_for_update_454() THEN
      LET p_man_apont_454a.* = p_man_apont_454.*
      IF POL1124_entrada_454("MODIFICACAO") THEN
...
#---------------------------------------#
 FUNCTION POL1124_paginacao_454(p_funcao)
#---------------------------------------#
...
#---------------------------------------#
 FUNCTION POL1124_emite_relatorio_454()
#---------------------------------------#
   LET p_count = 0
   LET p_comprime    = ascii 15
   LET p_descomprime = ascii 18
   ...
   FOREACH cq_apont1 INTO p_man_apont_454.*
      ...
      OUTPUT TO REPORT POL1124_relat_454() 
...
#----------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         CALL log150_procura_caminho("LST") RETURNING p_caminho
         LET p_caminho = p_caminho CLIPPED, "POL1124.tmp"
         START REPORT POL1124_imp_454 TO p_caminho
      ELSE
         START REPORT POL1124_imp_454 TO p_nom_arquivo
      END IF
   ELSE
      IF p_ies_impressao = "S" THEN
         START REPORT POL1124_imp_454 TO PIPE p_nom_arquivo
      ELSE
         START REPORT POL1124_imp_454 TO p_nom_arquivo
      END IF
   END IF
   ...
   FOREACH cq_lista1 INTO lr_relat_454.*
      OUTPUT TO REPORT POL1124_imp_454(lr_relat_454.*)
      LET l_lista = TRUE
   END FOREACH

   FINISH REPORT POL1124_imp_454
...
#-----------------------------------#
REPORT POL1124_imp_454(lr_relat_454)
#-----------------------------------#
   DEFINE lr_relat_454        RECORD
          num_ordem           LIKE ordens.num_ordem, 
          cod_operac          LIKE ord_oper.cod_operac, 
