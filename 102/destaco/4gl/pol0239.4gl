...
# PROGRAMA: POL0239                                                         #
# MODULOS.: POL0239  - LOG0010 - LOG0040 - LOG0050 - LOG0060                #
#           LOG0280  - LOG0380 - LOG1300 - LOG1400                          #
# OBJETIVO: IMPRESSAO DAS NOTAS FISCAIS FATURA - SERVICO - DESTACO          #
...
#            dat_saida           LIKE wfat_servico_mest.dat_emissao,
 ...
 { Corpo da nota contendo os itens da mesma. Pode conter ate 999 itens }
   ...
   DEFINE pr_txt     ARRAY[5] OF RECORD 
          texto      CHAR(65)
   END RECORD
...
MAIN
   ...
   DEFER INTERRUPT
   CALL log140_procura_caminho("pol0239.iem") RETURNING comando
   OPTIONS
      HELP    FILE comando
...
#  CALL log001_acessa_usuario("VDP")
   ...
   IF p_status = 0 THEN
      CALL pol0239_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol0239_controle()
#-------------------------#
   CALL log006_exibe_teclas("01", p_versao)

   CALL log130_procura_caminho("pol0239") RETURNING comando    
   OPEN WINDOW w_pol0239 AT 2,3 WITH FORM comando
   ...
   MENU "OPCAO"
      COMMAND "Informar"   "Informar parametros "
         HELP 0009
         MESSAGE ""
         CALL pol0239_inicializa_campos()
         IF log005_seguranca(p_user,"VDP","POL0239","CO") THEN
            IF pol0239_entrada_parametros() THEN
               NEXT OPTION "Listar"
            END IF
         END IF
      COMMAND "Listar"  "Lista as Notas Fiscais Fatura"
         HELP 1053
         IF log005_seguranca(p_user,"VDP","POL0239","CO") THEN
            IF pol0239_imprime_nff() THEN
               IF  pol0239_verifica_param_exportacao() = TRUE THEN 
               END IF
               NEXT OPTION "Fim"
            END IF
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0239
END FUNCTION

#-----------------------------------#
FUNCTION pol0239_entrada_parametros()
#-----------------------------------#

   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol0239
...
#----------------------------------#
FUNCTION pol0239_inicializa_campos()
#----------------------------------#
...
#------------------------------------------#
FUNCTION pol0239_verifica_param_exportacao()
#------------------------------------------#
...
#----------------------------#
FUNCTION pol0239_imprime_nff()
#----------------------------#    

 IF log028_saida_relat(15,41) IS NOT NULL THEN 
    MESSAGE " Processando a extracao do relatorio ... " ATTRIBUTE(REVERSE)
    IF p_ies_impressao = "S" THEN 
       IF g_ies_ambiente = "U" THEN
          START REPORT pol0239_relat TO PIPE p_nom_arquivo
       ELSE 
          CALL log150_procura_caminho ('LST') RETURNING p_caminho
          LET p_caminho = p_caminho CLIPPED, 'pol0239.tmp' 
          START REPORT pol0239_relat TO p_caminho 
       END IF 
    ELSE
       START REPORT pol0239_relat TO p_nom_arquivo
    END IF
 ELSE
    RETURN FALSE
 END IF  
   ...
   CALL pol0239_busca_dados_empresa()
   ...
   FOREACH cq_fat_nf_mestre INTO p_fat_nf_mestre_serv.*
      ...
      CALL pol0239_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol0239_busca_cof_compl()

      CALL pol0239_busca_nome_pais()
      LET p_nff.den_pais           = p_paises.den_pais              

      CALL pol0239_busca_dados_duplicatas()

      CALL pol0239_carrega_end_cobranca()
    
      CALL pol0239_carrega_corpo_nff()  {le os itens pertencentes a nf}
    	...
    	SELECT PCT_IRRF, PCT_ISS 
			...
			WHERE COD_EMPRESA = p_cod_empresa
			 ...
			 IF SQLCA.SQLCODE <> 0 THEN
			 		...
			 		LET p_pct_iss = 0
			 END IF 
      CALL pol0239_carrega_tabela_temporaria() {corpo todo da nota}

      CALL pol0239_carrega_historico()

#  osvaldo (val_tot_mercadoria)
#     LET p_nff.val_tot_mercadoria = p_wfat_servico_mest.val_tot_mercadoria
#  osvaldo (val_tot_mercadoria)

			SELECT val_tributo_tot 
			INTO p_nff.val_tot_iss
			FROM fat_mestre_fiscal
			WHERE  empresa = p_cod_empresa
			AND trans_nota_fiscal = p_fat_nf_mestre_serv.trans_nota_fiscal
			AND tributo_benef = 'ISS_REC'
			 
			IF SQLCA.SQLCODE <> 0 THEN
				LET p_nff.val_tot_iss = 0			 					 
			END IF 
...
#      LET p_nff.val_tot_iss        = p_fat_nf_mestre_serv.val_mercadoria * (1+( p_pct_iss/100 ))
      ...
      LET p_nff.den_cnd_pgto = pol0239_den_cnd_pgto()
      LET p_ies_lista = TRUE

      CALL pol0239_calcula_total_de_paginas()

      CALL pol0239_monta_relat()
...
#---------------------------------------#
FUNCTION pol0239_cria_tabela_temporaria()
#---------------------------------------#
...
#----------------------------#
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      OUTPUT TO REPORT pol0239_relat(p_wnotalev.*)
...
#-----------------------------------------#
 FUNCTION pol0239_busca_dados_duplicatas()
#-----------------------------------------#
...
#-------------------------------------#
FUNCTION pol0239_carrega_end_cobranca()
#-------------------------------------#
...
#----------------------------------#
FUNCTION pol0239_carrega_corpo_nff()
#----------------------------------#
...
#--------------------------------------#
FUNCTION pol0239_verifica_ctr_unid_med()
#--------------------------------------#
   ...
   FROM ctr_unid_med
   WHERE cod_empresa = p_cod_empresa
     AND cod_cliente = p_wfat_servico_mest.cod_cliente
   ...
   ELSE
      RETURN 1, p_wfat_serv_item.cod_unid_med
   END IF

END FUNCTION

#------------------------------------------#
FUNCTION pol0239_carrega_tabela_temporaria()
#------------------------------------------#
...
#  osvaldo
   ...
   LET p_des_texto = " "
   CALL pol0239_insert_array(p_des_texto)
...
#-----------------------------------------#
FUNCTION pol0239_calcula_total_de_paginas()
#-----------------------------------------#
...
#------------------------------------#
FUNCTION pol0239_busca_cof_compl()
#------------------------------------#
...
#------------------------------------#
FUNCTION pol0239_busca_dados_empresa()            
#------------------------------------#
...
#---------------------------------------------------#
FUNCTION pol0239_busca_dados_clientes(p_cod_cliente)
#---------------------------------------------------#
...
#--------------------------------#
FUNCTION pol0239_busca_nome_pais()                   
#--------------------------------#
...
#------------------------------------------------#
FUNCTION pol0239_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#----------------------------------------#
FUNCTION pol0239_insert_array(p_des_texto)
#----------------------------------------#
   ...
   INSERT INTO wnotalev
      VALUES (p_num_seq,3,"","","",0,0,0,0,0, 
              p_des_texto,0)
	
END FUNCTION 

#------------------------------#
REPORT pol0239_relat(p_wnotalev)
#------------------------------#
   ...
   PAGE HEADER
      ...
      PRINT p_6lpp,
            p_descomprime
      PRINT COLUMN 107, p_nff.num_nff               USING "&&&&&&"
#     PRINT COLUMN 077, "X" 
      SKIP 4 LINES
#     IF p_cod_fiscal_compl  = 0   THEN
         PRINT COLUMN 001, p_nff.den_nat_oper,      
#              COLUMN 044, p_nff.cod_fiscal         USING "&&&&",
               COLUMN 056, p_nff.ins_estadual_trib    
#     ELSE
#        PRINT COLUMN 001, p_nff.den_nat_oper,      
#              COLUMN 039, p_nff.cod_fiscal         USING "&&&",
#              COLUMN 042, ".",                                   
#              COLUMN 043, p_cod_fiscal_compl       USING "&",
#              COLUMN 045, p_nff.ins_estadual_trib    
#     END IF 

      SKIP 3 LINES
      PRINT COLUMN 001, p_nff.nom_destinatario,
            COLUMN 081, p_nff.num_cgc_cpf,
            COLUMN 104, p_nff.dat_emissao        USING "dd/mm/yyyy"
      SKIP 1 LINE
      PRINT COLUMN 001, p_nff.end_destinatario,
            COLUMN 062, p_nff.den_bairro, 
            COLUMN 092, p_nff.cod_cep,
            COLUMN 104, TODAY                    USING "dd/mm/yyyy"
      SKIP 1 LINE
      PRINT COLUMN 001, p_nff.den_cidade, 
            COLUMN 042, p_nff.num_telefone,
            COLUMN 072, p_nff.cod_uni_feder,
            COLUMN 081, p_nff.ins_estadual 
#           COLUMN 104, TIME 
      SKIP 2 LINES
   ...
   {  IF p_nff.val_duplic2 > 0 THEN
         PRINT COLUMN 001, p_comprime,
                           p_nff.num_nff            USING "&&&&&&",
               COLUMN 032, "1- ",                                      
               COLUMN 035, p_nff.val_duplic1 - p_nff.val_tot_irrf 
                                                    USING "###,##&.&&", 
               COLUMN 046, "/ 2- ",                                      
               COLUMN 052, p_nff.val_duplic2        USING "###,##&.&&", 
               COLUMN 065, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 071, p_nff.dig_duplic2        USING "&&", 
               COLUMN 073, " / ",                                
               COLUMN 077, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 083, p_nff.dig_duplic2        USING "&&", 
               COLUMN 100, p_nff.dat_vencto_sd1     USING "dd/mm/yyyy",   
               COLUMN 111, " - ",                                
               COLUMN 114, p_nff.dat_vencto_sd2     USING "dd/mm/yyyy",   
               COLUMN 133, p_nff.cod_cliente                         
      ELSE 
         PRINT COLUMN 001, p_comprime,
                           p_nff.num_nff            USING "&&&&&&",
               COLUMN 035, p_nff.val_duplic1 - p_nff.val_tot_irrf 
                                                    USING "###,##&.&&", 
               COLUMN 068, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 105, p_nff.dat_vencto_sd1     USING "dd/mm/yyyy",   
               COLUMN 131, p_nff.cod_cliente
      END IF   }
      ...
      IF p_nff.val_duplic2 > 0 THEN
         PRINT COLUMN 001, p_comprime,
               ...
               COLUMN 052, p_nff.val_duplic2        USING "###,##&.&&", 
               COLUMN 065, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 071, p_nff.dig_duplic1        USING "&&", 
               COLUMN 073, " / ",                                
               COLUMN 077, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 083, p_nff.dig_duplic2        USING "&&", 
               COLUMN 100, p_nff.dat_vencto_sd1     USING "dd/mm/yyyy",   
               ...
               COLUMN 114, p_nff.dat_vencto_sd2     USING "dd/mm/yyyy",   
               COLUMN 133, p_nff.cod_cliente                         
         {     COLUMN 149, p_nff.num_pedido_cli[1,7],
               COLUMN 163, p_nff.num_pedido   }
         PRINT COLUMN 001, p_nff.num_nff            USING "&&&&&&",
               COLUMN 032, "3- ",                                      
               COLUMN 035, p_nff.val_duplic3        USING "###,##&.&&", 
               COLUMN 046, "/ 4- ",                                      
               COLUMN 052, p_nff.val_duplic4        USING "###,##&.&&", 
               COLUMN 065, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 071, p_nff.dig_duplic3        USING "&&", 
               COLUMN 073, " / ",                                
               COLUMN 077, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 083, p_nff.dig_duplic4        USING "&&", 
               COLUMN 100, p_nff.dat_vencto_sd3     USING "dd/mm/yyyy",   
               COLUMN 111, " - ",                                
               COLUMN 114, p_nff.dat_vencto_sd4     USING "dd/mm/yyyy"   
      ELSE 
         PRINT COLUMN 001, p_comprime,
                           p_nff.num_nff            USING "&&&&&&",
               COLUMN 035, p_nff.val_duplic1 - p_nff.val_tot_irrf 
  ...
  {19}SKIP 20 LINES
   ...
   ON EVERY ROW
      ...
      CASE
         ...
         #  PRINT COLUMN 001, p_wnotalev.cod_item[1,12]
            PRINT COLUMN 001, p_wnotalev.den_item
         #        COLUMN 095, p_wnotalev.qtd_item       USING "####&.&&&",
         #        COLUMN 112, p_wnotalev.pre_unit       USING "##,##&.&&",
         #        COLUMN 135, p_wnotalev.val_liq_item   USING "#,###,##&.&&",
         #        COLUMN 148, p_wnotalev.val_tot_iss    USING "###,###,##&.&&"
         ...
         WHEN p_wnotalev.ies_tip_info = 3
            PRINT COLUMN 015, p_wnotalev.des_texto 

            LET p_linhas_print = p_linhas_print + 1

         WHEN p_wnotalev.ies_tip_info = 4
            PRINT 
            PRINT COLUMN 183, p_nff.val_tot_iss USING "###,###,##&.&&"
            PRINT 
            PRINT COLUMN 183, p_nff.val_tot_nff USING "###,###,##&.&&"
            PRINT 

            LET p_linhas_print = p_linhas_print + 5

         #  PRINT COLUMN 001, p_nff.pct_irrf USING "##&.&&", " % IRRF",
         #        COLUMN 134, p_nff.val_tot_irrf USING "#,###,##&.&&" 
         #  PRINT COLUMN 148, p_nff.val_tot_iss USING "###,###,##&.&&"
         #  WHILE TRUE
         #     IF p_linhas_print < 7  THEN 
         #        IF p_linhas_print = 3 THEN 
         #           PRINT COLUMN 148, p_nff.val_tot_iss USING "###,###,##&.&&"
         #        ELSE
         #           IF p_linhas_print = 5  THEN 
         #              PRINT COLUMN 148, p_val_tot_servico 
         #              USING "###,###,##&.&&"
         #              PRINT COLUMN 148, p_nff.val_tot_nff 
         #              USING "###,###,##&.&&"
         #           ELSE
         #              PRINT
         #           END IF 
      ...
      IF p_linhas_print = 10 THEN { nr. de linhas do corpo da nota }
         SKIP 19 LINES   
         PRINT p_descomprime, 
            COLUMN 004, p_nff.num_nff USING "&&&&&&"
         LET p_linhas_print = 0
      END IF     
...
#----------------------------------#
FUNCTION pol239_carrega_historico()
#----------------------------------#
	  ...
	  DECLARE cq_texto CURSOR FOR 
	   SELECT des_texto  
		...
		FOREACH cq_texto INTO  l_texto
			IF LENGTH(l_texto) > 0 THEN 
			   CALL substr(l_texto,65,5,'N')	
			        RETURNING pr_txt[1].texto,pr_txt[2].texto,
			                  pr_txt[3].texto,pr_txt[4].texto,pr_txt[5].texto
			   FOR p_ind = 1 TO 5
			       IF pr_txt[p_ind].texto IS NOT NULL THEN
                CALL pol0239_insert_array(pr_txt[p_ind].texto)
             END IF
         END FOR
			END IF 
		END FOREACH  
...
#----------------------------------#
FUNCTION pol0239_carrega_historico()
#----------------------------------#
