#-------------------------------------------------------------------#
# PROGRAMA: POL0758                                                 #
# OBJETIVO: RELATORIO DE COMISSOES FUNCIONARIOS - GERENCIAL         #
#-------------------------------------------------------------------#
DATABASE logix
...
GLOBALS
   ...
   DEFINE p_cod_empresa         LIKE empresa.cod_empresa,
          ...
          p_val_garantia        LIKE repres_885.val_garantia,
          p_dat_exp_gar         DATE, 
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "POL0758-10.02.00"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0758.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol0758_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0758_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0758") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0758 AT 3,03 WITH FORM p_nom_tela 
   ...
   MENU "OPCAO"
      ...
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0758","IN") THEN
         SELECT * INTO p_empresas_885.* 
           FROM empresas_885 
          WHERE cod_emp_oficial = p_cod_empresa   
         IF pol0758_entrada_dados() THEN
            LET p_ies_cons = TRUE
      ...
      LET p_achou = FALSE
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0758","MO") THEN
         IF p_ies_cons = TRUE THEN 
           IF pol0758_calcula_valores()  THEN 
            IF log028_saida_relat(16,40) IS NOT NULL THEN
               MESSAGE " Processando a extracao do relatorio ... " 
                  ATTRIBUTE(REVERSE)
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT pol0758_relat TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'pol0758.tmp'
                     START REPORT pol0758_relat  TO p_caminho
                  END IF
               ELSE
                  START REPORT pol0758_relat TO p_nom_arquivo
               END IF
               CALL pol0758_emite_relatorio()   
               IF p_count = 0 THEN
      ...
      EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0758

END FUNCTION

#----------------------------------------#
 FUNCTION pol0758_entrada_dados()
#----------------------------------------#
...
#---------------------------------#
 FUNCTION pol0758_calcula_valores()
#---------------------------------#
...
WHENEVER ERROR STOP
 ...
 SELECT count(*)
  ...
  WHERE a.cod_repres  = b.cod_repres 
    AND a.cod_empresa = p_empresas_885.cod_emp_gerencial
...
IF p_ies_calc_pag = 'S' THEN
   ...
   SELECT cod_gerente,cod_repres,val_garantia,dat_exp_gar
     FROM repres_885
    WHERE tip_comis = 'V' 
   ORDER BY cod_gerente,cod_repres
   FOREACH cq_repres INTO p_cod_gerente,p_cod_repres,p_val_garantia,p_dat_exp_gar
...
#              END IF
    ...
    IF p_val_garantia > t_tot_rep.val_bruto OR  
       t_tot_rep.val_bruto = 0 THEN 
...
#---------------------------------#
 FUNCTION pol0758_emite_relatorio()
#---------------------------------#
DEFINE l_cod_rep   LIKE representante.cod_repres
   ...
   FOREACH  cq_reprel INTO l_cod_rep, p_cod_gerente         
      ...
      FOREACH cq_relat INTO p_lanc_com_885.*
         ...
         OUTPUT TO REPORT pol0758_relat(p_relat.*,p_nom_repres,p_num_cgc_cpf,p_nom_gerente) 
...
#----------------------------------------------------------------------#
 REPORT pol0758_relat(p_relat,p_nom_repres,p_num_cgc_cpf,p_nom_gerente)
#----------------------------------------------------------------------#
   ...
   FORMAT
      ...
      PAGE HEADER  
         ...
         PRINT COLUMN 001, "GERENTE : ", p_relat.cod_gerente, " - ",
                           p_nom_gerente                    
         PRINT COLUMN 001, "VENDEDOR: ", p_relat.cod_repres, " - ",
                           p_nom_repres,                          
