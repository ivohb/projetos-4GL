...
#--------------------------#
   ...
   DELETE FROM ar_aparas_885 
    WHERE cod_empresa = p_cod_empresa
      AND num_aviso_rec NOT IN 
        (SELECT num_aviso_rec FROM aviso_rec WHERE cod_empresa = p_cod_empresa)

   IF STATUS <> 0 THEN
      CALL log003_err_sql('DELETE','ar_aparas_885')
   END IF
   ...
   MENU "OPCAO"
      ...
      COMMAND "Modificar" "Modifica dados na Tabela"
         IF p_ies_cons THEN
            IF pol0895_le_status('A') THEN
               ...
               IF p_status THEN
                  CALL pol0895_atualiza_cabec_885() RETURNING p_status
               ...
               END IF
            ELSE
               LET p_msg = 'Deseja alterar Motorista/Placa?'
      ...
      COMMAND KEY("N")"coNsolidar" "Conclui o lancamento e exporta p/ o Trim"
         IF p_ies_cons THEN
...
#------------------------#
   ...
   CALL pol0895_del_umd() RETURNING p_status
...
#------------------------#
   ...
   INPUT p_motorista, p_placa WITHOUT DEFAULTS
    FROM motorista, placa
   ...
   IF p_cabec THEN
      IF NOT pol0895_atualiza_cabec_885() THEN
         RETURN FALSE
      ELSE
         RETURN TRUE
      END IF
...
#--------------------------#
   ...
   INITIALIZE p_motorista, p_placa TO NULL
   ...
   INPUT BY NAME p_tela.* WITHOUT DEFAULTS  
      ...
      AFTER FIELD num_aviso_rec
          ...
          IF p_tipo = '7' THEN
             ERROR 'NF ainda está em trãnsito.'
             NEXT FIELD num_aviso_rec
          END IF             
...
#------------------------#
   ...
   INSERT INTO ar_aparas_885(
    ...
    VALUES(p_cod_empresa,
           p_num_ar, 'D','N','F','N',0,p_motorista,p_placa)
...
#-------------------------#
   ...
   INSERT INTO umd_aparas_885(
    ...
    VALUES(p_cod_empresa,
           ...
           pr_item[p_index].cod_motivo,0,0,'N')
...
#-------------------------------#
   ...
   IF p_cod_status MATCHES '[IP]' THEN
   ...
   ELSE
      IF p_cod_status MATCHES '[CL]' THEN
         LET p_msg = 'AR esta Consolidado! - Acesso restrito'
         CALL log0030_mensagem(p_msg,'excla')
         RETURN FALSE
      END IF
   END IF
...
#-----------------------#
   ...
   IF p_count > 0 THEN
      ...
      'AR já possui contagem e/ou inspeção','excla')
      RETURN FALSE
   END IF
...
#------------------------------#
   ...
   DISPLAY p_tipo TO tipo
   DISPLAY p_pct_umd_pad TO pct_umd_pad
   DISPLAY p_motorista TO motorista
   DISPLAY p_placa TO placa
...
#------------------------------------#
FUNCTION pol0895_atualiza_cabec_885()#
#------------------------------------#

   UPDATE ar_aparas_885
      SET motorista = p_motorista,
          placa = p_placa
    WHERE cod_empresa   = p_cod_empresa
      AND num_aviso_rec = p_tela.num_aviso_rec
