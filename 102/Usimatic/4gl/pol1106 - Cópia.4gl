...
#----------------------------#
   ...
   foreach cq_qtd into 
      ...
      foreach cq_sdo_ops into p_num_op, p_cod_operac 
            ...
            IF pr_item[p_index].cod_item IS NOT NULL then
               ...
               IF p_qtd_prod > pr_item[p_index].qtd_sdo_ops then
                  ...
                  IF p_ies_forca_apont = 'N' then
                     ERROR 'Qtd Boas + Qtd_refugo deve ser menor ou igual o saldo das ordens!'
                     NEXT FIELD qtd_boas
                  ELSE
...
#-----------------------------#
   ...
   declare cq_rop cursor for
    select num_ordem, cod_operac
      from man_apo_nest_405
     where cod_empresa  = p_cod_empresa
       and num_programa = p_tela.num_programa
       and cod_item     = p_cod_item

   foreach cq_rop INTO p_num_ordem, p_cod_operac
...
#-------------------------------#
   ...
   foreach cq_refug INTO p_num_ordem, p_cod_operac
      ...
      let p_qtd_saldo = p_qtd_saldo - p_qtd_boas
...
#-----------------------------#
   ...
   IF p_criticou THEN
      LET p_msg = 'ALGUNS COMPONENTES ESTÃO SEM SALDO\n',
                  'PARA ABIXAR. CONSULTE A TELA DE ERROS!'
      call pol1106_del_man_apo_405()
      call pol1106_critica_man_nest() RETURNING p_staus
      RETURN TRUE
   END IF
...
#-------------------------------#
   ...
   FOREACH cq_consiste into p_programa.*
      ...
      let p_man.qtd_hor = p_qtd_hor_unit * p_man.qtd_boas
...
#----------------------------#
   ...
   LET p_qtd_baixar = p_programa.pes_unit * p_man.qtd_boas
   
   let p_man.qtd_baixar = p_qtd_baixar
   ...
   LET p_man.id_man_apont = p_id_registro
   ...
   let p_man.qtd_hor = p_qtd_hor_unit * p_man.qtd_refugo
   
   LET p_qtd_baixar = p_programa.pes_unit * p_man.qtd_refugo
   let p_man.qtd_baixar = p_qtd_baixar
   ...
   IF STATUS <> 0 THEN 
      let p_cod_erro = STATUS
      let p_msg = 'ERRO INSERINDO PEÇAS REJEITADAS NA TABELA MAN_APO_LOGIX_405'
      RETURN FALSE
   END IF
...
#---------------------------------#
   ...
   FOREACH cq_man INTO p_man.*
      ...
      If p_man.qtd_refugo > 0 then
         LET p_qtd_movto = p_man.qtd_refugo
         LET p_ies_situa = 'R'
         If not pol1106_le_est_lote_ender() then
            RETURN true
         End if
      ...
      LET p_cod_operacao = NULL
      LET p_qtd_prod = 
          p_man.qtd_boas + p_man.qtd_refugo + p_man.qtd_sucata
...
#-------------------------------#
   ...
   FOREACH cq_cm INTO 
      ...
      IF p_cod_compon = p_man.cod_compon then
         LET p_qtd_baixar = p_man.qtd_baixar
      ELSE
...
#-------------------------------#
   ...
   FOREACH cq_bm INTO 
      ...
      FOREACH cq_neces INTO p_num_neces
