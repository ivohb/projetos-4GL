...
drop table pedido_erro_sical; -- banco logix
...
alter table cnpj_empresa add dt_emissao varchar(10)

create unique index ix_cnpj_empresa on
 cnpj_empresa(num_cnpj);
 
 preciso que rode os comando abaixo na base do logix. Confira o cnpj e coloque a empresa correta.
 insert into cnpj_empresa(num_cnpj, cod_empresa) values ('005.872.541/0001-23',?)
 insert into cnpj_empresa(num_cnpj, cod_empresa) values ('005.872.541/0004-76',?)
 
 
drop table de_para_cond_pgto;
create table de_para_cond_pgto (
   cod_empresa           varchar(02),
   cod_sical             integer,
   cod_logix             integer,
   primary key(cod_empresa, cod_sical)
