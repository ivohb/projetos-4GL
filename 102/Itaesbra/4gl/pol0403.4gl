      ...
      COMMAND "Informar" "Informa Parametros para Processamento"
         ...
         LET INT_FLAG = 0
            ...
            ELSE
               ERROR "Função Cancelada"
            END IF
      COMMAND "Processar" "Processa a Criação da O.M."         
         HELP 002
         MESSAGE ""
            IF m_informou THEN
               IF log004_confirm(21,45) THEN
                  IF pol0403_processa() THEN
                     LET p_msg = 'PROCESSAMENTO EFETUADO COM SUCESSO\n',
                                 'NÚMERO DA OM..: ', p_num_om,
                                 ' NÚMERO DO LOTE: ', mr_tela.num_lote
   ...
   INPUT BY NAME mr_tela.* WITHOUT DEFAULTS
      ...
      AFTER FIELD num_lote
         ...
         IF p_num_item_fat >= p_qtd_linhas_nf THEN
            ERROR "Limite de linhas por NF já atinjido !!!"
            NEXT FIELD num_lote
         END IF                    
...
##-- Alteracao Manuel 26/03/2010  
         ...
         IF mr_pedidos.cod_tip_venda <> p_cod_tip_ant THEN
            CALL log0030_mensagem('Pedido não pertence à mesma PLANTA !!!', 'excla')
            RETURN FALSE
         END IF 
         ...
         IF m_ies_gm THEN
            ...
            ELSE
               CALL log0030_mensagem('Lote não é da GM','info')
               NEXT FIELD num_lote
            END IF
...
#-------------------------#
   ...
   IF p_count = 0 THEN
      IF mr_tela.cod_doca IS NOT NULL THEN
         LET p_msg = 'Não a sequencia/item com saldo para\n o pedido/doca ',
             l_pedido CLIPPED, '/',mr_tela.cod_doca
      ELSE
         LET p_msg = 'Não a sequencia/item com saldo para o pedido indormado.'
      END IF  
      CALL log0030_mensagem(p_msg,'info')
      RETURN FALSE
   END IF
...
#  LET sql_stmt = sql_stmt CLIPPED, " ORDER BY a.prz_entrega, a.num_sequencia "
   ...
   IF l_ind = 1 THEN
      MESSAGE 'Não há itens com saldo para faturar !!!' ATTRIBUTE(REVERSE)
      RETURN FALSE
   ELSE 
      LET m_ind = l_ind - 1 
   ...
   INPUT ARRAY ma_tela WITHOUT DEFAULTS FROM s_om.*
      ...
      BEFORE FIELD qtd_reservada 
        ...
        IF pr_reser[pa_curr].qtd_reservada < 0 THEN
           ERROR 'Quantidade reservada do estoque está negativa!!!'
         ...
         IF ma_tela[pa_curr].qtd_reservada > 0 THEN
            IF pr_reser[pa_curr].qtd_reservada < 0 THEN
               CALL log0030_mensagem(
                 "Quantidade reservada na tabela estoque está negativa","exclamation")    
...
#------------------------------------#
   ...
   IF STATUS = 100 THEN
      LET p_msg = 'Embalagem padrão não cadastrada no pol0364'
      CALL log0030_mensagem(p_msg,'excla')
      RETURN TRUE
   ELSE
      IF STATUS <> 0 THEN
   ...
   FOR l_ind = 1 TO p_qtd_item
      ...
      ELSE
         ...
         IF p_ies_ctr_estoque = 'S' THEN
            IF p_qtd_reservada > 0 THEN
               CALL log085_transacao("ROLLBACK")
               LET p_msg = 'Item ',p_item CLIPPED, '. Não há saldo suficiente\n',
...
#  COMMIT WORK
   ...
   #  MESSAGE "Não foi Gerada O.M. !!!" ATTRIBUTE(REVERSE)
      RETURN TRUE 
   ELSE
      RETURN TRUE
   END IF    
...
#------------------------------ FIM DE PROGRAMA BL-------------------------------#
{ALTERAÇÕES:
24/08/12: gravação da est_loc_reser_end a partir da estoque_lote_ender
04/09/12: ajsutes na rotina de reserva para item que não controla lote
