      ...
      AFTER INPUT
         IF NOT INT_FLAG THEN         
            FOR p_ind = 1 TO ARR_COUNT()
                IF pr_gemea[p_ind].cod_peca_gemea IS NOT NULL THEN
                   IF pr_gemea[p_ind].qtd_peca_gemea IS NULL THEN
                      ERROR 'A coluna QTD não está devidamente preenchida!'
                      NEXT FIELD qtd_peca_gemea
                   END IF
               END IF
            END FOR
...
#-----------------------------#
   ...
   ELSE  
		  ...
		  LET p_texto = 'Exclusao de registro principal= ', p_tela.cod_peca_princ 
		  
		  IF pol0454_inclui_audit() THEN
		  END IF 
      ...
      FOR p_ind = 1 TO ARR_COUNT()
		  ...
		  INITIALIZE p_texto TO NULL  
		  LET p_texto = 'Inclusão de registro principal= ', p_tela.cod_peca_princ, 'e peca simetrica = ', pr_gemea[p_ind].cod_peca_gemea
...
#------------------------------#
	...
	LET l_hora = 		Time
          ...
          IF sqlca.sqlcode <> 0 THEN 
             LET p_houve_erro = TRUE
             MESSAGE "Erro na inclusão audit_ega_454" ATTRIBUTE(REVERSE)
           END IF	
