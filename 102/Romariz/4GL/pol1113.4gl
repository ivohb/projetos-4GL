...
#------------------------------#
   ...
   SELECT cod_local_insp  
	...
	ELSE
     ERROR "Lote informado não existe na tab estoque_lote "
	   RETURN FALSE
  END IF 
  ...
  END IF		
   ...
   ELSE
	...
	ELSE
       ERROR "Lote informado não existe na tab estoque_ender_ender "
	   RETURN FALSE
  END IF 
  ...
  IF p_inventario = 'S' THEN
     ERROR 'item/local/identificação/endereço está em inventário e não pode ser liberado'
     RETURN FALSE
  END IF
