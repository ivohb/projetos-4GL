...
#----------------------------#
   ...
   UPDATE prog_ordem_sup_com                                 
      SET val_receb = val_receb + p_val_etapa                
    WHERE cod_empresa = p_cod_empresa                        
      AND num_oc = p_num_oc                                  
...
#----------------------------#
   ...
   FOREACH cq_estorno  INTO p_cos_pagto.*
      ...
      IF STATUS <> 0 THEN
         CALL log003_err_sql('UPDATE', 'cos_oc_etapa:oc')
         RETURN FALSE
      END IF
      ...
      FOREACH cq_ext_progs INTO l_num_prog,
             l_versao_oc
      ...
      IF p_cos_pagto.val_pagar < l_val_receb THEN
         UPDATE prog_ordem_sup_com 
            SET val_receb = val_receb - p_cos_pagto.val_pagar
          WHERE cod_empresa = p_cod_empresa
            AND num_oc = l_num_oc
            AND num_versao = l_versao_oc
            AND num_prog_entrega = l_num_prog
