...
GLOBALS
   DEFINE p_cod_empresa            LIKE empresa.cod_empresa,
          ...
          p_cod_emp_oper           LIKE par_desc_oper.cod_emp_oper,          
          p_juros                  DECIMAL(5,3),                        
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol0551_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol0551_controle()
#-------------------------#
   CALL log006_exibe_teclas("01", p_versao)

   CALL log130_procura_caminho("pol0551") RETURNING comando    
   OPEN WINDOW w_pol0551 AT 2,3 WITH FORM comando
   ...
   MENU "OPCAO"
      COMMAND "Informar"   "Informar parametros "
         HELP 0009
         MESSAGE ""
         CALL pol0551_inicializa_campos()
         IF log005_seguranca(p_user,"VDP","pol0551","CO") THEN
            IF pol0551_entrada_parametros() THEN
               NEXT OPTION "Listar"
            END IF
         END IF
      COMMAND "Listar"  "Lista as Notas Fiscais Fatura"
         HELP 1053
         IF log005_seguranca(p_user,"VDP","pol0551","CO") THEN
            IF pol0551_imprime_nff() THEN
               IF  pol0551_verifica_param_exportacao() = TRUE THEN 
               END IF
               NEXT OPTION "Fim"
            END IF
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0551
END FUNCTION


#-----------------------------------#
FUNCTION pol0551_entrada_parametros()
#-----------------------------------#
   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol0551
...
#----------------------------------#
FUNCTION pol0551_inicializa_campos()
#----------------------------------#
   INITIALIZE p_nff.*, 
              p_duplic.*, 
              pa_corpo_nff, 
   ...
   LET p_juros = p_per_jur_dia  
   LET p_per_jur_dia = p_per_jur_dia / 100

END FUNCTION

#------------------------------------------#
FUNCTION pol0551_verifica_param_exportacao()
#------------------------------------------#
...
#----------------------------#
FUNCTION pol0551_imprime_nff()
#----------------------------#    
 IF log028_saida_relat(14,40) IS NOT NULL THEN 
    MESSAGE " Processando a extracao do relatorio ... " ATTRIBUTE(REVERSE)
    IF p_ies_impressao = "S" THEN 
       IF g_ies_ambiente = "U" THEN
          START REPORT pol0551_relat TO PIPE p_nom_arquivo
       ELSE 
          CALL log150_procura_caminho ('LST') RETURNING p_caminho
          LET p_caminho = p_caminho CLIPPED, 'pol0551.tmp' 
          START REPORT pol0551_relat TO p_caminho 
       END IF 
    ELSE
       START REPORT pol0551_relat TO p_nom_arquivo
    END IF
 ELSE
    RETURN TRUE  
 END IF

   CALL pol0551_busca_dados_empresa()
   ...
   FOREACH cq_wfat_mestre INTO p_wfat_mestre.*
      ...
      CALL pol0551_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol0551_busca_nome_pais()
      LET p_nff.den_pais           = p_paises.den_pais              

      CALL pol0551_busca_dados_duplicatas()

      CALL pol0551_carrega_end_cobranca()
    
      CALL pol0551_carrega_corpo_nff()  {le os itens pertencentes a nf}

      CALL pol0551_busca_dados_pedido() 

      LET p_val_tot_it = 0

      CALL pol0551_carrega_tabela_temporaria() {corpo todo da nota}
      ...
      CALL pol0551_busca_dados_transport(p_wfat_mestre.cod_transpor)
      CALL pol0551_busca_dados_cidades(p_transport.cod_cidade)
      ...
      LET p_nff.raz_social   = pol0551_representante()
      LET p_nff.num_suframa  = p_clientes.num_suframa
      LET p_nff.num_om       = p_wfat_item.num_om
      LET p_nff.des_especie1 = pol0551_especie(1)
      LET p_nff.des_especie2 = pol0551_especie(2)
      LET p_nff.des_especie3 = pol0551_especie(3)
      LET p_nff.den_cnd_pgto = pol0551_den_cnd_pgto()

      LET p_ies_lista = TRUE

      CALL pol0551_grava_dados_end_entrega()

      CALL pol0551_grava_dados_consig(p_wfat_mestre.cod_consig)

      CALL pol0551_grava_historico_nf_pedido()

      CALL pol0551_calcula_total_de_paginas()

      CALL pol0551_monta_relat()
...
#---------------------------------------#
FUNCTION pol0551_cria_tabela_temporaria()
#---------------------------------------#
   WHENEVER ERROR CONTINUE
   BEGIN WORK
...
#---------------------------------------#
FUNCTION pol0551_cria_temp_w_cla_fisc()
#---------------------------------------#
  WHENEVER ERROR CONTINUE
...
#----------------------------#
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      IF p_wnotalev.ies_tip_info > 2 THEN 
         CONTINUE FOREACH
      END IF
      ...
      OUTPUT TO REPORT pol0551_relat(p_wnotalev.*, p_duplic.*)
...
#---------------------------------------#
FUNCTION pol0551_busca_dados_duplicatas()
#---------------------------------------#
...
#-------------------------------------#
FUNCTION pol0551_carrega_end_cobranca()
#-------------------------------------#
...
#----------------------------------#
FUNCTION pol0551_carrega_corpo_nff()
#----------------------------------#
 ...
 IF p_nat_operacao.ies_tip_controle = "4" THEN
   DECLARE cq_wfat_item_rt CURSOR FOR
    SELECT wfat_item.*
     ...
     WHERE wfat_item.cod_empresa = p_cod_emp_oper
       AND wfat_item.num_nff     = p_wfat_mestre.num_nff
       AND item.cod_empresa      = p_cod_emp_oper
   ...
   FOREACH cq_wfat_item_rt INTO p_wfat_item.*
          ...
          IF p_wfat_item.cod_cla_fisc IS NOT NULL AND
             ...
             SELECT COUNT(*) INTO p_count FROM w_cla_fisc
              WHERE cod_cla_fisc = p_wfat_item.cod_cla_fisc
                 IF p_count = 0 THEN
               CALL pol0551_carrega_classificacoes(p_wfat_item.cod_cla_fisc)
                 END IF
          END IF 
     ...
     IF p_wfat_item.num_pedido > 0 THEN  
      ...
      CALL pol0551_item_cliente()

      LET pa_corpo_nff[p_ind].cod_cla_fisc     = pol0551_carrega_clas_fiscal()
...
#------------------------------------------------------#
FUNCTION pol0551_carrega_classificacoes(l_cod_cla_fisc)
#------------------------------------------------------#
...
#------------------------------------#
FUNCTION pol0551_carrega_clas_fiscal()
#------------------------------------#
...
#-----------------------------------------#
FUNCTION pol0551_carrega_historico_fiscal()
#-----------------------------------------#  
   ...
   IF p_wfat_historico.tex_hist1_1 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist1_1)
   END IF
   IF p_wfat_historico.tex_hist2_1 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist2_1)
   END IF
   IF p_wfat_historico.tex_hist3_1 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist3_1)
   END IF
   IF p_wfat_historico.tex_hist4_1 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist4_1)
   END IF
   IF p_wfat_historico.tex_hist1_2 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist1_2)
   END IF
   IF p_wfat_historico.tex_hist2_2 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist2_2)
   END IF
   IF p_wfat_historico.tex_hist3_2 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist3_2)
   END IF
   IF p_wfat_historico.tex_hist4_2 <> " " THEN
      CALL pol0551_insert_array(p_wfat_historico.tex_hist4_2)
   END IF
   ...
   THEN LET p_des_texto = "BASE DE CALCULO ICMS ST.... R$ ", p_wfat_mestre.val_tot_base_ret USING "###,###,##&.&&",
        ...
        CALL pol0551_insert_array(p_des_texto)
...
#------------------------------------------#
FUNCTION pol0551_carrega_tabela_temporaria()
#------------------------------------------#
   DEFINE i, j          SMALLINT,
   ...
   FOR i = 1 TO 999   {insere as linhas de corpo da nota na TEMP}
      ...
      LET p_wnotalev.qtd_it_conv     =  pa_corpo_nff[i].qtd_item * p_fat_conver

      LET p_num_seq = p_num_seq + 1

      LET p_wnotalev.num_seq         =  p_num_seq
      LET p_wnotalev.ies_tip_info    =  1       
      LET p_wnotalev.cod_item        =  pa_corpo_nff[i].cod_item
      LET p_wnotalev.den_item        =  pa_corpo_nff[i].den_item1
      LET p_wnotalev.cod_unid_med    =  pa_corpo_nff[i].cod_unid_med
      LET p_wnotalev.qtd_item        =  pa_corpo_nff[i].qtd_item

      LET p_wnotalev.pre_bruto       =  pa_corpo_nff[i].pre_bruto
      LET p_wnotalev.val_desc        =  pa_corpo_nff[i].val_desc
      LET p_wnotalev.pct_desc        =  pa_corpo_nff[i].pct_desc

      LET p_wnotalev.pre_unit        =  pa_corpo_nff[i].pre_unit
      LET p_wnotalev.val_liq_item    =  pa_corpo_nff[i].val_liq_item
      LET p_wnotalev.num_nff         =  0 
      INSERT INTO wnotalev VALUES ( p_wnotalev.* )
      ...
      FOR j = 1 TO 05
         IF pol0551_verifica_texto_ped_it(pa_corpo_nff[i].num_pedido,i) THEN
...
#-----------------------------------------#
FUNCTION pol0551_calcula_total_de_paginas()
#-----------------------------------------#
...
#------------------------------------------#
FUNCTION pol0551_busca_dados_subst_trib_uf()
#------------------------------------------#
...
#------------------------------------#
FUNCTION pol0551_busca_dados_empresa()            
#------------------------------------#
...
#------------------------------#
FUNCTION pol0551_representante()
#------------------------------#
...
#---------------------------------------#
FUNCTION pol0551_grava_dados_historicos()
#---------------------------------------#
   INITIALIZE p_wfat_historico.* TO NULL
...
#------------------------------#
FUNCTION pol0551_especie(p_ind)
#------------------------------#
...
#---------------------------------------------------#
FUNCTION pol0551_busca_dados_clientes(p_cod_cliente)
#---------------------------------------------------#
...
#--------------------------------#
FUNCTION pol0551_busca_nome_pais()                   
#--------------------------------#
   INITIALIZE p_paises.* ,
              p_uni_feder.*    TO NULL 
...
#----------------------------------------------------#
FUNCTION pol0551_busca_dados_transport(p_cod_transpor)
#----------------------------------------------------#
...
#------------------------------------------------#
FUNCTION pol0551_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#-----------------------------------#
FUNCTION pol0551_busca_dados_pedido()
#-----------------------------------#  
...
#-----------------------------------------------#
FUNCTION pol0551_grava_dados_consig(p_cod_consig)
#-----------------------------------------------#
...
#----------------------------------------#
FUNCTION pol0551_grava_dados_end_entrega()
#----------------------------------------#
   WHENEVER ERROR CONTINUE
   SELECT wfat_end_ent.end_entrega,
     ...
     FROM wfat_end_ent,
          cidades
    WHERE wfat_end_ent.cod_empresa = p_cod_emp_oper
...
#------------------------------------------#
FUNCTION pol0551_grava_historico_nf_pedido()
#------------------------------------------#
   DEFINE i       SMALLINT
   ...
   IF p_wfat_mestre.cod_texto1 <> 0 OR
      ...
      FOREACH cq_texto_nf INTO p_des_texto
         IF p_des_texto IS NOT NULL OR
            p_des_texto <> "  "     THEN
            CALL pol0551_insert_array(p_des_texto[1,75])
            IF p_des_texto[76,120] IS NOT NULL AND
               p_des_texto[76,120] <> " " THEN
               CALL pol0551_insert_array(p_des_texto[76,120])
            END IF
         END IF
      END FOREACH
   END IF
   ...
   IF pol0551_verifica_texto_ped_it(pa_corpo_nff[1].num_pedido,0) THEN
      FOR i = 1 TO 05
...
#-------------------------------------------------------------------#
FUNCTION pol0551_verifica_texto_ped_it(p_num_pedido, p_num_sequencia)
#-------------------------------------------------------------------#
...
#----------------------------------------------#
FUNCTION pol0551_carrega_classificacao_fiscal()
#----------------------------------------------#
...
#----------------------------------------#
FUNCTION pol0551_insert_array(p_des_texto)
#----------------------------------------#
...
#----------------------------------------#
REPORT pol0551_relat(p_wnotalev, p_duplic)
#----------------------------------------#
   ...
   FORMAT
      ...
      PAGE HEADER
         ...
         PRINT p_6lpp,
               p_comprime
         PRINT COLUMN 001, "pol0551",
               COLUMN 041, "PEDIDO DE VENDA",
               COLUMN 078, "DATA: ", TODAY,
               COLUMN 102, "HORA: ", TIME,
               COLUMN 123, "PAG.: ", PAGENO USING "###&"
         PRINT COLUMN 001, "",
               COLUMN 114, "PEDIDO FATURA TOTAL"
         PRINT COLUMN 001, p_traco                         

         PRINT COLUMN 004, "NUMERO DOCUMENTO: ",
               COLUMN 022, p_nff.num_nff USING "#####&",
               COLUMN 034, "FILIAL DE ORIGEM: ", p_cod_emp_oper, " - ",
               p_den_empresa[1,17], 
               COLUMN 075, "DATA PEDIDO: ", p_nff.dat_emissao,
               COLUMN 104, "PEDIDO CLIENTE: ", p_nff.num_pedido_cli
         PRINT COLUMN 004, "CODIGO CLIENTE..: ", p_nff.cod_cliente,
               COLUMN 052, "CGC/MF..........: ", p_nff.num_cgc_cpf,
               COLUMN 104, "INSC.ESTAD.: ", p_nff.ins_estadual
         PRINT COLUMN 004, "NOME CLIENTE....: ", p_nff.nom_destinatario,
               COLUMN 070, "FONE.......: ", p_nff.num_telefone
         PRINT COLUMN 004, "ENDERECO........: ", p_nff.end_destinatario,
               COLUMN 070, "BAIRRO: ", p_nff.den_bairro[1,15],
               COLUMN 092, "CIDADE: ", p_nff.den_cidade[1,25],
               COLUMN 127, "UF: ", p_nff.cod_uni_feder 
         PRINT COLUMN 004, "VENDEDOR........: ", p_nff.cod_repres, " - ",
               p_nff.raz_social
         PRINT COLUMN 004, "TRANSPORTADORA..: ", p_nff.num_cgc_trans, " - ",
               p_nff.nom_transpor[1,25],
               COLUMN 070, "FONE: ", p_nff.num_telefone_trans
         PRINT COLUMN 004, "REDESPACHO......: ", p_wfat_mestre.cod_consig,
               " - ", p_consignat.den_consignat[1,27],
               COLUMN 070, "FONE: ", p_consignat.num_telefone
         PRINT COLUMN 004, "QUANTIDADE......: ", p_nff.qtd_volume,
               COLUMN 033, "ESPECIE: ", p_nff.des_especie1[1,15],
               COLUMN 058, "MARCA: ", p_nff.den_marca[1,2],
               COLUMN 070, "NUMERO: ", 
               COLUMN 084, "PESO BRUTO: ", p_nff.pes_tot_bruto,
               COLUMN 115, "LIQ.: ", p_nff.pes_tot_liquido
         PRINT COLUMN 001, "-------------------------",   
                           "-------------------------",   
                           "--***",   
               COLUMN 057, "Q U A N T I D A D E ",   
               COLUMN 077, "***----------------------",   
                           "-------------------------",   
                           "------"
         PRINT COLUMN 001, "PRODUTO",              
               ...
               COLUMN 095, "VL.DESC",                                       
               COLUMN 103, "%DESC",                                       
               COLUMN 109, "PR.LIQ.UN",                                       
               COLUMN 121, "PR LIQUIDO"                                       
         PRINT COLUMN 001, p_traco                         
         ...
         CASE
            WHEN p_wnotalev.ies_tip_info = 1   
               PRINT COLUMN 001, p_wnotalev.cod_item,
                     COLUMN 017, p_wnotalev.den_item[1,45],
                     COLUMN 062, p_wnotalev.cod_unid_med,
                     COLUMN 064, p_wnotalev.qtd_item     USING "#####.#&",
                  #  COLUMN 074, p_wnotalev.entregar     USING "#########",
                     COLUMN 084, p_wnotalev.pre_bruto    USING "#####.###&",
                     COLUMN 095, p_wnotalev.val_desc     USING "###&.&&",
                     COLUMN 103, p_wnotalev.pct_desc     USING "#&.&&",
                     COLUMN 108, p_wnotalev.pre_unit     USING "###,###.&&",
                     COLUMN 119, p_wnotalev.val_liq_item USING "#,###,##&.&&" 

               PRINT COLUMN 001, p_traco                         

        
            WHEN p_wnotalev.ies_tip_info = 2
               PRINT COLUMN 017, p_wnotalev.den_item[1,45] 
               PRINT COLUMN 001, p_traco                         
      ...
      AFTER GROUP OF p_wnotalev.num_nff

         NEED 15 LINES

         PRINT COLUMN 002, "Ref. ",
               COLUMN 008, p_duplic.num_nf_orig,
               COLUMN 084, "VALOR MERCADORIAS E SERVICOS.: ", 
               COLUMN 115, GROUP SUM(p_wnotalev.val_liq_item)
                           USING "###,###,###,##&.&&"
         PRINT COLUMN 084, "ABATIMENTO...................: ",
               COLUMN 115, p_duplic.val_desc_cred_icm 
               USING "###,###,###,##&.&&" 
         PRINT COLUMN 084, "VALOR LIQUIDO DO PEDIDO......: ",
               COLUMN 115, GROUP SUM(p_wnotalev.val_liq_item) - 
