...
//Mercado Interno
  ...
  $del_pedido="delete from lt1200_ped_mapa where
               cod_usuario='".$ifx_user."'";
  $res_del_pedido = $cconnect("lt1200",$ifx_user,$ifx_senha);
  $result_del_pedido = $cquery($del_pedido,$res_del_pedido);

  $selec_pedido=" insert into lt1200:lt1200_ped_mapa
          SELECT 'EMITIDOS','".$ifx_user."',
                 (b.num_pedido||'-'||b.cod_empresa) as ped_emp,
                  b.num_pedido,b.cod_empresa,
                  b.pct_desc_financ,
                  (b.pct_desc_adic) as pct_desc_adic_ped,
                  b.pct_frete,b.ies_sit_pedido,
		  ...
		  year(b.dat_pedido) as ano_ped ,month(b.dat_pedido) as mes_ped,
                  ...
                  '','',
                  '' as sit_erp,
                  (b.cod_nat_oper/1) as cod_tip_cli   ,
                   b.cod_repres     
             from pedidos b,
                  ped_itens e,
                  item_vdp k,
  ...
  $selec_pedidos="select * from lt1200_ped_mapa
                  where cod_usuario='".$ifx_user."'
                    and tipo_dados='EMITIDOS' 
              
    order by ano,mes,num_pedido,cod_repres,cod_empresa";  
  $res_pedido = $cconnect("lt1200",$ifx_user,$ifx_senha);
  $result_pedido = $cquery($selec_pedidos,$res_pedido);
...
//CARTEIRA
 ...
 $selec_pedido=" insert into lt1200:lt1200_ped_mapa
         ...
         (b.num_pedido||'-'||b.cod_empresa) as ped_emp,
		  ...
		  year(b.dat_pedido) as ano_ped ,month(b.dat_pedido) as mes_ped,
                  m.cod_familia,m.cod_item,m.den_item,
                  n.cod_lin_prod,n.den_estr_linprod,'' as sit_erp,0,b.cod_repres   
             from pedidos b,
                  ped_itens e,
                  item_vdp k,
                  grupo_item l,
  ...
  $selec_pedido=" insert into lt1200:lt1200_ped_mapa
		  ...
		  year(o.dt_import) as ano_ped ,month(o.dt_import) as mes_ped,
                  m.cod_familia,m.cod_item,m.den_item,
                  n.cod_lin_prod,n.den_estr_linprod,b.sit_erp,0,b.cdrepr
             from vnxeorca b,
                  vnxeorit e,
                  vnempre f,
                  item_vdp k,
  ...
  $selec_pedidos="select * from lt1200_ped_mapa
                  where cod_usuario='".$ifx_user."'
                    and tipo_dados='CARTEIRA' 
           order by cod_lin_prod,cod_familia,den_grupo_item,den_item,cod_item";
