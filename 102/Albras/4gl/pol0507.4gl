...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol0507_controle()
   END IF
END MAIN

#---------------------------#
 FUNCTION pol0507_controle()
#---------------------------#
   CALL log006_exibe_teclas("01", p_versao)
   CALL log130_procura_caminho("pol0507") RETURNING comando    

   OPEN WINDOW w_pol0507 AT 2,2  WITH FORM comando
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informar Parametros"
         HELP 2926
         MESSAGE ""
         CALL pol0507_inicializa_campos()
         IF log005_seguranca(p_user,"SUPRIMEN","pol0507","CO") THEN
            LET p_ies_informou = FALSE
            IF pol0507_entrada_parametros() THEN
               NEXT OPTION "Listar"
            END IF
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0507

END FUNCTION

#------------------------------------#
 FUNCTION pol0507_entrada_parametros()
#------------------------------------#

   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol0507

   DISPLAY p_cod_empresa TO cod_empresa
   INITIALIZE p_num_ar_ini, p_num_ar_fim, p_dat_fat_numero TO NULL
   INITIALIZE p_nff.*, p_nf_sup.* TO NULL

   CALL pol0507_busca_par_sup_pad("ies_num_nf_entrada")
   ...
   INPUT BY NAME p_num_ar_ini,
     ...
     AFTER FIELD p_num_ar_ini
         IF p_num_ar_ini IS NOT NULL THEN
            IF pol0507_verifica_aviso_rec(p_num_ar_ini) = FALSE THEN
      ...
      AFTER FIELD p_num_ar_fim
         IF p_num_ar_ini IS NOT NULL THEN
            ...
            IF pol0507_verifica_aviso_rec(p_num_ar_fim) = FALSE THEN
               ERROR "Aviso de recebimento nao cadastrado na nota fiscal do suprimentos."
               NEXT FIELD p_num_ar_fim
            END IF
         ELSE
...
#---------------------------------------------------#
 FUNCTION pol0507_busca_par_sup_pad(p_cod_parametro)
#---------------------------------------------------#
...
#---------------------------------------------#
 FUNCTION pol0507_verifica_aviso_rec(p_num_ar)
#---------------------------------------------#
...
#-----------------------------------#
 FUNCTION pol0507_busca_num_nf_vdp()
#-----------------------------------#
...
#------------------------------#
 FUNCTION pol0507_imprime_nff()
#------------------------------#

   DEFINE sql_stmt CHAR(1000)

   CALL pol0507_busca_par_sup_pad("ser_ssr_nf_entrada")
   ...
   IF log028_saida_relat(14,34) IS NOT NULL THEN
    ...
    IF p_ies_impressao = "S" THEN
         IF g_ies_ambiente = "W" THEN
            ...
            LET p_caminho = p_caminho CLIPPED, "pol0507.tmp"
            START REPORT pol0507_relat TO p_caminho
         ELSE
            START REPORT pol0507_relat TO PIPE p_nom_arquivo
         END IF 
      ELSE
         START REPORT pol0507_relat TO p_nom_arquivo
      END IF
  
      WHENEVER ERROR CONTINUE
      CALL log085_transacao("BEGIN") 
   #  BEGIN WORK
      LOCK TABLE t_pol0507 IN EXCLUSIVE MODE
      CALL log085_transacao("COMMIT") 
   #  COMMIT WORK
      CALL sup997_cria_t_pol0507()
      WHENEVER ERROR STOP
      ...
      FOREACH cq_nf_sup_temp INTO p_nr_aviso_rec
         INSERT INTO t_pol0507 VALUES (p_nr_aviso_rec)
      END FOREACH
   ...
   #  BEGIN WORK
      ...
      IF pol0507_imprime_relat() = FALSE THEN
         CALL log085_transacao("ROLLBACK") 
      #  ROLLBACK WORK
         RETURN TRUE
      ELSE
         IF p_atualiza THEN
            IF p_ies_num_nf_vdp = "S" THEN
               IF pol0507_atualiza_fat_numero() = FALSE THEN
                  CALL log085_transacao("ROLLBACK") 
               #  ROLLBACK WORK
                  RETURN FALSE
               END IF
            ELSE
               IF pol0507_atualiza_par_sup_pad() = FALSE THEN
                  CALL log085_transacao("ROLLBACK") 
               #  ROLLBACK WORK
                  RETURN FALSE
               END IF
...
#--------------------------------#
 FUNCTION pol0507_imprime_relat()
#--------------------------------#
   DECLARE cq_nf_sup CURSOR WITH HOLD FOR 
    SELECT num_aviso_rec FROM t_pol0507 ORDER BY 1
   ...
   WHILE sqlca.sqlcode = 0
    ...
    # *** reimpressao *** 
      ...
      ELSE
         ...
         IF pol0507_deleta_nf_sup_erro()= FALSE THEN
            RETURN FALSE
         END IF

         IF pol0507_modifica_nfe_sup_compl() = FALSE THEN
            RETURN FALSE
         END IF

         IF pol0507_atualiza_nf_sup() = FALSE THEN
            RETURN FALSE
         END IF    
      END IF
      FETCH cq_nf_sup INTO p_num_aviso_rec
...
#--------------------------------------#
 FUNCTION pol0507_atualiza_fat_numero()
#--------------------------------------#
...
#---------------------------------------#
 FUNCTION pol0507_atualiza_par_sup_pad()
#---------------------------------------#
...
#----------------------------------#
 FUNCTION pol0507_atualiza_nf_sup()
#----------------------------------#
   WHENEVER ERROR CONTINUE
...
#-------------------------------------#
 FUNCTION pol0507_deleta_nf_sup_erro()
#-------------------------------------#
...
#------------------------------------#
 FUNCTION pol0507_inicializa_campos()
#------------------------------------#
...
#--------------------------------#
 FUNCTION pol0507_imprime_dados()
#--------------------------------#
   ...
   FOREACH cq_nf_sup_mst INTO p_nf_sup.*
      ...
      CALL pol0507_cria_tabela_temporaria()
      CALL pol0507_busca_dados_compl()
      ...
      LET p_nff.den_nat_oper = pol0507_den_nat_oper()
      ...
      CALL pol0507_busca_dados_subst_trib_uf()
      ...
      CALL pol0507_busca_dados_fornec(p_nf_sup.cod_fornecedor)
      ...
      CALL pol0507_busca_dados_cidades(p_fornecedor.cod_cidade)
      ...
      CALL pol0507_carrega_corpo_nff()  {le os itens pertencentes a nf}

      CALL pol0507_carrega_tabela_temporaria() {sera o corpo todo da nota}

      CALL pol0507_calcula_total_de_paginas()
...
# imprime base de calculo
      ...
      IF p_nf_sup.cod_transpor IS NOT NULL AND
         ...
         CALL pol0507_busca_dados_transport(p_nf_sup.cod_transpor)
         CALL pol0507_busca_dados_cidades(p_transport.cod_cidade)
...
{     IF p_nff.val_tot_base_icm = 0 THEN
      ...
      CALL pol0507_busca_dados_historicos()  {le wfat_historico}

      LET p_nff.des_especie = p_nfe_sup_compl.den_embal

      CALL pol0507_prepara_linhas_texto()

      LET p_ies_lista  = TRUE

      CALL pol0507_monta_relat()
      CALL pol0507_inicializa_campos()
   END FOREACH
...
#--------------------------------#
 FUNCTION sup997_cria_t_pol0507()
#--------------------------------#

   WHENEVER ERROR CONTINUE

    DROP TABLE t_pol0507 
    IF sqlca.sqlcode <> 0 THEN
       DELETE FROM t_pol0507
    END IF
 
    CREATE TEMP TABLE t_pol0507
...
#----------------------------------------#
 FUNCTION pol0507_cria_tabela_temporaria()
#----------------------------------------#
   WHENEVER ERROR CONTINUE
...
#------------------------------------#
 FUNCTION pol0507_busca_dados_compl()
#------------------------------------#
...
#---------------------------------#
 FUNCTION pol0507_busca_texto_nf()
#---------------------------------#
...
#--------------------------------------------#
 FUNCTION pol0507_busca_dados_subst_trib_uf()
#--------------------------------------------#
...
#-----------------------------------------------------#
 FUNCTION pol0507_busca_dados_fornec(p_cod_fornecedor)
#-----------------------------------------------------#
...
#--------------------------------------------------#
 FUNCTION pol0507_busca_dados_cidades(p_cod_cidade)
#--------------------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0507_modifica_nfe_sup_compl()
#-----------------------------------------#
...
# fim criação indice de classificação fiscal
   ...
   FOREACH cq_dados_nota INTO p_dados_nota.*
      ...
      SELECT num_item
        #INTO p_dados_nota.cod_origem
        INTO p_cod_ref_clas
        FROM clas_fisc_temp
       WHERE cod_cla_fisc = p_dados_nota.cod_cla_fisc

      OUTPUT TO REPORT pol0507_relat(p_dados_nota.*, p_clas_fisc.*, p_textos.*,p_cod_cla_h, p_cod_cla_i)
...
#------------------------------------#
 FUNCTION pol0507_carrega_corpo_nff()
#------------------------------------#
...
#--------------------------------------------#
 FUNCTION pol0507_carrega_tabela_temporaria()
#--------------------------------------------#
 DEFINE i, j         SMALLINT
...
#-------------------------------------------#
 FUNCTION pol0507_calcula_total_de_paginas()
#-------------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0507_busca_dados_historicos()
#----------------------------------------#
...
#------------------------------------------------------#
 FUNCTION pol0507_busca_dados_transport(p_cod_transpor)
#------------------------------------------------------#
...
#-------------------------------------------------#
 FUNCTION pol0507_busca_dados_consig(p_cod_consig)
#-------------------------------------------------#
...
#---------------------------------------#
 FUNCTION pol0507_prepara_linhas_texto()
#---------------------------------------#
 DEFINE i            SMALLINT,
        p_count      SMALLINT,
        p_texto      CHAR(20)
...
#------------------------------------------------------------------------------#
 REPORT pol0507_relat(p_dados_nota,p_clas_fisc,p_textos,p_cod_cla_h,p_cod_cla_i)
#------------------------------------------------------------------------------#
 ...
 FORMAT
   ...
   PAGE HEADER
      ...
      PRINT COLUMN 001, p_descomprime,
            COLUMN 072, p_nff.num_nff USING "&&&&&&"
      PRINT COLUMN 59, "X"
      ...
      IF p_cod_fiscal_compl IS NOT NULL  
         ...
         PRINT COLUMN 001, p_nff.den_nat_oper[1,19],
               COLUMN 035, p_nff.cod_fiscal      USING "&&&&"
      ELSE
         PRINT COLUMN 001, p_nff.den_nat_oper[1,30],
               COLUMN 035, p_nff.cod_fiscal      USING "&&&",
               COLUMN 038, ".",
               COLUMN 039, p_cod_fiscal_compl    USING "&"
      END IF 
                            
      SKIP 2 LINES
      PRINT COLUMN 001, p_nff.nom_destinatario,
            COLUMN 095, p_nff.num_cgc_cpf,
            COLUMN 123, p_nff.dat_emissao     USING "DD/MM/YYYY"  
      PRINT 
      PRINT COLUMN 001, p_nff.end_destinatario,
            COLUMN 073, p_nff.den_bairro[1,12],
            COLUMN 105, p_nff.cod_cep
            #COLUMN 071, TODAY USING "DD/MM/YYYY"       
      PRINT 
      PRINT COLUMN 001, p_nff.den_cidade[1,22],
            COLUMN 055, p_nff.num_telefone[1,13],
            COLUMN 081, p_nff.cod_uni_feder,
            COLUMN 100, p_nff.ins_estadual
            #COLUMN 071, TIME 
      SKIP 6 LINES
      ...
      CASE
         WHEN p_dados_nota.ies_tip_info = 1
            PRINT COLUMN 001, p_dados_nota.cod_item[1,10],
                  COLUMN 005, p_dados_nota.den_item[1,50],
                  COLUMN 064, p_cod_ref_clas              USING "&",
                  COLUMN 067, p_dados_nota.cod_origem     USING "&",
                  COLUMN 068, p_dados_nota.cod_tributacao USING "&&",
                  COLUMN 074, p_dados_nota.cod_unid_med,
                  COLUMN 078, p_dados_nota.qtd_item      USING "###&.&&",
                  COLUMN 087, p_dados_nota.pre_unit      USING "####&.&&&&",
                  COLUMN 105, p_dados_nota.val_liq_item  USING "####,##&.&&",
                  COLUMN 120, p_dados_nota.pct_icm       USING "#&",
                  COLUMN 125, p_dados_nota.pct_ipi       USING "#&",
                  COLUMN 129, p_dados_nota.val_ipi       USING "####&.&&"
            LET p_linhas_print = p_linhas_print + 1

         WHEN p_dados_nota.ies_tip_info = 2
            PRINT COLUMN 017, p_dados_nota.des_texto
            LET p_linhas_print = p_linhas_print + 1
      ...
      IF p_num_pagina = p_tot_paginas THEN { numero de linhas corpo da nota }
         ...
         IF p_num_pagina > 1 AND
           ...
           (p_num_pagina = 1 AND
            ...
            PRINT 
            PRINT
            PRINT COLUMN 007, p_nff.val_tot_base_icm   USING "###,###,##&.&&",
                  COLUMN 031, p_nff.val_tot_icm        USING "###,###,##&.&&",
                  COLUMN 065, p_nff.val_tot_base_ret   USING "###,###,##&.&&",
                  COLUMN 092, p_nff.val_tot_icm_ret    USING "###,###,##&.&&",
                  COLUMN 120, p_nff.val_tot_mercadoria USING "###,###,##&.&&"
            PRINT  
            PRINT COLUMN 007, p_nff.val_tot_frete      USING "###,###,##&.&&", 
                  COLUMN 031, p_nff.val_tot_seguro     USING "###,###,##&.&&",
                  COLUMN 065, p_nff.val_tot_despesas   USING "###,###,##&.&&",
                  COLUMN 092, p_nff.val_tot_ipi        USING "###,###,##&.&&",
                  COLUMN 120, p_nff.val_tot_nff        USING "###,###,##&.&&"
            PRINT 
            PRINT 
            ...
            ELSE
               PRINT COLUMN 070, "2";
            END IF
            PRINT COLUMN 079, p_nff.num_placa,
                  COLUMN 098, p_nff.cod_uni_feder_trans,
                  COLUMN 112, p_nff.num_cgc_trans
            PRINT
            PRINT COLUMN 001, p_nff.end_transpor,
                  COLUMN 074, p_nff.den_cidade_trans[1,22],
                  COLUMN 098, p_nff.cod_uni_feder_trans,
                  COLUMN 118, p_nff.ins_estadual_trans
            PRINT 
            PRINT COLUMN 003, p_nff.qtd_volume      USING "###,##&",
                  COLUMN 025, p_nff.des_especie[1,14] CLIPPED,
                  COLUMN 035, p_nff.den_marca,
                  COLUMN 079, p_nff.num_nff         USING "&&&&&&",
                  COLUMN 100, p_nff.pes_tot_bruto   USING "###,##&.&&&",
                  COLUMN 125, p_nff.pes_tot_liquido USING "###,##&.&&&"
            
            {imprime os textos históricos}
            ...
            FOREACH cq_texto INTO p_dados_nota.des_texto
               
               PRINT COLUMN 001, p_dados_nota.des_texto
               LET p_controle = p_controle + 1

               IF p_controle > 4 THEN 
                  EXIT FOREACH
               END IF
               
            END FOREACH
            
            {imprime a classificação fiscal}
            
            LET p_den_texto = 'Clas.Fisc.:'
         
            DECLARE cq_imp_indice CURSOR FOR
             SELECT *
               FROM clas_fisc_temp
         
            FOREACH cq_imp_indice INTO 
                    p_cod_clas_fisc,
                    p_num_item
         
               LET p_den_texto = p_den_texto CLIPPED, ' ',
                                 p_num_item CLIPPED,  '=',
                                 p_cod_clas_fisc CLIPPED
            END FOREACH

            IF LENGTH(p_den_texto) > 74 THEN
               CALL substr(p_den_texto,74,2,'N') 
                    RETURNING p_texto1, p_texto2   
            ELSE
               LET p_texto1 = p_den_texto
               LET p_texto2 = NULL
            END IF
                  
            PRINT COLUMN 001, p_texto1
            PRINT COLUMN 013, p_texto2
            LET p_controle = p_controle + 2      

            {imprime o Num_NF no canhoto}      
            
            FOR p_ind = p_controle TO 8
                PRINT
            END FOR
            PRINT COLUMN 001, p_descomprime,
                  COLUMN 067, p_nff.num_nff USING "&&&&&&"
            SKIP 3 LINES
            LET p_num_pagina = 0
         END IF
      ELSE
