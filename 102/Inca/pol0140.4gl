...
# PROGRAMA: POL0140                                               #
# MODULOS.: POL0140 - LOG0010 - LOG0030 - LOG0040 - LOG0050       #
...
#         p_pct_acrescimo         LIKE acre_cnd_pg_polo.pct_acrescimo,
#         p_pct_acre              LIKE acre_cnd_pg_polo.pct_acrescimo,
...
MAIN
  ...
  IF   p_status = 0
  THEN CALL pol0140_controle()
  END IF
END MAIN

#--------------------------#
 FUNCTION pol0140_controle()
#--------------------------#
  ...
  CALL  pol0140_cria_t_mestre()

  CALL log006_exibe_teclas("01 02", p_versao)
  WHENEVER ERROR CONTINUE
  CALL log130_procura_caminho("pol0140") RETURNING p_nom_tela
  OPEN WINDOW w_pol0140 AT 2,02 WITH FORM p_nom_tela    
  ...
  MENU "OPCAO"
    COMMAND "Incluir" "Inclui Pedido "
      HELP 0001
      MESSAGE ""
      IF   log005_seguranca(p_user,"VDP","pol0140","IN")     
      THEN CALL pol0140_inclusao_pedido()
  ...
  END MENU
  WHENEVER ANY ERROR CONTINUE  
  CLOSE WINDOW w_pol0140 
  WHENEVER ANY ERROR STOP
END FUNCTION

#-----------------------#
FUNCTION pol0140_sobre()
#-----------------------#
...
#----------------------------------------------------------------------#
 FUNCTION  pol0140_insert_t_mestre(p_num_pedido,      p_cod_nat_oper,
...
#-------------------------------#
 FUNCTION pol0140_cria_t_mestre()
#-------------------------------#
 WHENEVER ERROR CONTINUE
 DROP TABLE t_mestre
 DROP TABLE t_item
...
#---------------------------------#
 FUNCTION pol0140_inclusao_pedido()
#---------------------------------#
 ...
 WHILE p_flag = 1
    CASE
      WHEN p_tela = 1
           CALL pol0140_entrada_dados_mestr("INCLUSAO")   RETURNING p_status
      WHEN p_tela = 2
           CALL pol0140_entrada_dados_ent_obs("INCLUSAO") RETURNING p_status
      WHEN p_tela = 3
           CALL pol0140_entrada_dados_item("INCLUSAO")    RETURNING p_status
      WHEN p_tela = 4
           CALL pol0140_total("INCLUSAO")                 RETURNING p_status
    END CASE
 END WHILE
  
 IF   p_status = 0  THEN 
      CALL pol0140_efetiva_inclusao()
      CALL log006_exibe_teclas("01 02", p_versao)
      CURRENT WINDOW IS w_pol0140
      CALL pol0140_exibe_dados()
      IF   p_erro = FALSE
      THEN LET p_msg = " Inclusao efetuada com sucesso "
      ELSE LET p_msg = " Pedido Consistido " 
      END IF
 ELSE 
      ...
      CALL log006_exibe_teclas("01 02", p_versao)
      CURRENT WINDOW IS w_pol0140
      let p_msg = " Inclusao Cancelada "
 END IF
 
 END FUNCTION

#---------------------------------------------#
 FUNCTION pol0140_entrada_dados_mestr(p_funcao)
#---------------------------------------------#
 ...
 INPUT BY NAME p_pedido_dig_mest.cod_empresa,
       ...
       AFTER  FIELD cod_cliente
              ...
              IF p_pedido_dig_mest.cod_cliente IS NOT NULL THEN
                 ...
                 IF sqlca.sqlcode = 0
                    THEN DISPLAY BY NAME p_pedido_dig_mest.cod_transpor
                 END IF 
       ...
       AFTER  FIELD cod_nat_oper
         ...
         IF p_pedido_dig_mest.cod_nat_oper IS NULL THEN
            ERROR 'Campo com preenchimento obrigatório!'
            NEXT FIELD cod_nat_oper
         END IF

         SELECT natureza_oper_ref 
           INTO p_nat_oper_ref
           FROM vdp_cli_nat_op_ref 
          WHERE empresa = p_cod_empresa
            AND cliente = p_pedido_dig_mest.cod_cliente
            AND natureza_operacao = p_pedido_dig_mest.cod_nat_oper
         
         IF STATUS = 0 THEN
            IF p_nat_oper_ref > 0 THEN
               LET p_pedido_dig_mest.cod_nat_oper = p_nat_oper_ref
            END IF
         ELSE
            IF STATUS <> 100 THEN
               CALL log003_err_sql('Lendo', 'vdp_cli_nat_op_ref')
               NEXT FIELD cod_nat_oper
            END IF
         END IF
       
         IF pol0140_verifica_natureza_operacao() THEN 
            ERROR " Natureza da operacao nao cadastrada. "
            NEXT FIELD cod_nat_oper
         end if
          
         IF NOT pol0140_checa_validade(p_pedido_dig_mest.cod_nat_oper) THEN
         ...
         IF pol0140_verifica_par_fisc() = FALSE THEN 
            NEXT FIELD cod_nat_oper
         END IF
       ...
       AFTER  FIELD dat_prazo_entrega
              ...
              THEN ERROR " Data de entrega nao pode ser Nula "
                   NEXT FIELD dat_prazo_entrega
              END IF
              IF   pol0140_confirma_prazo_entrega() = FALSE
       ...
       AFTER  FIELD cod_repres
              IF pol0140_verifica_repres(p_pedido_dig_mest.cod_repres) = FALSE
              THEN 
       ...
       AFTER  FIELD cod_repres_adic
              IF   pol0140_verifica_repres(p_pedido_dig_mest.cod_repres_adic) = FALSE
       ...
       AFTER  FIELD num_list_preco
              ...
              IF   p_pedido_dig_mest.num_list_preco IS NOT NULL OR
                   ...
                   IF pol0140_verifica_lista_preco() 
       ...
       AFTER  FIELD cod_cnd_pgto
	             IF   pol0140_verifica_cnd_pagamento()
       ...
       AFTER FIELD num_pedido_repres 
          ...
          IF p_pedido_dig_mest.num_pedido_repres IS NOT NULL AND
             p_pedido_dig_mest.num_pedido_repres <> 0 THEN
             IF  p_par_vdp.num_prx_pedido <> 0
             THEN IF pol0140_verifica_pedido_repres()
       ...
       AFTER  FIELD cod_transpor
              IF   p_pedido_dig_mest.cod_transpor IS NOT NULL AND
                   p_pedido_dig_mest.cod_transpor <> " "
              THEN IF   pol0140_verifica_transportadora(p_pedido_dig_mest.cod_transpor) = FALSE
       ...
       AFTER  FIELD cod_consig  
              IF   p_pedido_dig_mest.cod_consig IS NOT NULL AND
                   p_pedido_dig_mest.cod_consig <> " "
              THEN IF   pol0140_verifica_transportadora(p_pedido_dig_mest.cod_consig) = FALSE
       ...
       AFTER FIELD ies_finalidade 
              IF pol0140_verifica_finalidade() = FALSE
       ...
       AFTER  FIELD cod_moeda
              IF   pol0140_verifica_moeda() = FALSE
       ...
       AFTER  FIELD cod_tip_carteira
              IF   pol0140_verifica_carteira() = FALSE
...
#---------------------------------------------#
FUNCTION pol0140_checa_validade(p_cod_nat_oper)
#---------------------------------------------#
   ...
   SELECT natureza_operacao
     FROM vdp_valid_nat_oper
    WHERE natureza_operacao = p_cod_nat_oper
      AND dat_ini_valid    <= p_dat_atual
      AND dat_final_valid  >= p_dat_atual
         
   IF STATUS <> 0 AND STATUS <> 100 THEN
      CALL log003_err_sql('lendo', 'vdp_valid_nat_oper')
      RETURN FALSE
   END IF

   IF STATUS = 100 THEN
      LET p_msg = 'Operação fora do prazo de validade!'
      RETURN FALSE
   END IF
...
#-------------------------------#
 FUNCTION pol0140_mostra_zoom()
#-------------------------------#
 DISPLAY "( Zoom )" AT 3,68
 END FUNCTION

#------------------------------#
 FUNCTION pol0140_apaga_zoom()
#------------------------------#
 DISPLAY "--------" AT 3,68
 END FUNCTION

#------------------------------------------#
 FUNCTION pol0140_confirma_prazo_entrega()
#------------------------------------------#
...
#-------------------------------------------------#
 FUNCTION pol0140_entrada_dados_ent_obs(p_funcao)
#-------------------------------------------------#
 DEFINE p_funcao                CHAR(12)
  
 CALL log006_exibe_teclas("01 02 07", p_versao)
 WHENEVER ERROR CONTINUE
 CALL log130_procura_caminho("pol01401") RETURNING p_nom_tela
 OPEN WINDOW w_pol01401 AT 2,02 WITH FORM p_nom_tela     
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 WHENEVER ERROR STOP  
 CURRENT WINDOW IS w_pol01401
 #DISPLAY FORM f_pol01401 
 ...
 INPUT p_num_sequencia,
  ...
  FROM num_sequencia,
    ...
    AFTER  FIELD num_sequencia
	          ...
	          THEN 
	          ELSE IF   pol0140_verifica_endeco_entrega()
...
#--------------------------------------#
 FUNCTION pol0140_verifica_lista_preco()
#--------------------------------------#
  SELECT *
...
#---------------------------------------------------------#
 FUNCTION pol0140_verifica_transportadora(p_cod_transpor)
#---------------------------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0140_verifica_endeco_entrega()
#-----------------------------------------#
...
#----------------------------------------------#
 FUNCTION pol0140_entrada_dados_item(p_funcao)
#----------------------------------------------#
 DEFINE p_funcao          CHAR(12),  
 ...
 WHENEVER ERROR CONTINUE
 CALL log130_procura_caminho("pol01402") RETURNING p_nom_tela
 OPEN WINDOW w_pol01402 AT 2,02 WITH FORM p_nom_tela    
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 WHENEVER ERROR STOP  
 CURRENT WINDOW IS w_pol01402
 #DISPLAY FORM f_pol01402 
 ...
 INPUT ARRAY t_pedido_dig_item WITHOUT DEFAULTS FROM s_pedido_dig_item.*
       ...
       BEFORE FIELD cod_item
              CALL pol0140_mostra_zoom()
              CALL pol0140_mostra_estoque(t_pedido_dig_item[pa_curr].cod_item)
       ...
       AFTER  FIELD cod_item
              ...
              IF   p_pedido_dig_mest.ies_comissao = "S" AND 
                   ...
                   IF p_achou = FALSE THEN 
                      let p_msg = " Percentual de comissao nao encontrado."
                      call log0030_mensagem(p_msg,'excla')
                        NEXT FIELD cod_item
                   END IF
              END IF
       ...
       AFTER  FIELD  qtd_pecas_solic
              ...
              THEN IF   pol0140_entrada_ped_itens_rem() = FALSE
...
#          END IF 
       ...
       AFTER  FIELD pct_desc_adic 
              ...
              THEN LET t_pedido_dig_item[pa_curr].pct_desc_adic = 0
              END IF
              CALL pol0140_verifica_valor_pedido() 
...
################### PREVISAO - PRODUCAO - INICIO  ###################
    IF p_par_vdp.par_vdp_txt[14,14] = "S" THEN
       IF pol0140_verifica_prevprod()= FALSE
          THEN NEXT FIELD qtd_pecas_solic
       END IF
    END IF
...
################### PREVISAO - PRODUCAO - INICIO  ###################
    ...
    AFTER  FIELD ies_incl_txt
       ...
       ON KEY (control-w, f1)
              CALL pol0140_help_itens()
    
       ON KEY (control-z)
              CALL pol0140_popup(3)
 END INPUT
  IF   int_flag = 0
  THEN LET p_status = 0
...
#-------------------------------------------------------#
 FUNCTION pol0140_calcula_pre_unit(p_pre_unit,p_pct_desc)
#-------------------------------------------------------#
...
#-----------------------------------#
 FUNCTION pol0140_verifica_prevprod()
#-----------------------------------#
 ...
 INITIALIZE p_prev_producao.* TO NULL
{
 ...
 ELSE IF (p_prev_producao.qtd_pedido + p_qtd_saldo) >
      ...
      LET p_prev_producao.qtd_pedido = p_prev_producao.qtd_pedido +
                                       p_qtd_saldo
      CALL pol0140_atualiza_prevprod()
      LET p_audit_logix.cod_empresa = p_cod_empresa
...
#-----------------------------------------------#
 FUNCTION pol0140_verifica_repres(p_cod_repres)
#-----------------------------------------------#
...
#--------------------------------------#
FUNCTION pol0140_verifica_repres_canal()
#--------------------------------------#
...
#----------------------------------#
 FUNCTION pol0140_atualiza_prevprod()
#----------------------------------#
  WHENEVER ERROR CONTINUE
...
#----------------------------------#
 FUNCTION pol0140_insere_prevprod()
#----------------------------------#
  WHENEVER ERROR CONTINUE
...
#----------------------------------#
 FUNCTION pol0140_busca_num_pedido()
#----------------------------------#
...
#-----------------------------------#
 FUNCTION pol0140_verifica_pedido()
#-----------------------------------#
 SELECT * FROM pedidos
...
#-------------------------------------------#
 FUNCTION pol0140_verifica_natureza_operacao()
#-------------------------------------------#
  WHENEVER ERROR CONTINUE
...
#------------------------------------#
 FUNCTION pol0140_verifica_cliente()
#------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0140_verifica_par_fisc()
#-------------------------------------#
   ...
   FOREACH cd_oof INTO p_tributo_benef
   
      IF STATUS <> 0 THEN
         CALL log003_err_sql('Lendo', 'cd_oof')
         RETURN FALSE
      END IF
      ...
      IF STATUS = 100 THEN
         if p_msg is null then
            let p_msg = 'Operação ',p_cod_nat_oper, ' sem parametros fiscais para:\n'
         end if
         LET p_msg = p_msg CLIPPED, p_tributo_benef CLIPPED, '/'
      ELSE
         IF STATUS <> 0 THEN
            CALL log003_err_sql('Lendo','obf_tributo_benef:cd_oof')
            RETURN FALSE
         END IF
      END IF
...
#------------------------------------------#
 FUNCTION pol0140_verifica_cnd_pagamento()
#------------------------------------------#
...
#---------------------------------------#
 FUNCTION pol0140_verifica_pedido_repres()
#---------------------------------------#
  WHENEVER ERROR CONTINUE
   ...
   ELSE RETURN TRUE
   END IF
   SELECT num_pedido FROM pedido_dig_mest
   ...
   ELSE RETURN TRUE
   END IF
   SELECT pedidos_hist.num_pedido FROM pedidos_hist
...
#------------------------------------#
 FUNCTION pol0140_verifica_tipo_venda()
#------------------------------------#
  WHENEVER ERROR CONTINUE
...
#----------------------------------#
 FUNCTION pol0140_verifica_moeda()
#----------------------------------#
...
#--------------------------------#
 FUNCTION pol0140_verifica_cidade()
#--------------------------------#
  WHENEVER ERROR CONTINUE
...
#---------------------------------#
 FUNCTION pol0140_verifica_item()
#---------------------------------#
 ...
 DEFINE      p_ies_situacao    LIKE item.ies_situacao,
    ...
    LET p_cod_parc = "%",t_pedido_dig_item[pa_curr].cod_item CLIPPED
  ...
  SELECT item.cod_item,
    ...
    INTO p_cod_item,
         ...
         p_den_item        
    FROM item, item_vdp
    WHERE item.cod_item  LIKE   p_cod_parc                             
      AND item.cod_item        = item_vdp.cod_item
      AND item.cod_empresa     = p_cod_empresa
      AND item_vdp.cod_empresa = item.cod_empresa 
  ...
  IF STATUS = 100  THEN
     SELECT item.cod_item,
       ...
       IF STATUS = 100  THEN
          let p_msg = " Produto nao cadastrado "
          call log0030_mensagem(p_msg,'excla')
          LET p_status = 0
  ...
  ELSE
       IF STATUS = -284  THEN 
            let p_msg =  "Existe mais de um item com cod. Parcial.Entre com o cod completo"
            call log0030_mensagem(p_msg,'excla')
            LET p_status = 0
            RETURN  p_status, p_qtd_padr_embal 
       END IF
  ...
  THEN
  ELSE 
       call log0030_mensagem("Produto cancelado", 'excla')
       LET p_status = 0
       RETURN  p_status, p_qtd_padr_embal
  END IF
 
  IF pol0140_existe_nat_oper_refer() THEN
  ...
  THEN SELECT qtd_padr_embal
        ...
        WHERE item_embalagem.cod_empresa = p_cod_empresa
         ...
         IF   sqlca.sqlcode = NOTFOUND
         THEN 
              let p_msg = "Item nao cadastrado na tabela item_embalagem"
              call log0030_mensagem(p_msg,'excla')
              LET p_status = 0
              RETURN p_status, p_qtd_padr_embal
         END IF
  ...
  IF   p_pedido_dig_mest.num_list_preco = 0 OR
       ...
       p_pedido_dig_mest.num_list_preco = "    "
  THEN
  ELSE CALL pol0140_busca_preco_lista(p_cod_item, p_cod_lin_prod,
       ...
       IF   p_status = 0 THEN  
            ...
            ELSE CALL pol0140_calcula_pre_unit(p_pre_unit_bruto,
                                              p_desc_bruto_tab)
            ...
            END IF
       ELSE 
            let p_msg = "Produto nao cadastrado na lista de preco"
            call log0030_mensagem(p_msg,'excla')
            LET p_status = 0
            RETURN  p_status, p_qtd_padr_embal
       END  IF
...
#-----------------------------------------#
 FUNCTION pol0140_verifica_valor_pedido()
#-----------------------------------------#
  ...
  IF p_pedido_dig_mest.cod_moeda > 0 THEN 
     ...
     IF sqlca.sqlcode = 0 THEN
     ELSE 
        OPEN WINDOW w_pol01407 AT 10,10 WITH 5 ROWS, 30 COLUMNS
...
#    p_pedido_dig_mest.ies_aceite_finan = "N" THEN 
     FOR p_dep = 1 to pa_curr
         ...
         CALL pol0140_calcula_pre_unit(p_pre_unit_liq,
                                      p_pct_desc_m)
         ...
         IF (p_valor_pedido + p_val_dup_aberto + p_val_ped_carteira) > 
             ...
             OPEN WINDOW w_pol01408 AT 10,30 WITH 6 ROWS, 40 COLUMNS
                  ATTRIBUTE(BORDER, PROMPT LINE LAST)
...
#---------------------------------#
 FUNCTION pol0140_efetiva_inclusao()
#---------------------------------#
 CALL pol0140_inclui_mestre()
 CALL pol0140_insert_ped_list()
 IF   p_pedido_dig_ent.end_entrega IS NOT NULL
 THEN CALL pol0140_inclui_end_entr()
 END IF
 IF   p_pedido_dig_obs.tex_observ_1 IS NOT NULL
 THEN CALL pol0140_inclui_observ()
 END IF
 CALL pol0140_inclui_itens()
 CALL vdp106_inclui_texto()  
#CALL pol0140_inclui_itens_bnf()
 CALL pol0140_inclui_ped_itens_desc()
 END FUNCTION

#--------------------------------#
 FUNCTION pol0140_popup(p_status)
#--------------------------------#
DEFINE  p_cod_transpor    LIKE transport.cod_transpor,
 ...
 CASE
    ...
    WHEN infield(pct_desc_adic) AND p_status = 1
         CALL pol0140_controle_peditdesc(p_cod_empresa,
                                          p_pedido_dig_mest.num_pedido,
                                          0 )
         CALL log006_exibe_teclas("01 02 03 07", p_versao)
         CURRENT WINDOW IS w_pol0140
...
#------------------------------#
 FUNCTION pol0140_inclui_mestre()
#------------------------------#
 ...
 THEN IF   p_consist_cred = TRUE 
      ...
      THEN IF   p_par_vdp.par_vdp_txt[22,22] = "S"
           THEN CALL pol0140_insert_t_mestre(p_pedidos.num_pedido,
...
#--------------------------------#
 FUNCTION pol0140_insert_ped_list()
#--------------------------------#
 WHENEVER ERROR CONTINUE
 IF   p_erro = FALSE
...
#--------------------------------#
 FUNCTION pol0140_inclui_end_entr()
#---------------------------------#
...
#------------------------------#
 FUNCTION pol0140_inclui_observ()
#------------------------------#
 ...
 FOR pa_curr = 1 TO  500 
    IF   t_pedido_dig_item[pa_curr].cod_item IS NOT NULL OR
         ...
         IF   p_par_vdp.par_vdp_txt[22,22] = "S" THEN 
               CALL vdp596_pesquisa_ctr_meta(p_cod_empresa,
              ...
              IF   p_achou = TRUE THEN 
                   IF   p_pedido_dig_mest.ies_aceite_comer = "S" OR
                        p_par_vdp.par_vdp_txt[19,19] = "N"     THEN
                   ELSE 
                      IF   pol0140_verifica_meta_venda() THEN 
                      END IF
                   END IF
              END IF
       ...
       END IF
       IF   p_ies_tip_controle = "2"
       THEN CALL pol0140_insert_ped_itens_rem()
       END IF
   ELSE EXIT FOR
   END IF
 END FOR
...
#-----------------------------------#
#FUNCTION pol0140_inclui_itens_bnf()
#-----------------------------------#
#FOR pa_curr = 1 TO  99  
...
#                 THEN
#                 ELSE IF   pol0140_verifica_meta_venda()
#                      THEN 
#                      END IF
#                 END IF
#            END IF
...
#--------------------------------#
 FUNCTION pol0140_incl_ped_of_pcp()
#--------------------------------#
...
#---------------------------------#
 FUNCTION pol0140_total(p_funcao)
#---------------------------------#
 ...
 CALL log130_procura_caminho("pol01403") RETURNING p_nom_tela
 OPEN WINDOW w_pol01403 AT 2,02 WITH FORM p_nom_tela    
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 
 WHENEVER ERROR STOP  
 
 CURRENT WINDOW IS w_pol01403
 ...
 FOR p_count = 1 TO 500
    IF   t_pedido_dig_item[p_count].cod_item IS NOT NULL  THEN 
        
        LET p_totalc.quantidade = p_totalc.quantidade +
         ...
         CALL pol0140_calcula_pre_unit(t_pedido_dig_item[p_count].pre_unit,
                                      p_pct_desc_m)
             RETURNING p_pre_unit_liq
...
#---------------------------------------------------------------#
 FUNCTION pol0140_busca_preco_lista(p_cod_item, p_cod_lin_prod,
                                  p_cod_lin_recei, p_cod_seg_merc,
                                  p_cod_cla_uso)
#---------------------------------------------------------------#
 DEFINE      p_cod_item        LIKE item_vdp.cod_item,
...
#----------------------------------------------#
 FUNCTION pol0140_verifica_finalidade_tipo_cli()
#----------------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0140_verifica_finalidade()
#-------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0140_verifica_credito_cliente()
#-----------------------------------------#
...
SELECT qtd_dias_atr_dupl,
 ...
 THEN WHENEVER ERROR CONTINUE
      CURRENT WINDOW IS w_pol01401
      OPEN WINDOW w_pol01401 AT 10,10 WITH 5 ROWS, 30 COLUMNS
           ATTRIBUTE(BORDER, PROMPT LINE LAST)
      WHENEVER ERROR STOP
  ...
  DEFINE p_ped_itens_txt    RECORD LIKE ped_itens_texto.*
   ...
   FOREACH cq_ped_txt INTO p_ped_itens_txt.*
     ...
     IF sqlca.sqlcode = NOTFOUND THEN
        INSERT INTO ped_itens_texto VALUES (p_ped_itens_txt.*)
     ELSE 
...
#-------------------------------------#
 FUNCTION pol0140_verifica_meta_venda()
#-------------------------------------#
 IF (p_pedido_dig_item.qtd_pecas_solic + p_ctr_meta.qtd_venda) >
     p_ctr_meta.qtd_remanejada THEN 
     ERROR "QTDE PEDIDO EXCEDEU META DE VENDA"
     LET p_erro = TRUE
     RETURN FALSE
 END IF
...
#--------------------------------------------#
#FUNCTION pol0140_mostra_estoque(p_cod_item)
#--------------------------------------------#
...
#-----------------------------------------------#
 FUNCTION pol0140_mostra_estoque(p_cod_item_est)
#-----------------------------------------------#
...
#------------------------------------------------------------------#
FUNCTION pol0140_controle_peditdesc(p_cod_emp, p_num_ped, p_num_seq)
#------------------------------------------------------------------#
   ...
   CALL log130_procura_caminho("pol01404") RETURNING p_nom_tela
   OPEN WINDOW w_pol01404 AT 2,2  WITH FORM p_nom_tela
        ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   CURRENT WINDOW IS w_pol01404
   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol01404
   ...
   CLOSE WINDOW w_pol01404
   LET int_flag = 0
END FUNCTION

#---------------------------------------#
 FUNCTION pol0140_inclui_ped_itens_desc()
#---------------------------------------#
...
#------------------------------------------------------#
 FUNCTION pol0140_busca_desc_adic_unico(p_num_sequencia,
				       p_desc)
#------------------------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0140_verifica_carteira()
#-------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0140_existe_nat_oper_refer()
#----------------------------------------#

   SELECT cod_nat_oper_ref 
     INTO m_cod_nat_oper_ref 
     FROM nat_oper_refer
    WHERE cod_empresa  = p_cod_empresa
...
#---------------------------------------#
 FUNCTION pol0140_insert_ped_itens_rem()
#---------------------------------------#
...
#--------------------------------------#
FUNCTION pol0140_entrada_ped_itens_rem()
#--------------------------------------#
  ...
  CALL log006_exibe_teclas("01 02 07", p_versao)
  WHENEVER ERROR CONTINUE
  CALL log130_procura_caminho("pol01405") RETURNING p_comando
  OPEN WINDOW w_pol01405 AT 2,2 WITH FORM p_comando
       ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
  WHENEVER ERROR STOP
  CURRENT WINDOW IS w_pol01405
  ...
  CALL pol0140_verifica_motivo_remessa()      RETURNING p_status
  CALL pol0140_verifica_area_negocio()        RETURNING p_status
  CALL pol0140_verifica_linha_negocio()       RETURNING p_status
  ...
  INPUT BY NAME p_ped_itens_rem.* WITHOUT DEFAULTS
    ...
    AFTER  FIELD cod_motivo_remessa
           IF   pol0140_verifica_motivo_remessa() = FALSE
    ...
    AFTER  FIELD cod_area_negocio
           ...
           END IF
           IF   pol0140_verifica_area_negocio() = FALSE
    ...
    AFTER  FIELD cod_lin_negocio
           ...
           END IF
           IF   pol0140_verifica_linha_negocio() = FALSE
           THEN ERROR " Linha de Negocio nao cadastrada "
                NEXT FIELD cod_lin_negocio   
           END IF
           IF   pol0140_verifica_area_lin_negocio() = FALSE
...
# osvaldo       CALL con088_verifica_cod_conta(p_cod_empresa,
#                                              p_ped_itens_rem.num_conta, 
#                                              "S",
#                                              " ")
# osvaldo            RETURNING p_plano_contas.*, p_plano 
                IF   p_plano = FALSE
    ...
    AFTER  FIELD num_pedido_compra  
           ...
           THEN IF   pol0140_verifica_pedido_compra() = FALSE
                THEN NEXT FIELD num_pedido_compra
                END IF
           END IF 
    
    ON KEY (control-w, f1)
           CALL pol0140_help_rem()
    
    ON KEY (control-z)
           CALL pol0140_popup_rem()
  END INPUT
  
  CLOSE WINDOW w_pol01405
...
#----------------------------#
 FUNCTION pol0140_popup_rem()
#----------------------------#
  DEFINE p_mot_rem      LIKE ped_itens_rem.cod_motivo_remessa
  
  CASE
    WHEN infield(cod_motivo_remessa)
##aquiooooo LET p_mot_rem = sup260_popup_motivo_remessa(p_cod_empresa)
         CALL log006_exibe_teclas("01 02 07", p_versao)
         CURRENT WINDOW IS w_pol01405
...
#------------------------------------------#
 FUNCTION pol0140_verifica_motivo_remessa()
#------------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0140_verifica_area_negocio()
#----------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0140_verifica_linha_negocio()
#-----------------------------------------#
...
#--------------------------------------------#
 FUNCTION pol0140_verifica_area_lin_negocio()
#--------------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0140_verifica_pedido_compra()
#-----------------------------------------#
