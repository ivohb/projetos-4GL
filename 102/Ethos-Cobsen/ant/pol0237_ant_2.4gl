...
# PROGRAMA: POL0237                                                         #
# MODULOS.: POL0237  - LOG0010 - LOG0040 - LOG0050 - LOG0060                #
...
MAIN
   ...
   DEFER INTERRUPT
   CALL log140_procura_caminho("pol0237.iem") RETURNING comando
   OPTIONS
      HELP    FILE comando
   ...
   IF p_status = 0 THEN
      CALL pol0237_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol0237_controle()
#-------------------------#
   CALL log006_exibe_teclas("01", p_versao)

   CALL log130_procura_caminho("pol0237") RETURNING comando    
   OPEN WINDOW w_pol0237 AT 2,3 WITH FORM comando
   ...
   MENU "OPCAO"
      COMMAND "Informar"   "Informar parametros "
         HELP 0009
         MESSAGE ""
         CALL pol0237_inicializa_campos()
         IF log005_seguranca(p_user,"VDP","POL0237","CO") THEN
            IF pol0237_entrada_parametros() THEN
               NEXT OPTION "Listar"
            END IF
         END IF
      COMMAND "Listar"  "Lista as Notas Fiscais Fatura"
         HELP 1053
         IF log005_seguranca(p_user,"VDP","POL0237","CO") THEN
            IF pol0237_imprime_nff() THEN
               IF  pol0237_verifica_param_exportacao() = TRUE THEN 
               END IF
               NEXT OPTION "Fim"
            END IF
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0237
END FUNCTION

#-----------------------------------#
FUNCTION pol0237_entrada_parametros()
#-----------------------------------#

   CALL log006_exibe_teclas("01 02 08", p_versao)
   CURRENT WINDOW IS w_pol0237
...
#----------------------------------#
FUNCTION pol0237_inicializa_campos()
#----------------------------------#
...
#------------------------------------------#
FUNCTION pol0237_verifica_param_exportacao()
#------------------------------------------#
 ...
 IF log028_saida_relat(14,41) IS NOT NULL THEN 
      ...
      IF p_ies_impressao = "S" THEN 
         IF g_ies_ambiente = "U" THEN
            START REPORT pol0237_relat TO PIPE p_nom_arquivo
         ELSE 
            CALL log150_procura_caminho ('LST') RETURNING p_caminho
            LET p_caminho = p_caminho CLIPPED, 'pol0237.tmp' 
            START REPORT pol0237_relat TO p_caminho 
         END IF 
      ELSE
         START REPORT pol0237_relat TO p_nom_arquivo
      END IF
   ELSE
      RETURN TRUE
   END IF  
   ...
   CALL pol0237_busca_dados_empresa()
   ...
   DECLARE cq_fat_nf_mestre CURSOR WITH HOLD FOR
  	SELECT DISTINCT a.* from fat_nf_mestre a, servico_par c
   ...
   FOREACH cq_fat_nf_mestre INTO p_fat_nf_mestre_serv.*
      ...
      CALL pol0237_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol0237_busca_cof_compl()

      CALL pol0237_busca_nome_pais()
      LET p_nff.den_pais = p_paises.den_pais              

      CALL pol0237_busca_dados_duplicatas()

      CALL pol0237_carrega_end_cobranca()

      CALL pol0237_carrega_historico()
      ...
      CALL pol0237_carrega_corpo_nff()  {le os itens pertencentes a nf}

      LET p_nff.val_tot_nff = p_fat_nf_mestre_serv.val_nota_fiscal

      CALL pol0237_carrega_tabela_temporaria() {corpo todo da nota}
      ...
      LET p_nff.den_cnd_pgto = pol0237_den_cnd_pgto()
      LET p_ies_lista = TRUE

      CALL pol0237_calcula_total_de_paginas()

      CALL pol0237_monta_relat()
...
#---------------------------------------#
FUNCTION pol0237_cria_tabela_temporaria()
#---------------------------------------#
...
#----------------------------#
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      OUTPUT TO REPORT pol0237_relat(p_wnotalev.*)
...
#-----------------------------------------#
 FUNCTION pol0237_busca_dados_duplicatas()
#-----------------------------------------#
...
#-------------------------------------#
FUNCTION pol0237_carrega_end_cobranca()
#-------------------------------------#
...
#----------------------------------#
FUNCTION pol0237_carrega_historico()
#----------------------------------#
...
#----------------------------------#
FUNCTION pol0237_carrega_corpo_nff()
#----------------------------------#
...
#--------------------------------------#
FUNCTION pol0237_verifica_ctr_unid_med()
#--------------------------------------#
...
#------------------------------------------#
FUNCTION pol0237_carrega_tabela_temporaria()
#------------------------------------------#
...
#  osvaldo
   ...
   LET p_des_texto = " "
   CALL pol0237_insert_array(p_des_texto)
   LET p_des_texto = " "
   CALL pol0237_insert_array(p_des_texto)
...
#-----------------------------------------#
FUNCTION pol0237_calcula_total_de_paginas()
#-----------------------------------------#
   ...
   IF p_num_linhas IS NOT NULL AND 
      p_num_linhas > 0 THEN 
      LET p_tot_paginas = (p_num_linhas - (p_num_linhas MOD 8 )) / 8
      IF (p_num_linhas MOD 8 ) > 0 THEN 
...
#------------------------------------#
FUNCTION pol0237_busca_cof_compl()
#------------------------------------#
...
#------------------------------------#
FUNCTION pol0237_busca_dados_empresa()            
#------------------------------------#
...
#---------------------------------------------------#
FUNCTION pol0237_busca_dados_clientes(p_cod_cliente)
#---------------------------------------------------#
...
#--------------------------------#
FUNCTION pol0237_busca_nome_pais()                   
#--------------------------------#
...
#------------------------------------------------#
FUNCTION pol0237_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#----------------------------------------#
FUNCTION pol0237_insert_array(p_des_texto)
#----------------------------------------#
...
#------------------------------#
REPORT pol0237_relat(p_wnotalev)
#------------------------------#
  ...
  {30}SKIP 3 LINES
   ...
   ON EVERY ROW
      ...
      IF p_linhas_print = 8  THEN { nr. de linhas do corpo da nota }
         SKIP 1 LINES  
