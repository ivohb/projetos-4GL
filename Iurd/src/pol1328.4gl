...
#--------------------------------#
   ...
   IF l_ano_mes_rec <= l_ano_mes_con THEN
      LET m_msg = 'Modulo contabil fechado para o período'
      IF NOT pol1328_grava_erro() THEN
         RETURN FALSE
      END IF
   END IF      
   ...
   IF mr_nota.ies_gera_nota MATCHES '[FS]' THEN 
      ...
      IF l_ano_mes_rec <= l_ano_mes_fec THEN
         LET m_msg = 'Modulo financeiro fechado para o período'
         IF NOT pol1328_grava_erro() THEN
            RETURN FALSE
         END IF
      END IF      
   ...
   IF mr_nota.ies_gera_nota MATCHES '[AS]' THEN 
      ...
      IF l_ano_mes_rec <= l_ano_mes_fec THEN
         LET m_msg = 'Modulo suprimento fechado para o período'
         IF NOT pol1328_grava_erro() THEN
            RETURN FALSE
         END IF
      END IF      
...
#-----------------------------#
   ...
   IF LENGTH(mr_nota.den_observacao CLIPPED) > 3950 THEN
      LET m_msg = 'TEXTO DA FATURA SUPERIOR 3950 POSIÇÕES, O LOGIX NÃO ACEITA'
      IF NOT pol1328_grava_erro() THEN
         RETURN FALSE
      END IF
   END IF      
   ...
   FOREACH cq_ap INTO 
      ...
      IF l_cod_for IS NOT NULL THEN
         IF mr_gi_ap.cod_banco_fav IS NOT NULL AND
            ...
            IF l_count = 0 THEN
               LET m_msg = 'Dados banários enviados não são do fornecedor/favorecido'
               IF NOT pol1328_grava_erro() THEN
                  RETURN FALSE
               END IF            
            END IF
         ELSE
            IF mr_gi_ap.cod_banco_fav IS NOT NULL THEN
               IF mr_gi_ap.num_agencia_fav IS NULL OR mr_gi_ap.num_conta_banco_fav IS NULL THEN
                  LET m_msg = 'Dados banários da tabela gi_ap_9a2 estão imcompletos'
                  IF NOT pol1328_grava_erro() THEN
                     RETURN FALSE
                  END IF            
               END IF
