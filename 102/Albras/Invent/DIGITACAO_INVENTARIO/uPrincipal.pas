...
type
  TfrmPrincipal = class(TForm)
    ...
    cxGrid1DBTableView1preparado: TcxGridDBColumn;
...
var ies_dimensional, endereco, peca, serie : string;
    ...
    bItemPreparadoInventario : String;
begin
  // Validações
  ...
  Update;
    ...
    try
      ...
      bItemPreparadoInventario:='N';
      // caso esteja na tabela itens_invent grava com S identificando que o item estava previsto para ser iventariado do contrario grava com N
      if(not qryGen.IsEmpty)then
        bItemPreparadoInventario:='S';// se achar o item na itens_invent
          ...
          if Application.MessageBox('Item não está preparado para inventário. Deseja inclui-lo?','Mensagem',MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2) = IdYes then
            begin
              cmd.CommandText := 'INSERT INTO itens_invent_912 (cod_empresa,cod_item,num_contagem,local_fabrica,cod_local,ies_dimensional, num_lote,endereco, num_volume, dat_hor_producao, dat_hor_validade, comprimento, largura, altura, diametro , '+
                                 '                              num_peca, num_serie,qtd_contada,ies_situa_qtd,num_folha,cod_usuario,dat_hor_inclusao,preparado) VALUES '+
                                 ...
                                 ' ,'+StringReplace(StringReplace(edtqtd.Text,'.','',[rfReplaceAll]),',','.',[rfReplaceAll])+','+QuotedStr(copy(trim(edtSit.Text),1,1))+','+edtFolha.Text+','+QuotedStr(cod_usuario_so)+','+fData(QuotedStr(DateToStr(now)))+','+aspas(bItemPreparadoInventario)+' )';
              cmd.Execute;
            end;
        end
      else
            ...
            if ((not qryGen.Eof) and (qryGen.Fields[0].AsInteger > 0)) then
              begin
                cmd.CommandText := ' UPDATE itens_invent SET      '+
                                   ...
                                   ' qtd_estoque_cont = '+StringReplace(StringReplace(qryItens.FieldByName('qtd_contada').Value,'.','',[rfReplaceAll]),',','.',[rfReplaceAll])+
                                   ' ,ies_situacao = ''D'' '+
                                   ' WHERE cod_empresa = '+QuotedStr(qryItens.FieldByName('cod_empresa').Value)+
                                   ' AND cod_item = '+QuotedStr(qryItens.FieldByName('cod_item').Value)+
                                   ' AND cod_local_estoq = '+QuotedStr(qryItens.FieldByName('cod_local').Value)+
