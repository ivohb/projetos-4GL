 ...
 $ano_ref=substr($dfin,6,4);
  ...
  if($ger=="")
  {
   $cons_comis="insert into lt1200_ctr_comis (cod_empresa,mes_ref,ano_ref,
          sit_func,sit_rep,sit_aut,sit_fr,sit_tel)
          values('".$empresa."','".$mes_ref."','".$ano_ref."','X','X','X','X','X')
          ";
  ...
  if($procede=="S")
  {
   ...
   $res_comis = $cconnect("lt1200",$ifx_user,$ifx_senha);
   $result_comis = $cquery($cons_comis,$res_comis);
   //notas nao zona franca
   $cons_comis="insert into lt1200_comissoes
              SELECT unique 'NF','G','".$mes_ref."','".$ano_ref."','".$empresa."'
                    ,a1.representante,a.nota_fiscal,a.nota_fiscal,CAST(a.dat_hor_emissao AS DATE),
                     a.cliente,a.val_mercadoria,a.val_nota_fiscal,'',d.cod_nivel_3,
                     f.num_pedido,f.num_pedido_repres,
                     f.dat_emis_repres,f.dat_pedido
              from logix:fat_nf_mestre a,
              	   logix:fat_nf_repr a1,
              ...
              where
               a.empresa='".$empresa."'
               and CAST(a.dat_hor_emissao AS DATE) between '".$dini."' and '".$dfin."'
               and not EXISTS (SELECT trans_nota_fiscal FROM logix:fat_nf_item_fisc x 
                               WHERE x.empresa=a.empresa
                               AND x.trans_nota_fiscal=a.trans_nota_fiscal
                               AND x.cod_fiscal IN ('6109','6110'))
               and a.sit_nota_fiscal  <>'C'
               and a.serie_nota_fiscal='1'
               AND a1.empresa=a.empresa
               AND a1.trans_nota_fiscal=a.trans_nota_fiscal
               and b.cod_repres=a1.representante
               and c.cod_repres=b.cod_repres
               and c.tipo='".$tipo."'
               and c.pct_nff > 0
               and d.cod_nivel_4=a1.representante
               AND NOT EXISTS (SELECT * FROM lt1200_comissoes
                               WHERE ies_tipo_lancto='G'
                               AND cod_empresa=a.empresa
                               AND num_nff=CAST(a.nota_fiscal AS VARCHAR(10))
                               AND cod_cliente=a.cliente
                               AND cod_repres=a1.representante
                               AND dat_emissao > '01/02/2009')
               and e.empresa=a.empresa
               and e.trans_nota_fiscal=a.trans_nota_fiscal
               and f.cod_empresa=e.empresa
               and f.num_pedido=e.pedido
               ";
   $res_comis = $cconnect("lt1200",$ifx_user,$ifx_senha);
   $result_comis = $cquery($cons_comis,$res_comis);
   ...
   $cons_comis="insert into lt1200_comissoes
              SELECT unique 'NF','G','".$mes_ref."','".$ano_ref."','".$empresa."'
                    ,d.cod_nivel_3,a.nota_fiscal,a.nota_fiscal,CAST(a.dat_hor_emissao AS DATE),
                     a.cliente,a.val_mercadoria,a.val_nota_fiscal,'',d.cod_nivel_3,
                     f.num_pedido,f.num_pedido_repres,
                     f.dat_emis_repres,f.dat_pedido
              from logix:fat_nf_mestre a,
   ...
   $cons_comis="insert into lt1200_comissoes
              ...
              where
               ...
               and c.tipo='".$tipo."'
               and c.pct_nff > 0
               and d.cod_nivel_3=a1.representante
               and d.cod_nivel_4=0
               AND NOT EXISTS (SELECT * FROM lt1200_comissoes
                               WHERE ies_tipo_lancto='G'
                               AND cod_empresa=a.empresa
  ...
  // Notas Zona Franca
   $cons_comis="insert into lt1200_comissoes
              SELECT unique 'NF','G','".$mes_ref."','".$ano_ref."','".$empresa."'
                    ,a1.representante,a.nota_fiscal,a.nota_fiscal,CAST(a.dat_hor_emissao AS DATE),
                     a.cliente,a.val_nota_fiscal+a.val_desc_merc,a.val_nota_fiscal,'',d.cod_nivel_3,
                     f.num_pedido,f.num_pedido_repres,
                     f.dat_emis_repres,f.dat_pedido
              from logix:fat_nf_mestre a,
              	   logix:fat_nf_repr a1,
              ...
              where
               a.empresa='".$empresa."'
               and CAST(a.dat_hor_emissao AS DATE) between '".$dini."' and '".$dfin."'
               and EXISTS (SELECT trans_nota_fiscal FROM logix:fat_nf_item_fisc x 
                           WHERE x.empresa=a.empresa
                           AND x.trans_nota_fiscal=a.trans_nota_fiscal
                           AND x.cod_fiscal IN ('6109','6110'))
               and a.sit_nota_fiscal  <>'C'
...
//fim Zona Franca
   ...
   $cons_comis="insert into lt1200_comissoes
               SELECT a.ies_tip_docum,'G','".$mes_ref."','".$ano_ref."',
                   ...
                   b.num_docum_origem,b.num_docum,max(a.dat_pgto) as dat_pgto,
                    b.cod_cliente,b.val_liquido,sum(a.val_pago) as val_pago,'',e.cod_nivel_3,
                    '','','',''
               ...
               AND b.ies_tip_docum=a.ies_tip_docum
               ...
               AND NOT EXISTS (SELECT * FROM lt1200_comissoes
                               ...
                               AND num_docum=a.num_docum
                               AND cod_cliente=b.cod_cliente
                               AND cod_repres=b.cod_repres_1
                               AND dat_emissao > '01/02/2009')
		  GROUP BY a.ies_tip_docum,c.cod_repres,b.num_docum_origem,b.num_docum,b.cod_cliente,
           b.val_liquido,e.cod_nivel_3  
               ";
   $res_comis = $cconnect("lt1200",$ifx_user,$ifx_senha);
   $result_comis = $cquery($cons_comis,$res_comis);
...
//dados de Notas de Devolução
   ...
   $cons_comis="SELECT unique 'NF' as ies_tip_docto,'G' as ies_tipo_lancto,
    ...
    where a.cod_operacao[3,5] in ('101','102','201','202','410','411')
...
*/
  ...
  /*
  $ajusta_zona="select distinct cod as cod_repres,cdfilial
                  from vnpfisic" ;
  */
  $ajusta_zona="SELECT DISTINCT b.cod_repres,a.cdfilial
				FROM vnempre a 
				INNER JOIN lt1200:lt1200_representante b ON lpad(b.cod_repres,10,'0') = a.cod";
