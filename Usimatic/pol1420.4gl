...
#---------------------------------------#
    ...
    CALL _ADVPL_set_property(l_label,"TEXT","Período firme:")    
    CALL _ADVPL_set_property(l_label,"TRANSPARENT",TRUE)    
    CALL _ADVPL_set_property(l_label,"FONT",NULL,NULL,TRUE,FALSE)
    ...
    CALL _ADVPL_set_property(l_label,"TEXT","Até data:")    
    CALL _ADVPL_set_property(l_label,"TRANSPARENT",TRUE)    
...
#----------------------------#
   ...
   IF mr_arquivo.per_firme IS NULL OR mr_arquivo.per_firme < 0 THEN
      LET m_msg = 'Período firme inválido.'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
...
#-------------------------#
   ...
   IF m_count > 1 THEN
      LET m_msg = 'Arquivo contém programações\n de mais de um cliente.'
   ...
   IF m_cod_cliente <> mr_arquivo.cod_cliente THEN
      LET m_msg = 'O cliente do arquivo é diferente\n do cliente informado na tela.'
      CALL log0030_mensagem(m_msg,'info')
      RETURN FALSE
   END IF
...
#------------------------------#
   ...
   IF p_status = 1 THEN
      LET m_msg = 'Não foi possivel renomear o arquivo de .csv para .csv-proces'
      CALL log0030_mensagem(m_msg,'info')      
      RETURN FALSE
   END IF
...
#-----------------------------#
   ...
   IF m_count = 0 THEN
      LET m_msg = 'Arquivo selecionado contém pedidos.'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF
   ...
   FOREACH cq_agupa INTO 
      ...
      IF mr_pedido.cod_cliente IS NULL THEN
         LET m_msg = m_msg CLIPPED, ' - Cliente inválido;'
         LET m_cliticou = TRUE
      END IF
      ...
      FOREACH cq_itens_ped INTO                                                
         ...
         ELSE
            LET m_msg = m_msg CLIPPED, ' - Pendência inválida'
            LET m_cliticou = TRUE
         ...
         IF mr_itens.prz_entrega IS NULL THEN
            LET m_msg = m_msg CLIPPED, ' - Prazo inválido'
            LET m_cliticou = TRUE
            LET mr_itens.situacao = 'E'
         END IF
...
#-----------------------------#
   ...
   IF m_count = 0 THEN
      LET m_msg = m_msg CLIPPED, ' - Item cliente não cadastrado no man10021;'
      LET m_cliticou = TRUE
   ELSE
      IF m_count > 1 THEN
   ...
   IF mr_pedido.cod_item IS NULL THEN
      LET m_msg = m_msg CLIPPED, ' - Não há item logix p/ o item cliente no man10021;'
      LET m_cliticou = TRUE
      RETURN TRUE
   END IF
...
#-----------------------------#
   ...
   FOREACH cq_carteira INTO m_num_pedido
      ...
      LET ma_pedido[m_ind].mensagem = 'Não enviado no EDI'
...
#--------------------------------#
   ...
   IF l_count > 0 THEN
      LET m_msg = 'Erro limpando tabela temporária w_progs_komatsu'
      CALL log0030_mensagem(m_msg,'info')
      RETURN
   END IF      
   ...
   ELSE   
     ...
     FOREACH cq_le_prog INTO  
        ...
        IF m_ind > 1000 THEN                                                                                  
           LET m_msg = 'Limite de programações ultrapassou a 1000.'                                        
           CALL log0030_mensagem(m_msg,'info')                                                                
           EXIT FOREACH                                                                                       
        END IF                                                                                                
   ...
   FOREACH cq_prog_temp INTO 
      ...
      IF m_ind > 1000 THEN                                                                                  
         LET m_msg = 'Limite de programações previstas\n ultrapassou a 1000'                                        
         CALL log0030_mensagem(m_msg,'info')                                                                
         EXIT FOREACH                                                                                       
      END IF                                                                                                
...
#----------------------------------#
   ...
   FOREACH cq_prog_antes INTO                                                                            
      ...
      LET m_ies_origem = 'C'       
      LET m_ies_situa = 'N'       
      LET m_mensagem = 'PROG NÃO ENVIADA'   
      LET m_id_item = 0
      LET m_num_pc = NULL
      LET m_seq_pc = 0                                              
...
#---------------------------#
   ...
   IF m_id_arquivo IS NULL THEN
      LET m_msg = 'Cliente não possui carga a consultar'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF
...
#---------------------------#    
   ...
   IF ma_pedido[m_lin_atu].situacao = 'E' THEN
      LET m_msg = 'O item selecionado contém erro.'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF   

   IF ma_pedido[m_lin_atu].situacao = 'P' THEN
      LET m_msg = 'O item selecionado já foi processado.'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF   
   ...
   LET m_msg = 'Marque as programações que desena processar'
   CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
...
#--------------------------------#
   ...
   IF m_count = 0 THEN
      LET m_msg = 'Nunhuma programação foi selecionada'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF   

   IF l_firme > 0 THEN
      LET m_msg = 'Programações firmes foram selecionadas.\nDeseja continuar ?\n'
   ELSE
      LET m_msg = 'A carteira de pedidos será atualzada.\nDeseja continuar ?\n'
   END IF         
...
#------------------------------#
   ...
   IF ma_prog[l_lin_atu].situacao = 'E' THEN
      ...
      LET m_msg = 'Essa programação contem erros.' 
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
   END IF
...
#---------------------------#
   ...
   IF m_qtd_erro = 0 THEN
      LET m_msg = 'Nenhum item importado comtem erro (situação = E)'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF        
...
#-----------------------------#
   ...
   FOREACH cq_chec_it INTO  
      ...
      IF l_qtd_solic IS NULL THEN                                              
         LET m_msg = m_msg CLIPPED, ' - Qtd EDI inválida'                
         LET m_cliticou = TRUE        
         LET mr_itens.situacao = 'E'                                        
      END IF                                                                  
...
#--------------------------#
   ...
   DEFINE l_menubar     VARCHAR(10),
    ...
    CALL _ADVPL_set_property(m_form_proc,"TITLE","SELECÇÃO DE PROGRAMAÇÃO")
...
#--------------------------------------#
   ...
   LET l_label = _ADVPL_create_component(NULL,"LLABEL",l_layout)
   CALL _ADVPL_set_property(l_label,"TEXT","Dat proces até:")    
