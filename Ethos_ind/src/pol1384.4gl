...
#---------------------------#
   ...
   IF ma_itens[l_linha].prz_entrega <> ma_itens[l_linha].prz_ent_ant THEN
      LET ma_itens[l_linha].ship_date = ma_itens[l_linha].prz_entrega
      LET l_data = ma_itens[l_linha].prz_entrega
      CALL _ADVPL_set_property(m_browse,"COLUMN_VALUE","ship_date",l_linha,l_data)
   END IF
...
#----------------------------#
   ...
   FOREACH cq_le_it INTO 
      ...
      FOREACH cq_ordem INTO l_op
         IF STATUS <> 0 THEN
            CALL log003_err_sql('SELECT','ord_ped_item_547:cq_le_it')
            RETURN FALSE
         END IF
         LET ma_itens[l_ind].num_ordem = l_op
         EXIT FOREACH
      END FOREACH
          ...
          ELSE
             IF STATUS = 100 THEN
                INSERT INTO ped_itens_ethos
                 VALUES(p_cod_empresa, m_num_pedido, 
                        ma_itens[l_linha].num_seq, ma_itens[l_linha].prz_entrega)
                IF STATUS <> 0 THEN
                   CALL log003_err_sql('INSERT','ped_itens_ethos')
                   RETURN FALSE
                END IF
             END IF                         
          END IF
