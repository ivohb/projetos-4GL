...
- Tipo de movimento = F: apontamento liberado (situação do estoque = L)    #
- Tipo de movimento = E: apontamento c/ restrição (situação do estoque = E)#
- Tipo de movimento = R: apontar como refugo (situação do estoque = R). Em #
    seguida, transferir para o item do campo parametros_885.cod_item_refugo#
- Tipo de movimento = S: apontar diretamento no item sucata da tabela      #
    parametros_885.cod_item_sucata                                         #
- Pedido com divisão de quantidade: apontar a parte por dentro no item da  #
    ordem e o restante diretamente no item sucata                          #
- Produção de bobina a partir de outra bobina. Nesse caso, os campos       #
   apont_papel_885.bobinaconsumida e apont_papel_885.itemconsumido deverão #
   estar preenchido. O Logix efetuará um apontamento normal, porém a       #
   estrutura da OF conterá apenas o item aparas do campo                   #
   parametros_885.cod_item_retrab e a bobina consumida deverá ser transfe- #
   rida para o item aparas antes da realização do consumo pela estrutura   #
   da OF.}                                                                 #
#--------------------------------------------------------------------------#           
...
DEFINE p_statusregistro     LIKE apont_papel_885.statusregistro,                  
       ...
       p_cod_local_refugo   LIKE estoque_lote.cod_local,                          
       p_cod_local_sucata   LIKE estoque_lote.cod_local,                          
       p_cod_local_retrab   LIKE estoque_lote.cod_local, 
       p_cod_item_refugo    LIKE item.cod_item,
       p_cod_item_sucata    LIKE item.cod_item,
       p_cod_item_retrab    LIKE item.cod_item,
...
#--------------------------#
  ...
  #lds CALL LOG_refresh_display()	
   ...
   IF NOT p_status THEN
      CALL pol1270_grava_apont_papel() RETURNING p_status
   END IF
...
#------------------------------#
   ...
   SELECT cod_local_estoq
     INTO p_cod_local_refugo
     FROM item
    WHERE cod_empresa = p_cod_empresa
      AND cod_item = p_parametros_885.cod_item_refugo
      
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO(',STATUS,')LENDO LOCAL DE ESTOQUE DO ITEM REFUGO'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO(',STATUS,')LENDO LOCAL DE ESTOQUE DO ITEM SUCATA'
      RETURN FALSE
   END IF
...
#---------------------------------#
   ...
   FOREACH cq_op INTO 
      ...
      IF NOT pol1270_proces_apont() THEN
         ...
         IF p_ind_men > 0 THEN
            FOR p_ind = 1 TO p_ind_men
                LET p_msg = p_men[p_ind].mensagem
                CALL pol1270_insere_erro() RETURNING p_status
            END FOR
         END IF
...
#-------------------------------#
   ...
   IF p_tipo_processo = 3 THEN
      LET p_ies_retrabalho = TRUE
   ELSE
      LET p_ies_retrabalho = FALSE
   END IF
   ...
   IF p_count > 0 THEN
      LET p_msg = 'JA FOI APONTADO UM REGISTRO COM ESSA SEQUENCIA'
      IF NOT pol1270_insere_erro() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF p_ies_retrabalho THEN
      IF p_man.bobinaconsumida IS NULL OR
          p_man.bobinaconsumida = ' ' THEN
         LET p_msg = 'APONTAMENTO DE RETRABALHO SEM ENVIO DA BOBINA CONSUMIDA '
         IF NOT pol1270_insere_erro() THEN
            RETURN FALSE
         END IF
      END IF
      IF p_man.itemconsumido IS NULL OR
          p_man.itemconsumido = ' ' THEN
         LET p_msg = 'APONTAMENTO DE RETRABALHO SEM ENVIO DO ITEM CONSUMIDA '
         IF NOT pol1270_insere_erro() THEN
            RETURN FALSE
         END IF
      END IF
      IF NOT p_criticou THEN
         IF p_man.qtd_movto > 0 THEN
            ...
            IF NOT pol1270_le_estoque() THEN
               RETURN FALSE
            END IF
            IF p_man.qtd_movto > p_qtd_saldo THEN
               LET p_msg = 'NAO HA SALDO NA BOBINA ', 
                     p_num_lote CLIPPED, ' P/ CONSUMIR OU TRANSFERIR '
               IF NOT pol1270_insere_erro() THEN
                  RETURN FALSE
               END IF
            END IF
   ...
   IF NOT p_criticou THEN
      ...
      IF p_count = 0 THEN
         ...
         END IF
      END IF
      IF p_man.qtd_movto > 0 AND 
           p_man.tip_movto MATCHES '[FE]' AND p_ies_oper_final = 'S' THEN
...
#-----------------------------#
   ...
   IF p_ies_apontamento = 'N' THEN                                         
      ...
      END IF                                                               
   ELSE                                                                    
      IF NOT pol1270_le_seq_oper() THEN                                  
         RETURN FALSE                                                      
      END IF                                                               
   END IF                                                                  
...
#-------------------------------#
   ...
   IF p_man.qtd_movto < 0 THEN
      ...
      LET p_qtd_saldo_apon = p_qtd_boas + p_qtd_refug + p_qtd_sucata
      
      IF p_qtd_saldo_apon < p_qtd_baixar THEN
         LET p_msg = 'QTD A ESTORNAR > QTD JA APONTADAS NA TABELA ORD_OPER'
      END IF
   END IF
...
#----------------------------#
   ...
   FOREACH cq_apont INTO p_man.*
      ...
      IF p_man.qtd_movto > 0 THEN   #Apontamento 
         ...
         IF p_man.tip_movto = 'S' THEN  #apto de sucata(refiles)
         ...
         IF p_man.tip_movto MATCHES '[FE]' THEN  #apto de peças boas ou excepcional
            ...
            IF p_qtd_apontar > 0 THEN
               ...
               IF NOT pol1270a_aponta_producao() THEN #aponta a produção como L ou E
                  RETURN FALSE
               END IF
            END IF
         ...
         IF p_man.tip_movto = 'R' THEN  #apto de refugo 
            ...
            IF NOT pol1270a_aponta_producao() THEN  
               RETURN FALSE
            END IF
            
            #transfere do item da OF para o item refugo ou sucata
            ...
            IF NOT pol1270a_movto_estoque() THEN  #fará a saida do item da OF
               RETURN FALSE
            END IF
            ...
            LET p_man.item = p_parametros_885.cod_item_refugo
            LET p_num_lote = p_parametros_885.num_lote_refugo
            LET p_man.comprimento = 0
            LET p_man.largura = 0    
            LET p_man.altura = 0    
            LET p_man.diametro = 0 
            ...
            IF NOT pol1270a_movto_estoque() THEN  
               RETURN FALSE
            END IF

            LET p_est_trans_relac.num_transac_dest = p_num_trans_atual
            LET p_est_trans_relac.cod_item_dest = p_parametros_885.cod_item_refugo
...
#----------------------------#
FUNCTION pol1270_le_estoque()#
#----------------------------#

   SELECT ies_situa_qtd,
         cod_local
     INTO p_ies_situa_bob,
          p_cod_local_bob
     FROM estoque_lote
    WHERE cod_empresa = p_cod_empresa
   ...
   ELSE
      IF STATUS <> 0 THEN
         LET p_msg = 'ERRO:(',STATUS, ') LENDO BOBINA NA TAB ESTOQUE_LOTE'  
         RETURN FALSE
      END IF  
   END IF
   ...
   SELECT SUM(qtd_saldo)
		...
		WHERE cod_empresa   = p_cod_empresa
		  AND cod_item      = p_cod_item
		  AND cod_local     = p_cod_local_bob
		  AND num_lote      = p_num_lote
		  AND ies_situa_qtd = p_ies_situa_bob

   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') LENDO SALDO NA TAB ESTOQUE_LOTE_ENDER'  
      RETURN FALSE
   END IF  
   ...
   IF STATUS <> 0 THEN                                                
      LET p_msg = 'ERRO:(',STATUS, ') LENDO RESERVA NA TAB ESTOQUE_LOC_RESER'     
      RETURN FALSE                                                    
   END IF                                                             
