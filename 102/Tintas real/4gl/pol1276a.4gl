   ...
   #--------------------------------------------#
		...
		IF manr24_cria_w_comp_baixa (0) THEN
		   ...
		   FOREACH cq_baixa INTO 
		      ...
		      ELSE
			     IF manr24_inclui_w_comp_baixa(p_compon.*,1) THEN 
		         END IF
		      END IF				   
			 
			 
			 LET l_qtd_saldo	= 0
			 INITIALIZE p_dat_hor_producao, p_dat_hor_validade TO NULL
			 ...
			 DECLARE cq_estoque_lote  CURSOR FOR
					...
					AND   ies_situa_qtd	= 'L'
					AND   cod_local 	= p_w_apont_prod.cod_local
			 ...
			 FOREACH cq_estoque_lote INTO 
						  ...
						  ELSE					     
							 LET p_compon.dat_producao = EXTEND(p_estoque_lote_ender.dat_hor_producao, YEAR TO DAY)
							 LET p_compon.hor_producao = EXTEND(p_estoque_lote_ender.dat_hor_producao, HOUR TO SECOND)
 							 LET p_compon.dat_valid = EXTEND(p_estoque_lote_ender.dat_hor_producao, YEAR TO DAY)
							 LET p_compon.hor_valid = EXTEND(p_estoque_lote_ender.dat_hor_producao, HOUR TO SECOND)
...
#------------------------------#
	...
	IF p_erro THEN 																			#INSERE O ERRO
	    ...
	    LET l_man_log_apo_prod912[p_index].erro = 0
			...
			let l_man_log_apo_prod912[p_index].num_serie =  p_num_serie
