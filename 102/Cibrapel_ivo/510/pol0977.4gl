#-------------------------------------------------------------------#
# OBJETIVO: APONTAMENTO AUTOMÁTICO DE CONSUMO  - GUAPI              #
# DATA....: 08/08/2008                                              #
# CONVERSÃO 10.02: 01/08/2014 - IVO                                 #
# FUNÇÕES: FUNC002                                                  #
#-------------------------------------------------------------------#
...
END GLOBALS
   ...
   DEFINE pr_erro           ARRAY[2000] OF RECORD 
...
MAIN
   ...
   LET p_caminho = log140_procura_caminho('pol0977.iem')

  CALL log001_acessa_usuario("VDP","LIC_LIB")
        RETURNING p_status, p_cod_empresa, p_user

  IF p_status = 0  THEN
     CALL pol0977_controle()
  END IF

END MAIN       

#--------------------------#
 FUNCTION pol0977_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol0977") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0977 AT 2,2 WITH FORM p_caminho
        ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   
   IF NOT pol0977_le_empresa() THEN 
      RETURN 
   END IF 
   ...
   MENU "OPCAO"
      COMMAND "Processar" "Processa a importação/baixa do consumo"
         CALL pol0977_processar() RETURNING p_status
         IF p_status THEN
            CALL log0030_mensagem('Processamento concluído!','exclamation')
         ELSE
            CALL log0030_mensagem(p_msg,'exclamation')
            CLEAR FORM
            DISPLAY p_cod_empresa TO cod_empresa
         END IF
      COMMAND "Consultar" "Consulta de baixas do consumo com erros"
         IF pol0977_verifica_erros() THEN 
            CALL pol0977_consultar() RETURNING p_status
            IF p_status THEN
      ...
      COMMAND "Pendentes" "Exibe meses que ainda não foram baixados"
         CALL pol0977_exibe_prendentes() RETURNING p_status
         ...
         END IF            
      COMMAND "Listar" "Listagem de consumo"
         CALL pol0977_listagem() RETURNING p_status
         IF p_status THEN
         ELSE
            ERROR 'Operação cancelada !!!'
         END IF
...
#---------------------------#
   ...
   IF NOT pol0977_le_parametros() THEN
      RETURN FALSE
   END IF

   #CALL pol0977_deleta_erros()

   LET p_dat_movto = TODAY
   
   {LET p_ies_apon = 'P'
   
   IF NOT pol0977_importa_apont() THEN
      CALL pol0977_grava_erro() RETURNING p_status
   END IF}

   LET p_ies_apon = 'C'
   
   IF NOT pol0977_importa_consumo() THEN
      CALL pol0977_grava_erro() RETURNING p_status
   END IF
...
#------------------------------#
FUNCTION pol0977_importa_apont()
#------------------------------#
   ...
   DECLARE cq_op CURSOR WITH HOLD FOR
    SELECT numsequencia,
           ...
           codmaquina,
           datproducao,
           #dateadd(MINUTE,tempoproducao,datproducao),
           pesobalanca,
           tipmovto,
           numlote,
           largura,
   ...
   FOREACH cq_op INTO 
      ...
      SELECT COUNT(num_ordem)
        INTO p_count
        FROM ord_criticada_912
       WHERE num_ordem  = p_man.ordem_producao
      ...
      IF NOT pol0977_consiste_apont() THEN
         IF NOT pol0977_insere_erro() THEN
            RETURN FALSE
         END IF
      END IF

      IF NOT p_criticou THEN
         IF pol0977_insere_apont() THEN
            IF pol0977_processa_apont() THEN
      ...
      IF NOT pol0977_grava_apont_papel() THEN
         RETURN FALSE
      END IF
...
#-------------------------------#
FUNCTION pol0977_consiste_apont()
#-------------------------------#
   ...
   IF NOT pol0977_consiste_turno() THEN
      RETURN FALSE
   END IF
   ...
   IF NOT pol0977_checa_dimensional() THEN
      RETURN FALSE
   END IF

   IF p_man.tip_movto <> 'S' THEN
      IF NOT pol0977_consiste_dimen() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF NOT p_criticou THEN   
      ...
      IF STATUS = 100 THEN
         IF NOT pol0977_insere_operacao() THEN
            RETURN FALSE
         END IF
      ...
      IF p_ies_apontamento = 'N' THEN
         ...
         END IF
      ELSE
         IF NOT pol0977_consiste_qtds() THEN
            RETURN FALSE
         END IF
      END IF
   END IF
   ...
   IF NOT p_criticou THEN
      IF NOT pol0977_consiste_qtd_apont('T') THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF (p_man.qtd_boas < 0 AND p_ies_oper_final = 'S') OR 
      ...
      IF NOT pol0977_cheka_estoque() THEN
         RETURN FALSE
      END IF
...
#--------------------------------#
FUNCTION pol0977_consiste_turno()
#--------------------------------#
   ...
   IF STATUS <> 0 THEN
      ...
      CALL pol0977_insere_erro() RETURNING p_status
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol0977_consiste_datas()
#--------------------------------#
...
#---------------------------------#
FUNCTION pol0977_insere_operacao()
#---------------------------------#
...
#-------------------------------#
FUNCTION pol0977_consiste_qtds()
#-------------------------------#
...
#--------------------------------------------#
FUNCTION pol0977_le_item_ctr_grade(p_cod_item)
#--------------------------------------------#
...
#----------------------------------#
FUNCTION pol0977_checa_dimensional()
#----------------------------------#
  
   IF NOT pol0977_le_item_ctr_grade(p_cod_prod) THEN
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol0977_consiste_dimen()
#--------------------------------#
   ...
   IF p_ies_largura = 'S' THEN
      IF p_man.largura IS NULL THEN
         LET p_msg = 'DIMENSIONAL LARGURA NAO ENVIADO PELO TRIM - ', p_dim
         IF NOT pol0977_insere_erro() THEN
            RETURN FALSE
         END IF
      ELSE
         IF p_man.largura <> p_largura_ped THEN
            LET p_msg = 'DIMENSIONAL LARGURA DIFERENTE DO PEDIDO:', p_dim
            IF NOT pol0977_insere_erro() THEN
               RETURN FALSE
            END IF
         END IF
   ...
   IF p_ies_altura = 'S' THEN
      IF p_man.altura IS NULL THEN
         LET p_msg = 'DIMENSIONAL ALTURA NAO ENVIADO PELO TRIM - ', p_dim
         IF NOT pol0977_insere_erro() THEN
            RETURN FALSE
         END IF
      ELSE
         IF p_man.altura <> p_altura_ped THEN
            LET p_msg = 'DIMENSIONAL ALTURA DIFERENTE DO PEDIDO:',p_dim
            IF NOT pol0977_insere_erro() THEN
               RETURN FALSE
            END IF
         END IF
   ...
   IF p_ies_diametro = 'S' THEN
      IF p_man.diametro IS NULL THEN
         LET p_msg = 'DIMENSIONAL DIAMETRO NAO ENVIADO PELO TRIM - ', p_dim
         IF NOT pol0977_insere_erro() THEN
            RETURN FALSE
         END IF
      ELSE
         IF p_man.diametro <> p_diametro_ped THEN
            LET p_msg = 'DIMENSIONAL DIAMETRO DIFERENTE DO PEDIDO:',p_dim
            IF NOT pol0977_insere_erro() THEN
               RETURN FALSE
            END IF
         END IF
...
#----------------------------------#
FUNCTION pol0977_grava_apont_papel()
#----------------------------------#
...
#------------------------------#
FUNCTION pol0977_le_parametros()
#------------------------------#
...
#--------------------------------#          
FUNCTION pol0977_processa_apont()
#--------------------------------#          
   ...
   FOREACH cq_apont INTO p_man.*
      ...
      IF pol0977_consiste_dados() THEN
         LET p_item_ant = p_man.item
         IF pol0977_aponta_op() THEN
            LET p_retorno = TRUE
         END IF
      END IF

      IF NOT pol0977_grava_man() THEN
         LET p_retorno = FALSE
      END IF
...
#--------------------------------#
FUNCTION pol0977_consiste_dados()
#--------------------------------#
   ...
   ELSE
      ...
      ELSE
         ...
         IF NOT pol0977_le_desc_nat_oper_885() THEN
            RETURN FALSE
         ELSE         
   ...
   IF NOT pol0977_le_ord_oper() THEN
      RETURN FALSE
   END IF
   ...
   IF NOT pol0977_consiste_qtd_apont('L') THEN
      RETURN FALSE
   END IF
...
#-------------------------#
FUNCTION pol0977_de_para()
#-------------------------#
   
   DEFINE p_situac CHAR(01)
   ...
   LET p_situac = p_ies_situa
   LET p_num_transac = p_num_transac_o
   
   IF NOT pol0977_depara_item() THEN
      RETURN FALSE
   END IF
...
#------------------------------#
FUNCTION pol0977_depara_item()
#------------------------------#

   IF NOT pol0977_le_oper_orig() THEN
      RETURN FALSE
   END IF   
   ...
   #IF NOT pol0977_baixa_acessorios() THEN
   #   RETURN FALSE
   #END IF

   CALL pol0977_seta_orig_dest()

   IF NOT pol0977_update_estoque() THEN
      RETURN FALSE
   END IF
   ...
   IF NOT pol0977_le_oper_dest() THEN
      RETURN FALSE
   END IF   
   ...
   IF NOT pol0977_trata_fabricacao() THEN
      RETURN FALSE
   END IF

   IF NOT pol0977_insere_est_relac() THEN
      RETURN FALSE
   END IF

   IF NOT pol0977_insere_baixa() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
FUNCTION pol0977_baixa_acessorios()
#---------------------------------#
   ...
   IF NOT pol0977_baixa_estoque_lote() THEN
      RETURN FALSE
   END IF
...
#------------------------------------#
FUNCTION pol0977_baixa_estoque_lote()
#------------------------------------#
   ...
   IF STATUS = 100 THEN
      LET p_estorno = TRUE
      LET p_num_lote_orig = p_num_lote
      IF NOT pol0977_insere_lote() THEN
         RETURN FALSE
      END IF
      RETURN TRUE
   END IF

   CALL pol0977_update_ender()
   ...
   IF NOT pol0977_grava_estoq_trans() THEN
      RETURN FALSE
   END IF
...
#-----------------------------#
   ...
   IF STATUS = 100 THEN
      ...
      IF NOT pol0977_grava_erro() THEN
         RETURN FALSE
      END IF   
   ELSE
      IF STATUS <> 0 THEN
...
#-------------------------------------#
FUNCTION pol0977_le_equipto(p_equipto)
#-------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0977_consiste_qtd_apont(p_ch)
#----------------------------------------#
...
#---------------------------#
   ...
   CALL pol0977_aponta_na_zero() RETURNING p_status
...
#--------------------------------#
FUNCTION pol0977_aponta_na_zero()
#--------------------------------#

   IF p_man.tip_movto MATCHES "[FERS]" THEN
      IF p_ies_oper_final = "S"  OR p_man.tip_movto MATCHES '[RS]' THEN
         IF p_man.tip_movto MATCHES "[RS]" THEN
            CALL pol0977_move_estoq() RETURNING p_status
         ELSE
            LET p_cod_oper = 'E'
            CALL pol0977_entrada_prod() RETURNING p_status
         END IF
      END IF
   END IF

   IF p_status THEN
      CALL pol0977_grava_ordens() RETURNING p_status
   END IF
...
#------------------------------#
FUNCTION pol0977_grava_ordens()
#------------------------------#

   IF p_man.tip_movto MATCHES '[FRES]' THEN
      IF NOT pol0977_atualiza_ordens() THEN
         RETURN FALSE
      END IF
   END IF

   IF p_man.qtd_boas < 0 THEN
      IF NOT pol0977_deleta_tabs(p_man.operacao, p_man.sequencia_operacao) THEN
         RETURN FALSE
      END IF
      LET p_cod_tip_movto = 'R'
   ELSE
      LET p_cod_tip_movto = 'N'
      IF NOT pol0977_insere_tabs(p_man.operacao, p_man.sequencia_operacao) THEN
         RETURN FALSE
      END IF
      IF p_parametros[128,128] = "S" THEN
         IF NOT pol0977_integra_min() THEN
            RETURN FALSE
          END IF
      END IF
   END IF   
...
#---------------------------------#
FUNCTION pol0977_atualiza_ordens()
#---------------------------------#
   ...
   IF p_man.sequencia_operacao > 1 THEN
      ...
      IF NOT pol0977_nao_apontaveis() THEN
         RETURN FALSE
      END IF
   END IF
...
#--------------------------------#
FUNCTION pol0977_nao_apontaveis()
#--------------------------------#
   ...
   IF STATUS = 0 THEN
      ...
      ELSE #Marcelo pediu p/ não apontar apo_oper e suas parceiras - 27/07/09
         {IF p_man.qtd_boas < 0 THEN
            IF NOT pol0977_deleta_tabs(p_operacao,p_num_seq_ant) THEN
               RETURN FALSE
            END IF
         ELSE
            LET p_cod_tip_movto = 'N'
            IF NOT pol0977_insere_tabs(p_operacao,p_num_seq_ant) THEN
               RETURN FALSE
            END IF
         END IF}
      END IF
...
#-------------------------------------------------#
FUNCTION pol0977_deleta_tabs(p_cod_oper, p_num_seq)
#-------------------------------------------------#
...
#--------------------------------------------------#
FUNCTION pol0977_insere_tabs(p_cod_oper, p_num_seq)
#--------------------------------------------------#
...
#------------------------------#
   ...
   SELECT COUNT(cod_equip)
    ...
    IF p_count > 0 THEN
       IF NOT pol0977_atualiza_min(p_man.eqpto) THEN
        RETURN FALSE
      END IF
    END IF
   ...
   SELECT COUNT(cod_equip)
    ...
    IF p_count > 0 THEN
       IF NOT pol0977_atualiza_min(p_man.ferramenta) THEN
        RETURN FALSE
      END IF
    END IF
...
#------------------------------------------#
 FUNCTION pol0977_atualiza_min(l_cod_equip)
#------------------------------------------#
...
#----------------------------#
FUNCTION pol0977_move_estoq()
#----------------------------#

   IF p_man.tip_movto = 'S' THEN
      IF NOT pol0977_grava_sucata() THEN
         RETURN FALSE
      END IF
   ELSE
      IF NOT pol0977_le_item() THEN
         RETURN FALSE
      END IF
      ...
      LET p_cod_oper = 'E'
      IF NOT pol0977_entrada_prod() THEN
         RETURN FALSE
      END IF
      IF p_man.tip_movto = 'R' THEN
         IF NOT pol0977_transf_refugo() THEN
            RETURN FALSE
         END IF
      END IF
   END IF
...
#-------------------------------#
FUNCTION pol0977_transf_refugo()
#-------------------------------#
...
#----------------------------------#
FUNCTION pol0977_insere_est_relac()
#----------------------------------#
...
#------------------------------#
FUNCTION pol0977_baixa_consumo()
#------------------------------#

   LET p_est_relac = TRUE
   ...
   END IF
	 ...
	 IF p_cod_empresa = p_cod_emp_ofic THEN
      ...
      IF p_qtd_baixar > 0 THEN
         ...
         #baixa aparas dos itens alternativos

    	   IF NOT pol0977_bx_alternat() THEN
	          RETURN FALSE
	       END IF
...
#-----------------------------#
FUNCTION pol0977_bx_alternat()
#-----------------------------#

   DECLARE cq_alternat CURSOR FOR
    SELECT cod_item
      FROM apara_alternat_885
     WHERE cod_empresa = p_cod_empresa
      
   FOREACH cq_alternat INTO p_cod_prod
   
      IF STATUS <> 0 THEN                                                            
         LET p_msg = 'ERRO:(',STATUS, ') LENDO APARAS ALTERNATIVAS'                       
         RETURN FALSE                                                                
      END IF                                                                         
      ...
      FOREACH cq_lt INTO p_num_transac, p_num_lote, p_cod_local, p_qtd_lote, p_ies_situa
      
         IF STATUS <> 0 THEN                                                            
            LET p_msg = 'ERRO:(',STATUS, ') LENDO LOTES DE APARAS ALTERNATIVAS'                       
            RETURN FALSE                                                                
         END IF                                                                         

         IF p_qtd_lote < p_qtd_baixar THEN
            LET p_qtd_movto = p_qtd_lote
            LET p_qtd_baixar = p_qtd_baixar - p_qtd_lote   
         ELSE
            LET p_qtd_baixar = 0                                            
         END IF                                                                         
...
#----------------------------#
FUNCTION pol0977_up_estoque()
#----------------------------#

   IF p_tip_operacao = 'E' THEN
   ...
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') ATUALIZANDO TAB ESTOQUE:UE'
      RETURN FALSE
   END IF      
...
#--------------------------------#
FUNCTION pol0977_update_estoque()
#--------------------------------#
   ...
   IF NOT pol0977_atualiza_lote() THEN
      RETURN FALSE
   END IF
...
#------------------------------#
   ...
   IF p_ies_situa != 'R' THEN
    ...
    ELSE
      ...
      IF NOT pol0977_carrega_campos() THEN
         RETURN
      END IF
...
#--------------------------------#
FUNCTION pol0977_seta_orig_dest()
#--------------------------------#
...
#----------------------------------#
FUNCTION pol0977_trata_fabricacao()
#----------------------------------#

   IF NOT pol0977_grava_estoque() THEN
      RETURN FALSE
   END IF
   
   CALL pol0977_le_lote()
   ...
   IF STATUS = 0 THEN
      IF p_ies_refugo THEN
         ...
         END IF
      ELSE
         IF NOT pol0977_update_lote() THEN
            RETURN FALSE
         END IF
      END IF
   ELSE
...
#-------------------------------#
FUNCTION pol0977_grava_estoque()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol0977_atualiza_lote()
#-------------------------------#
...
#----------------------------#
   ...
   ELSE
      ...
      IF NOT pol0977_insere_ender() THEN
         RETURN FALSE
      END IF
   END IF 
...
#-------------------------------#
FUNCTION pol0977_carrega_campos()
#-------------------------------#
   ...
   IF NOT pol0977_le_item_ctr_grade(p_estoque_lote.cod_item) THEN
      RETURN FALSE
   END IF
...
#-----------------------------------#
FUNCTION pol0977_grava_estoq_trans()
#-----------------------------------#
   ...
   IF p_cod_operacao IS NULL THEN
      IF NOT pol0977_le_operacao() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF p_cod_tip_movto = 'R' AND p_num_transac_normal IS NULL THEN
      ...
      DECLARE cq_rev CURSOR FOR
       SELECT num_transac
        ...
        WHERE cod_empresa        = p_estoque_trans.cod_empresa
          ...
          AND num_prog           = p_estoque_trans.num_prog
          AND ies_tip_movto      = 'N'
...
#------------------------------------#
   ...
   IF NOT pol0977_grava_end() THEN
      RETURN FALSE
   END IF
   
   IF p_flag <> '3' AND p_man.qtd_boas > 0 THEN
      IF NOT pol0977_insere_chf_compon() THEN
         RETURN FALSE
      END IF
   END IF
...
#---------------------------#
  ...
  INSERT INTO estoque_auditoria 
     VALUES(p_cod_empresa, p_num_transac_orig, p_user, getdate(),'pol0977')
...
#-----------------------------------#
FUNCTION pol0977_insere_chf_compon()
#-----------------------------------#
...
#----------------------------------#
FUNCTION pol0977_elimina_estornos()
#----------------------------------#
...
#---------------------------------#
FUNCTION pol0977_importa_consumo()
#---------------------------------#
   ...
   DELETE FROM estoque_lote
    WHERE (cod_empresa = p_cod_emp_ger OR cod_empresa = p_cod_emp_ofic)
      AND qtd_saldo   <= 0
   ...
   IF NOT pol0977_elimina_estornos() THEN
      RETURN FALSE
   END IF
   ...
   DECLARE cq_baixa CURSOR WITH HOLD FOR
    SELECT codempresa,
     ...
     WHERE codempresa = p_cod_empresa
       AND statusregistro IN (0,2)
       AND datconsumo >= p_tela.dat_ini
       AND datconsumo <= p_tela.dat_fim
     ORDER BY numordem,
              coditem,
              numlote,
              datconsumo,
   ...
   FOREACH cq_baixa INTO 
      ...
      IF NOT pol0977_apaga_erros() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT p_criticou THEN  
         IF NOT pol0977_consiste_info() THEN
            IF NOT pol0977_grava_erro() THEN
               CALL log085_transacao("ROLLBACK")  
               RETURN FALSE
            END IF
      ...
      IF p_criticou THEN
      ELSE
         IF NOT pol0977_proces_info() THEN
            CALL log085_transacao("ROLLBACK")  
            CALL log085_transacao("BEGIN")  
            IF NOT pol0977_grava_erro() THEN
               CALL log085_transacao("ROLLBACK")  
      ...
      IF NOT pol0977_grava_cons_trim() THEN
         CALL log085_transacao("ROLLBACK")  
      ELSE
         CALL log085_transacao("COMMIT")  
      END IF
...
#-------------------------------#
FUNCTION pol0977_consiste_info()
#-------------------------------#
   ...
   IF p_consu.dat_consumo IS NOT NULL THEN
    ...
    IF p_dat_fecha_ult_man IS NOT NULL THEN
      IF p_consu.dat_consumo <= p_dat_fecha_ult_man THEN
         LET p_msg = 'CONSUMO APOS FECHAMENTO DA MANUFATURA NAO E PERMITIDO'
         IF NOT pol0977_grava_erro() THEN
            RETURN FALSE
         END IF
      END IF
    ...
    IF p_dat_fecha_ult_sup IS NOT NULL THEN
      IF p_consu.dat_consumo <= p_dat_fecha_ult_sup THEN
         LET p_msg = 'CONSUMO APOS FECHAMENTO DO ESTOQUE NAO E PERMITIDO'
         IF NOT pol0977_grava_erro() THEN
            RETURN FALSE
         END IF
      END IF
   ...
   IF p_consu.cod_item IS NULL OR p_consu.cod_item = 0 THEN
      ...
      END IF
   ELSE
      IF NOT pol0977_checa_item(p_consu.cod_item) THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF NOT pol0977_le_item_man() THEN
      RETURN FALSE
   END IF
   ...
   ELSE
      ...
      IF p_consu.cod_estorno = 0 THEN
         IF NOT pol0977_checa_estoque() THEN
            RETURN FALSE
         END IF
      ELSE
   ...
   IF p_consu.cod_itemrefugo IS NOT NULL THEN
      IF p_ies_apara = 'S' THEN
         IF NOT pol0977_checa_item(p_consu.cod_itemrefugo) THEN
            RETURN FALSE
         END IF
...
#--------------------------------------#
FUNCTION pol0977_checa_item(p_cod_item)
#--------------------------------------#
...
END FUNCTION
#-------------------------------#
FUNCTION pol0977_checa_estoque()
#-------------------------------#
   ...
   IF NOT pol0977_ve_estoque() THEN
      RETURN FALSE
   END IF
   
   IF p_sem_estoque THEN 
      IF p_ies_apara = 'S' THEN
         LET p_qtd_bx_oh = p_qtd_saldo
         LET p_sem_estoque = FALSE
      ELSE
         RETURN TRUE
      END IF
   ELSE
      LET p_qtd_bx_oh = p_qtd_baixar
   END IF
   ...
   IF p_ies_apara = 'S' THEN       #se faltar estoque no item original, vai baixar dos
      LET p_sem_estoque = FALSE    #itens alternativos e o item apara_nobre também é 1
      {IF p_sem_estoque THEN       #item alternativo
         IF NOT pol0977_ve_sdo_nobre() THEN
            LET p_cod_empresa = p_cod_emp_ger
            RETURN FALSE
         END IF
      END IF}
...
#-------------------------------#
   ...
   SELECT qtd_saldo,
    ...
    WHERE cod_empresa   = p_cod_empresa
      ...
      AND cod_local     = p_cod_local
      AND ies_situa_qtd = p_ies_situa
      AND ((num_lote = p_num_lote AND p_ctr_lote = 'S') OR
           (num_lote IS NULL AND p_ctr_lote = 'N'))
   ...
   IF p_qtd_saldo < p_qtd_baixar THEN
      LET p_qtd_dif = p_qtd_baixar - p_qtd_saldo
      IF p_qtd_dif < 1 THEN
         LET p_qtd_baixar = p_qtd_saldo
      ELSE
...
#-----------------------------#
FUNCTION pol0977_ve_sdo_nobre()
#-----------------------------#

   DEFINE p_sdo_nobre DECIMAL(10,3)
   
   SELECT cod_local_estoq
     INTO p_loc_est_nobre
     FROM item
    WHERE cod_empresa   = p_cod_empresa
      AND cod_item      = p_cod_apara_nobre
     
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') LENDO LOCAL ESTOQUE APARAS NOBRE'  
      RETURN FALSE
   END IF  
   ...
   SELECT qtd_saldo,
    ...
    WHERE cod_empresa   = p_cod_empresa
      AND cod_item      = p_cod_apara_nobre
      AND cod_local     = p_loc_est_nobre
      AND num_lote      = p_num_lote_nobre
      AND ies_situa_qtd = p_ies_situa

   IF STATUS <> 0 AND STATUS <> 100 THEN
      LET p_msg = 'ERRO:(',STATUS, ') LENDO SALDO APARAS NOBRE - ESTOQUE_LOTE'  
      RETURN FALSE
   END IF  
...
#---------------------------------#
FUNCTION pol0977_grava_cons_trim()
#---------------------------------#
...
#----------------------------#
FUNCTION pol0977_proces_info()
#----------------------------#

   LET p_man.ordem_producao = p_consu.num_ordem
   LET p_cod_oper   = 'B'
   LET p_est_relac  = FALSE
   ...
   IF NOT pol0977_procs_baixa() THEN
      RETURN FALSE
   END IF
...
#-----------------------------#
FUNCTION pol0977_procs_baixa()
#-----------------------------#

   IF p_ies_bobina AND p_consu.cod_estorno = 1 THEN
      IF NOT pol0977_estorna_mp() THEN
         RETURN FALSE
      END IF
      LET p_ies_bobina = FALSE
      RETURN TRUE
...
#--------------------------#
FUNCTION pol0977_baixa_mp()
#--------------------------#

   LET p_num_transac = p_num_transac_o

   IF NOT pol0977_baixa_consumo() THEN
      RETURN FALSE
   END IF
...
#---------------------------#
   ...
   DECLARE cq_estorna CURSOR FOR
    SELECT cod_empresa,
     ...
     WHERE cod_empresa IN (p_cod_emp_ger,p_cod_emp_ofic)
       AND num_seq_cons = p_num_seq_cons
     ORDER BY cod_empresa DESC, num_transac_et DESC
   ...
   FOREACH cq_estorna INTO 
      ...
      IF STATUS <> 0 THEN
         LET p_msg = 'ERRO:(',STATUS, ') LENDO A CONS_INSUMO_885:CQ_ESTORNA'
         RETURN FALSE
      END IF

      IF NOT pol0977_le_transacoes() THEN
         RETURN FALSE
      END IF

      IF p_tip_operacao IS NULL OR p_tip_operacao = ' ' THEN
         LET p_msg = 'TIPO DE OPERACAO INVALIDO NA TAB CONS_INSUMO_885'
         RETURN FALSE
      END IF
      ...
      IF STATUS = 100 THEN
         ...
         ELSE
            LET p_msg = 'ITEM',p_cod_prod CLIPPED,
                        ' S/ ESTOQ P/ ESTORNAR NA ESTOQU_LOTE'
            RETURN FALSE
         END IF
      ELSE
         IF STATUS = 0 THEN
            IF p_tip_operacao = 'S' THEN
               ...
               CALL pol0977_up_lote()
            ELSE
               IF p_qtd_movto <= p_qtd_lote THEN
                  CALL pol0977_up_lote()
               ELSE
                  LET p_msg = 'ITEM',p_cod_prod CLIPPED,
                              ' S/ ESTOQ P/ ESTORNAR NA ESTOQU_LOTE'
            ...
            END IF
         ELSE
            LET p_msg = 'ERRO:(',STATUS, ') LENDO A ESTOQUE_LOTE:CQ_ESTORNA'
            RETURN FALSE
         END IF      
      END IF
      
      IF STATUS <> 0 THEN
         LET p_msg = 'ERRO:(',STATUS, ') ATUALIZANDO TAB ESTOQUE_LOTE:CQ_ESTORNA'
         RETURN FALSE
      END IF
      ...
      IF STATUS = 100 THEN
         ...
         ELSE
            LET p_msg = 'ITEM',p_cod_prod CLIPPED,
                        ' S/ ESTOQ P/ ESTORNAR NA ESTOQU_LOTE_ENDER'
            RETURN FALSE
         END IF
      ELSE
         IF STATUS = 0 THEN
            ...
            END IF            
         ELSE
            LET p_msg = 'ERRO:(',STATUS, ') LENDO A ESTOQUE_LOTE_ENDER:CQ_ESTORNA'
            RETURN FALSE
         END IF      
      END IF
      
      IF NOT pol0977_up_estoque() THEN
         RETURN FALSE
      END IF
         
   END FOREACH
   
   DELETE FROM baixa_consu_885
    WHERE cod_empresa IN (p_cod_emp_ger,p_cod_emp_ofic)
      AND num_seq_cons = p_num_seq_cons
   
   IF STATUS <> 0 THEN
      CALL log003_err_sql('deletando','baixa_consu_885')
      RETURN FALSE
   END IF
...
#-------------------------------#
FUNCTION pol0977_le_transacoes()
#-------------------------------#
   ...
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') LENDO estoque_trans:lt'  
      RETURN FALSE
   END IF

   IF p_tip_operacao IS NULL OR p_tip_operacao = ' ' THEN
      IF p_estoque_trans.cod_operacao = 'BPRD' OR
         p_estoque_trans.cod_operacao = 'STRC' THEN
         LET p_tip_operacao = 'S'
      ELSE
         IF p_estoque_trans.cod_operacao = 'PARA' THEN
            LET p_tip_operacao = 'E'
         END IF
      END IF
   END IF
   ...
   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') LENDO estoque_trans_end:lt'  
      RETURN FALSE
   END IF
...
#--------------------------------------#
FUNCTION pol0977_le_desc_nat_oper_885()
#--------------------------------------#
...
#-------------------------------#
FUNCTION pol0977_cheka_estoque()
#-------------------------------#
...
#--------------------------------#
 FUNCTION pol0977_verifica_erros()
#--------------------------------#
...
#---------------------------#
 FUNCTION pol0977_consultar()
#---------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol09771") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol09771 AT 2,2 WITH FORM p_nom_tela
...
#---------------------------------#
FUNCTION pol0977_exibe_prendentes()
#---------------------------------#
   ...
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0977a") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol0977a AT 04,14 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
...
#--------------------------#
FUNCTION pol0977_listagem()#
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol0977b") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0977b AT 5,7 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
   
   LET p_status = FALSE
   
   IF pol0977_parametros() THEN
      IF pol0977_cria_tab() THEN
         IF pol0977_coleta_dados() THEN
            IF pol0977_imprimir() THEN
               LET p_status = TRUE
            END IF
         END IF
      END IF
...
#----------------------------#
FUNCTION pol0977_parametros()#
#----------------------------#

   LET INT_FLAG = FALSE 
   
   INITIALIZE m_tela.* TO NULL 
   LET m_tela.sumarizar = 'N'
   LET m_tela.deletar = 'N'
   
   INPUT BY NAME m_tela.* WITHOUT DEFAULTS 
   
   AFTER FIELD cod_familia
      IF m_tela.cod_familia IS NOT NULL THEN  
         SELECT den_familia
           INTO p_den_familia
           FROM familia
          WHERE cod_empresa = p_cod_empresa
            AND cod_familia = m_tela.cod_familia
   ...
   AFTER FIELD cod_item
      IF m_tela.cod_item IS NOT NULL THEN  
         SELECT den_item_reduz
           INTO p_den_item_reduz
           FROM item
          WHERE cod_empresa = p_cod_empresa
            AND cod_item    = m_tela.cod_item
   ...
   AFTER FIELD dat_ini
      IF m_tela.dat_ini > TODAY THEN 
         ERROR "A data inicial não pode ser maior que a data atual !!!"
         NEXT FIELD dat_ini
      END IF 
      
   AFTER INPUT 
      IF m_tela.dat_ini IS NOT NULL AND m_tela.dat_fim IS NOT NULL THEN 
   ...
   CASE
      WHEN INFIELD(cod_familia)
         ...
         IF p_codigo IS NOT NULL THEN
            LET m_tela.cod_familia = p_codigo CLIPPED
...
#--------------------------#
FUNCTION pol0977_cria_tab()#
#--------------------------#

   DROP TABLE criticado_tmp_885;
   
   CREATE TEMP TABLE criticado_tmp_885(
       numsequencia         INTEGER,
       datconsumo           DATE,
       coditem              CHAR(15),
       numlote              CHAR(15),
	 ...
	 IF STATUS <> 0 THEN 
			CALL log003_err_sql("CRIANDO","criticado_tmp_885")
			RETURN FALSE
	 END IF
...
#------------------------------#
   ...
   LET p_query =
   "SELECT cons_insumo_885.numsequencia, cons_insumo_885.datconsumo, ",
           "cons_insumo_885.coditem, cons_insumo_885.numlote, ",
   "       cons_insumo_885.qtdconsumida, cons_insumo_885.numordem, item.den_item_reduz ",
   "  FROM cons_insumo_885, item ",
   " WHERE cons_insumo_885.codempresa = '",p_cod_empresa,"' ",
   "   AND cons_insumo_885.iesrefugo = 'N' ",
   "   AND cons_insumo_885.numsequencia IN ( ",
   "       SELECT DISTINCT cons_erro_885.numsequencia FROM cons_erro_885 ",
   "        WHERE cons_erro_885.codempresa = cons_insumo_885.codempresa) ",
   "   AND item.cod_empresa = cons_insumo_885.codempresa ",
   "   AND item.cod_item = cons_insumo_885.coditem "
      
   IF m_tela.cod_familia IS NOT NULL THEN
      LET p_query = p_query CLIPPED, " AND item.cod_familia = '",m_tela.cod_familia,"' "
   END IF

   IF m_tela.cod_item IS NOT NULL THEN
      LET p_query = p_query CLIPPED, " AND item.cod_item = '",m_tela.cod_item,"' "
   END IF
   
   IF m_tela.dat_ini IS NOT NULL THEN
      LET p_query = p_query CLIPPED, " AND cons_insumo_885.datconsumo >= '",m_tela.dat_ini,"' "
   END IF

   IF m_tela.dat_fim IS NOT NULL THEN
      LET m_tela.dat_fim = m_tela.dat_fim + 1
      LET p_query = p_query CLIPPED, " AND cons_insumo_885.datconsumo <= '",m_tela.dat_fim,"' "
   END IF

   LET p_query = p_query CLIPPED, " ORDER BY cons_insumo_885.DATCONSUMO, ",
     " cons_insumo_885.coditem, cons_insumo_885.numlote, cons_insumo_885.numordem "

   PREPARE v_query FROM p_query   
   
   IF STATUS <> 0 THEN
      CALL log003_err_sql('PREPARE', 'v_query')
      RETURN FALSE
   END IF
   
   DECLARE cq_criticados CURSOR FOR v_query
   
   FOREACH cq_criticados INTO 
           p_relat.numsequencia,
           p_datconsumo,    
           p_relat.coditem,       
           p_relat.numlote,       
           p_relat.qtdconsumida,  
           p_relat.numordem,      
           p_relat.den_item_reduz

      IF STATUS <> 0 THEN
         CALL log003_err_sql('FOREACH', 'cq_criticados')
         RETURN FALSE
      END IF

      LET p_relat.datconsumo = DATE(p_datconsumo)  
      
      INSERT INTO criticado_tmp_885
       VALUES(p_relat.*)
       
      IF STATUS <> 0 THEN
         CALL log003_err_sql('INSERT', 'criticado_tmp_885')
         RETURN FALSE
      END IF
...
#--------------------------------#
 FUNCTION pol0977_le_den_empresa()
#--------------------------------#
...
#--------------------------#
FUNCTION pol0977_imprimir()#
#--------------------------#
         
   IF NOT pol0977_le_den_empresa() THEN
      RETURN FALSE 
   END IF   
   ...
   IF m_tela.sumarizar = 'S' THEN
      CALL pol0977_imp_sumarizado() RETURNING p_status
   ELSE
      CALL pol0977_imp_detalhado() RETURNING p_status
   END IF
   ...
   IF p_status AND p_count > 0 THEN
      IF m_tela.deletar = 'S' THEN
         CALL log085_transacao("BEGIN") 
         IF NOT pol0977_del_criticados() THEN
            CALL log085_transacao("ROLLBACK") 
         ELSE
            CALL log085_transacao("COMMIT") 
         END IF
...
#--------------------------------#
FUNCTION pol0977_imp_sumarizado()#
#--------------------------------#

   IF p_ies_impressao = "S" THEN
      IF g_ies_ambiente = "U" THEN
         START REPORT pol0977_sumariz TO PIPE p_nom_arquivo
      ELSE
         CALL log150_procura_caminho ('LST') RETURNING p_caminho
         LET p_caminho = p_caminho CLIPPED, 'pol0977.tmp'
         START REPORT pol0977_sumariz  TO p_caminho
      END IF
   ELSE
      START REPORT pol0977_sumariz TO p_nom_arquivo
   END IF

   INITIALIZE p_relat TO NULL
   
   DECLARE cq_sumarizado CURSOR FOR
    SELECT coditem, numlote, SUM(qtdconsumida) 
      FROM criticado_tmp_885 
     GROUP BY coditem, numlote
   
   FOREACH cq_sumarizado INTO 
           p_relat.coditem, 
           p_relat.numlote, 
           p_relat.qtdconsumida
           
      IF STATUS <> 0 THEN
         CALL log003_err_sql('FOREACH', 'cq_sumarizado')
         RETURN FALSE
      END IF
      ...
      OUTPUT TO REPORT pol0977_sumariz() 
      
      LET p_count = 1
      
   END FOREACH

   FINISH REPORT pol0977_sumariz  
...
#----------------------#
   ...
   FORMAT
      ...
      FIRST PAGE HEADER
         ...
         PRINT COLUMN 001, "POL0977",
               COLUMN 012, "CONSUMOS CRITICADOS - ENVIADOS PELO TRIM",
               COLUMN 056, "EMIS: ", TODAY USING "dd/mm/yyyy", " ", TIME
         PRINT COLUMN 001, "--------------------------------------------------------------------------------"
         PRINT
         PRINT COLUMN 001, '          ITEM            DESCRICAO          LOTE            QUANTIDADE'
         PRINT COLUMN 001, '          --------------- ------------------ --------------- -----------'
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 011, p_relat.coditem,
               COLUMN 027, p_relat.den_item_reduz,
               COLUMN 046, p_relat.numlote,
               COLUMN 062, p_relat.qtdconsumida
...
#-------------------------------#
FUNCTION pol0977_imp_detalhado()#
#-------------------------------#

   IF p_ies_impressao = "S" THEN
      IF g_ies_ambiente = "U" THEN
         START REPORT pol0977_detalhe TO PIPE p_nom_arquivo
      ELSE
         CALL log150_procura_caminho ('LST') RETURNING p_caminho
         LET p_caminho = p_caminho CLIPPED, 'pol0977.tmp'
         START REPORT pol0977_detalhe  TO p_caminho
      END IF
   ELSE
      START REPORT pol0977_detalhe TO p_nom_arquivo
   END IF

   INITIALIZE p_relat TO NULL
   
   DECLARE cq_detalhe CURSOR FOR
    SELECT * 
      FROM criticado_tmp_885 
   
   FOREACH cq_detalhe INTO p_relat.*
           
      IF STATUS <> 0 THEN
         CALL log003_err_sql('FOREACH', 'cq_detalhe')
         RETURN FALSE
      END IF
            
      OUTPUT TO REPORT pol0977_detalhe() 
      
      LET p_count = 1
      
   END FOREACH

   FINISH REPORT pol0977_detalhe  
...
#----------------------#
   ...
   FORMAT
      ...
      FIRST PAGE HEADER
         ...
         PRINT COLUMN 001, "--------------------------------------------------------------------------------"
         PRINT
         PRINT COLUMN 001, 'DT CONSUMO ITEM            DESCRICAO          LOTE            QUANTIDAD ORDEM'
         PRINT COLUMN 001, '---------- --------------- ------------------ --------------- --------- --------'
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 001, p_relat.datconsumo,
               COLUMN 012, p_relat.coditem,
               COLUMN 028, p_relat.den_item_reduz,
               COLUMN 047, p_relat.numlote,
               COLUMN 063, p_relat.qtdconsumida USING '####&.&&&',
               COLUMN 073, p_relat.numordem USING '#######&'
...
#--------------------------------#
FUNCTION pol0977_del_criticados()#
#--------------------------------#
   
   DEFINE m_msg  CHAR(300)
          
   LET m_msg = 'Confirma a exclusão dos\n',
               'registros listados?'
   IF NOT log0040_confirm(20,25,m_msg) THEN
      RETURN FALSE
   END IF

   DELETE FROM cons_erro_885 WHERE codempresa = p_cod_empresa
      AND numsequencia IN (SELECT DISTINCT numsequencia FROM criticado_tmp_885)
      
   IF STATUS <> 0 THEN
      CALL log003_err_sql('DELETE','cons_erro_885')
      RETURN FALSE
   END IF

   DELETE FROM cons_insumo_885 WHERE codempresa = p_cod_empresa
      AND numsequencia IN (SELECT DISTINCT numsequencia FROM criticado_tmp_885)
      
   IF STATUS <> 0 THEN
      CALL log003_err_sql('DELETE','cons_insumo_885')
      RETURN FALSE
   END IF
