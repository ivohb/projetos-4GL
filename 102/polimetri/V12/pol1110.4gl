#------------------------------------------------------------------------------#
# SISTEMA.: INTEGRAÇÃO DO LOGIX x EGA - MAN912E                                #
# PROGRAMA: pol1110                                                            #
# OBJETIVO: IMPORTAÇÃO DO EGA x LOGIX 10.02                                    #
# DATA....: 18/02/2010                                                         #
# ALTERADO:                                                                    #
#------------------------------------------------------------------------------#
 DATABASE logix
 ...
 DEFINE  p_den_prog    CHAR(7)

MAIN
   CALL log0180_conecta_usuario()
   
   LET p_versao = 'pol1110-12.00.00  ' 
   ...
   LET m_caminho = log140_procura_caminho('pol1110.iem')
...
{  CALL log001_acessa_usuario("ESPEC999","")
   ...
   CALL log130_procura_caminho("pol1110") RETURNING m_caminho
   LET m_caminho = m_caminho CLIPPED 
   OPEN WINDOW w_pol1110 AT 2,2 WITH FORM m_caminho
       ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   DISPLAY p_cod_empresa TO cod_empresa

   IF  p_status = 0 THEN
       CALL pol1110_controle()
   END IF

   CLOSE WINDOW w_pol1110
   
END MAIN

#--------------------------#
 FUNCTION pol1110_controle()
#--------------------------#
	DEFINE p_hora_atu            LIKE proc_apont_man912.hor_ini

   IF NUM_ARGS() > 0  THEN
      LET p_den_prog = ARG_VAL(1)
   ELSE
      LET p_den_prog = NULL
   END IF
...
#---------------------------#
FUNCTION pol1110_processa()
#---------------------------#

   IF NOT pol1110_cria_temp() THEN
      RETURN
   END IF
   
   CALL log085_transacao("BEGIN")

   IF NOT pol1110_elimina_duplicidade() THEN
      CALL log085_transacao("ROLLBACK")
      RETURN
   END IF
   ...
   IF NOT pol1110_grava_historico() THEN
      CALL log085_transacao("ROLLBACK")
      RETURN
   END IF

   IF NOT pol1110_monta_seq_oper() THEN
      CALL log085_transacao("ROLLBACK")
      RETURN 
   END IF
   ...
   IF NOT pol1110_importa_dados() THEN  #Consiste e carrega os apontamento para
...
#------------------------------------#
FUNCTION pol1110_elimina_duplicidade()
#------------------------------------#
   ...
   INSERT INTO apont_temp
    SELECT DISTINCT *
     ...
     WHERE chav_seq IS NULL
       ...
       AND (qtd_boas > '00000000' OR qtd_refugo > '00000000')
...
#---------------------------------#
FUNCTION pol1110_grava_historico()
#---------------------------------#
   ...
   FOREACH cq_hist INTO p_aponta.*,p_rowid
      ...
      #ivo - 20/09/2011 - Ega não manda o item
      select cod_item
        into p_cod_item
        from ordens
       WHERE cod_empresa = p_cod_empresa
      ...
      INSERT INTO apont_hist_man912
       VALUES(p_chav_seq,1,
              ...
              p_aponta.cod_mov,
              p_aponta.arq_orig,
              NULL,"pol1110","pol1110",p_cod_empresa)

      IF STATUS <> 0 THEN 
         CALL log003_err_sql("INCLUSÃO","APONT_HIST_MAN912")
         RETURN FALSE
      END IF
...
#--------------------------------#
FUNCTION pol1110_monta_seq_oper()
#--------------------------------#
   ...
   FOREACH cq_sequenica INTO
      ...
      IF STATUS <> 0 THEN 
         CALL log003_err_sql("UPDATE","apont_ega_man912 2")
         RETURN FALSE
      END IF
...
#--------------------------------#
 FUNCTION pol1110_importa_dados()
#--------------------------------#
   ...
   FOREACH cq_importa INTO p_aponta.*
      ...
      LET p_tex_observ = NULL
	    ...
	    IF   p_aponta.num_op  = '000000000'   THEN 
			IF NOT pol1110_deleta_apont() THEN
				RETURN FALSE
			END IF
	        CONTINUE FOREACH
	    END IF 	 
     ...
     SELECT COUNT(chav_seq)
       ...
       WHERE cod_empresa = p_cod_empresa          
         ...
         AND dat_producao=  p_aponta.dat_producao
      ...
      END IF           		
	    ...
	    if p_qtd_nivel_aen > 0 then #filtra apontamento por area e linha
         ...
         select cod_lin_prod,
	       ...
	       if status <> 0 then
			      LET m_msg = 'OP ou Item NÃO CADASTRADO - pol1110'
            CALL pol1110_insere_erro()
            CONTINUE FOREACH
	       end if
	       ...
	       if status = 100 then
            IF NOT pol1110_envia_hist() THEN
               RETURN FALSE
            END IF
	          CONTINUE FOREACH
	       else
      ...
      SELECT cod_operac
        ...
        FROM oper_ega_man912
       WHERE cod_empresa    = p_cod_empresa
         AND cod_operac_ega = p_aponta.cod_operac

      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'OPERACAO NÃO CADASTRADA NA TAB OPER_EGA_MAN912 - pol1110'
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF
      ...
      IF l_qtd_boas < l_qtd_refugo THEN
         LET m_msg = 'QTD PRODUZIDAS MENOR QUE QTD REFUGADAS - pol1110'
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF #ivo - 20/09/2011 - até aqui
      
      IF pol1110_consiste_ordem() = FALSE THEN
   ...
   #   DISPLAY 'Ordem: ', p_num_ordem AT 12,20
      ...
      IF p_qtd_ordem > 0 THEN
         LET m_msg = 'INCONSISTIDO POR CONSEQUENCIA DE ERRO ANTERIOR - pol1110'
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF      

      IF p_aponta.num_seq_operac IS NULL THEN
         LET m_msg = 'OPERACAO SEM NUMERO DE SEQUENCIA - pol1110'
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF      

      #ivo - 20/09/2011 - Ega só mada fabricação
      IF p_aponta.tip_mov <> 'F' then
          LET m_msg = 'TIPO DE MOVIMENTO INVÁLIDO - pol1110: ', p_aponta.tip_mov
          CALL pol1110_insere_erro()
          CONTINUE FOREACH
      END IF 
         
      IF pol1110_consiste_operacao() = FALSE THEN
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF
      ...
      CALL pol1110_consiste_dados() RETURNING p_status
      ...
      IF NOT pol1110_consiste_gemeas() THEN
         CONTINUE FOREACH
      END IF

      LET p_tex_observ = NULL

      IF NOT pol1110_le_qtd_planej() THEN
         LET m_msg = 'ERRO AO LER A QTD.PLANEJADA DA ORDEM - pol1110'
         CALL pol1110_insere_erro()
         CONTINUE FOREACH
      END IF

      CALL pol1110_calc_dat_hor()
            
      LET p_qtd_hor = (p_qtd_seg_fim - p_qtd_seg_ini) / 3600

# ALTERADO POR MANUEL EM 12-11-1012 A PEDIDO DO WARLEY E MARCIO, PARA DIVIDIR O TEMPO TOTAL PELO NUMERO DE SIMETRICAS	  

	  IF p_qtd_gemea   > 0 THEN 
	     LET p_qtd_hor = p_qtd_hor / (p_qtd_gemea + 1)
	  END IF 
      ...
      if p_qtd_prod > 0 then
         IF NOT pol1110_tem_material() THEN     
            CALL pol1110_insere_erro()
            CONTINUE FOREACH
         END IF
      end if
        
      IF NOT pol1110_grava_man() THEN
         RETURN FALSE
      END IF

      IF p_tem_gemea THEN
         IF NOT pol1110_grava_gem() THEN
            RETURN FALSE
         END IF
      END IF
...
#----------------------------#
FUNCTION pol1110_erro_gemea()
#----------------------------#

     LET m_msg = 'TRANSFERENCIA DE BOAS P/ REJEITADA SO DA PECA PRINCIPAL - pol1110'
...
#------------------------------#
   ...
   FOREACH cq_comp INTO 
      ...
      IF p_qtd_saldo IS NULL OR p_qtd_saldo = 0 THEN
         LET m_msg = 'COMP.',p_cod_item,' SEM MATERIAL SUF LOC PROD-pol1110'
         RETURN FALSE
      END IF
      
      IF p_qtd_saldo < p_qtd_baixar THEN
         LET m_msg = 'COMP.',p_cod_item,' S/MATERIAL SUF LOC PROD-pol1110'
         RETURN FALSE
      END IF
   ...
   IF p_tem_gemea THEN
      IF NOT pol1110_gemea_t_estoq() THEN
         RETURN FALSE
      END IF
   END IF
...
#-------------------------------#
FUNCTION pol1110_gemea_t_estoq()
#-------------------------------#
   ...
   DEFINE p_local_baixa   LIKE ord_compon.cod_local_baixa,
          ...
          p_apont_gemea   LIKE man_apont_454.qtd_boas

   let p_apont_gemea = p_qtd_prod         
   ...
   FOREACH cq_est_gem INTO p_cod_peca_gemea, p_qtd_peca_gemea
      ...
      LET p_qtd_apontar = p_apont_gemea * p_qtd_peca_gemea
         ...
         END IF
            ...
            LET p_mat_empenhado = p_qtd_geme_apo * p_qtd_necessaria
			      ...
			      IF p_qtd_saldo < p_qtd_mat_neces THEN
			         LET m_msg = 'OP SIMETRICA: ',p_num_opg,' COMP.',p_cod_item,' Sem MATERIAL SUF LOC PROD-pol1110'
			         RETURN FALSE
			      END IF
...
#----------------------------------#
FUNCTION pol1110_calcula_apontadas()
#----------------------------------#
...
#---------------------------------#
FUNCTION pol1110_le_qtd_planej()
#---------------------------------#
...
{
#--------------------------------#
FUNCTION pol1110_compatibiliza_op()
#--------------------------------#
...
#----------------------------#
   ...
   INSERT INTO man_apont_hist_454
    VALUES(man_apont.*,'I','pol1110', 'pol1110')
   ...
   IF p_chamada <> 'G' THEN
      IF NOT pol1110_atualiza_tabs() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------#
FUNCTION pol1110_atualiza_tabs()
#------------------------------#
...
#--------------------------------#
FUNCTION pol1110_consiste_gemeas()
#--------------------------------#
   ...
   FOREACH cq_peca INTO p_cod_peca_gemea
      ...
      FOREACH cq_op INTO p_op_gemea, p_qtd_planej, p_dat_entrega
		...
		END IF
         ...
         IF SQLCA.SQLCODE <> 0 THEN 
            CALL log003_err_sql("inclusão","ordens_gemeas")
            RETURN FALSE
         END IF      
...
#--------------------------------#
 FUNCTION pol1110_consiste_dados()
#--------------------------------#
      
      LET p_retorno = TRUE
      
      IF pol1110_consiste_item() = FALSE THEN
         CALL pol1110_insere_erro()
         LET p_retorno = FALSE
      END IF
      
      #ivo - 20/09/2011 - EGA não manda máquina
      
      {IF pol1110_consiste_maquina() = FALSE THEN
         CALL pol1110_insere_erro()
         LET p_retorno = FALSE
      END IF} #ivo - 20/09/2011 - até aqui

      IF pol1110_consiste_matricula() = FALSE THEN
         CALL pol1110_insere_erro()
         LET p_retorno = FALSE
      END IF
...
#-----------------------------------#
 FUNCTION pol1110_consiste_operacao()
#-----------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'OPERACAO ',p_cod_operac CLIPPED, 
                  ' NÃO CADASTRADA NA TAB OPERACAO - pol1110'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET m_msg = 'OPERACAO FINAL NÃO CADASTRADA - ORD_OPER - pol1110'
      RETURN FALSE
   END IF
   ...
   IF p_aponta.cod_mov = '01120' AND
      ...
      IF p_ies_apontamento <> 'F' THEN
         LET m_msg = 'TENTATIVA DE ENCERRAR OPER S/ ENCERRAR OPER ANT - pol1110'
         RETURN FALSE
      END IF
   END IF 
...
#--------------------------------#
 FUNCTION pol1110_consiste_ordem()
#--------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ORDEM DE PRODUÇÃO NÃO EXISTE - pol1110'
      CALL pol1110_insere_erro()
      RETURN FALSE
   END IF
   ...
   ELSE
      ...
      IF p_envia_hist = 'N' THEN
         LET m_msg = 'ORDEM DE PRODUÇÃO NÃO ESTA EM PRODUCAO - pol1110'
         CALL pol1110_insere_erro()
      END IF
      RETURN FALSE
   END IF
   ...
   IF LENGTH(p_cod_cent_trab) = 0 THEN
      LET m_msg = 'CENTRO DE TRABALHO INVALIDO NA ORD_OPER NA OP ', p_aponta.num_op, ' - pol1110'
      CALL pol1110_insere_erro()
      RETURN FALSE
   END IF
      
   IF LENGTH(p_cod_arranjo) = 0 or p_cod_arranjo = ' ' THEN
      LET m_msg = 'ARRANJO INVALIDO NA ORD_OPER - pol1110'
      CALL pol1110_insere_erro()
      RETURN FALSE
   END IF

   #ivo - 20/09/2011 - o Ega na envia a máquina
...
#-------------------------------#
 FUNCTION pol1110_consiste_item()
#-------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ITEM: ', p_aponta.cod_item, ' NÃO CADASTRADO - pol1110'
      RETURN FALSE
   END IF
...
#----------------------------------#
 FUNCTION pol1110_consiste_maquina()
#----------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_aponta.cod_maquina,
                  ' NÃO CADASTRADA - MAQ_EGA_MAN912 - pol1110'
      RETURN FALSE
   END IF

   IF p_cod_equip IS NULL THEN
      LET m_msg = 'COD.EQPTO. NULLO NA TABELA MAQ_EGA_MAN912 - pol1110'
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_cod_maquina,' NÃO CADASTRADA - RECURSO - pol1110'
      RETURN FALSE
   END IF
...
#------------------------------------#
 FUNCTION pol1110_consiste_matricula()
#------------------------------------#
   ...
   IF sqlca.sqlcode <> 0 or p_aponta.mat_operador is null or p_aponta.mat_operador = ' ' THEN #ivo - 20/09/2011
      ...
      IF STATUS <> 0 THEN
         LET m_msg = 'MATRIC.OPERADOR:', p_aponta.mat_operador,
                     ' NÃO CADASTRADA - FUNCIONARIO - pol1110'
         RETURN FALSE
      ELSE
         ...
         IF STATUS <> 0 THEN
            LET m_msg = 'OPERADOR PADRAO:', p_aponta.mat_operador,
                        ' NÃO CADASTRADO - FUNCIONARIO - pol1110'
            RETURN FALSE
         END IF
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol1110_consiste_movto()
#--------------------------------#
...
#----------------------------------#               
FUNCTION pol1110_ajusta_neces_gem()               
#----------------------------------#               
   ...
   FOREACH cq_pec_gem INTO p_cod_peca_gemea, p_qtd_peca_gemea
      ...
      FOREACH cq_neces_gem INTO p_num_ordem
         ...
         IF NOT pol1110_ajusta_necessidades() THEN
            RETURN FALSE
         END IF
...
#--------------------------------------#
FUNCTION pol1110_ajusta_necessidades()
#--------------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS, ' ATUALIZANDO TABELA ORDENS - pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS, ' INSERINDO NA AUDIT_LOGIX - pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS, ' ATUALIZANDO TABELA ORD_OPER- pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   FOREACH cq_comp_neces INTO 
      ...
      IF NOT pol1110_movimenta_estoque() THEN
         RETURN FALSE
      END IF
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS, ' ATUALIZANDO TABELA NECECIDADES- pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
...
#-----------------------------------#
 FUNCTION pol1110_movimenta_estoque()
#-----------------------------------#
   ...
   FOREACH cq_lot INTO 
      ...
      IF p_novo_saldo = 0 THEN
         IF NOT pol1110_deleta_lote() THEN
            RETURN FALSE
         END IF
      ELSE
         IF NOT pol1110_atualiza_lote() THEN
            RETURN FALSE
         END IF
      END IF
...
#------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS, ' DELETANDO DA ESTOUQE_LOTE - pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS, ' DELETANDO DA ESTOUQE_LOTE_ENDER-pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   ELSE
      IF NOT pol1110_atualiza_local_prod() THEN
         RETURN FALSE
      END IF
   END IF

   IF NOT pol1110_insere_est_trans() THEN
      RETURN FALSE
   END IF
...
#------------------------------#
 FUNCTION pol1110_atualiza_lote()
#------------------------------#
   ...
   IF p_num_lote IS NULL THEN
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS, ' ATUALISANDO A ESTOUQE_LOTE-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS, ' ATUALISANDO A ESTOUQE_LOTE_ENDER-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
   ...
   ELSE
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE_LOTE-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE_LOTE_ENDER-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
...
#--------------------------------------#
FUNCTION  pol1110_atualiza_local_prod()
#--------------------------------------#
   ...
   IF SQLCA.sqlcode = NOTFOUND THEN
      IF NOT pol1110_insere_local_prod() THEN
         RETURN FALSE
      END IF
   ELSE
      IF NOT pol1110_altera_local_prod() THEN
         RETURN FALSE
      END IF
   END IF
...
#-----------------------------------#
FUNCTION  pol1110_insere_local_prod()
#-----------------------------------#
      ...
      INSERT INTO estoque_lote
          VALUES (p_cod_empresa,
                  ...
                  0)
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' INSERINDO ESTOQUE_LOTE-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' INSERINDO ESTOQUE_LOTE_ENDER-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
...
#-----------------------------------#
FUNCTION  pol1110_altera_local_prod()
#-----------------------------------#
...
#----------------------------------#
 FUNCTION pol1110_insere_est_trans()
#----------------------------------#
   ...
   LET mr_estoque_trans.num_prog           = "POL1110"
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ERRO ',STATUS,' INSERINDO ESTOQUE_TRANS-pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF p_tex_observ IS NOT NULL THEN
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' INSERINDO ESTOQUE_OBS-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
   ...
   IF NOT pol1110_ins_est_trans_end() THEN
      RETURN FALSE
   END IF

   IF p_mov_mat = 'M' THEN
      IF NOT pol1110_insere_op_lote() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------------#
 FUNCTION pol1110_ins_est_trans_end()
#------------------------------------#
   ...
   IF SQLCA.SQLCODE <> 0 THEN
      LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE_TRANS_END-pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
  ...
  INSERT INTO estoque_auditoria 
     VALUES(p_cod_empresa, m_num_transac_orig, p_user, TODAY,'pol1110')

  IF SQLCA.SQLCODE <> 0 THEN 
     LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE_AUDITORIA-pol1110'
     CALL pol1110_insere_erro()
     LET p_sem_estoque = TRUE
     RETURN FALSE
  END IF
...
#--------------------------------#
 FUNCTION pol1110_insere_op_lote()
#--------------------------------#
   ...
   IF p_num_lote IS NULL THEN
      SELECT endereco
       ...
       WHERE cod_empresa     = p_cod_empresa
         ...
         AND num_lote IS NULL
      IF STATUS <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' LENDO ESTOQUE_LOTE_ENDER-pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
      ...
      IF STATUS <> 0 AND STATUS <> 100 THEN
         LET m_msg = 'ERRO ',STATUS,' LENDO OP_LOTE - pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
   ...
   IF STATUS = 0 THEN         
      UPDATE op_lote
       ...
       WHERE rowid = p_row
      IF SQLCA.SQLCODE <> 0 THEN
         LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO OP_LOTE - pol1110'
         CALL pol1110_insere_erro()
         LET p_sem_estoque = TRUE
         RETURN FALSE
      END IF
   ...
   IF SQLCA.SQLCODE <> 0 THEN
      LET m_msg = 'ERRO ',STATUS,' INSERINDO NA OP_LOTE - pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
...
#------------------------------#
FUNCTION pol1110_tira_das_boas()
#------------------------------#
   ...
   IF NOT pol1110_tem_estoque() THEN
      RETURN TRUE
   END IF
   
   if p_aponta_refugo = 'S' then
      RETURN TRUE
   end if

   LET p_novo_saldo = p_saldo - p_qtd_saldo
   
       #se for tratar rejeição de gemeas, vide
       #bloco comentado no fim do programa
   
   IF NOT pol1110_troca_sit_estoq() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
FUNCTION pol1110_troca_sit_estoq()
#---------------------------------#
   ...
   IF STATUS <> 0 THEN
      LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE-pol1110'
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF p_saldo_disp < p_aponta.qtd_refugo THEN
      IF p_tip_peca = 'G' THEN
         LET m_msg = 'PECA GEMEA ',p_aponta.cod_item CLIPPED,
                     ' S/ ESTOQ-TRASNF.BOA P/REFUGO-pol1110'
      ELSE    
         if p_aponta_refugo = 'N' then     
            LET m_msg = 'PECA ',p_aponta.cod_item CLIPPED,
                     ' S/ ESTOQ-TRASNF.BOA P/ REFUGO - pol1110'
         else
            LET m_msg = 'PECA ',p_aponta.cod_item CLIPPED,
                     ' S/ ESTOQ PARA ESTORNAR - pol1110'
         end if
      END IF
      CALL pol1110_insere_erro()
      LET p_sem_estoque = TRUE
...
#------------------------#
   ...
   IF NOT pol1110_cria_w_parada() THEN
      RETURN
   END IF
   ...
   DECLARE cq_apont SCROLL CURSOR WITH HOLD FOR 	
    SELECT empresa,
	 ...
	 FOREACH cq_apont INTO 	
      ...
      LET p_hor_ini = x_hor_ini[1,5]
      LET p_hor_fim = x_hor_fim[1,5]

      SELECT COUNT(m.seq_reg_mestre)
        INTO p_count
        FROM man_apo_mestre m,
             man_apo_detalhe d,
             man_tempo_producao t
       WHERE m.empresa = p_cod_empresa
         AND m.ordem_producao = p_w_apont_prod.num_ordem
         AND d.empresa = m.empresa
         AND d.seq_reg_mestre = m.seq_reg_mestre
         AND d.sequencia_operacao = p_w_apont_prod.num_seq_operac
         AND t.empresa = m.empresa
         AND t.seq_reg_mestre = m.seq_reg_mestre
         AND t.data_ini_producao = p_w_apont_prod.dat_ini_prod
         AND t.hor_ini_producao = p_hor_ini
         AND t.dat_final_producao = p_w_apont_prod.dat_fim_prod
         AND t.hor_final_producao = p_hor_fim
               
      IF STATUS <> 0 THEN
         LET p_count = 0
      END IF
      
      IF p_count > 0 THEN #REGISTRO JÁ APONTADO
         
         LET p_dat_proces = TODAY
         
		     UPDATE man_apont_454
		        SET dat_atualiz = p_dat_proces
		      WHERE rowid = l_rowid
				 ...
				 UPDATE man_apont_hist_454 
	 			  ...
	 			  WHERE empresa = p_w_apont_prod.cod_empresa
				    ...
				    AND sequencia_operacao = p_w_apont_prod.num_seq_operac
						...
						AND hor_inicial      	= x_hor_ini 
						AND hor_fim        		= x_hor_fim
      ...
      END IF   
			...
			LET p_w_apont_prod.num_lote = pol1110_calcula_lote(p_w_apont_prod.num_ordem)
      ...
      LET p_w_apont_prod.ies_sit_qtd	        =	'L'
			...
			LET p_w_apont_prod.num_programa 				= 'POL1110'
         ...
         END IF
	 		   ...
	 		   IF manr24_inclui_w_apont_prod(p_w_apont_prod.*,1) THEN # incuindo apontamento
 	 			    ...
 	 			    IF p_w_apont_prod.ies_defeito = 1  THEN             #apontando defeitos
		 			     IF pol1110_cria_w_defeito() THEN 
               ...
               LET p_dat_proces = TODAY
  	 	         ...
  	 	         IF p_qtd_transf > 0 THEN
	 	              CALL log085_transacao("BEGIN")
	 	              IF NOT pol1110_transf_refugo() THEN
	 	                 CALL log085_transacao("ROLLBACK")
	 	              ELSE
	 	                 CALL log085_transacao("COMMIT")
	 	              END IF
...
#----------------------------------#
FUNCTION pol1110_calcula_lote(p_op)
#----------------------------------#
...
#---------------------------------#
 FUNCTION pol1110_cria_w_defeito()#
#---------------------------------#
...
#-------------------------------#
 FUNCTION pol1110_cria_w_parada()
#-------------------------------#
...
#------------------------------#
FUNCTION pol1110_transf_refugo()
#------------------------------#
   ...
   IF NOT pol1110_transf_movto() THEN
      RETURN FALSE
   END IF
   
   IF NOT pol1110_transf_estoque() THEN
      RETURN FALSE
   END IF
...
#------------------------------#
   ...
   LET p_estoque_trans.num_prog = 'POL1110'
...
#--------------------------------#
FUNCTION pol1110_transf_estoque()
#--------------------------------#
   ...
   IF NOT pol1110_atu_lote_ender() THEN
      RETURN FALSE
   END IF
   ...
   IF NOT pol1110_atu_estoque_lote() THEN
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol1110_atu_lote_ender()
#--------------------------------#
...
#---------------------------------#
FUNCTION pol1110_atu_estoque_lote()
#---------------------------------#
...
#-------------------------------- FIM DE PROGRAMA -----------------------------#
   ...
   #--- Uma peça gemea pode ter n OP's e isso teria gerado
   #--- n lotes diferentes no estoque. Se precisar apontar
   #--- refugos em OP's de peças gemeas, de qual lote/OP eu
   #--- tiraria das boas????? Manuel pediu p/ não apontar.
