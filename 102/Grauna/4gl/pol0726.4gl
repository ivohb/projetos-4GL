...
# DATA....: 24/01/2008                                              #
# POR.....: IVO H BARBOSA                                           #
# ALTERADO: THIAGO - DATA:10/02/2009                                #
#  					ADICIONAR FILTRO DE ITENS POR DATA                      #
#						Thiago - Mudar o processo regenerativo por linha,				#
#						tirar a obrigatoriedade do botão informar, caso					#
#						ele infome alguma coisa fazer o processo pelo						#
#						parametro informado caso contrario fazer o							#
#						processamento de todas AS linhas de montagem	29/5/2009	#
# 16/09/09Ivo - Gravar na ordens o ies_lista_ordem da item_man      #
#-------------------------------------------------------------------#
...
GLOBALS
  ...
  DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
         ...
         p_ies_cons1           SMALLINT,	{<--alterado-10/03/2009--}
         p_data_in						LIKE 	ped_itens.prz_entrega,{<--alterado-10/03/2009--}
         p_data_fi						LIKE  ped_itens.prz_entrega{<--alterado-10/03/2009--}
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol0726_controle()
   END IF

END MAIN

#--------------------------#
 FUNCTION pol0726_controle()
#--------------------------#
   
   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0726") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0726 AT 2,2 WITH FORM p_nom_tela 
...
#      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
   ...
   MENU "OPCAO"
   	COMMAND "Informar" "Informar datas p/ filtro" {<--alterado-10/03/2009---THIAGO}
       IF pol0726_informa_data() THEN 
       	 LET p_ies_cons1 = TRUE
       	 NEXT OPTION "Seletivo por pedido"
       ELSE 
       	LET p_ies_cons1 = FALSE 
       END IF 
    COMMAND "Seletivo por pedido" "Seleção de pedidos p/ geração de ordens"
       IF p_ies_cons1 THEN 
       	CALL pol0726_pedido()
       ELSE 
       	 ERROR 'Informe datas previamente !!!'
         NEXT OPTION 'Informar'
       END IF 
    COMMAND "Seletivo por item" "Seleção de itens p/ geração de ordens"
       IF p_ies_cons1 THEN 
       	CALL pol0726_item()
       ELSE 
       	ERROR 'Informe datas previamente !!!'
         NEXT OPTION 'Informar'
       END IF 
    COMMAND "Regenerativo por linha" "Seleção de area/linha p/ geração de ordens"
       IF p_ies_cons1 THEN 
       	CALL pol0726_linha()
       ELSE 
       	 ERROR 'Informe datas previamente !!!'
   ...
   CLOSE WINDOW w_pol0726

END FUNCTION

#----------------------#
FUNCTION pol0726_pedido()
#----------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol07264") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol07264 AT 5,7 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   MENU "OPCAO"
    COMMAND "Informar" "Informa parâmetros p/ o processamento"
       IF pol0726_informar('P') THEN
...
#----------------------#
FUNCTION pol0726_item()
#----------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol07261") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol07261 AT 5,7 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   MENU "OPCAO"
    COMMAND "Informar" "Informa parâmetros p/ o processamento"
       IF pol0726_informar('I') THEN
          ERROR 'Parâmetros infomados com sucesso !!!'
          LET p_ies_cons = TRUE
          NEXT OPTION 'Processar'
       ELSE
...
END FUNCTION
#-----------------------#
FUNCTION pol0726_linha()
#-----------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol07262") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol07262 AT 5,7 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   MENU "OPCAO"
    COMMAND "Informar" "Informa parâmetros p/ o processamento"
       IF pol0726_informar('L') THEN
          ERROR 'Parâmetros infomados com sucesso !!!'
          LET p_ies_cons = TRUE
          NEXT OPTION 'Processar'
       ELSE
...
#-------------------------------#
FUNCTION pol0726_informar(p_info)
#-------------------------------#
   
   DEFINE p_info CHAR(01)
   
   IF NOT pol0726_cria_item_tmp() THEN
      RETURN FALSE
   END IF
   ...
   INPUT ARRAY pr_pedido
      ...
      AFTER FIELD num_pedido
         IF pr_pedido[p_index].num_pedido IS NOT NULL THEN
            IF pol0726_repetiu_pedido() THEN
               ERROR "Pedido já Indormado !!!"
               NEXT FIELD num_pedido
            END IF
            IF NOT pol0726_le_pedido(pr_pedido[p_index].num_pedido) THEN
               RETURN FALSE
            END IF
   ...
   INPUT ARRAY pr_item
      ...
      AFTER FIELD cod_item
         IF pr_item[p_index].cod_item IS NOT NULL THEN
            IF pol0726_repetiu_cod() THEN
               ERROR "Item já Indormado !!!"
               NEXT FIELD cod_item
            END IF
            IF NOT pol0726_le_item(pr_item[p_index].cod_item) THEN
               RETURN FALSE
            END IF
            IF p_msg IS NOT NULL THEN
               ERROR p_msg
...
END FUNCTION
#-------------------------------#
FUNCTION pol0726_repetiu_pedido()
#-------------------------------#
...
#----------------------------------#
FUNCTION pol0726_le_item(p_cod_item)
#----------------------------------#
...
END FUNCTION
#--------------------------------------#
FUNCTION pol0726_le_pedido(p_num_pedido)
#--------------------------------------#
...
DEFINE l_status 	SMALLINT 							#Essa alteraçao foi feita para selecionar todas as 
   ...
   IF l_status THEN 
	   INPUT ARRAY pr_linha
	      ...
	      AFTER FIELD cod_lin_prod
	         IF pr_linha[p_index].cod_lin_prod IS NOT NULL THEN
	            IF pol0726_repetiu_linha() THEN
	               ERROR "Area/Linha já Indormado !!!"
	               NEXT FIELD cod_lin_prod
	            END IF
	            IF NOT pol0726_le_linha_prod() THEN
	               RETURN FALSE
	            END IF
	            IF p_msg IS NOT NULL THEN
	               ERROR p_msg
	   ...
	   END FOR
   ELSE 
			DECLARE cd_vdp_todo CURSOR FOR  SELECT UNIQUE a.cod_item 						#vai ler todos os itens de todas as
			...
			FOREACH cd_vdp_todo INTO p_cod_item
				IF STATUS <> 0 THEN
					CALL log003_err_sql('Lendo','item/item_vdp')
					RETURN FALSE
				END IF
...
#-------------------------------#
FUNCTION pol0726_repetiu_linha()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol0726_le_linha_prod()
#-------------------------------#
...
#-----------------------#
   ...
   CASE
      ...
      WHEN INFIELD(cod_lin_prod)
         LET p_codigo = pol0726_exibe_linha()
         CURRENT WINDOW IS w_pol07262
         IF p_codigo IS NOT NULL THEN
...
#-----------------------------#
   ...
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol07263") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol07263 AT 8,20 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
...
#-------------------------------#
FUNCTION pol0726_cria_item_tmp()
#-------------------------------#
   ...
   END IF
    ...
    DROP TABLE pedido_tmp_1040

   IF STATUS = 0 OR STATUS -206 THEN 
 
      CREATE TABLE pedido_tmp_1040(
         num_pedido      DEC(6,0) 
       );
         
      IF SQLCA.SQLCODE <> 0 THEN 
         CALL log003_err_sql("CRIACAO","pedido_tmp_1040")
         RETURN FALSE
      END IF
   END IF
...
#--------------------------------#
FUNCTION pol0726_cria_tab_strut()
#--------------------------------#
...
#------------------------------------#
FUNCTION pol0726_processa(p_processa)
#------------------------------------#

   DEFINE p_processa CHAR(01)
   
   IF NOT pol0726_bloqueia_tab() THEN
      RETURN FALSE
   END IF

   IF NOT pol0726_cria_tab_strut() THEN
      RETURN FALSE
   END IF
   ...
   FOREACH cq_tmp INTO p_cod_item
       ...
       IF NOT pol0726_le_tip_local(p_cod_item) THEN
          RETURN FALSE
       END IF   

       IF NOT pol0726_insere_item() THEN
          RETURN FALSE
       END IF

       IF NOT pol0726_explode_estrutura() THEN
          RETURN FALSE
       END IF
       
       IF p_processa  = 'P'   THEN 
          IF NOT pol0726_gera_ordens_ped() THEN
             RETURN FALSE
          END IF
       ELSE
          IF NOT pol0726_gera_ordens() THEN
             RETURN FALSE
          END IF
       END IF
       
       IF NOT pol0726_deleta_estrut() THEN
          RETURN FALSE
       END IF
...
#---------------------------------------#
FUNCTION pol0726_le_tip_local(p_cod_item)
#---------------------------------------#
...
#------------------------------#
   ...
   LOCK TABLE pedido_tmp_1040 IN EXCLUSIVE MODE

   IF STATUS <> 0 THEN
      LET p_msg = 'ERRO:(',STATUS, ') BLOQUEANDO PEDIDO_TMP_1040'
      RETURN FALSE
   END IF
...
#------------------------------#
FUNCTION pol0726_deleta_estrut()
#------------------------------#
...
#-----------------------------------#
FUNCTION pol0726_explode_estrutura()
#-----------------------------------#
   ...
   WHILE TRUE
    ...
    FOREACH cq_exp INTO p_sequencia, p_cod_item_pai, p_qtd_prodcomp
       ...
       FOREACH cq_est INTO p_cod_item_compon, p_qtd_necessaria, p_pct_refug
          ...
          IF NOT pol0726_le_tip_item() THEN
             RETURN FALSE
          END IF
          ...
          IF NOT pol0726_le_tip_local(p_cod_item_compon) THEN
             RETURN FALSE
          END IF   
          ...
          DISPLAY p_cod_item_compon AT 18,30
...
#-----------------------------#
   ...
   FOREACH cq_gera INTO 
      ...
      END IF
       ...
       IF NOT pol0726_deleta_ops() THEN
          RETURN FALSE
       END IF
       
       IF pol0726_deleta_oc() = FALSE THEN
          RETURN FALSE
       END IF
   
      IF NOT pol0726_grava_ordens() THEN
         RETURN FALSE
      END IF
...
#---------------------------------#
FUNCTION pol0726_gera_ordens_ped()
#---------------------------------#
...
#-------------------------------#
   ...
   FOREACH cq_op INTO 
      ...
      IF NOT pol0726_le_estoque() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT pol0726_le_item_man() THEN
         RETURN FALSE
      END IF 
      ...
      IF p_gerar = 'OP' THEN
         CALL pol0726_gera_op() RETURNING p_status
      ELSE
         CALL pol0726_gera_oc() RETURNING p_status
      END IF
...
#-------------------------#
FUNCTION pol0726_gera_op()
#-------------------------#

   IF NOT pol0726_insere_ordens() THEN
      RETURN FALSE
   END IF      

   IF NOT pol0726_insere_op_compl() THEN
      RETURN FALSE
   END IF

   IF NOT pol0726_le_compon() THEN
      RETURN FALSE
   END IF      
...
#-------------------------------#
FUNCTION pol0726_insere_ordens()
#-------------------------------#

   IF NOT pol0726_prx_num_op() THEN
      RETURN FALSE
   END IF
...
#-------------------------------#
 FUNCTION pol0726_prx_num_neces()
#-------------------------------#
...
#---------------------------------#
 FUNCTION pol0726_insere_op_compl()
#---------------------------------#
...
#-----------------------------#
   ...
   FOREACH cq_compon INTO p_cod_item_compon, p_qtd_necessaria
      ...
      IF p_ies_tip_item  = "T"  THEN
         ...
         IF NOT pol0726_trata_fantasma() THEN
            RETURN FALSE
         END IF
      ELSE
         IF NOT pol0726_insere_necessidades() THEN
            RETURN FALSE
         END IF
      END IF
...
#--------------------------------#          
FUNCTION pol0726_trata_fantasma()
#--------------------------------#
...
#-------------------------------------#
FUNCTION pol0726_insere_necessidades()
#-------------------------------------#

   IF NOT pol0726_prx_num_neces() THEN
      RETURN FALSE
   END IF
...
#-------------------------#
   ...
   IF NOT pol0726_le_item_sup() THEN
      RETURN FALSE
   END IF

   IF pol0726_prx_num_oc() = FALSE THEN
      RETURN FALSE
   END IF
   
   IF pol0726_insere_oc() = FALSE THEN
      RETURN FALSE
   END IF

   IF pol0726_insere_prog_oc() = FALSE THEN
      RETURN FALSE
   END IF

   IF pol0726_insere_dest_oc() = FALSE THEN
      RETURN FALSE
   END IF

   IF p_ies_tip_item = 'B' THEN
      IF pol0726_insere_estrut() = FALSE THEN
         RETURN FALSE
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol0726_insere_prog_oc()
#--------------------------------#
...
#--------------------------------#
 FUNCTION pol0726_insere_dest_oc()
#--------------------------------#
...
#-------------------------------#
 FUNCTION pol0726_insere_estrut()
#-------------------------------#
...
#-------------------------------#
 FUNCTION pol0726_informa_data()		{<--------Alterado----------THIAGO-----10/03/2009----}
#-------------------------------#
	 CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0726") RETURNING p_nom_tela
