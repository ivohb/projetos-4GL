...
DATABASE logix
  ...
  DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
   ...
   DEFINE p_den_critica    LIKE man_erro_405.den_critica,
          p_email_orig     CHAR(50), 
          p_email_dest     CHAR(50), 
          p_assunto        CHAR(30), 
          p_den_caminho    CHAR(80),
          p_user_orig      CHAR(08),
          p_user_dest      CHAR(08),
          p_arquivo        CHAR(30),
          p_imp_linha      CHAR(80),
...
#----------------------------#
   ...
   IF l_ies_proces = 'S' AND pol1186_retornar() THEN
...
#---------------------------------------------------#
 FUNCTION pol1186_proces_apto(p_user_log, p_emp_log)#
#---------------------------------------------------#
   
   DEFINE p_user_log   CHAR(08),
          p_emp_log    CHAR(02),
          l_dat_proces CHAR(19)

   WHENEVER ANY ERROR CONTINUE
   SET ISOLATION TO DIRTY READ
   SET LOCK MODE TO WAIT 300
   LET p_versao = "pol1186-12.00.16  "
   CALL func002_versao_prg(p_versao)

   LET p_user = p_user_log
   LET p_cod_empresa = p_emp_log
   LET p_qtd_critica = 0
   LET p_houve_erro = TRUE

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol1186") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1186 AT 10,20 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
   
   LET p_msg = NULL
   
   IF pol1186_ies_proces() THEN
      CALL pol1186_erro_critico()
      RETURN p_msg, p_qtd_critica
   END IF
                  
   IF NOT pol1186_cria_tabs() then
      LET p_houve_erro = TRUE
      RETURN p_msg, p_qtd_critica
   END IF

   IF NOT pol1186_le_parametros() THEN
      RETURN p_msg, 0
   END IF
   ...
   FOREACH cq_programa INTO p_num_programa
      ...
      IF not pol1186_grava_id_nest() THEN
         LET p_houve_erro = TRUE
         CALL pol1186_erro_critico()
         EXIT FOREACH
      END IF
      ...
      IF NOT pol1186_proces_programa() THEN
   ...
   UPDATE proces_apont_405
      SET ies_proces = 'N',
          dat_proces = l_dat_proces,
          cod_usuario = ''
    WHERE cod_empresa = p_cod_empresa
   
   IF p_qtd_critica > 0 THEN
      CALL pol1186_logix_envia_email()
      #CALL pol1186_java_envia_email()
   END IF
...
#----------------------------------#
FUNCTION pol1186_java_envia_email()#
#----------------------------------#
...
#-----------------------------------#
FUNCTION pol1186_logix_envia_email()#
#-----------------------------------#
   ...
   SELECT parametro_texto
    ...
    WHERE empresa = p_cod_empresa
      AND parametro = 'USUARIO_ORIG_APONT'

   IF STATUS <> 0 THEN
      LET p_msg = 'Usuario emitente não cadastrado na tabela min_par_modulo'
      RETURN
   END IF
   
   LET p_user_orig = log9900_conversao_minusculo(p_user_orig)
   ...
   SELECT e_mail,
     ...
     INTO p_email_orig,
          p_nom_emitente
     FROM usuarios
    WHERE cod_usuario = p_user_orig

   IF STATUS <> 0 OR p_email_orig IS NULL THEN
      LET p_msg = 'Email do usuario emitente não cadstrado na tab usuarios'
      RETURN
   END IF
   ...
   SELECT parametro_texto
     ...
     FROM min_par_modulo 
    WHERE empresa = p_cod_empresa
      AND parametro = 'USUARIO_DEST_APONT'

   IF STATUS <> 0 THEN
      LET p_msg = 'Usuario recebedor não cadastrado na tabela min_par_modulo'
      RETURN
   END IF
   
   LET p_user_dest = log9900_conversao_minusculo(p_user_dest)
   ...
   SELECT e_mail,
     ...
     INTO p_email_dest,
          p_nom_usuario
     FROM usuarios
    WHERE cod_usuario = p_user_dest

   IF STATUS <> 0 OR p_email_dest IS NULL THEN
      LET p_msg = 'Email do usuario recebedor não cadstrado na tab usuarios'
      RETURN
   END IF
   ...
   SELECT nom_caminho
    ...
    WHERE usuario = p_user_dest
      AND empresa = p_cod_empresa 
      AND sistema_fonte = 'LST' 
      AND ambiente = 'W'
  
   IF STATUS <> 0 OR p_den_caminho IS NULL THEN
      LET p_msg = 'Caminho de relatório do recebedor não cadastrado na tab log_usu_dir_relat'
      RETURN
   END IF

   LET p_arquivo = p_user_orig CLIPPED, '-', p_user_dest CLIPPED, '.lst'
   LET p_den_caminho = p_den_caminho CLIPPED, p_arquivo
         
   START REPORT pol1186_relat TO p_den_caminho
   ...
   DECLARE cq_le_docs CURSOR FOR
    SELECT num_programa,
      ...
      FROM man_erro_405
     WHERE cod_empresa = p_cod_empresa
     ORDER BY num_programa, num_ordem     
   ...
   FOREACH cq_le_docs INTO  
      ...
      IF STATUS <> 0 THEN
         LET p_cod_erro = STATUS
         LET p_msg = 'Erro ', p_cod_erro, ' lendo man_erro_405 - enviando email'
         EXIT FOREACH
      END IF
      
      LET p_imp_linha = p_num_programa CLIPPED, ' - ', p_num_ordem USING '<<<<<<<<<', ' - ', p_den_critica
      
      OUTPUT TO REPORT pol1186_relat() 
      
   END FOREACH

   FINISH REPORT pol1186_relat  
   
   LET p_assunto = 'Apontamento de produção'
   LET p_titulo = 'PROGRAMAS CRITICADOS'
   
   CALL log5600_envia_email(p_email_orig, p_email_dest, p_assunto, p_den_caminho, 2)

END FUNCTION

#---------------------#
 REPORT pol1186_relat()
#---------------------#
...
#-------------------------------#
FUNCTION pol1186_grava_id_nest()
#-------------------------------#
...
#----------------------------------#
 FUNCTION pol1186_proces_programa()#
#----------------------------------#
   ...
   if not pol1186_deleta_erro() then
      RETURN false
   end if
   
   let p_tem_critica = false
   
   if not pol1186_consiste_dados() then
      RETURN false
   end if
   
   IF p_tem_critica then
      call pol1186_critica_man_nest() RETURNING p_status
      RETURN TRUE
   END IF
   ...
   FOREACH cq_mat INTO
      ...
      CALL pol1186_exib_mensagem()
   
      IF NOT pol1186_le_material() THEN 
         RETURN FALSE
      END IF

      IF NOT pol1186_le_sucata() THEN 
         RETURN FALSE
      END IF
   ...
   IF NOT pol1186_checa_material() THEN 
      RETURN FALSE
   END IF

   IF NOT pol1186_checa_sucata() THEN 
      RETURN FALSE
   END IF
   ...
   IF not pol1186_aponta_programa() then
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET p_cod_erro = STATUS
      LET p_msg = 'ERRO ', p_cod_erro CLIPPED, 'CONFERINDO ATUALIZACAO DA TABELA MAN_APO_LOGIX_405'
      RETURN FALSE
   END IF

   IF p_count > 0 THEN
      LET p_msg = 'NAO FOI POSSIVEL AUTALIZAR O STATUS DA TABELA MAN_APO_LOGIX_405'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      let p_cod_erro = STATUS
      let p_msg = 'ERRO ',p_cod_erro, ' AO ATUALIZAR A TABELA MAN_APO_NEST_405'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET p_cod_erro = STATUS
      LET p_msg = 'ERRO ',p_cod_erro, ' CONFERINDO ATUALIZACAO DA TABELA MAN_APO_NEST_405'
      RETURN FALSE
   END IF

   IF p_count > 0 THEN
      LET p_msg = 'NÃO FOI POSSIVEL ATUALIZAR A TABELA MAN_APO_NEST_405'
      RETURN FALSE
   END IF
...
#---------------------------------------#
FUNCTION pol1186_forca_apont(p_cod_item)
#---------------------------------------#
...
#------------------------------#
FUNCTION pol1186_le_parametros()
#------------------------------#
...
#---------------------------------#
FUNCTION pol1186_critica_man_nest()
#---------------------------------#
...
#-------------------------------#
FUNCTION pol1186_consiste_dados()
#-------------------------------#
   ...
   FOREACH cq_consiste into p_programa.*
      ...
      if not pol1186_checa_ordem() then
         return false
      end if
      
      IF p_man.cod_item IS NULL THEN
         let p_msg = 'CODIGO DO ITEM PRODUZIDO ESTA NULO'
         call pol1186_insere_erro()
      END IF
      
      if not pol1186_checa_operacao() then
         return false
      end if

      if not pol1186_checa_operador() then
         return false
      end if

      if not pol1186_checa_componente() then
         return false
      end if
      ...
      if p_qtd_apontar <= 0 then
         let p_msg = 'QUANTIDADE A APONTAR INVALIDA'
         call pol1186_insere_erro()
      end if
      
      IF p_sdo_op < p_qtd_apontar THEN
         LET p_ies_forca_apont = pol1186_forca_apont(p_man.cod_item)
      ...
      IF p_criticou then
         LET p_tem_critica = true
      ELSE
         if not pol1186_insere_man() then
            return false
         END IF
      END IF
...
#------------------------------#
FUNCTION pol1186_exib_mensagem()
#------------------------------#
...
#-----------------------------#
FUNCTION pol1186_checa_ordem()
#-----------------------------#

   if empty(p_programa.num_ordem) OR p_programa.num_ordem = 0  then
...
#--------------------------------#
FUNCTION pol1186_checa_operacao()
#--------------------------------#
...
#----------------------------------#
FUNCTION pol1186_checa_componente()
#----------------------------------#
...
#--------------------------------#
FUNCTION pol1186_checa_operador()
#--------------------------------#
...
#---------------------------------#
FUNCTION pol1186_checa_tempo_unit()
#---------------------------------#
...
#-------------------------------#
FUNCTION pol1186_calc_data_hora()
#-------------------------------#
   ...
   CALL pol1186_calcula_turno(p_hi)
...
#-----------------------------------#
FUNCTION pol1186_calcula_turno(p_hi)
#-----------------------------------#
...
#----------------------------#
FUNCTION pol1186_insere_man()
#----------------------------#
   
   DEFINE p_id_man_apont INTEGER
   
   LET p_man.nom_prog = 'pol1186'
   ...
   if not pol1186_le_ferramenta() then
      return false
   end if

   if not pol1186_le_uni_funcio() then
      return false
   end if
...
#-------------------------------#
FUNCTION pol1186_le_ferramenta()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1186_le_uni_funcio()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol1186_aponta_programa()
#---------------------------------#
   ...
   FOREACH cq_man INTO p_man.*
      ...
      if not pol1186_atualiza_man() then
         return false
      end if

      IF NOT pol1186_ins_mestre() THEN
         RETURN FALSE
      END IF

      IF NOT pol1186_ins_detalhe() THEN
         RETURN FALSE
      END IF
      
      IF NOT pol1186_ins_tempo() THEN
         RETURN FALSE
      END IF

      IF NOT pol1186_atuali_ord_oper() THEN
         RETURN FALSE
      END IF
      ...
      IF (p_qtd_boas + p_qtd_refug + p_qtd_sucata) > 0 THEN
         If not pol1186_grava_ordens() THEN
            Return false
         End if
      End If
      ...
      If p_man.qtd_refugo > 0 then
         ...
         IF NOT pol1186_gra_tabs_velhas() THEN
            RETURN FALSE
         END IF
         ...
         If not pol1186_le_est_lote_ender() then
            RETURN FALSE
         End if
         
         LET p_qtd_prod = p_qtd_refug
         LET p_cod_operacao = NULL
         IF NOT pol1186_aponta_estoque() THEN
            RETURN FALSE
         END IF
            
         IF NOT pol1186_insere_man_item() THEN
            RETURN FALSE
         END IF

         LET p_tip_movto = 'E'
   
         IF NOT pol1186_insere_chf_componente() THEN
            RETURN FALSE
         END IF
            
         IF NOT pol1186_ins_man_def() THEN
            RETURN FALSE
         END IF
         ...
         IF NOT pol1186_baixa_material() THEN 
            RETURN FALSE
         END IF

         IF NOT pol1186_baixa_sucata() THEN 
            RETURN FALSE
         END IF
      ...
      IF NOT pol1186_ins_apont_proces() THEN
         RETURN FALSE
      END IF
      
      let p_cod_status = 'A'
      
      IF NOT pol1186_atualiza_apo_logix() THEN 
         RETURN FALSE
      END IF
...
#------------------------------#
   ...
   FOREACH cq_bs INTO 
      ...
      IF NOT pol1186_le_item_man() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT pol1186_baixa_compon() THEN
         RETURN FALSE
      END IF
...
#-----------------------------------#
FUNCTION pol1186_atualiza_apo_logix()
#-----------------------------------#
...
#---------------------------------#
FUNCTION pol1186_atuali_ord_oper()
#---------------------------------#
...
#---------------------------------#
FUNCTION pol1186_gra_tabs_velhas()
#---------------------------------#
...
#----------------------------------#   
FUNCTION pol1186_ins_apont_proces()
#----------------------------------#
...
#---------------------------#
FUNCTION pol1186_cria_tabs()
#---------------------------#

   DROP TABLE consumo_tmp_405
   
   CREATE TEMP  TABLE consumo_tmp_405(
      cod_empresa CHAR(02),
      cod_compon  CHAR(15),
      cod_local   CHAR(15),
   ...
   CREATE TEMP TABLE sucata_tmp_405(
      cod_empresa CHAR(02),
      cod_compon  CHAR(15),
      cod_local   CHAR(15),
      qtd_neces   DECIMAL(10,3),
...
#-------------------------------#
FUNCTION pol1186_checa_material()
#-------------------------------#
   ...
   FOREACH cq_cm INTO 
      ...
      IF NOT pol1186_checa_estoque() THEN
         RETURN FALSE
      END IF
...
#-------------------------------#
FUNCTION pol1186_checa_estoque()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1186_baixa_material()
#-------------------------------#
...
#------------------------------#
   ...
   FOREACH cq_bel INTO p_estoque_lote_ender.*
      ...
      IF NOT pol1186_baixa_lote() THEN
         RETURN FALSE
      END IF
      
      IF NOT pol1186_baixa_estoque() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT pol1186_grava_estoq_trans() THEN
         RETURN FALSE
      END IF
      
      IF p_tip_oper = 'A' THEN
         IF NOT pol1186_insere_man_consumo() THEN
            RETURN FALSE
         END IF
         IF NOT pol1186_insere_chf_componente() THEN
            RETURN FALSE
...
#-------------------------------#
FUNCTION pol1186_baixa_estoque()
#-------------------------------#
...
#-----------------------------------#
FUNCTION pol1186_grava_estoq_trans()
#-----------------------------------#
   ...
   IF NOT pol1186_le_operacao() THEN
      RETURN FALSE
   END IF
   ...
   IF NOT pol1186_ins_est_trans_end() THEN
      RETURN FALSE
   END IF

   IF NOT pol1186_insere_estoq_audit() THEN
      RETURN FALSE
   END IF

   IF p_tip_oper = 'A' THEN
      IF NOT pol1186_apont_transac() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------------#
 FUNCTION pol1186_ins_est_trans_end()
#------------------------------------#
...
#-----------------------------------#
FUNCTION pol1186_insere_estoq_audit()
#-----------------------------------#
...
#-------------------------------#
FUNCTION pol1186_apont_transac()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol1186_insere_man_item()
#---------------------------------#
...
#-----------------------------------#
FUNCTION pol1186_insere_man_consumo()
#-----------------------------------#
...
#--------------------------------------#
FUNCTION pol1186_insere_chf_componente()
#--------------------------------------#
...
#----------------------------------#
FUNCTION pol1186_le_est_lote_ender()
#----------------------------------#

   Let p_tem_lote = false

   IF NOT pol1186_le_item() THEN
      RETURN FALSE
   END IF

   IF NOT pol1186_le_ctr_grade(p_man.cod_item) THEN
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol1186_aponta_estoque()
#--------------------------------#
   ...
   IF p_tem_lote THEN
      ...
      IF NOT pol1186_atualiza_lote_ender() THEN
         RETURN FALSE
      END IF
   ELSE
      CALL pol1186_ender_carrega()
      IF NOT pol1186_insere_lote_ender() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF STATUS = 0 THEN
      IF NOT pol1186_atualiza_lote() THEN
         RETURN FALSE
      END IF
   ELSE
      IF STATUS = 100 THEN
         CALL pol1186_lote_carrega()
         IF NOT pol1186_insere_lote() THEN
            RETURN FALSE
         END IF
      ELSE
         let p_cod_erro = STATUS
   ...
   IF NOT pol1186_atualiza_estoque() THEN
      RETURN FALSE
   END IF
...
#---------------------------------------#
FUNCTION pol1186_le_ctr_grade(p_cod_item)
#---------------------------------------#
...
#------------------------------------#
FUNCTION pol1186_atualiza_lote_ender()
#------------------------------------#
...
#-------------------------------#
FUNCTION pol1186_ender_carrega()
#-------------------------------#
...
#-----------------------------------#
FUNCTION pol1186_insere_lote_ender()
#-----------------------------------#
...
#-------------------------------#
FUNCTION pol1186_atualiza_lote()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol1186_atualiza_estoque()
#---------------------------------#
