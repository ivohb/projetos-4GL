...
#-----------------------#
   ...
   COMMAND "Processar" "Gera ordens enviadas pelo drummer"
	    LET p_msg = 'As ordens planejadas serão eliminadas\n',
	                'e novas ordens enviadas pelo Drummer\n',
	                'serão criadas. Deseja continuar ?' 
   ...
   FOREACH cq_ins INTO m_cod_item, m_dat_entrega,m_dat_abertura_oc,
       ...
       IF STATUS = 100 THEN
          LET p_msg = 'OC ', p_erro CLIPPED, 
               '. ITEM ', m_cod_item CLIPPED, ' ENVIADO PELO DRUMMER NÃO EXISTE'
          RETURN FALSE
       ELSE
          IF STATUS <> 0 THEN
             LET p_erro = STATUS
...
#---------------------------#
   ...
   {SELECT cod_progr INTO l_cod_progr   #márcio pediu para tirar.
     FROM programador
    WHERE cod_empresa = p_cod_empresa
      AND login       = p_user
