...
# PROGRAMA: pol0809                                               #
# QUALQUER ALTERACAO NESTE PROGRAMA DEVERA SER FEITO A MESMA      #
#   NO POL0921 - ROTINA AUTOMATICA                                #
#-----------------------------------------------------------------#
DATABASE logix
...
MAIN
  ...
  LET p_versao = "POL0809-10.02.00" #Favor nao alterar esta linha (SUPORTE)
  ...
  IF p_status = 0 THEN 
    CALL pol0809_controle()
  END IF
END MAIN

#---------------------------#
 FUNCTION pol0809_controle()
#---------------------------#
  CALL log006_exibe_teclas("01", p_versao)

  CALL log130_procura_caminho("pol0809") RETURNING p_nom_tela 
  OPEN WINDOW w_pol0809 AT 5,3  WITH FORM p_nom_tela 
       ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

  MENU "OPCAO"
    COMMAND  "Informar"     "Informa parâmetros para o processamento "
       IF pol0809_entrada_parametros() THEN
          LET p_ies_cons = TRUE
    ...
    IF p_ies_cons THEN
      ...
      CALL pol0809_processa_or()
      CALL pol0809_processa_sd()
      CALL pol0809_notas()
    ...
    COMMAND "Fim" "Retorna ao Menu Anterior"
      ...
      EXIT MENU
  END MENU
  CLOSE WINDOW w_pol0809
END FUNCTION

#------------------------------------#
 FUNCTION pol0809_entrada_parametros()
#------------------------------------#
   CALL log006_exibe_teclas("01 02 07",p_versao)
   CURRENT WINDOW IS w_pol0809
      ...
      AFTER FIELD dat_fim
        ...
        ELSE
           IF p_tela.dat_fim < p_tela.dat_ini THEN
              ERROR "Data final deve ser superior ou igual a inicial" 
              NEXT FIELD dat_ini
           END IF          
        END IF
...
#-----------------------------#
 FUNCTION pol0809_processa_or()
#-----------------------------#

  DISPLAY 'PROCESSANDO PEDIDO ORIG'  AT 7,10
  ...
  DECLARE cq_pedo1 CURSOR FOR
    SELECT UNIQUE a.* 
      FROM pedidos a, ped_itens b 
     WHERE a.cod_empresa  IN (p_cod_empg1, p_cod_empg2, p_cod_empg3, p_cod_empg4, p_cod_empg5) 
       AND a.ies_sit_pedido  <> '9'
       AND a.cod_empresa = b.cod_empresa
       AND a.num_pedido = b.num_pedido
  FOREACH cq_pedo1 INTO p_pedidos.*
     ...
     SELECT * 
       INTO p_empresas_885.*
       FROM empresas_885 
      WHERE cod_emp_gerencial = p_pedidos.cod_empresa
     ...
     ELSE
        ...
        ELSE
           IF p_desc_nat_oper_885.pct_desc_qtd > 0 THEN
              CALL pol0809_processa_qtd_or()
           ELSE   
              CALL pol0809_processa_val_or()
           END IF 
        END IF
     END IF
  END FOREACH              
...
#---------------------------------#
 FUNCTION pol0809_processa_100_or()
#---------------------------------#
  ...
  FOREACH cq_peio100 INTO p_ped_itens.*
    ...
    LET p_ped_itens_orig_885.qtd_pecas_solic  = p_ped_itens.qtd_pecas_solic 
    LET p_ped_itens_orig_885.qtd_pecas_cancel = p_ped_itens.qtd_pecas_cancel
    LET p_ped_itens_orig_885.qtd_pecas_atend  = p_ped_itens.qtd_pecas_atend
    ...
    IF p_ies_tip_controle = '1' THEN 
       ...
       LET p_ped_itens_orig_885.qtd_pecas_atend = p_qtd_remessa
    END IF   
...
#---------------------------------#
 FUNCTION pol0809_processa_val_or()
#---------------------------------#
...
#---------------------------------#
 FUNCTION pol0809_processa_qtd_or()
#---------------------------------#
  ...
  FOREACH cq_peioqtd INTO p_ped_itens.*
    ...
    LET p_ped_itens_orig_885.pre_unit_qtd  = p_ped_itens.pre_unit 
    LET p_ped_itens_orig_885.qtd_saldo     = p_ped_itens.qtd_pecas_solic - p_ped_itens.qtd_pecas_cancel     
    LET p_ped_itens_orig_885.qtd_pecas_solic  = p_ped_itens.qtd_pecas_solic + p_ped_itens_01.qtd_pecas_solic
    LET p_ped_itens_orig_885.qtd_pecas_atend  = p_ped_itens.qtd_pecas_atend
    LET p_ped_itens_orig_885.qtd_pecas_cancel = p_ped_itens.qtd_pecas_cancel
    LET p_ped_itens_orig_885.prz_entrega   = p_ped_itens.prz_entrega
...
END FUNCTION
#-----------------------------#
 FUNCTION pol0809_processa_sd()
#-----------------------------#

  DISPLAY 'PROCESSANDO PEDIDOS SALDO' AT 7,10

  DELETE FROM ped_itens_peso_885 
WHERE cod_empresa  IN (p_cod_emp1, p_cod_emp2, p_cod_emp3, p_cod_emp4, p_cod_emp5, p_cod_empg1, p_cod_empg2, p_cod_empg3, p_cod_empg4, p_cod_empg5)

  DECLARE cq_peds1 CURSOR FOR
    SELECT UNIQUE a.* 
      FROM pedidos a, ped_itens b 
     WHERE a.cod_empresa  IN (p_cod_empg1, p_cod_empg2, p_cod_empg3, p_cod_empg4, p_cod_empg5) 
       AND a.ies_sit_pedido  <> '9'
  ...
  FOREACH cq_peds1 INTO p_pedidos.*
     ...
     SELECT den_cnd_pgto[1,15]
       INTO p_ped_itens_peso_885.den_cnd_pgto
       FROM cond_pgto
      WHERE cod_cnd_pgto =  p_pedidos.cod_cnd_pgto
     ...
     ELSE
        ...
        ELSE
           IF p_desc_nat_oper_885.pct_desc_qtd > 0 THEN
              CALL pol0809_processa_qtd_sd()
           ELSE   
              CALL pol0809_processa_val_sd()
           END IF 
        END IF
     END IF
  END FOREACH              
...
#---------------------------------#
 FUNCTION pol0809_processa_100_sd()
#---------------------------------#
  ...
  FOREACH cq_peis100 INTO p_ped_itens.*
    ...
    LET p_ped_itens_peso_885.qtd_pecas_solic  = p_ped_itens.qtd_pecas_solic 
    LET p_ped_itens_peso_885.qtd_pecas_cancel = p_ped_itens.qtd_pecas_cancel
    LET p_ped_itens_peso_885.qtd_pecas_atend  = p_ped_itens.qtd_pecas_atend
    ...
    IF p_ies_tip_controle = '1' THEN 
       ...
       LET p_ped_itens_peso_885.qtd_pecas_atend = p_qtd_remessa
    END IF   
...
#---------------------------------#
 FUNCTION pol0809_processa_val_sd()
#---------------------------------#
...
#---------------------------------#
 FUNCTION pol0809_processa_qtd_sd()
#---------------------------------#
  ...
  FOREACH cq_peisqtd INTO p_ped_itens.*
    ...
    LET p_ped_itens_peso_885.pre_unit_qtd  = p_ped_itens.pre_unit 
    LET p_ped_itens_peso_885.qtd_saldo     = p_ped_itens.qtd_pecas_solic - p_ped_itens.qtd_pecas_atend - p_ped_itens.qtd_pecas_cancel
    LET p_ped_itens_peso_885.qtd_pecas_solic  = p_ped_itens.qtd_pecas_solic + p_ped_itens_01.qtd_pecas_solic
    LET p_ped_itens_peso_885.qtd_pecas_atend  = p_ped_itens.qtd_pecas_atend
    LET p_ped_itens_peso_885.qtd_pecas_cancel = p_ped_itens.qtd_pecas_cancel
    LET p_ped_itens_peso_885.prz_entrega   = p_ped_itens.prz_entrega
...
###       AND ies_situacao <> 'C'
  FOREACH cq_not INTO p_nf_mestre.*
     ...
     ELSE
        ...
        ELSE
           IF p_desc_nat_oper_885.pct_desc_qtd > 0 THEN
              CALL pol0809_proc_nf_qtd()
           ELSE   
              CALL pol0809_proc_nf_val()
           END IF 
        END IF
     END IF
  END FOREACH              
