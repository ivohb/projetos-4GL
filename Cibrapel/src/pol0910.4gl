...
END GLOBALS
   ...
   DEFINE p_num_seq_ar         LIKE aviso_rec.num_seq,
          ...
          p_qtd_liber_calc     LIKE aviso_rec.qtd_liber,
          p_qtd_excep_calc     LIKE aviso_rec.qtd_liber_excep,
          p_qtd_rejeit_calc    LIKE aviso_rec.qtd_rejeit,
          ...
          p_qtd_trim           LIKE estoque_trans.qtd_movto,
          ...
          p_lote_trim          LIKE estoque_lote.num_lote
   ...
   DEFINE p_num_ar             INTEGER,
          ...
          p_qtd_txt            CHAR(11),
          p_dat_emis_nf        DATE,
          p_tip_inspecao       CHAR(01),
          p_fat_conversao      DECIMAL(17,10),
...
#----------------------------#
   ...
   DEFINE p_tot_insp DECIMAL(10,3)
   ...
   INPUT ARRAY pr_item WITHOUT DEFAULTS FROM sr_item.*
      ...
      AFTER ROW
         ...
         IF NOT INT_FLAG THEN
            LET p_tot_insp = pr_item[p_index].qtd_liber + 
                pr_item[p_index].qtd_liber_excep + pr_item[p_index].qtd_rejeit
         
            IF p_tot_insp <> pr_item[p_index].qtd_contagem THEN
               ERROR 'Total inspecionado não confere com a colun contagem'
               NEXT FIELD qtd_liber
            END IF
         END IF
   ...
   ELSE
      DELETE FROM aviso_rec_rej
       WHERE cod_empresa = p_cod_empresa
         AND num_aviso_rec = p_tela.num_aviso_rec
      IF STATUS <> 0 THEN 
         CALL log003_err_sql('DELETE','aviso_rec_rej')
         RETURN FALSE
      END IF
   END IF
...
#----------------------------#
   ...
   IF p_tip_inspecao MATCHES '[PC]' THEN
   ELSE
      LET p_msg = 'Tipo de inspeção de aparas\n não parametrizado no POL0779.'
      CALL log0030_mensagem(p_msg,'INFO')
      RETURN FALSE
   END IF}
   
   LET p_tip_inspecao = 'C'
   ...
   IF NOT pol0910_prepara_inspecao() THEN
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   END IF
...
#----------------------------------#
FUNCTION pol0910_prepara_inspecao()#
#----------------------------------#

   IF p_tip_inspecao = 'C' THEN
      CALL pol0910_calc_inspecao() RETURNING p_status
   ELSE
      CALL pol0910_compatib_inspecao() RETURNING p_status
   END IF
...
#Calcula as quantidades a serem inspecionadas no Logix

#-------------------------------#
FUNCTION pol0910_calc_inspecao()
#-------------------------------#

   DEFINE p_qtd_insp_calc LIKE cont_aparas_885.qtd_contagem,
   ...
   FOREACH cq_umd INTO
      ...
      LET p_fat_conversao = c_fat_conversao
      ...
      FOREACH cq_dif INTO
         ...
         IF p_qtd_liber_calc > p_qtd_dif THEN
            LET p_qtd_liber_calc = p_qtd_liber_calc - p_qtd_dif
         ELSE
            IF p_qtd_excep_calc > p_qtd_dif THEN
               LET p_qtd_excep_calc = p_qtd_excep_calc  - p_qtd_dif
            ELSE
               IF p_qtd_rejeit_calc > p_qtd_dif THEN
                  LET p_qtd_rejeit_calc = p_qtd_rejeit_calc - p_qtd_dif
               ELSE
                  CALL log0030_mensagem(
         ...
         UPDATE cont_aparas_885
            SET qtd_liber_calc  = p_qtd_liber_calc,
                qtd_excep_calc  = p_qtd_excep_calc,
                qtd_rejeit_calc = p_qtd_rejeit_calc
          WHERE cod_empresa   = p_cod_empresa
            AND num_aviso_rec = p_tela.num_aviso_rec
            AND num_seq_ar    = p_num_seq_ar
...
#utilização do peso balança p/ o Logix

#-----------------------------------#
FUNCTION pol0910_compatib_inspecao()#
#-----------------------------------#

   UPDATE cont_aparas_885
      SET qtd_liber_calc  = qtd_liber,
          qtd_excep_calc  = qtd_liber_excep,
          qtd_rejeit_calc = qtd_rejeit
     WHERE cod_empresa   = p_cod_empresa
       AND num_aviso_rec = p_tela.num_aviso_rec
            
   IF STATUS <> 0 THEN
      CALL log003_err_sql('Atualizanado','cont_aparas_885:pci')
      RETURN FALSE
   END IF
...
#-----------------------------# 
   ...
   FOREACH cq_ar INTO p_num_seq_ar, p_cod_item_tr, c_fat_conversao
      ...
      FOREACH cq_lanc INTO
         ...
         LET p_qtd_movto = p_qtd_liber_calc + p_qtd_excep_calc + p_qtd_rejeit_calc
         ...
         IF p_qtd_liber_calc > 0 THEN 
            LET p_qtd_movto = p_qtd_liber_calc
            LET p_qtd_trim = p_qtd_liber
            LET p_ies_situa = 'L'
         ...
         IF p_qtd_excep_calc > 0 THEN 
            LET p_qtd_movto = p_qtd_excep_calc
            LET p_qtd_trim = p_qtd_excep
            LET p_ies_situa = 'E'
            IF NOT pol0910_insere_estoque() THEN 
               RETURN FALSE 
            END IF
         ...
         IF p_qtd_rejeit_calc > 0 THEN 
            LET p_qtd_movto = p_qtd_rejeit_calc
            LET p_qtd_trim = p_qtd_rejeit
            LET p_ies_situa = 'R'
            IF NOT pol0910_insere_estoque() THEN 
               RETURN FALSE 
            END IF
...
#-----------------------------#
   ...
   IF p_qtd_liber_calc IS NULL THEN
      LET p_qtd_liber_calc = 0
   END IF

   IF p_qtd_excep_calc IS NULL THEN
      LET p_qtd_excep_calc = 0
   END IF

   IF p_qtd_rejeit_calc IS NULL THEN
      LET p_qtd_rejeit_calc = 0
   END IF
   
   SELECT cod_unid_med_nf,
          num_oc
     INTO l_cod_unid_med,
          l_num_oc
     FROM aviso_rec
   ...
   IF UPSHIFT(l_cod_unid_med) = 'KG' THEN      
   ELSE
      IF l_num_oc IS NULL OR l_num_oc = 0 THEN
         LET l_fat_conver = pol0910_le_item_fornec()
      ELSE
         ...
         IF STATUS <> 0 OR l_fat_conver IS NULL THEN
            LET l_fat_conver = 0
         END IF
         
         IF l_fat_conver = 0 THEN 
            LET l_fat_conver = pol0910_le_item_fornec()
         END IF
      END IF
   END IF
   ...
   IF l_fat_conver > 0 THEN
      LET p_qtd_rejeit_calc = p_qtd_rejeit_calc / l_fat_conver
      LET p_qtd_liber_calc = p_qtd_liber_calc / l_fat_conver
      LET p_qtd_excep_calc = p_qtd_excep_calc / l_fat_conver
   END IF
   ...
   UPDATE aviso_rec
      SET ies_liberacao_insp = 'S',
          ...
          qtd_rejeit         = p_qtd_rejeit_calc,
          qtd_liber          = p_qtd_liber_calc,
          qtd_liber_excep    = p_qtd_excep_calc
    WHERE cod_empresa   = p_cod_empresa
      AND num_aviso_rec = p_tela.num_aviso_rec
      AND num_seq       = p_num_seq_ar
...
#--------------------------------#
FUNCTION pol0910_le_item_fornec()
#--------------------------------#

   DEFINE l_fator   LIKE item_fornec.fat_conver_unid
   ...
   IF STATUS <> 0 OR l_fator IS NULL THEN
      LET l_fator = 0
   END IF
...
#--------------------------------#
   ...
   IF NOT pol0910_exporta_trim() THEN
      RETURN FALSE
   END IF
...
#------------------------------#      
   ...
   IF p_num_lote IS NULL OR p_num_lote = ' ' THEN      
      SELECT *
...
#----------------------------#
   ...
   LET p_qtd_liberada  = p_qtd_liberada  + p_qtd_liber_calc 
   LET p_qtd_lib_excep = p_qtd_lib_excep + p_qtd_excep_calc
   LET p_qtd_rejeitada = p_qtd_rejeitada + p_qtd_rejeit_calc
...
#-----------------------------------#
   ...
   LET p_estoque_lote_ender.identif_estoque    = NULL
...
#------------------------------------#
   ...
   INSERT INTO estoque_lote_ender(
          ...
          VALUES(p_estoque_lote_ender.cod_empresa,
                 ...
                 p_estoque_lote_ender.identif_estoque)
...
#----------------------------#
   ...
   DECLARE cq_mov_ar CURSOR FOR
    SELECT cod_operacao
      FROM aviso_rec_compl 
     WHERE cod_empresa = p_cod_empresa 
       AND num_aviso_rec = p_tela.num_aviso_rec
   
   FOREACH cq_mov_ar INTO p_cod_operacao
      IF STATUS <> 0 THEN
   ...
   IF p_cod_operacao IS NULL THEN
      LET p_msg = 'Não foi possivel ler a\n',
                  'operação do movto AR'
      CALL log0030_mensagem(p_msg,'info')
      RETURN FALSE
   END IF
   ...
   FOREACH cq_le_umd INTO p_num_seq_ar
      ...
      FOREACH cq_le_insp INTO p_num_transac, p_tip_movto, p_sequencia
         ...
         IF p_ies_situa = 'L' THEN
            LET p_qtd_liber_calc = p_qtd_movto
         ELSE
            IF p_ies_situa = 'E' THEN
               LET p_qtd_excep_calc = p_qtd_movto
            ELSE
               IF p_ies_situa = 'R' THEN
                  LET p_qtd_rejeit_calc = p_qtd_movto
               END IF
            END IF
         END IF
...
#------------------------------#
FUNCTION pol0910_exporta_trim()#
#------------------------------#

   INITIALIZE p_insumo TO NULL
            
   LET p_insumo.num_sequencia = p_num_transac
   LET p_insumo.cod_empresa = p_estoque_trans.cod_empresa
   LET p_insumo.num_lote = p_lote_trim
   LET p_insumo.cod_item = p_estoque_trans.cod_item
   LET p_insumo.largura = 0
   LET p_insumo.diametro = 0   
   LET p_insumo.tubete = 0  
   LET p_insumo.num_nf = p_tela.num_nf    
   LET p_insumo.num_ar = p_tela.num_aviso_rec
   LET p_insumo.cod_fornecedor = p_tela.cod_fornecedor
   LET p_insumo.nom_fornecedor = p_tela.nom_fornecedor
   LET p_insumo.dat_emis_nf = p_dat_emis_nf
   LET p_insumo.dat_movto = p_estoque_trans.dat_movto
   LET p_insumo.qtd_movto = p_qtd_trim
   LET p_insumo.val_movto = p_val_movto
   LET p_insumo.qtd_fardos = p_qtd_fardo   
   LET p_insumo.tip_movto = p_estoque_trans.ies_tip_movto   
   LET p_insumo.cod_status = 0
   LET p_insumo.ies_bobina = 'N'   
   LET p_insumo.num_seq_ar = p_estoque_trans.num_seq  
   LET p_insumo.dat_entrada_nf = p_tela.dat_entrada
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('INSERT','insumo_885:N')
      RETURN FALSE
   END IF
...
#--------------------------#
   ...
   SELECT *
     ...
     FROM insumo_885
    WHERE cod_empresa = p_cod_empresa
      AND num_sequencia = p_trans_rev   #transação sendo revetida
   ...
   LET p_insumo.num_sequencia = p_num_transac  #transação da reversão
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('INSERT','insumo_885:R')
      RETURN FALSE
   END IF
