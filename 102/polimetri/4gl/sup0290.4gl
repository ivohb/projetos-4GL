...
# SISTEMA.: SUPRIMENTOS                                               #
# PROGRAMA: SUP0290                                                   #
# MODULOS.: SUP0290 - LOG0010 - LOG0030 - LOG0040 - LOG0050 - LOG0060 #
#           SUP1000 - SUP1040 - SUP1280 - SUP1310 - SUP1320 - SUP1360 #
#           SUP1370 - SUP3740 - SUP4110 - CON0880 - CAP0580 - SUP7020 #
# OBJETIVO: MANUTENCAO ORDEM DE COMPRA - MATERIAL DE ESTOQUE          #
# AUTOR...: GILBERTO TAMANINI                                         #
# DATA....: 15/12/1992                                                #
#---------------------------------------------------------------------#
DATABASE logix
...
MAIN
  ...
  CALL fgl_setenv("VERSION_INFO","L10-SUP0290-10.02.$Revision: 24 $p") #Informacao da versao do programa controlado pelo SourceSafe - Nao remover esta linha.
  LET p_versao = "SUP0290-10.02.24p" #Favor nao alterar esta linha (SUPORTE)
...
#--------------------------#
  ...
  COMMAND "Q_prog.entrega" "Modifica os programas de entrega "
    ...
    LET p_num_oc = gr_dados_tela_com.num_oc #ivo
    LET p_qtd_solic = gr_dados_tela_com.qtd_solic #ivo

    CALL log120_procura_caminho("sup0360") RETURNING comando
...
#------------------------#
FUNCTION sup029_trava90()
#------------------------#

   DEFINE p_qtd_ordem    LIKE ordem_sup.num_oc,
   ...
   IF p_qtd_ordem = p_qtd_solic THEN
      RETURN
   END IF

   IF p_qtd_ordem < p_qtd_solic THEN
      IF p_ies_situa_oc <> 'X' THEN
         RETURN
      END IF
   END IF
   
   #CALL pol1234_trava90(p_num_oc) RETURNING p_status #ivo
  ...
  IF sup029_entrada_dados("INCLUSAO",1)  THEN
     ...
     IF NOT l_houve_erro THEN
        ...
        IF sqlca.sqlcode <> 0 THEN
           ...
           LET l_houve_erro = TRUE
        END IF
        #ivo - chama função, que irá analisar as regras do trava90
        #se a ordem não atender os requisitos, ficará bloqueada (status X)
        #IF NOT pol1234_trava90(p_prog_ordem_sup.num_oc) THEN #ivo
        #   LET l_houve_erro = TRUE                           #ivo
        #END IF                                               #ivo
     END IF
...
#---fim----OS704186#
        ...
        IF NOT l_houve_erro THEN
           ...
           DELETE FROM dest_prog_ord_sup
            WHERE dest_prog_ord_sup.cod_empresa = p_ordem_sup.cod_empresa
              AND dest_prog_ord_sup.num_oc      = p_ordem_sup.num_oc
              AND dest_prog_ord_sup.num_versao  = p_ordem_sup.num_versao
           WHENEVER ERROR STOP
           IF sqlca.sqlcode <> 0 THEN
              CALL log003_err_sql("EXCLUSAO","DEST_PROG_ORD_SUP")
              LET l_houve_erro = TRUE
           END IF
        END IF
...
#-------------------------------#
 FUNCTION sup0290_version_info()
#-------------------------------#
  RETURN "$Archive: /logix10R2/suprimentos/suprimentos/programas/sup0290.4gl $|$Revision: 22 $|$Date: 18/01/11 9:16 $|$Modtime: 10/01/11 13:51 $" #Informações do controle de versão do SourceSafe - Não remover esta linha (FRAMEWORK)
 END FUNCTION
