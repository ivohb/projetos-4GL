...
GLOBALS
   ...
   DEFINE t_estrut_item ARRAY[100] OF RECORD
      ...
      ies_arredonda LIKE estrut_item_indus.ies_arredonda
   END RECORD
...
#--------------------------------------#
   ...
   IF pol0272_entrada_dados() THEN
      ...
      IF pol0272_entrada_itens("INCLUSAO") THEN
         ...
         FOR p_i = 1 TO 100
            IF t_estrut_item[p_i].seq_estrut IS NULL AND
               t_estrut_item[p_i].cod_item_ret IS NULL THEN
               EXIT FOR
            ELSE
               ...
               LET p_estrut_item_indus.ies_arredonda = 
                   t_estrut_item[p_i].ies_arredonda
...
#------------------------------#
   ...
   FOR p_ind = 1 TO ARR_COUNT()
       IF p_ind = pa_curr THEN
          CONTINUE FOR
       END IF
       IF t_estrut_item[p_ind].cod_item_ret = t_estrut_item[pa_curr].cod_item_ret THEN
          RETURN TRUE
          EXIT FOR
       END IF
   END FOR
   ...
   INPUT ARRAY t_estrut_item WITHOUT DEFAULTS FROM s_estrut_item.*
      ...
      AFTER FIELD cod_item_ret
         ...
         IF t_estrut_item[pa_curr].cod_item_ret IS NOT NULL THEN
            ...
            IF pol0272_repetiu_item() THEN
               CALL log0030_mensagem("Item já Lançado !!!","excla")
               NEXT FIELD cod_item_ret
            END IF
            ...
            IF SQLCA.SQLCODE <> 0 THEN                            
               ERROR "Cod Item Retorno nao Cadastrado"            
               NEXT FIELD cod_item_ret                            
            ELSE                                                  
            ...
            IF t_estrut_item[pa_curr].seq_estrut IS NULL THEN
               ...
               DISPLAY pa_curr TO s_estrut_item[sc_curr].seq_estrut
            END IF           
         END IF
      ...
      AFTER FIELD qtd_item_ret
         IF t_estrut_item[pa_curr].qtd_item_ret IS NULL THEN
            LET t_estrut_item[pa_curr].qtd_item_ret = 0
            DISPLAY t_estrut_item[pa_curr].qtd_item_ret TO 
                    s_estrut_item[sc_curr].qtd_item_ret
         END IF

      AFTER FIELD ies_arredonda
         IF t_estrut_item[pa_curr].ies_arredonda MATCHES '[SN]' THEN
         ELSE
            NEXT FIELD ies_arredonda
         END IF
   ...
   FORMAT
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 001, p_relat.seq_estrut USING "####&",
               ...
               COLUMN 077, p_relat.ies_arredonda
...
CREATE TABLE estrut_item_indus
    (
    cod_empresa  CHAR (2),
    cod_item_prd CHAR (15),
    dat_inclusao DATE,
    cod_cliente  CHAR (15),
    seq_estrut   INTEGER,
    cod_item_ret CHAR (15),
    qtd_item_ret DECIMAL(12,7),
    ies_arredonda CHAR(01)
    );

CREATE UNIQUE INDEX ix_estrut_1
    ON estrut_item_indus (cod_empresa, cod_item_prd, dat_inclusao, cod_cliente, seq_estrut);
