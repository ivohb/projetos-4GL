...
MAIN
   ...
   LET p_versao = "pol1328-12.00.104 "
...
#-------------------------------#
   ...
   DEFINE l_cod_fatura    INTEGER,
          l_num_seq       INTEGER,
          l_manut         LIKE audit_cap.desc_manut,
          l_hora          LIKE audit_cap.hora_manut,
          l_data          LIKE audit_cap.data_manut,
          l_num_ad        INTEGER
   ...
   FOREACH cq_del_ad INTO 
      ...
      DELETE FROM ctb_lanc_ctbl_cap WHERE empresa = p_cod_empresa AND num_ad_ap = l_num_ad
      
      IF STATUS <> 0 THEN
         RETURN FALSE
      END IF

      DELETE FROM lanc_cont_cap WHERE cod_empresa = p_cod_empresa AND num_ad_ap = l_num_ad
