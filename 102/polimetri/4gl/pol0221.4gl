...
# AUTOR...: Ivo  (conversão para versão 10.02)                      #
# DATA....: 25/06/2010                                              #
#-------------------------------------------------------------------#
DATABASE logix
...
GLOBALS
   ...
   DEFINE p_wfat_mestre        RECORD LIKE wfat_mestre.*,
          ...
          p_fat_rems_item_nf   RECORD LIKE fat_rems_item_nf.*,
          ...
          p_fat_resv_item_nf   RECORD LIKE fat_resv_item_nf.*,
          p_sup_par_resv_est   RECORD LIKE sup_par_resv_est.*,
...
MAIN
   ...
   WHENEVER ANY ERROR CONTINUE
      ...
      DEFER INTERRUPT
   LET p_versao = "pol0221-10.02.18"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0221.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol0221_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0221_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0221") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0221 AT 2,2 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   DISPLAY p_cod_empresa TO cod_empresa
   
   CALL pol0221_cria_temporaria()

   IF NOT pol0221_le_param() THEN
      RETURN FALSE
   END IF
   ...
   MENU "OPCAO"
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0221

END FUNCTION

#-----------------------#
FUNCTION pol0221_sobre()
#-----------------------#
...
#--------------------------------------#
 FUNCTION pol0221_inclusao()
#--------------------------------------#
 
   LET p_houve_erro = FALSE
   CLEAR FORM
   IF pol0221_entrada_dados() THEN
      ...
      IF pol0221_info_preco() THEN
         CALL log085_transacao("BEGIN")
         IF pol0221_processa() THEN
...
#-------------------------------#
 FUNCTION pol0221_entrada_dados()
#-------------------------------#

   DISPLAY p_cod_empresa TO cod_empresa
   CALL log006_exibe_teclas("01 02 07",p_versao)
   CURRENT WINDOW IS w_pol0221
   INITIALIZE p_wfat_mestre.* TO NULL
   INITIALIZE p_item_de_terc.* TO NULL
   INITIALIZE p_texto.*, pr_nf TO NULL
   LET p_wfat.tip_carteira     = '02'
   ...
   INPUT BY NAME p_wfat_mestre.cod_cliente, 
      ...
      AFTER FIELD num_nff1
         ...
         IF NOT pol0221_checa_nf(p_wfat.num_nff1) THEN
            NEXT FIELD num_nff1
         END IF
      ...
      AFTER FIELD num_nff2
         ...
         IF p_wfat.num_nff2 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff2) THEN
               NEXT FIELD num_nff2
            END IF
      ...
      AFTER FIELD num_nff3
         ...
         IF p_wfat.num_nff3 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff3) THEN
               NEXT FIELD num_nff3
            END IF
      ...
      AFTER FIELD num_nff4
         ...
         IF p_wfat.num_nff4 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff4) THEN
               NEXT FIELD num_nff4
            END IF
      ...
      AFTER FIELD num_nff5
         ...
         IF p_wfat.num_nff5 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff5) THEN
               NEXT FIELD num_nff5
            END IF
      ...
      AFTER FIELD num_nff6
         ...
         IF p_wfat.num_nff6 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff6) THEN
               NEXT FIELD num_nff6
            END IF
      ...
      AFTER FIELD num_nff7
         ...
         IF p_wfat.num_nff7 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff7) THEN
               NEXT FIELD num_nff7
            END IF
      ...
      AFTER FIELD num_nff8
         ...
         IF p_wfat.num_nff8 IS NOT NULL THEN
            IF NOT pol0221_checa_nf(p_wfat.num_nff8) THEN
               NEXT FIELD num_nff8
            END IF
...
#----------------------------------#
FUNCTION pol0221_checa_nf(p_num_nf)
#----------------------------------#
...
#--------------------------------#
FUNCTION pol0221_cria_temporaria()
#--------------------------------#
...
#-----------------------------#
   ...
   INPUT ARRAY t_fat_emb WITHOUT DEFAULTS FROM s_fat_emb.*
    ATTRIBUTES(INSERT ROW = FALSE, DELETE ROW = FALSE)
      ...
      IF t_fat_emb[pa_curr].qtd_item > 0  THEN 
         IF NOT pol0221_verifica_estoque() THEN
            ERROR "Não existe saldo suficiente para este item"
            NEXT FIELD qtd_item 
         END IF
      END IF
...
END FUNCTION
#----------------------------------#
FUNCTION pol0221_verifica_estoque()
#----------------------------------#
   DEFINE w_qtd_estoque       LIKE estoque_lote.qtd_saldo,
          w_qtd_reserva       LIKE estoque_loc_reser.qtd_reservada,
          w_qtd_saldo         LIKE estoque_loc_reser.qtd_reservada,
          w_ies_ctr_estoque   LIKE item.ies_ctr_estoque
          
          
   LET    w_qtd_estoque       =  0 
   LET    w_qtd_reserva       =  0 
   LET    w_qtd_saldo         =  0     
   INITIALIZE w_ies_ctr_estoque    TO NULL             
          
        SELECT ies_ctr_estoque
        INTO w_ies_ctr_estoque
        FROM item
       WHERE cod_empresa = p_cod_empresa
         AND cod_item    = t_fat_emb[pa_curr].cod_item
              
             
        IF SQLCA.SQLCODE <> 0 THEN 
          CALL log003_err_sql("Lendo 2","ITEM")
          RETURN FALSE
        END IF
          
          
        IF w_ies_ctr_estoque  <>   'S'   THEN 
           RETURN TRUE 
        END IF    
          ...
          WHERE cod_empresa = p_cod_empresa
            AND cod_item    = t_fat_emb[pa_curr].cod_item
            AND cod_local   in (select cod_local_estoq from ITEM
                  where   cod_empresa = p_cod_empresa
                  AND     cod_item    = t_fat_emb[pa_curr].cod_item)
            AND ies_situa_qtd='L'

                     
           IF w_qtd_estoque   IS NULL THEN 
              LET w_qtd_estoque = 0 
           END IF         
          ...
          WHERE cod_empresa = p_cod_empresa
           ...
           IF w_qtd_reserva  IS NULL THEN 
              LET w_qtd_reserva = 0 
           END IF    
           
           IF w_qtd_reserva  < 0 THEN 
              LET w_qtd_reserva = 0 
           END IF    
           
           
           LET w_qtd_saldo = w_qtd_estoque - w_qtd_reserva
             
           IF  w_qtd_saldo < t_fat_emb[pa_curr].qtd_item THEN
               RETURN FALSE
           END IF     
...
END FUNCTION 
#--------------------------#
FUNCTION pol0221_processa()
#--------------------------#

   IF NOT pol0221_fat_mestre() THEN
      RETURN FALSE
   END IF
   
   IF NOT pol0221_le_me() THEN
      RETURN FALSE
   END IF
   ...
   FOR p_ind = 1 TO p_qtd_item
       ...
       IF NOT pol0221_le_item() THEN
          RETURN FALSE
       END IF
       
       MESSAGE 'Lendo parâmetros fiscais'
       
       IF NOT pol0221_le_param_fisc() THEN
          RETURN FALSE
       END IF
       ...
       IF NOT pol0221_fat_item() THEN
          RETURN FALSE
       END IF
          
       IF NOT pol0221_item_fisc() THEN
          RETURN FALSE
       END IF
       
       IF NOT pol0221_reserva_estoque() THEN
          RETURN FALSE
       END IF

       IF NOT pol0221_rems_item() THEN
          RETURN FALSE
       END IF

   END FOR

   IF NOT pol0221_mestre_fisc() THEN
      RETURN FALSE
   END IF

   IF NOT pol0221_atu_fat_mestre() THEN
      RETURN FALSE
   END IF

   IF NOT pol0221_grava_texto() THEN
      RETURN FALSE
   END IF
...
#-------------------------------#
FUNCTION pol0221_le_param_fisc()
#-------------------------------#
   ...
   IF NOT pol0221_le_tip_item() THEN
      RETURN FALSE
   END IF
   ...
   FOREACH  cq_tributos INTO
      ...
      FOREACH cq_acesso INTO p_num_ctr_acesso, p_seq_acesso
         ...
         CALL pol0221_pega_chave()

         IF NOT pol0221_checa_tributo() THEN
            RETURN FALSE
         END IF
...
#-------------------------------#
FUNCTION pol0221_checa_tributo()
#-------------------------------#
   ...
   FOREACH cq_chave INTO p_chave
      ...
      WHEN 'REGIAO' 
         IF NOT pol0221_le_obf_regiao() THEN
            RETURN FALSE
         END IF
      ...
      WHEN 'GRP_CLASSIF' 
         IF NOT pol0221_le_obf_cl_fisc() THEN
            RETURN FALSE
         END IF
      ...
      WHEN 'GRP_ITEM' 
         IF NOT pol0221_le_obf_fisc_item() THEN
            RETURN FALSE
         END IF
      ...
      WHEN 'GRP_CLIENTE' 
         IF NOT pol0221_le_obf_fisc_cli() THEN
            RETURN FALSE
         END IF
...
#-------------------------------#
FUNCTION pol0221_le_obf_regiao()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol0221_le_obf_cl_fisc()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol0221_le_obf_fisc_item()
#---------------------------------#
...
#---------------------------------#
FUNCTION pol0221_le_obf_fisc_cli()
#---------------------------------#
...
END FUNCTION
#---------------------------------#
FUNCTION pol0221_reserva_estoque()
#---------------------------------#
   DEFINE p_qtd_estoque       LIKE estoque_lote.qtd_saldo,
          p_saldo_nota        LIKE estoque_lote.qtd_saldo,
          p_qtd_a_faturar     LIKE estoque_lote.qtd_saldo,
    ...
    LET   p_qtd_estoque       = 0 
    LET   p_saldo_nota        = 0 
    LET   p_qtd_a_faturar     = 0 
    LET   p_qtd_reservada     = 0 


     INITIALIZE p_estoque_loc_reser.*, p_fat_resv_item_nf.*, p_est_loc_reser_end.* , p_sup_par_resv_est.* TO NULL 
     
   
#---Alterado em 02/08/2010 por Manuel - Verifica se o  item controla lote 

         IF p_ies_ctr_estoque <> 'S' THEN 
            RETURN TRUE
         END IF    
         
         LET p_saldo_nota  = p_qtd_fat
 
           
#---Alterado em 02/08/2010 por Manuel - Verifica se ha saldo suficiente no estoque 

         DECLARE cq_estoque CURSOR FOR 
         SELECT *  
           FROM estoque_lote
          WHERE cod_empresa   = p_cod_empresa
            AND ies_situa_qtd = 'L'
       ...
       FOREACH cq_estoque INTO p_estoque_lote.*       
         
           IF p_estoque_lote.qtd_saldo IS NULL THEN 
              LET p_saldo = 0 
              CONTINUE FOREACH
           END IF 

		   #---Inicio da alteraçao de 24/07/2012 versão 12 por Manuel - Verifica quanto já existe de reserva para o item/lote       
         ...
         END IF   
		   ...
		   #---Fim da alteraçao de 24/07/2012 versão 12 por Manuel  		   
		   
         LET p_qtd_estoque = p_estoque_lote.qtd_saldo - p_qtd_reservada
          ...
          IF  p_qtd_estoque < p_saldo_nota  THEN 
              LET p_saldo_nota    = p_saldo_nota - p_qtd_estoque
              LET p_qtd_a_faturar = p_qtd_estoque
          ELSE    
              LET p_qtd_a_faturar = p_saldo_nota
              LET p_saldo_nota = 0
          END IF     
         

#---Alterado em 02/08/2010 por Manuel - para gravar a reserva na tabela  ESTOQUE_LOC_RESER

          LET p_estoque_loc_reser.cod_empresa     = p_cod_empresa
          LET p_estoque_loc_reser.num_reserva     = 0
          LET p_estoque_loc_reser.cod_item        = p_cod_item
          LET p_estoque_loc_reser.cod_local       = p_local_estoq
          LET p_estoque_loc_reser.qtd_reservada   = p_qtd_a_faturar
          LET p_estoque_loc_reser.num_lote        = p_estoque_lote.num_lote
          ...
          WHENEVER ERROR CONTINUE
            ...
            VALUES(p_cod_empresa,                                              
                  ...
                  IF p_estoque_lote.num_lote IS NOT NULL THEN
                     SELECT num_transac 
                       INTO   p_num_transac
                  ...
                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("INCLUSAO","SUP_RESV_LOTE_EST")                                                                                             
                     RETURN FALSE                                                       
                  END IF    
                  ...
                  WHERE SUP_PAR_RESV_EST.EMPRESA=p_cod_empresa 
                    AND SUP_PAR_RESV_EST.RESERVA=p_est_loc_reser_end.num_reserva 
                    AND SUP_PAR_RESV_EST.PARAMETRO='efetiva_parcial     '


                  LET p_sup_par_resv_est.empresa 					= p_cod_empresa
                  LET p_sup_par_resv_est.reserva 					= p_est_loc_reser_end.num_reserva
                  LET p_sup_par_resv_est.parametro 				= 'efetiva_parcial     '
                  LET p_sup_par_resv_est.des_parametro 		= 'Reserva de vendas que permite efetivação parcial            '
                  

                  INSERT INTO SUP_PAR_RESV_EST 
                              VALUES(p_sup_par_resv_est.*)
                  
                                 
                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("INCLUSAO 1","SUP_PAR_RESV_EST")                                                                                             
                     RETURN FALSE                                                       
                  END IF  
                  ...
                  WHERE SUP_PAR_RESV_EST.EMPRESA=p_cod_empresa 
                    AND SUP_PAR_RESV_EST.RESERVA=p_est_loc_reser_end.num_reserva 
                    AND SUP_PAR_RESV_EST.PARAMETRO='exige_saldo_reserva '


                  LET p_sup_par_resv_est.empresa 					= p_cod_empresa
                  LET p_sup_par_resv_est.reserva 					= p_est_loc_reser_end.num_reserva
                  LET p_sup_par_resv_est.parametro 				= 'exige_saldo_reserva '
                  LET p_sup_par_resv_est.des_parametro 		= 'Reserva incluida com saldo de estoque para ser atendida.    '
                  ...
                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("INCLUSAO 2","SUP_PAR_RESV_EST")                                                                                             
                     RETURN FALSE                                                       
                  END IF  
                  ...
                  LET p_sup_par_resv_est.empresa 					= p_cod_empresa
                  LET p_sup_par_resv_est.reserva 					= p_est_loc_reser_end.num_reserva
                  LET p_sup_par_resv_est.parametro 				= 'sit_est_reservada   '
                  LET p_sup_par_resv_est.des_parametro 		= 'Situação do estoque que está sendo reservado.               '
                  LET p_sup_par_resv_est.parametro_ind   	= 'L'
                  ...
                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("INCLUSAO 3","SUP_PAR_RESV_EST")                                                                                             
                     RETURN FALSE                                                       
                  END IF  
                  
                  UPDATE estoque SET qtd_reservada= qtd_reservada + p_qtd_a_faturar 
                  WHERE  cod_empresa = p_cod_empresa 
                  AND cod_item = p_cod_item


                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("ATUALIZAÇÃO 3","ESTOQUE")                                                                                             
                     RETURN FALSE                                                       
                  END IF  


                  LET p_fat_resv_item_nf.empresa 						= p_cod_empresa
                  LET p_fat_resv_item_nf.trans_nota_fiscal 	= p_trans_nf 
                  LET p_fat_resv_item_nf.seq_item_nf        = p_ind
                  LET p_fat_resv_item_nf.reserva_estoque    = p_est_loc_reser_end.num_reserva
                  LET p_fat_resv_item_nf.qtd_reservada      = p_qtd_a_faturar
                  LET p_fat_resv_item_nf.trans_estoque      = p_num_transac
                  LET p_fat_resv_item_nf.seq_tabulacao      = 1


                  INSERT INTO FAT_RESV_ITEM_NF 
                              VALUES(p_fat_resv_item_nf.*)
                  
                                 
                  IF SQLCA.SQLCODE <> 0 THEN                                            
                     CALL log003_err_sql("INCLUSAO","FAT_RESV_ITEM_NF ")                                                                                             
                     RETURN FALSE                                                       
                  END IF  
                  
                  IF p_saldo_nota <= 0   THEN
                     EXIT FOREACH
                  END IF      
      END FOREACH

      IF p_saldo_nota > 0   THEN
         CALL log003_err_sql("FALTA ESTOQUE","ESTOQUE_LOTE ")   
         RETURN FALSE 
      END IF   
...
END FUNCTION
#-----------------------------# 
FUNCTION pol0221_rems_item()
#-----------------------------#
   DEFINE p_cod_remessa    LIKE motivo_remessa.cod_motivo_remessa             
       

   MESSAGE 'Gravando fat_rems_item_nf!'   
   
   INITIALIZE p_fat_rems_item_nf TO NULL

#   SELECT PAR_TXT[1, 2]
#   INTO  
#   p_cod_remessa
#   FROM PAR_SUP_PAD WHERE PAR_SUP_PAD.COD_EMPRESA=p_cod_empresa
#    AND PAR_SUP_PAD.COD_PARAMETRO='cod_motivo_remessa'
#    
#   IF STATUS <> 0 THEN
...
#   END IF
   ...
   LET p_fat_rems_item_nf.empresa              = p_cod_empresa   
   LET p_fat_rems_item_nf.trans_nota_fiscal    = p_trans_nf      
   LET p_fat_rems_item_nf.seq_item_nf          = p_ind           
   LET p_fat_rems_item_nf.motivo_remessa       = '03'
   LET p_fat_rems_item_nf.val_estoque          = 0
   LET p_fat_rems_item_nf.area_negocio         = p_cod_lin_prod
   LET p_fat_rems_item_nf.linha_negocio        = p_cod_lin_recei
   LET p_fat_rems_item_nf.usuario              = p_user
   LET p_fat_rems_item_nf.pedido_compra        = 0
             

      
      
   INSERT INTO fat_rems_item_nf
      VALUES(p_fat_rems_item_nf.*)
      
   IF STATUS <> 0 THEN
      CALL log003_err_sql('Inserindo','fat_rems_item_nf')
      RETURN FALSE
   END IF
   RETURN TRUE
...
#-----------------------------#
   ...
   FOREACH cq_sum INTO 
      ...
      IF STATUS <> 0 THEN
         CALL log003_err_sql('Lendo','cq_sum')
         RETURN FALSE
      END IF
...
#--------------------------------#
FUNCTION pol0221_atu_fat_mestre()
#--------------------------------#
