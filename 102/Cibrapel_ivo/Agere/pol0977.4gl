...
#------------------------------#
   ...
   IF NOT pol0977_cria_tab_temp() THEN
      RETURN FALSE
   END IF
   ...
   FOREACH cq_op INTO 
      ...
      IF NOT pol0977_deleta_erro() THEN
         RETURN FALSE
      END IF
...
#-------------------------------#
FUNCTION pol0977_cria_tab_temp()
#-------------------------------#

   CREATE TEMP TABLE ord_criticada_912
   (
     num_ordem  INTEGER,
     cod_operac CHAR(05)
   );
   ...
   IF SQLCA.SQLCODE <> 0 THEN 
      DELETE FROM ord_criticada_912
      IF SQLCA.SQLCODE <> 0 THEN 
         LET p_msg = 'ERRO:(',STATUS, ') DELETANDO ORD_CRITICADA_912'
         RETURN FALSE
      END IF
   END IF
...
#---------------------------------#
   ...
   IF NOT pol0977_deleta_lote() THEN
      RETURN FALSE
   END IF
...
#---------------------------#
   ...
   FOREACH cq_erro INTO 
      ...
      LET pr_erro[p_index].datconsumo = EXTEND(datconsumo_e, YEAR TO DAY)
