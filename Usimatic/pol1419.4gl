...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: pol1419                                                 #
# OBJETIVO: CARGA E PROCESSAMENTO DO EDI FORECAST  - USIMATIC       #
# AUTOR...: IVO                                                     #
# DATA....: 27/01/21                                                #
#-------------------------------------------------------------------#
...
#---------------------------#
   ...
   DROP TABLE w_prog_komatsi;
   
   CREATE TEMP TABLE w_prog_komatsi (
    tip_prog              VARCHAR(08),
    ano_plano             DECIMAL(4,0),
    mes_plano             DECIMAL(2,0),
    qtd_plano             DECIMAL(10,3)      
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','tabele w_prog_komatsi')
      RETURN FALSE
   END IF
   
   CREATE INDEX ix_w_prog_komatsi ON w_prog_komatsi(tip_prog)

   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','indice ix_w_prog_komatsi')
      RETURN FALSE
   END IF
...
#-----------------------------------------#
    ...
    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_panel_prog)
    ...
    CALL _ADVPL_set_property(l_tabcolumn,"VARIABLE","dat_prog")
    CALL _ADVPL_set_property(l_tabcolumn,"ORDER",TRUE)
    ...
    CALL _ADVPL_set_property(l_tabcolumn,"COLUMN_WIDTH",80)
    CALL _ADVPL_set_property(l_tabcolumn,"VARIABLE","tip_prog")
...
#---------------------------------#
   ...
   LET m_item_cliente = ma_plano[m_lin_atu].item
...
#----------------------------#
   ...
   IF STATUS = 100 THEN
      LET m_msg = 'Caminho do sistema PVK não cadastrado na LOG1100/log00098'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   ELSE
      IF STATUS <> 0 THEN
...
#-------------------------#
   ...
   FOREACH cq_qfp INTO m_registro
      ...
      IF mr_itens.local_geogr IS NULL THEN
         LET m_msg = m_msg CLIPPED, 'Região geográfica inválida;'
      END IF
   ...
   IF NOT l_ies_forec THEN
      LET m_msg = 'Arquivo selecionado está fora\n do lay-out para forecast'
...
#----------------------------#
   ...
   WHILE m_ind <= m_qtd_firme
      ...
      IF ma_data[m_ind].dat_prog IS NULL THEN
         LET ma_data[m_ind].erro = 'Data diaria do plano inválida'
      END IF
...
{      
   ...
   IF STATUS = 100 THEN
      LET m_msg = 'Item cliente não cadastrado no MAN10021'
      LET mr_itens.mensagem = m_msg
      LET mr_itens.situacao = 'E'
      RETURN TRUE
   ELSE
...
#--------------------------#
   ...
   IF l_data IS NULL THEN
      LET ma_data[m_ind].erro = 'Data inválida'
   ELSE
...
#--------------------------------#
   ...
   DELETE FROM w_prog_komatsi
   
   SELECT COUNT(*) INTO l_count FROM w_prog_komatsi
   
   IF l_count > 0 THEN
      LET m_msg = 'Erro limpando tabela temporária w_prog_komatsi'
      CALL log0030_mensagem(m_msg,'info')
      RETURN
   END IF      
   ...
   FOREACH cq_le_prog INTO             
      ...
      m_seq_pc                         
        ...
        ELSE
           IF NOT pol1419_le_ped_itens() THEN                     
              RETURN                                         
           END IF           
           IF m_ja_tem THEN
   ...
   IF m_ind <= 1000 AND m_ies_status <> 'E' THEN
      IF NOT pol1419_prog_nao_enviada() THEN
         RETURN
      END IF
   END IF
   ...
   FOREACH cq_prog_temp INTO 
      ...
      IF STATUS <> 0 THEN                                                                                
         CALL log003_err_sql('SELECT','w_prog_komatsi:cq_prog_temp')                                           
         EXIT FOREACH                                                                                       
      END IF                                                                                                
