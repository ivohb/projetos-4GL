...
#--------------------------#
   ...
   MENU "OPCAO"
      COMMAND "Consultar" "Consulta apontamento enviado pelo trim"
         CALL pol0915_consulta() RETURNING p_status
         IF p_status THEN
            LET p_ies_cons = TRUE 
         ELSE
            CLEAR FORM
...
#--------------------------#
   ...
   LET sql_stmt = "SELECT numsequencia, numpedido, numordem, codmaquina ",
                  "  FROM apont_trim_885 ",
                  " WHERE ", where_clause CLIPPED,
                  "   AND codempresa = '",p_cod_empresa,"' ",
                  " order by numpedido, numordem, codmaquina "
