...
#----------------------------------------#
   ...
   FOR l_pos_ini = 1 TO LENGTH(l_texto)
      IF l_texto[l_pos_ini] IS NULL OR l_texto[l_pos_ini] = ' ' THEN
      ELSE
         EXIT FOR
      END IF
   END FOR
   ...
   FOR l_pos_fim = LENGTH(l_texto) TO 1 STEP -1
      IF l_texto[l_pos_fim] IS NULL OR l_texto[l_pos_fim] = ' ' THEN
      ELSE
         EXIT FOR
      END IF
   END FOR
...
#-------------------------------------------#
#Criptografa o texto no padrão MD5          #
#-------------------------------------------#
# Parâmetro: texto a ser criptografado      #
...
#-------------------------------------------#
#Verifica se a senha contém pelo menos uma, #
# letra, um número e um carcatere especial  #
#-------------------------------------------#
# Parâmetro: senha                          #
# Retorno: True: senha forte                #
#       ou False: senha fraca               #
#-------------------------------------------#
FUNCTION func002_validaSenha(l_senha) 
#-------------------------------------------#
		
		DEFINE  l_senha    VARCHAR(15),
		        l_ind      INTEGER,
		        l_tamanho  INTEGER,
		        l_digito   CHAR(01),
		        l_maiusc   INTEGER,
		...
		LET l_tamanho = LENGTH(l_senha CLIPPED)

		IF l_tamanho < 4 OR l_tamanho > 15  THEN
			RETURN FALSE
    END IF		
		...
		FOR l_ind = 1 TO l_tamanho
			...
			LET l_digito = l_senha[l_ind]

			IF l_digito MATCHES "[ABCDEFGHIJKLMNOPQRSTUVWXYZ]"  THEN
			   LET l_maiusc = l_maiusc + 1
			ELSE
   			 IF l_digito MATCHES "[0123456789]"  THEN
		   	    LET l_numero = l_numero + 1
    		 ELSE
   			    IF l_digito MATCHES "[!@#$%&*()+?<>{}=-_|.,;:]"  THEN
		   	       LET l_especial = l_especial + 1
            ELSE
  			       IF l_digito MATCHES "[abcdefghijklmnopqrstuvwxyz]"  THEN
			            LET l_minusc = l_minusc + 1
			         END IF
		   	    END IF
		   	 END IF
		  END IF
   ...
   IF l_maiusc = 0 OR l_numero = 0 OR l_especial = 0 OR l_minusc = 0 THEN
			RETURN FALSE
   END IF
