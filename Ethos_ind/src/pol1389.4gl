...
#----------------------#
    ...
    CALL _ADVPL_set_property(l_proces,"TOOLTIP","Processa a alteração do plano")
...
#---------------------------------#
   ...
   IF m_count = 0 THEN
      LET m_msg = 'Não há ordens para o pedido/entrega informados'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF
...
#--------------------------#
   ...
   IF l_count <> m_count THEN
      ...
      LET m_msg = 'Número de registros da programação antiga: ',m_count USING '<<<<<', '\n',
                  'Número de registros da programação atual.: ',l_count USING '<<<<<'
      CALL log0030_mensagem(m_msg,'info')
      RETURN FALSE
   END IF
   ...
   FOREACH cq_comp_prog INTO l_programa
      ...
      IF l_qrt_prog <> l_count THEN
         LET m_msg = 'Plano: ', l_programa, 'Número de registros do TASK \n',
...
#-----------------------------#
   ...
   FOREACH cq_exib INTO l_programa,
      ...
      IF STATUS <> 0 THEN
         IF STATUS = 100 THEN
            LET m_msg = m_msg CLIPPED, '- programa/OP não existe em man_apo_nest;'
         ELSE
      ...
      ELSE
         IF ma_ordens[m_ind].tip_registro <> 'A' THEN
            LET m_msg = m_msg CLIPPED, '- plano não apontado;'
         END IF
      END IF
      ...
      IF STATUS <> 0 THEN
         IF STATUS = 100 THEN
            LET m_msg = m_msg CLIPPED, '- ordem antiga não existe no logix;'
         ELSE
      ...
      IF STATUS <> 0 THEN
         IF STATUS = 100 THEN
            LET m_msg = m_msg CLIPPED, '- não há uma OP nova correspondente;'
         ELSE
      ...
      IF ma_ordens[m_ind].ies_situa <> '4' THEN
         LET m_msg = m_msg CLIPPED, '- OP atul não está liberada;'
      END IF
...
#---------------------------#
   ...
   IF m_divergencia THEN
      LET m_msg = 'Plano com divergência não pode ser reaproveitado.'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   END IF
