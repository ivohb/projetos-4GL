...
#------------------------------------------------#
 FUNCTION vdp0070y_after_update_ped_itens()
#------------------------------------------------#
  DEFINE l_qtd_item_ant           DECIMAL(10,3),
         ...
         l_val_frete_unitario_ant DECIMAL(17,6),
         l_val_segr_unitario_ant  DECIMAL(17,6),
         l_status                 SMALLINT,
  ...
  LET l_val_frete_unitario_ant         = LOG_getVar('val_frete_unit_ant')
  LET l_val_segr_unitario_ant          = LOG_getVar('val_seguro_unit_ant')
  ...
  IF m_tipo IS NULL THEN #se for nulo é modificacao, entao deve verificar se teve alteracao. Senao grava o que veio dos sets
     ...
     IF l_val_frete_unitario_ant = lr_parametros.val_frete_unitario THEN
        LET lr_parametros.val_frete_unitario = NULL
     END IF

     IF l_val_segr_unitario_ant = lr_parametros.val_segr_unitario THEN
...
#------------------------------------------------#
 FUNCTION vdp0070y_after_insert_ped_itens()
#------------------------------------------------#

 LET m_tipo = "INCLUSAO"

 CALL vdp0070y_after_update_ped_itens()

END FUNCTION

#------------------------------------------------#
 FUNCTION vdp0070y_before_qtd_acresc()
#------------------------------------------------#
...
#------------------------------------------------#
 FUNCTION vdp0070y_before_pre_unit()
#------------------------------------------------#
...
#-------------------------------#
FUNCTION vdp0070y_version_info()
#-------------------------------#
RETURN "$Archive: /Logix/Fontes_Doc/Customizacao/10R2/_clientes_sem_guarda/kanaflex_sa_industria_de_plasticos/vendas/vendas/funcoes/vdp0070y.4gl $|$Revision: 1 $|$Date: 05/07/12 13:39 $|$Modtime: $" #Informações do controle de versão do SourceSafe - Não remover esta linha (FRAMEWORK)
END FUNCTION
