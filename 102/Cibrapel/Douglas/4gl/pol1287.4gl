...
#-----------------------------#
   ...
   INPUT p_cod_transpor WITHOUT DEFAULTS
    FROM cod_transpor
      ...
      IF p_count = 0 THEN
         ERROR "Transportador não consta do cadastro de Fornecedor/Transportadores !!!"
      ...
      IF p_count > 0 THEN
         ERROR "Já existem placas cadastradas para este fornecedor!!! - Use a opção modificar"
         NEXT FIELD cod_transpor
      END IF
   ...
   INPUT ARRAY pr_transpor
      ...
      AFTER FIELD num_placa
		...
		AFTER FIELD tara_minima
         ...
         IF pr_transpor[p_index].tara_minima = 0 THEN
            ERROR 'Não permitido valor de tara igual a zero !!!'
            NEXT FIELD num_placa
         END IF
