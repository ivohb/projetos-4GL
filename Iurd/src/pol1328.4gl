...
#----------------------------#       
   ...
   FOREACH cq_nota INTO mr_nota.*
      ...
      ELSE
         ...
         DECLARE cq_ad_ap_912 CURSOR FOR
         SELECT num_ap
           FROM ad_ap
          WHERE cod_empresa = mr_ad_mestre.cod_empresa
            AND num_ad = mr_ad_mestre.num_ad
         FOREACH cq_ad_ap_912 INTO m_num_ap

            IF STATUS <> 0 THEN 
               LET m_erro = STATUS USING '<<<<<'
               LET m_msg = 'Erro de status: ',m_erro
               LET m_msg = m_msg CLIPPED, ' lendo tabela ad_ap.cq_ad_ap_912'
               LET m_num_seq = 0
               CALL log085_transacao("ROLLBACK")
               RETURN FALSE
            END IF
            ...
            IF STATUS <> 0 THEN 
               LET m_erro = STATUS USING '<<<<<'
               LET m_msg = 'Erro de status: ',m_erro
               LET m_msg = m_msg CLIPPED, ' inserindo dados na tab gi_ad_ap_912'
               LET m_num_seq = 0
               RETURN FALSE
            END IF
