...
{
Quando for feito um apontamento de refugo, a quantidade apontada será
transferida para o item de retrabalho.
Quando for feito um apontamento de sucata, a quantidade apontada será
transferida para o item de sucata. Porém, ocorre que essa produção de
sucata vai para a prensa e se mistura com refiles não apontáveis. Com
isso, a quantidade de sucata armazenada no item sucata ficará menor
que a quantidade física contida na prensa. Para resolver essa situação,
no momento do envio dessa sucata para Guapi, a sucata deverá ser pesada
e um ajuste de estoque será feito no logix, antes de faturar a NF.

Colocar controle de lote nas composições
limpar estoque das mesmas e depois colocar o controle de lote.
...
DEFINE mr_apont          RECORD
       ...
       boa_com_retrab    CHAR(01),
       suc_com_retrab    CHAR(01)
END RECORD
...
#-----------------#
   ...
   SELECT parametro_numerico 
    ...
    IF min_par_modulo IS NULL THEN
...
#----------------------------------------#
    ...
    CALL _ADVPL_set_property(m_ordem,"VALID","pol1321_valida_op")
...
#---------------------------#
FUNCTION pol1321_valida_op()#
#---------------------------#
    
    DEFINE l_num_docum    CHAR(15),
           l_item_ordem   CHAR(15),
           l_item_pedido  CHAR(15),
           l_tipo_item    CHAR(10)
    
    IF mr_cabec.num_ordem IS NULL THEN
       LET m_msg = 'Informe o número da OF.'
       CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
       RETURN FALSE
    END IF   
    
    LET m_num_ordem = mr_cabec.num_ordem
    ...
    IF STATUS = 100 THEN
       CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",'OF não existe')
       RETURN FALSE
    END IF   
    ...
    CALL pol1321_pega_pedido(l_num_docum)
    ...
    IF  Status <> 0 THEN
        CALL log003_err_sql("SELECT","ped_itens:v_op")
        RETURN FALSE
    END IF

    LET l_tipo_item = pol1321_le_tipo(l_item_ordem)
    
    IF l_tipo_item = 'CHAPA' THEN
       IF l_item_ordem <> l_item_pedido THEN
          LET m_msg = 'Essa ordem tem apontamento automático.'
          CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
          RETURN FALSE
       END IF
       RETURN TRUE
...
#-----------------------------#
 FUNCTION pol1321_w_pesquisa()#
#-----------------------------#
	
	DROP TABLE w_pesquisa

	CREATE TEMP TABLE w_pesquisa(
				num_ordem		INTEGER
		);

   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','tabela:w_pesquisa')
      RETURN FALSE
   END IF 
   
   CREATE UNIQUE INDEX ix_w_pesquisa ON w_pesquisa(num_ordem)

   IF STATUS <> 0 THEN
      CALL log003_err_sql('CREATE','indice:w_pesquisa')
      RETURN FALSE
   END IF 
...
#------------------------------------------------------#
   ...
   IF NOT pol1321_w_pesquisa() THEN
      RETURN
   END IF
   
    IF  l_order_by IS NULL THEN
        LET l_order_by = "num_docum"
    END IF

   LET l_sql_stmt = "SELECT DISTINCT num_docum FROM ordens ",
   ...
   DECLARE cq_temp CURSOR FOR var_pesquisa
   ...
   FOREACH cq_temp INTO l_num_docum
      ...
      IF  Status <> 0 THEN
          CALL log003_err_sql("SELECT","ped_itens:cq_temp")
          RETURN 
      END IF
      ...
      IF Status <> 0 THEN
         CALL log003_err_sql("SELECT","ordens:cq_temp")
         RETURN 
      END IF
      
      LET l_tipo_item = pol1321_le_tipo(l_cod_item)
      ...
      DECLARE cq_ops CURSOR FOR
       SELECT num_ordem
        ...
        WHERE cod_empresa = p_cod_empresa
          AND num_docum = l_num_docum
          AND ( (cod_item_pai = l_cod_item AND l_tipo_item = 'CONJUNTO') OR
                (cod_item_pai = '0' AND l_tipo_item <> 'CONJUNTO') )
      
      FOREACH cq_ops INTO l_num_ordem
             
         IF  Status <> 0 THEN
             CALL log003_err_sql("SELECT","ordens:cq_ops")
             RETURN 
         END IF
      
         INSERT INTO w_pesquisa
          VALUES(l_num_ordem)

         IF  Status <> 0 THEN
             CALL log003_err_sql("INSERT","w_pesquisa:cq_ops")
             RETURN 
         END IF
   ...
   DECLARE cq_cons SCROLL CURSOR WITH HOLD FOR 
    SELECT num_ordem FROM w_pesquisa
...
#-------------------------------#
   ...
   IF m_count > 0 OR l_item[1] >= 'A' THEN
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('SELECT','ord_compon/item:1')
      RETURN l_tipo_item
   END IF
   
   IF m_count > 0 THEN 
      LET l_tipo_item = 'CONJUNTO'
      RETURN l_tipo_item
   END IF
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('SELECT','ord_compon/item:2')
      RETURN l_tipo_item
   END IF
   ...
   FOR l_ind = 1 to LENGTH(l_item CLIPPED)
       LET l_digito = l_item[l_ind]
       IF l_digito  MATCHES '[0123456789]' THEN
       ELSE
          LET l_texto = l_texto CLIPPED, l_digito
       END IF
   END FOR
   
   IF (l_texto IS NULL) OR (LENGTH(l_texto CLIPPED) > 1) THEN
      LET l_tipo_item = 'CAIXA'
   ELSE
...
#----------------------------------#
   ...
   WHILE TRUE
      ...
      ELSE
         ...
         ELSE
            IF STATUS = 0 THEN
               CALL pol1321_exibe_dados() RETURNING p_status
               LET l_achou = TRUE
               EXIT WHILE
            ELSE 
               CALL log003_err_sql("FETCH CURSOR","cq_cons")
...
#-------------------------#
   ...
   CALL _ADVPL_set_property(l_confirma,"EVENT","pol1321_conf_apont")  
   ...
   CALL _ADVPL_set_property(l_cancela,"EVENT","pol1321_canc_apont")     
...
#-----------------------------------#
    ...
    CALL _ADVPL_set_property(l_label,"TEXT","Usou retrabalho:")    

    LET m_boa_retrab = _ADVPL_create_component(NULL,"LCHECKBOX",l_panel)
    CALL _ADVPL_set_property(m_boa_retrab,"EDITABLE",m_acessorio) 
    CALL _ADVPL_set_property(m_boa_retrab,"POSITION",265,80)     
    CALL _ADVPL_set_property(m_boa_retrab,"VALUE_CHECKED","S")     
    CALL _ADVPL_set_property(m_boa_retrab,"VALUE_NCHECKED","N")     
    CALL _ADVPL_set_property(m_boa_retrab,"VARIABLE",mr_apont,"boa_com_retrab")
    ...
    LET m_sucata_retrab = _ADVPL_create_component(NULL,"LCHECKBOX",l_panel)
    CALL _ADVPL_set_property(m_sucata_retrab,"EDITABLE",m_acessorio) 
    CALL _ADVPL_set_property(m_sucata_retrab,"POSITION",265,110)     
    CALL _ADVPL_set_property(m_sucata_retrab,"VALUE_CHECKED","S")     
    CALL _ADVPL_set_property(m_sucata_retrab,"VALUE_NCHECKED","N")     
    CALL _ADVPL_set_property(m_sucata_retrab,"VARIABLE",mr_apont,"suc_com_retrab")

    LET l_label = _ADVPL_create_component(NULL,"LLABEL",l_panel)
    CALL _ADVPL_set_property(l_label,"POSITION",30,140)     
    CALL _ADVPL_set_property(l_label,"TEXT","Qtde retrabalho:")    
...
#----------------------------#
   ...
   IF NOT m_acessorio THEN
      LET mr_apont.boa_com_retrab = 'N'
      LET mr_apont.suc_com_retrab = 'N'
   ...
   IF NOT pol1321_le_pes_prod() THEN
      RETURN FALSE
   END IF
   
   IF m_acessorio THEN
      IF mr_apont.boa_com_retrab = 'S' THEN
         LET mr_apont.boa_retrab = mr_apont.boa 
      END IF
      
      IF mr_apont.suc_com_retrab = 'S' THEN
         LET mr_apont.sucata_retrab = mr_apont.sucata 
      END IF
      
   END IF   
         
   LET m_qtd_retrab = mr_apont.boa_retrab + mr_apont.sucata_retrab 
   ...
   LET m_qtd_retrab = m_qtd_retrab * m_pes_prod
   LET mr_apont.boa_retrab = mr_apont.boa_retrab * m_pes_prod
   LET mr_apont.sucata_retrab = mr_apont.sucata_retrab * m_pes_prod
...
#------------------------------#
   ...
   IF m_ies_lote = 'S' THEN
      LET mr_man_apont.num_lote = mr_cabec.num_docum CLIPPED
   END IF
   ...
   DECLARE cq_apont CURSOR WITH HOLD FOR 	
    SELECT cod_empresa,
	 ...
	 FOREACH cq_apont INTO 	
      ...
      LET m_qtd_produz = m_qtd_sucata + p_w_apont_prod.qtd_refug + p_w_apont_prod.qtd_boas
...
#---------------------------#
   ...
   LET m_cod_item_pai = p_w_apont_prod.cod_item                                                       
   ...
   ELSE                                                                                               
	 ...
	 END IF                                                                                          	
       ...
       LET m_cod_item_pai = m_cod_sucata                                                                                              
      ...
      IF pol1321_w_sucata() THEN                                                                      
         INSERT INTO w_sucata                                                                         
   		     VALUES(m_cod_sucata, m_qtd_sucata, m_fat_conver, m_qtd_conver, 0)                        
				 IF STATUS <> 0 THEN                                                  	                      	
   ...
   IF NOT manr24_cria_w_comp_baixa (0) THEN   
			LET m_msg = 'NÃO FOI POSSIVEL CRIAR A TABELA W_COMP_BAIXA' 	                      	
		  RETURN FALSE                                                          	                    	                                                            
   END IF                                                                                             
      
   IF m_acessorio AND m_qtd_retrab > 0 THEN
      IF NOT pol1321_grava_compon() THEN
         RETURN FALSE
      END IF
   ELSE
      IF mr_cabec.tip_produto[1,2] = 'CA' AND 
          (p_w_apont_prod.ies_sucata = 1 OR p_w_apont_prod.ies_defeito = 1) THEN
         IF NOT pol1321_carrega_baixa('F') THEN #não consumir tipo F
            RETURN FALSE
         END IF      
      ELSE
         IF NOT pol1321_carrega_baixa('9') THEN #não consumir tipo 9 (não existe)
            RETURN FALSE
         END IF      
      END IF
   END IF         
...
#--------------------------------#
   ...
   FOR m_ind = 1 TO l_qtd_lin
       IF ma_itens[m_ind].estornar = 'S' THEN
          ...
          IF STATUS <> 0 THEN
             CALL log003_err_sql('SELECT','man_apont_912')
             RETURN
          END IF
          ...
          IF m_cod_status = '2' THEN
             RETURN
          END IF    
          IF l_qtd_retrab IS NULL OR l_qtd_retrab <= 0 THEN      
...
#--------------------------------#
   ...
   IF m_tip_movto MATCHES '[R]' AND l_opc = '1' THEN #aqui
      CALL log085_transacao("BEGIN")
   ...
   END IF
	 ...
	 IF m_tip_movto = 'S' THEN
      ...
      IF POL1321_w_sucata() THEN 
	       INSERT INTO w_sucata 
		       VALUES(m_cod_sucata, m_qtd_estornar, m_fat_conver, m_qtd_conver, 0  )
         IF STATUS <> 0 THEN
            LET m_msg = 'ERRO: ',m_erro CLIPPED, ' INSERINDO NA TAB W_SUCATA'
            CALL pol1321_ins_erro()
	          RETURN FALSE
...
#--------------------------#
   ...
   IF STATUS <> 0 THEN
      LET m_erro = STATUS
      LET m_msg = 'ERRO ',m_erro CLIPPED, ' LENDO APONTAMENTO A ESTORNAR ', m_seq_reg_mestre
      CALL pol1321_ins_erro()
      RETURN FALSE
   END IF 
...
#------------------------------------#
   ...
   IF NOT pol1321_le_estoq_chapa(l_cod_item, mr_cabec.num_docum) THEN
      RETURN FALSE
   END IF
   
   IF m_estoq_chapa >= l_qtd_boas THEN
      RETURN TRUE
   END IF

   LET l_qtd_boas = l_qtd_boas - m_estoq_chapa
   ...
   LET mr_man_apont.qtd_boas = l_qtd_boas
   ...
   IF NOT pol1321_le_ctr_lote(l_cod_item) THEN
      RETURN FALSE
   END IF
...
#------------------------------------------------------#
FUNCTION pol1321_le_estoq_chapa(l_cod_item, l_num_lote)#
#------------------------------------------------------#

   DEFINE l_qtd_reservada   LIKE estoque_loc_reser.qtd_reservada,
          l_cod_item        LIKE estoque_lote_ender.cod_item,
          l_num_lote        LIKE estoque_lote_ender.num_lote,
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('SELECT','item:lec')     
      RETURN FALSE
   END IF
   ...
   SELECT SUM(qtd_saldo)
    ...
    WHERE cod_empresa   = p_cod_empresa
      ...
      AND ies_situa_qtd = 'L'
      AND ( (num_lote IS NULL AND l_ctr_lote = 'N') OR
            (num_lote = l_num_lote AND  l_ctr_lote = 'S') )
          
   IF STATUS <> 0 THEN
      CALL log003_err_sql('SELECT','estoque_lote_ender:lec')     
      RETURN FALSE
   END IF  

   IF m_estoq_chapa IS NULL THEN
      LET m_estoq_chapa = 0
      RETURN TRUE
   END IF
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('SELECT','estoque_loc_reser:lec')   
      RETURN FALSE
   END IF  
   ...
   LET m_estoq_chapa = m_estoq_chapa - l_qtd_reservada

   IF m_estoq_chapa IS NULL OR m_estoq_chapa <= 0 THEN
      LET m_estoq_chapa = 0
   END IF
...
#----------------------------------------#
   ...
   FOREACH cq_structure INTO 
      ...
      IF l_ies_tip_item = 'F' THEN #acessório
         LET l_qtd_necessaria = l_qtd_necessaria * mr_apont.boa
      ELSE
         LET l_qtd_necessaria = l_qtd_necessaria * l_parametro.qtd_apont
      END IF
      
      IF l_qtd_necessaria <= 0 THEN
         CONTINUE FOREACH
      END IF
...
#-----------------------------------------#
FUNCTION pol1321_carrega_baixa(l_ies_tipo)#																						
#-----------------------------------------#
   ...
   DECLARE cq_carrega CURSOR FOR
    SELECT cod_item_compon,
     ...
     WHERE cod_empresa = p_cod_empresa
       AND num_ordem = m_num_ordem
       AND qtd_necessaria > 0
       AND ies_tip_item <> l_ies_tipo
       GROUP BY cod_item_compon, cod_local_baixa
   
   FOREACH cq_carrega INTO  l_cod_compon, l_local, l_qtd_neces 
      ...
      LET l_qtd_neces = l_qtd_neces * m_qtd_produz
       ...
       SELECT num_lote,         
        ...
        FROM estoque_lote_ender
		   WHERE cod_empresa = p_cod_empresa
		     ...
		     AND ies_situa_qtd = 'L'
		     AND qtd_saldo > 0
		     AND ( (num_lote = m_num_docum AND l_ies_chapa = 'S') OR 
		           (1=1 AND l_ies_chapa = 'N'))
      ...
      FOREACH cq_lt_ender INTO
         ...
         IF STATUS <> 0 THEN
            LET m_erro = STATUS
            LET m_msg = 'ERRO ',m_erro CLIPPED,' LENDO CURSOR CQ_LT_ENDER'
            RETURN FALSE
         END IF		
         ...
         IF l_compon.qtd_transf = 0 THEN
            LET l_compon.qtd_transf = 0.001
         END IF
   
         LET l_compon.cod_item_pai = m_cod_item_pai                            
         ...
         IF STATUS <> 0 THEN                                                             
	          LET m_erro = STATUS                                                         
	          LET m_msg = 'ERRO ',m_erro CLIPPED,' INSERINDO DADOS NA TABELA W_COMP_BAIXA'
	       	  RETURN FALSE                                                                   
         END IF	                                                                         
      ...
      IF l_qtd_neces > 0.001 THEN
         LET m_msg = 'ITEM ',l_cod_compon CLIPPED, ' SEM SALDO SUFICIENTE P/ CONSUMIR '
         RETURN FALSE
      END IF
...
DEFINE l_dat_hor_producao LIKE estoque_lote_ender.dat_hor_producao,       
   ...
   IF STATUS <> 0 THEN
      LET m_erro = STATUS
      LET m_msg = 'ERRO ',m_erro CLIPPED,' LENDO TABELA ESTOQUE_LOTE_ENDER'
      RETURN FALSE
   END IF		
	 
	 LET l_compon.cod_item_pai = m_cod_item_pai
	 LET l_compon.cod_item = p_parametros_885.cod_item_retrab
   LET l_compon.dat_producao = EXTEND(l_dat_hor_producao, YEAR TO DAY)
   LET l_compon.hor_producao = EXTEND(l_dat_hor_producao, HOUR TO SECOND)
 	 LET l_compon.dat_valid    = EXTEND(l_dat_hor_validade, YEAR TO DAY)
   LET l_compon.hor_valid    = EXTEND(l_dat_hor_validade, HOUR TO SECOND)
  
   LET L_compon.qtd_transf = m_qtd_retrab
...
#--------------------------#
FUNCTION pol1321_le_conta()#
#--------------------------#

DEFINE l_cod_oper_sp         LIKE par_pcp.cod_estoque_sp,        
       l_cod_oper_rp         LIKE par_pcp.cod_estoque_rp,  
       l_ies_com_detalhe     CHAR(01)
   ...
   IF STATUS <> 0 THEN                                                  	                      	
      LET m_erro = STATUS
		  LET m_msg = 'ERRO: ',m_erro CLIPPED, ' LENDO TABELA PAR_PCP' 	                      	
		  RETURN FALSE                                                          	                    	
   END IF                                                                	                    	
   ...
   SELECT ies_com_detalhe                                                                                     
     ...
     FROM estoque_operac                                                                                      
    WHERE cod_empresa  = p_cod_empresa                                                                        
      AND cod_operacao = l_cod_oper_sp                                                                       
                                                                                                                 
   IF STATUS <> 0 THEN                                                  	                      	
      LET m_erro = STATUS
		  LET m_msg = 'ERRO: ',m_erro CLIPPED, ' LENDO TABELA ESTOQUE_OPERAC:SAIDA' 	                      	
		  RETURN FALSE                                                          	                    	
   END IF                                                                	                    	
                                                                                                                 
   IF l_ies_com_detalhe = 'S' THEN                                                                            
      SELECT num_conta_debito                                                                           
        INTO p_w_apont_prod.num_conta_saida                                                                                
      ...
      IF STATUS <> 0 THEN                                                  	                      	
         LET m_erro = STATUS
		     LET m_msg = 'ERRO: ',m_erro CLIPPED, ' LENDO TABELA ESTOQUE_OPERAC_CT:SAIDA' 	                      	
		     RETURN FALSE                                                          	                    	
      END IF                                                                	                    	
   END IF
   ...
   SELECT ies_com_detalhe                                                                                     
     ...
     FROM estoque_operac                                                                                      
    WHERE cod_empresa  = p_cod_empresa                                                                        
      AND cod_operacao = l_cod_oper_rp                                                                       
                                                                                                                 
   IF STATUS <> 0 THEN                                                  	                      	
      LET m_erro = STATUS
		  LET m_msg = 'ERRO: ',m_erro CLIPPED, ' LENDO TABELA ESTOQUE_OPERAC:ENTRADA' 	                      	
		  RETURN FALSE                                                          	                    	
   END IF                                                                	                    	
                                                                                                                 
   IF l_ies_com_detalhe = 'S' THEN                                                                            
      SELECT num_conta_credito                                                                           
        INTO p_w_apont_prod.num_conta_ent                                                                               
        FROM estoque_operac_ct                                                                          
      WHERE cod_empresa  = p_cod_empresa                                                               
        AND cod_operacao = l_cod_oper_rp                                                              
      ...
      IF STATUS <> 0 THEN                                                  	                      	
         LET m_erro = STATUS
		     LET m_msg = 'ERRO: ',m_erro CLIPPED, ' LENDO TABELA ESTOQUE_OPERAC_CT:ENTRADA' 	                      	
		     RETURN FALSE                                                          	                    	
      END IF                                                                	                    	
   END IF
