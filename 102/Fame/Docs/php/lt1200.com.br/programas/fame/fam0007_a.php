 ...
 if($vendas=="S")
 {
  $selec_pedidos="SELECT 
		  ...
		  (month(b.dat_hor_emissao)||'/'||year(b.dat_hor_emissao) ) as mesano,
                  c.pedido AS num_pedido,b.natureza_operacao AS cod_tip_cli,
  ...
  SELECT 
		  ...
		  (month(b.dat_hor_emissao)||'/'||year(b.dat_hor_emissao) ) as mesano,
             ...
             where b.empresa='".$empresa."'
               and b.serie_nota_fiscal='1'
               and b.nota_fiscal not in ('730545','730546','730547')
               and CAST(b.dat_hor_emissao AS DATE) between '".$ini."' and '".$fim."'
               and b.sit_nota_fiscal <> 'C'
               and i.cod_cliente=b.cliente
	       	   and c.empresa=b.empresa
  ...
  $selec_notas="SELECT (b.nota_fiscal||'-'||b.empresa) as nff_emp,
		  ...
		  month(b.dat_hor_emissao) as mes,
                  year(b.dat_hor_emissao)  as ano,
		  (month(b.dat_hor_emissao)||'/'||year(b.dat_hor_emissao) ) as mesano,
		  i.nom_cliente,i.cod_cliente,b.natureza_operacao as cod_tip_cli,
                  day(b.dat_hor_emissao) as dia
...
//Pedidos Exportacao
  ...
  while (is_array($mat_pedido))
  {
   ...
   $unit=(($mat_pedido["pre_unit"]-($mat_pedido["pre_unit"])));
...
//Mercado Interno
  ...
  $selec_pedido="SELECT (b.num_pedido||'-'||b.cod_empresa) as ped_emp,
                  b.num_pedido,b.cod_empresa,cast(b.cod_moeda as decimal(3,0)) as cod_moeda,
                  b.pct_desc_financ,
                  (b.pct_desc_adic) as pct_desc_adic_ped,
                  b.pct_frete,b.ies_sit_pedido,
                  e.pre_unit,
...
union all
          SELECT (b.cod||'-'||'01') as ped_emp,
                  (b.cod/1) as num_pedido,'01' as cod_empresa,cast(b.cod_moeda as decimal(3,0)) as cod_moeda,
                  0 as pct_desc_financ,
                  0 as pct_desc_adic_ped,
                  b.pct_frete,'N' as ies_sit_pedido,
                  (e.prunit-(e.prunit*e.descont/100)) as pre_unit,
  ...
  while (is_array($mat_pedido))
  {
   // Caso a moeda seja diferente de R$ busco a cotação no Logix para conversão
   // (26/10/2009) - Marcelo Peres.
   
   $cambio = 1;
   if($mat_pedido["cod_moeda"] != 1)
   	{
   	$selec_cambio="select val_cotacao from cotacao
   	               where cod_moeda = ".$mat_pedido["cod_moeda"]."
   	               and dat_ref = today";
    
   	$res_cambio = $cconnect("logix",$ifx_user,$ifx_senha);
  	$result_cambio = $cquery($selec_cambio,$res_cambio);
  	$mat_cambio=$cfetch_row($result_cambio);
  	
  	if(is_array($mat_cambio))
  		{
  		$cambio = $mat_cambio["val_cotacao"];  		
  		}
   ...
   $mes_rel=round($mat_pedido["mes"]);
   $ano_rel=round($mat_pedido["ano"]);
   $unit=($mat_pedido["pre_unit"]*$cambio);
   $unit=(round(($unit*100),0)/100);
   $unit=(($unit-($unit*$mat_pedido["pct_desc_bruto"]/100)));
   $unit=(round(($unit*100),0)/100);
   $unit=(($unit-($unit*$mat_pedido["pct_desc_adic_ped"]/100)));
 ...
 /* $pdf->ln();
  $pdf->setx(10);
  $pdf->SetFillColor(260);
  $pdf->Cell(20,5,$mat_pedido["num_pedido"],TRBL,0,'L',1);
  $pdf->Cell(25,5,$mat_pedido["ano"].'-'.$mat_pedido["mes"].'-'.$mat_pedido["dia"],TRBL,0,'L',1);
  $pdf->Cell(20,5,$mat_pedido["cod_repres"],TRBL,0,'L',1);
  $pdf->Cell(25,5,number_format($matimd,2,",","."),TRB,0,'R');
  $pdf->Cell(25,5,number_format($matim,2,",","."),TRB,0,'R');
  $pdf->Cell(25,5,number_format($qtd_pedid,2,",","."),TRB,0,'R');
  $pdf->Cell(25,5,number_format($qtd_pedi,2,",","."),TRB,0,'R');
*/
   $vlme=0;
   $vlmc=0;
   $vlma=0;
...
//CANCELADOS
  ...
  $selec_pedido="
SELECT (b.num_pedido||'-'||b.cod_empresa) as ped_emp,
                  b.num_pedido,b.cod_empresa,b.num_pedido_repres,b.cod_moeda,
                  b.ies_frete,b.pct_desc_financ,
                  (b.pct_desc_adic) as pct_desc_adic_ped,
                  b.pct_frete,b.ies_sit_pedido,
...
union all
  ...
  while (is_array($mat_pedido))
  {
   ...
   $unit=($mat_pedido["pre_unit"]*$cambio);	
   $unit=(round(($unit*100),0)/100);
   $unit=(($unit-($unit*$mat_pedido["pct_desc_adic"]/100)));
   $unit=(round(($unit*100),0)/100);
   $unit=(($unit-($unit*$mat_pedido["pct_desc_bruto"]/100)));
   $unit=(round(($unit*100),0)/100);
   $unit=(($unit-($unit*$mat_pedido["pct_desc_adic_ped"]/100)));
...
/* original
  ...
  */
  ...
  $ypos=$pdf->gety();
  $pdf->Settextcolor('255');
  $pdf->Cell(190,5,'ORDENS DE CARGA (VALOR TOTAL DA MERCADORIA)',TRL,0,'C',1);
  $pdf->SetFont('Arial','B',8);
  $pdf->Settextcolor('0','0','0');
  $pdf->SetFillColor(260);
  $pdf->ln();
  ...
  while (is_array($mat_pedido))
  {
   ...
   if($det_ped=="S")
   {
    if($ped_atu<>$ped_ant)
    {
      ...
      $pdf->setx(10);
      $pdf->SetFillColor(260);
      $pdf->Cell(40,5,"PEDIDO: ".round($ped_atu),TRB,0,'R',1);
    }
   }
   ...
   if($det=="S")
   {
    ...
    if($mes_ant<>$mes_atu)
    {
     ...
     $pdf->setx(10);
     $pdf->SetFillColor(260);
     $pdf->Cell(40,5,$mes_ant,TRBL,0,'R',1);
     $pdf->SetFillColor(220);
     $pdf->Cell(25,5,number_format($matim,2,",","."),TRB,0,'R',1);
     $pdf->Cell(25,5,number_format($qtd_pedi,0,",","."),TRB,0,'R',1);
     $pdf->SetFillColor(260);
