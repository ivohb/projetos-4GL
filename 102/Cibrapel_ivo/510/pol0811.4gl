...
MAIN
   LET p_versao = "POL0811-10.02.00" #Favor nao alterar esta linha (SUPORTE)
...
DEFINE l_count  INTEGER
   ...
   ELSE
      ...
      LET p_i = 1
      FOREACH c_ped_itens INTO p_ped_itens.*
            
         LET t_ped_itens[p_i].cod_item      = p_ped_itens.cod_item
         LET t_ped_itens[p_i].num_sequencia = p_ped_itens.num_sequencia
         LET t_ped_itens[p_i].qtd_solic     = p_ped_itens.qtd_pecas_solic - p_ped_itens.qtd_pecas_atend - p_ped_itens.qtd_pecas_cancel
         LET t_ped_itens[p_i].qtd_atend     = p_ped_itens.qtd_pecas_atend
         
         IF t_ped_itens[p_i].qtd_solic <= 0 THEN
            CONTINUE FOREACH
         END IF    
...
DEFINE l_qtd_saldo   LIKE ped_itens.qtd_pecas_solic,
   ...
   FOR w_i = 1 TO 500  
     ...
     IF t_ped_itens[w_i].qtd_atend > 0 THEN 
        LET l_total = 'N'
     END IF    
