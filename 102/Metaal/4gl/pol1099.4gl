...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: pol1099                                                 #
# OBJETIVO: CADASTRO DO CONTEÚDO DAS COLUNAS                        #
# AUTOR...: IVO                                                     #
# DATA....: 02/06/11                                                #
#-------------------------------------------------------------------#
...
#-----------------------------#
   ...
   INPUT p_cod_item WITHOUT DEFAULTS
    FROM cod_item
      ...
      IF p_count > 0 THEN
         ERROR 'Já existem conteúdos cadastrados ',
               'p/ esse item !!! - Use a opção modificar'
         NEXT FIELD cod_item
      END IF
...
#--------------------------------#
   ...
   FOR m_ind = 1 TO ARR_COUNT()
       IF pr_coluna[m_ind].cod_operac IS NOT NULL THEN
          ...
          IF p_count = 0 THEN
             LET p_msg = 'Operação ', pr_coluna[m_ind].cod_operac CLIPPED,
   ...
   INPUT ARRAY pr_linha
      ...
      AFTER FIELD conteudo
         ...
         IF LENGTH(pr_linha[p_ind].conteudo) > pr_coluna[p_index].tamanho THEN
            ERROR 'Informe um texto com no máximo ',p_tamanho
            NEXT FIELD conteudo
         END IF
