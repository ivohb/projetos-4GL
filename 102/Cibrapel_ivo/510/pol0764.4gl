#-------------------------------------------------------------------#
# PROGRAMA: POL0764                                                 #
# OBJETIVO: RELATORIO DE COMISSOES GERENTE        - GERENCIAL       #
#-------------------------------------------------------------------#
DATABASE logix
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "POL0764-10.02.00"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0764.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol0764_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0764_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0764") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0764 AT 3,03 WITH FORM p_nom_tela 
   ...
   MENU "OPCAO"
      ...
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0764","IN") THEN
         SELECT * INTO p_empresas_885.* 
           FROM empresas_885 
          WHERE cod_emp_oficial = p_cod_empresa   
         IF pol0764_entrada_dados() THEN
            LET p_ies_cons = TRUE
      ...
      LET p_achou = FALSE
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0764","MO") THEN
         IF p_ies_cons = TRUE THEN 
           IF pol0764_calcula_valores()  THEN 
            IF log028_saida_relat(16,40) IS NOT NULL THEN
               MESSAGE " Processando a extracao do relatorio ... " 
                  ATTRIBUTE(REVERSE)
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT pol0764_relat TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'pol0764.tmp'
                     START REPORT pol0764_relat  TO p_caminho
                  END IF
               ELSE
                  START REPORT pol0764_relat TO p_nom_arquivo
               END IF
               CALL pol0764_emite_relatorio()   
               IF p_count = 0 THEN
      ...
      EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0764

END FUNCTION

#----------------------------------------#
 FUNCTION pol0764_entrada_dados()
#----------------------------------------#
...
#---------------------------------#
 FUNCTION pol0764_calcula_valores()
#---------------------------------#
...
IF p_ies_calc_pag = 'S' THEN
   ...
   SELECT cod_gerente,val_gar_ger,dat_exp_gar 
     FROM ger_com_885
   ORDER BY cod_gerente 
   FOREACH cq_repres INTO p_cod_gerente,p_val_garantia,p_dat_exp_gar
...
#---------------------------------#
 FUNCTION pol0764_emite_relatorio()
#---------------------------------#
DEFINE l_cod_rep   LIKE representante.cod_repres
   ...
   FOREACH  cq_reprel INTO l_cod_rep
      ...
      FOREACH cq_relat INTO p_lanc_com_885.*
         ...
         OUTPUT TO REPORT pol0764_relat(p_relat.*,p_nom_repres,p_num_cgc_cpf) 
...
#-------------------------------------------------------#
 REPORT pol0764_relat(p_relat,p_nom_repres,p_num_cgc_cpf)
#-------------------------------------------------------#
