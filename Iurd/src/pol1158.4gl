...
#--------------------------# 
   ...
   DEFINE la_texto ARRAY[120] OF RECORD   
          texto               CHAR(40)
   END RECORD
   ...
   LET l_texto = FALSE
   LET p_index = 1
   INITIALIZE la_texto TO NULL
   
   DECLARE cq_obser CURSOR FOR
    SELECT obs_ad
      FROM cap_obs_ad 
     WHERE empresa = p_empresa
       AND apropriacao_desp = p_num_docum
   FOREACH cq_obser INTO la_texto[p_index].texto

      IF STATUS <> 0 THEN
         CALL log003_err_sql('Lendo','cap_obs_ad')
         RETURN
      END IF
   ...
   IF NOT l_texto THEN   
      LET la_texto[p_index].texto = l_observ
      LET p_index = p_index + 1
   END IF
   
   CALL SET_COUNT(p_index - 1)
         
   INPUT ARRAY la_texto WITHOUT DEFAULTS FROM sa_texto.*
      ATTRIBUTES(INSERT ROW = FALSE, DELETE ROW = FALSE)
      ...
      BEFORE FIELD texto
         LET p_texto = la_texto[p_index].texto
         
      AFTER FIELD texto
         
         LET la_texto[p_index].texto = p_texto
         DISPLAY p_texto to sa_texto[s_index].texto
         
         IF FGL_LASTKEY() = 27 OR FGL_LASTKEY() = 2000 OR FGL_LASTKEY() = 2016 THEN
         ELSE
            LET p_count = p_index + 1
            IF la_texto[p_count].texto IS NULL THEN
               NEXT FIELD texto
            END IF
         END IF
