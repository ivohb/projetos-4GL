...
GLOBALS
  ...
  DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
   ...
   DEFINE pr_chave             ARRAY[500] OF RECORD  
          id_registro          INTEGER  
   END RECORD

   DEFINE pr_compon            ARRAY[500] OF RECORD  
          cod_compon           CHAR(15)  
   END RECORD

   DEFINE pr_ar                ARRAY[500] OF RECORD  
          cod_compon           CHAR(15),
          den_item             CHAR(76),
          num_ar_1             DECIMAL(6,0),
          num_ar_2             DECIMAL(6,0),
          num_ar_3             DECIMAL(6,0),
          num_ar_4             DECIMAL(6,0),
          num_ar_5             DECIMAL(6,0),
          num_ar_6             DECIMAL(6,0),
          num_ar_7             DECIMAL(6,0),
          num_ar_8             DECIMAL(6,0)
   END RECORD
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol1106_controle()
   END IF

END MAIN

#--------------------------#
 FUNCTION pol1106_controle()
#--------------------------#
   
   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol1106") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1106 AT 2,1 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   DISPLAY p_cod_empresa TO cod_empresa

   MENU "OPCAO"
      COMMAND "Apontar" "Apontamento da produção"
         LET p_ies_etiq = FALSE
         IF pol1106_info_prog_apontar() THEN
            IF pol1106_proces_apto() THEN
               let p_cod_erro = 0
            ELSE
               ERROR 'Apontamento cancelado!'
            END IF
            CALL pol1106_erro_critico()
            CLOSE WINDOW w_pol11064
            IF p_msg IS NOT NULL THEN
               CALL log0030_mensagem(p_msg,'excla')
            END IF
            DISPLAY p_tela.ies_status to ies_status
         ELSE
            CALL pol1106_limpa_tela()
            ERROR "Operação Cancelada !!!"
         END IF
         CURRENT WINDOW IS w_pol1106
         NEXT OPTION "Fim"
      COMMAND "Criticados" "Exibe registros criticados"
         CALL pol1106_criticado() 
         CLOSE WINDOW w_pol11062
         CALL pol1106_limpa_tela()
   ...
   CLOSE WINDOW w_pol1106

END FUNCTION

#-----------------------#
FUNCTION pol1106_sobre()
#-----------------------#
...
#--------------------------#
FUNCTION pol1106_etiqueta()#
#--------------------------#

   DEFINE p_param   CHAR(42),
          p_comando CHAR(200)

   LET p_param = p_cod_empresa CLIPPED, '&', p_user CLIPPED, '&', p_tela.num_programa CLIPPED
...
#----------------------------------#
FUNCTION pol1106_info_prog_apontar()
#----------------------------------#
   ...
   INPUT BY NAME p_tela.* WITHOUT DEFAULTS
      ...
      AFTER FIELD num_programa
         ...
         if not pol1106_le_man_nest() then
      ...
      AFTER FIELD cod_profis
         ...
         IF p_tela.cod_profis IS NOT NULL THEN
            ...
            IF STATUS <> 0 THEN
               ERROR 'Código não cadastrado no RHU08300 !!!'
   ...
   if not pol1106_grava_id_nest() then
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   end if

   if not pol1106_grava_man_nest() then
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   end if

   if not pol1106_aceita_qtd() then
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   end if

   if not pol1106_rateia_qtd() then
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   end if
...
#-----------------------------#
   ...
   Foreach cq_tr into p_tela.ies_status
      ...
      if p_tela.ies_status = 'N' then
         let p_tela.den_status = 'Enviado'
      else
...
#-------------------------------#
FUNCTION pol1106_grava_id_nest()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1106_grava_man_nest()
#-------------------------------#
   ...
   foreach cq_gmn into p_num_ordem, p_cod_item, p_qtd_prod, p_qtd_apontada, p_id_registro
...
#----------------------------#
   ...
   INITIALIZE pr_item, pr_chave to null
   let p_index = 1
   ...
   foreach cq_qtd into 
      ...
      let pr_chave[p_index].id_registro = 0
      let pr_item[p_index].tip_oper = 'A'
      ...
      let pr_item[p_index].qtd_boas = 0 #Usimatic
   ...
   foreach cq_bx into 
           pr_item[p_index].cod_item, 
           pr_item[p_index].qtd_produzida,
           pr_item[p_index].qtd_apontada,
           pr_chave[p_index].id_registro

      if status <> 0 then
         call log003_err_sql('Lendo','man_apo_nest_405:cq_bx')
         RETURN FALSE
      end if
      
      let pr_item[p_index].tip_oper = 'B'
      let pr_item[p_index].qtd_refugo = 0
      let pr_item[p_index].qtd_boas   = 0
   ...
   INPUT ARRAY pr_item WITHOUT DEFAULTS FROM sr_item.*
      ...
      BEFORE FIELD cod_defeito
         ...
         IF pr_item[p_index].qtd_refugo IS NULL OR
            pr_item[p_index].qtd_refugo = 0 OR
            pr_item[p_index].tip_oper = 'B' THEN
            let pr_item[p_index].cod_defeito = null
            display ' ' to sr_item[s_index].cod_defeito  
            next field qtd_sdo_ops
         End if
      ...
      AFTER ROW
         ...
         IF NOT INT_FLAG then
            IF (pr_item[p_index].cod_item IS NOT NULL AND pr_item[p_index].tip_oper = 'A') then
               ...
               IF p_qtd_prod > pr_item[p_index].qtd_sdo_ops then
                  LET p_ies_forca_apont = pol1106_forca_apont(pr_item[p_index].cod_item)
...
#----------------------------#
FUNCTION pol1106_rateia_qtd()
#----------------------------#

   for p_ind = 1 to ARR_COUNT()
       if pr_item[p_ind].cod_item is not null then
        if pr_item[p_ind].tip_oper = 'A' then
          if pr_item[p_ind].qtd_boas > 0 then
             if not pol1106_rateia_boas() then
                RETURN FALSE
             end if
          end if
          if pr_item[p_ind].qtd_refugo > 0 then
             if not pol1106_rateia_refugo() then
                RETURN FALSE
             end if
          end if
        else
           if not pol1106_grava_baixa() then
              Return false
           end if 
        end if
       end if
...
#----------------------------#
FUNCTION pol1106_grava_baixa()
#----------------------------#

   update man_apo_nest_405
      set qtd_boas   = pr_item[p_ind].qtd_boas,
          qtd_refugo = pr_item[p_ind].qtd_refugo
    where cod_empresa  = p_cod_empresa
      and num_programa = p_tela.num_programa
      and num_ordem    = 0
      and id_registro  = pr_chave[p_ind].id_registro
       
   if status <> 0 then
      call log003_err_sql('update','man_apo_nest_405:gravando baixas')             
      RETURN FALSE
   end if
...
#-----------------------------#
   ...
   foreach cq_rop INTO p_num_ordem, p_cod_operac, p_sdo_prog, p_id_registro
      ...
      if not pol1106_grava_boas() then
         RETURN FALSE
      end if
...
#-------------------------------#
FUNCTION pol1106_rateia_refugo()
#-------------------------------#
   ...
   foreach cq_refug INTO p_num_ordem, p_cod_operac, p_sdo_prog, p_id_registro
      ...
      if not pol1106_grava_refugo() then
         RETURN FALSE
      end if
...
#---------------------------------------#
FUNCTION pol1106_forca_apont(p_cod_item)
#---------------------------------------#
...
#-----------------------#
   ...
   CASE
     ...
     WHEN INFIELD(num_prog)
        LET p_codigo = pol1106_sel_programa()

        CLOSE WINDOW w_pol11061
        CURRENT WINDOW IS w_pol11062
...
#------------------------------#
   ...
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol11061") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol11061 AT 5,10 WITH FORM p_nom_tela
...
#------------------------------#
FUNCTION pol1106_le_parametros()
#------------------------------#
...
#-----------------------------#
FUNCTION pol1106_proces_apto()
#-----------------------------#

   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol11064") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol11064 AT 10,10 WITH FORM p_nom_tela
   ...
   IF NOT pol1106_consistencia() THEN
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   END IF
   ...
   SELECT COUNT(cod_empresa)
     ...
     WHERE cod_empresa  = p_cod_empresa
       ...
       AND cod_status   = 'I'
       AND num_ordem > 0
       AND (qtd_boas + qtd_refugo) > 0

   IF STATUS <> 0 THEN
      let p_cod_erro = STATUS
      let p_msg = 'ERRO CONTANDO REGISTROS DA TABELA MAN_APO_LOGIX_405'
      RETURN FALSE
   END IF
   
   IF p_count > 0 THEN
      IF NOT pol1106_dig_ar() THEN
         call pol1106_del_man_apo_405()
         LET p_msg = 'USUARIO ABORTOU O PROCESSO'
         RETURN FALSE
      END IF
   END IF
...
#------------------------------#
FUNCTION pol1106_consistencia()#
#------------------------------#
   
   #if not pol1106_bloqueia_tab() then
   #   RETURN false
   #end if

   if not pol1106_cria_tabs() then
      RETURN false
   end if

   IF NOT pol1106_le_parametros() THEN
      RETURN FALSE
   END IF
   
   let p_tem_critica = false
   
   if not pol1106_consiste_dados() then
      RETURN false
   end if
   ...
   if p_tem_critica then
      ...
      call pol1106_del_man_apo_405()
      call pol1106_critica_man_nest() RETURNING p_staus
      LET p_ies_etiq = FALSE
      return true
   end if
   ...
   FOREACH cq_mat INTO
      ...
      CALL pol1106_exib_mensagem()
   
      IF NOT pol1106_le_material() THEN 
         RETURN FALSE
      END IF

      IF NOT pol1106_le_sucata() THEN 
         RETURN FALSE
      END IF
   ...
   IF NOT pol1106_checa_material() THEN 
      RETURN FALSE
   END IF

   IF NOT pol1106_checa_sucata() THEN 
      RETURN FALSE
   END IF
...
#-------------------------#
FUNCTION pol1106_apontar()#
#-------------------------#
   
   if not pol1106_deleta_erro() then
      RETURN false
   end if
   ...
   IF not pol1106_aponta_programa() then
      RETURN FALSE
   END IF

   let p_tip_oper = 'B'
   
   IF not pol1106_baixa_especial() then
      RETURN FALSE
   END IF
...
#-----------------------------------#
FUNCTION pol1106_del_man_apo_405()
#-----------------------------------#
...
#------------------------------#
   ...
   UPDATE man_apo_nest_405
    ...
    where cod_empresa  = p_programa.cod_empresa
      ...
      and id_registro  = p_programa.id_registro
...
#---------------------------------#
FUNCTION pol1106_critica_man_nest()
#---------------------------------#
...
#-------------------------------#
FUNCTION pol1106_consiste_dados()
#-------------------------------#
   ...
   FOREACH cq_consiste into p_programa.*
      ...
      if not pol1106_checa_ordem() then
         return false
      end if
      
      IF p_man.cod_item IS NULL THEN
         if not pol1106_atu_man_nest() then
            return false
         end if
         CONTINUE FOREACH
      END IF
      
      if not pol1106_checa_operacao() then
         return false
      end if

      #if not pol1106_checa_componente() then
      #   return false
      #end if
      ...
      if not pol1106_insere_man() then
         return false
      end if
...
#------------------------------#
FUNCTION pol1106_exib_mensagem()
#------------------------------#
...
#--------------------------------#
FUNCTION pol1106_checa_operacao()
#--------------------------------#
...
#----------------------------------#
FUNCTION pol1106_checa_componente()
#----------------------------------#
...
#---------------------------------#
FUNCTION pol1106_checa_tempo_unit()
#---------------------------------#
...
#-------------------------------#
FUNCTION pol1106_calc_data_hora()
#-------------------------------#
   ...
   CALL pol1106_calcula_turno(p_hi)
...
#-----------------------------------#
FUNCTION pol1106_calcula_turno(p_hi)
#-----------------------------------#
...
#-----------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      CALL log003_err_sql('Inserindo','man_erro_405')
   END IF                                           
...
#----------------------------#
FUNCTION pol1106_insere_man()
#----------------------------#
      
   LET p_man.nom_prog = 'POL1106'
   ...
   if not pol1106_le_ferramenta() then
      return false
   end if

   if not pol1106_le_uni_funcio() then
      return false
   end if
...
#-------------------------------#
FUNCTION pol1106_le_ferramenta()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1106_le_uni_funcio()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1106_valida_ar(p_ar)
#-------------------------------#
   ...
   LET p_cod_compon = pr_ar[p_ind].cod_compon
   
   SELECT cod_empresa
     FROM nf_sup
    WHERE cod_empresa = p_cod_empresa
      AND num_aviso_rec = p_ar
   ...
   IF p_count = 0 THEN
      ERROR 'Esse AR não é do componente\n consumido: ', p_cod_compon
      Return FALSE
   END IF
   
   LET p_count = 0
   
   IF p_ar = pr_ar[p_ind].num_ar_1 THEN
      LET p_count = p_count + 1
   END IF
   
   IF p_ar = pr_ar[p_ind].num_ar_2 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_3 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_4 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_5 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_6 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_7 THEN
      LET p_count = p_count + 1
   END IF

   IF p_ar = pr_ar[p_ind].num_ar_8 THEN
      LET p_count = p_count + 1
   END IF
   
   IF p_count > 1 THEN
      ERROR 'AR JÁ INFORMADO.'
      Return FALSE
   END IF
...
#------------------------#
FUNCTION pol1106_dig_ar()
#------------------------#

   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol11066") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol11066 AT 2,1 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)

   CALL pol1106_inf_ar() RETURNING p_status
   
   CLOSE WINDOW w_pol11066
...
#------------------------#
FUNCTION pol1106_inf_ar()
#------------------------#
   
   DEFINE p_num_seq   integer
   
   INITIALIZE pr_ar, pr_chave, pr_compon TO NULL
   LET p_ind = 1
   ...
   FOREACH cq_ar INTO 
      p_num_ordem,
      pr_ar[p_ind].cod_compon,
      p_num_seq

      IF STATUS <> 0 THEN
         let p_cod_erro = STATUS
         let p_msg = 'ERRO LENDO DADOS DA TABELA MAN_APO_LOGIX_405 - CQ_AR'
         RETURN TRUE
      END IF                                           
      ...
      SELECT COUNT(item_componente)
       ...
       WHERE empresa            = p_cod_empresa
         ...
         AND sequencia_operacao = p_num_seq
         AND item_componente    = pr_ar[p_ind].cod_compon

      IF STATUS <> 0 THEN
         CALL log003_err_sql('SELECT', 'man_op_componente_operacao')
         RETURN FALSE
      END IF

      IF STATUS <> 0 THEN
         let p_cod_erro = STATUS
         let p_msg = 'ERRO LENDO DADOS DA TABELA MAN_OP_COMPONENTE_OPERACAO - CQ_AR'
         RETURN TRUE
      END IF                                           

      IF p_count = 0 THEN
         
         LET pr_ar[p_ind].cod_compon = ''
         ...
         FOREACH cq_troca INTO p_cod_compon

            IF STATUS <> 0 THEN
               let p_cod_erro = STATUS
               let p_msg = 'ERRO LENDO DADOS DA TABELA MAN_OP_COMPONENTE_OPERACAO - CQ_TROCA'
               RETURN TRUE
            END IF                                           
            
            LET pr_ar[p_ind].cod_compon = p_cod_compon
            EXIT FOREACH
            
         END FOREACH
         
         IF pr_ar[p_ind].cod_compon = '' THEN
            LET p_msg = 'Não foi possivel obter o componend da tab man_op_componente_operacao'
   ...
   IF p_ind = 1 THEN
      LET p_msg = 'Não foi possível ler componen-\n',
                  'tes do programa: ', p_tela.num_programa
      CALL log0030_mensagem(p_msg,'info')
      RETURN TRUE
   END IF

   DELETE FROM item_ar_tmp
   
   LET p_index = p_ind - 1
   
   FOR p_ind = 1 to p_index
   
      IF pr_ar[p_ind].cod_compon IS NOT NULL THEN
         SELECT cod_compon
           FROM item_ar_tmp
          WHERE cod_compon = pr_ar[p_ind].cod_compon

         IF STATUS = 100 THEN
            SELECT den_item
              INTO pr_ar[p_ind].den_item
              FROM item
             WHERE cod_empresa = p_cod_empresa
               AND cod_item = pr_ar[p_ind].cod_compon
         
            INSERT INTO item_ar_tmp(cod_compon, den_item) 
             VALUES(pr_ar[p_ind].cod_compon, pr_ar[p_ind].den_item)
             
            IF STATUS <> 0 THEN
               let p_cod_erro = STATUS
               let p_msg = 'ERRO INSERINDO DADOS NA TABELA ITEM_AR_TMP'
               RETURN TRUE
            END IF                                           
         ELSE
            IF STATUS <> 0 THEN
               let p_cod_erro = STATUS
               let p_msg = 'ERRO LENDO DADOS NA TABELA ITEM_AR_TMP'
               RETURN TRUE
            END IF
         END IF
      END IF
   ...
   DECLARE cq_ar_tmp CURSOR FOR
    SELECT * FROM item_ar_tmp
   
   FOREACH cq_ar_tmp INTO pr_ar[p_ind].*

      IF STATUS <> 0 THEN
         let p_cod_erro = STATUS
         let p_msg = 'ERRO LENDO DADOS DA TABELA ITEM_AR_TMP - CQ_AR_TMP'
         RETURN TRUE
      END IF                                           
   ...
   INPUT ARRAY pr_ar WITHOUT DEFAULTS FROM sr_ar.*
      ...
      AFTER FIELD num_ar_1
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_1) THEN
            NEXT FIELD num_ar_1
         END IF

      AFTER FIELD num_ar_2
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_2) THEN
            NEXT FIELD num_ar_2
         END IF

      AFTER FIELD num_ar_3
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_3) THEN
            NEXT FIELD num_ar_3
         END IF

      AFTER FIELD num_ar_4
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_4) THEN
            NEXT FIELD num_ar_4
         END IF

      AFTER FIELD num_ar_5
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_5) THEN
            NEXT FIELD num_ar_5
         END IF

      AFTER FIELD num_ar_6
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_6) THEN
            NEXT FIELD num_ar_6
         END IF

      AFTER FIELD num_ar_7
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_7) THEN
            NEXT FIELD num_ar_7
         END IF

      AFTER FIELD num_ar_8
         
         IF NOT pol1106_valida_ar(pr_ar[p_ind].num_ar_8) THEN
            NEXT FIELD num_ar_8
         END IF

      AFTER ROW
         IF NOT INT_FLAG THEN
            IF FGL_LASTKEY() = 27 OR FGL_LASTKEY() = 2000 OR FGL_LASTKEY() = 4010 OR
               FGL_LASTKEY() = 2016 OR FGL_LASTKEY() = 2 THEN
            ELSE
               IF pr_ar[p_ind+1].cod_compon IS NULL THEN
                  NEXT FIELD num_ar_1
               END IF
            END IF
         END IF
      AFTER INPUT
         ...
         IF NOT INT_FLAG THEN
            FOR p_ind = 1 TO ARR_COUNT()
                IF pr_ar[p_ind].cod_compon IS NOT NULL THEN
                   IF pr_ar[p_ind].num_ar_1 IS NULL AND
                      pr_ar[p_ind].num_ar_2 IS NULL AND
                      pr_ar[p_ind].num_ar_3 IS NULL AND
                      pr_ar[p_ind].num_ar_4 IS NULL AND
                      pr_ar[p_ind].num_ar_5 IS NULL AND
                      pr_ar[p_ind].num_ar_6 IS NULL AND
                      pr_ar[p_ind].num_ar_7 IS NULL AND
                      pr_ar[p_ind].num_ar_8 IS NULL THEN
                      LET p_msg = 'Pelo menos um AR deve ser\n informado, para cada item'
                      CALL log0030_mensagem(p_msg,'excla')
                      NEXT FIELD num_ar_1
                   END IF
                END IF
...
#------------------------#
FUNCTION pol1106_grv_ar()#
#------------------------#
 
   FOR p_ind = 1 TO ARR_COUNT()
       IF pr_ar[p_ind].cod_compon IS NOT NULL THEN
          IF NOT pol1106_ins_ar_tmp() THEN
             Return FALSE
          END IF
       END IF
   END FOR

   IF not pol1106_ins_apo_impres() THEN
      Return false
   END IF
...
#----------------------------#
FUNCTION pol1106_ins_ar_tmp()#
#----------------------------#

   INSERT INTO item_ar_tmp
    VALUES(pr_ar[p_ind].cod_compon,
           pr_ar[p_ind].den_item,
           pr_ar[p_ind].num_ar_1,
           pr_ar[p_ind].num_ar_2,
           pr_ar[p_ind].num_ar_3,
           pr_ar[p_ind].num_ar_4,
           pr_ar[p_ind].num_ar_5,
           pr_ar[p_ind].num_ar_6,
           pr_ar[p_ind].num_ar_7,
           pr_ar[p_ind].num_ar_8)
           
   IF STATUS <> 0 THEN
      CALL log003_err_sql('INSERT','item_ar_tmp')
      Return FALSE
   END IF
...
#--------------------------------#
FUNCTION pol1106_ins_apo_impres()#
#--------------------------------#

   LET p_ind = 1
   
   DECLARE cq_id_man CURSOR FOR
    SELECT cod_compon,
           id_man_apont
      FROM man_apo_logix_405
     WHERE cod_empresa  = p_cod_empresa
   ...
   FOREACH cq_id_man INTO 
      p_cod_compon,
      p_id_man_apont

      IF STATUS <> 0 THEN
         let p_cod_erro = STATUS
         let p_msg = 'ERRO LENDO DADOS DA TABELA MAN_APO_LOGIX_405 - CQ_ID_MAN'
         RETURN TRUE
      END IF                                           
...
#-------------------------#
FUNCTION pol1106_grava_ar()
#-------------------------#

   IF pr_ar[p_ind].num_ar_1 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_1) THEN
         Return FALSE
      END IF
   END IF
   
   IF pr_ar[p_ind].num_ar_2 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_2) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_3 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_3) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_4 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_4) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_5 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_5) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_6 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_6) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_7 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_7) THEN
         Return FALSE
      END IF
   END IF

   IF pr_ar[p_ind].num_ar_8 IS NOT NULL THEN
      IF NOT pol1106_ins_ar(pr_ar[p_ind].num_ar_8) THEN
         Return FALSE
      END IF
   END IF
...
#--------------------------------#
FUNCTION pol1106_ins_ar(p_num_ar)
#--------------------------------#

   DEFINE p_num_ar INTEGER
   
   DELETE FROM apo_impres_ar_405
    WHERE cod_empresa = p_cod_empresa
      AND id_man_apont = p_id_man_apont
      AND num_aviso_rec = p_num_ar
    
   INSERT INTO apo_impres_ar_405
    VALUES(p_cod_empresa, p_id_man_apont, p_num_ar)

   IF STATUS <> 0 THEN
      CALL log003_err_sql('INSERT', 'apo_impres_ar_405')
      Return FALSE
   END IF
...
#---------------------------------#
FUNCTION pol1106_aponta_programa()
#---------------------------------#
   ...
   FOREACH cq_man INTO p_man.*
      ...
      if not pol1106_atualiza_man() then
         return false
      end if

      IF NOT pol1106_ins_mestre() THEN
         RETURN FALSE
      END IF

      IF NOT pol1106_ins_detalhe() THEN
         RETURN FALSE
      END IF
      
      IF NOT pol1106_ins_tempo() THEN
         RETURN FALSE
      END IF

      IF NOT pol1106_atuali_ord_oper() THEN
         RETURN FALSE
      END IF
      ...
      IF (p_qtd_boas + p_qtd_refug + p_qtd_sucata) > 0 THEN
         If not pol1106_grava_ordens() THEN
            Return false
         End if
      End If
      ...
      If p_man.qtd_refugo > 0 then
         ...
         IF NOT pol1106_gra_tabs_velhas() THEN
            RETURN FALSE
         END IF
         ...
         If not pol1106_le_est_lote_ender() then
            RETURN true
         End if
         
         LET p_qtd_prod = p_qtd_refug
         LET p_cod_operacao = NULL
         IF NOT pol1106_aponta_estoque() THEN
            RETURN FALSE
         END IF
            
         IF NOT pol1106_insere_man_item() THEN
            RETURN FALSE
         END IF

         LET p_tip_movto = 'E'
   
         IF NOT pol1106_insere_chf_componente() THEN
            RETURN FALSE
         END IF
            
         IF NOT pol1106_ins_man_def() THEN
            RETURN FALSE
         END IF
         ...
         IF NOT pol1106_baixa_material() THEN 
            RETURN FALSE
         END IF
      ...
      IF NOT pol1106_ins_apont_proces() THEN
         RETURN FALSE
      END IF
      
      let p_cod_status = 'A'
      
      IF NOT pol1106_atualiza_apo_logix() THEN 
         RETURN FALSE
      END IF
...
#-------------------------------#
FUNCTION pol1106_baixa_especial()
#-------------------------------#
   ...
   FOREACH cq_bx_esp into 
      ...
      if not pol1106_le_item_man() then
         Return false
      end if
      ...
      IF NOT pol1106_checa_estoque() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT pol1106_baixa_compon() THEN
         RETURN FALSE
      END IF
...
#-----------------------------------#
FUNCTION pol1106_atualiza_apo_logix()
#-----------------------------------#
...
#---------------------------------#
FUNCTION pol1106_atuali_ord_oper()
#---------------------------------#
...
#---------------------------------#
FUNCTION pol1106_gra_tabs_velhas()
#---------------------------------#
...
#----------------------------------#   
FUNCTION pol1106_ins_apont_proces()
#----------------------------------#
...
#---------------------------#
   ...
   DROP TABLE item_ar_tmp
   
   CREATE  TABLE item_ar_tmp(
      cod_compon  CHAR(15),
      den_item    CHAR(76),
      num_ar1     INTEGER,
      num_ar2     INTEGER,
	 ...
	 IF STATUS <> 0 THEN 
			CALL log003_err_sql("CRIANDO","item_ar_tmp")
			RETURN FALSE
	 END IF
...
#-------------------------------#
FUNCTION pol1106_checa_material()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1106_checa_estoque()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol1106_baixa_material()
#-------------------------------#
...
#------------------------------#
   ...
   FOREACH cq_bel INTO p_estoque_lote_ender.*
      ...
      IF NOT pol1106_baixa_lote() THEN
         RETURN FALSE
      END IF
      
      IF NOT pol1106_baixa_estoque() THEN
         RETURN FALSE
      END IF
      ...
      IF NOT pol1106_grava_estoq_trans() THEN
         RETURN FALSE
      END IF
      
      IF p_tip_oper = 'A' THEN
         IF NOT pol1106_insere_man_consumo() THEN
            RETURN FALSE
         END IF
         IF NOT pol1106_insere_chf_componente() THEN
            RETURN FALSE
...
#-------------------------------#
FUNCTION pol1106_baixa_estoque()
#-------------------------------#
...
#-----------------------------------#
FUNCTION pol1106_grava_estoq_trans()
#-----------------------------------#
   ...
   IF p_cod_operacao IS NULL THEN
      IF NOT pol1106_le_operacao() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   Else
      LET p_estoque_trans.num_docum = p_tela.num_programa
   END IF
   ...
   IF NOT pol1106_ins_est_trans_end() THEN
      RETURN FALSE
   END IF

   IF NOT pol1106_insere_estoq_audit() THEN
      RETURN FALSE
   END IF

   IF p_tip_oper = 'A' THEN
      IF NOT pol1106_apont_transac() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------------#
 FUNCTION pol1106_ins_est_trans_end()
#------------------------------------#
...
#-----------------------------------#
FUNCTION pol1106_insere_estoq_audit()
#-----------------------------------#
...
#-------------------------------#
FUNCTION pol1106_apont_transac()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol1106_insere_man_item()
#---------------------------------#
...
#-----------------------------------#
FUNCTION pol1106_insere_man_consumo()
#-----------------------------------#
...
#--------------------------------------#
FUNCTION pol1106_insere_chf_componente()
#--------------------------------------#
...
#----------------------------------#
FUNCTION pol1106_le_est_lote_ender()
#----------------------------------#

   Let p_tem_lote = false

   IF NOT pol1106_le_item() THEN
      RETURN FALSE
   END IF

   IF NOT pol1106_le_ctr_grade(p_man.cod_item) THEN
      RETURN FALSE
   END IF
...
#--------------------------------#
FUNCTION pol1106_aponta_estoque()
#--------------------------------#
   ...
   IF p_tem_lote THEN
      ...
      IF NOT pol1106_atualiza_lote_ender() THEN
         RETURN FALSE
      END IF
   ELSE
      CALL pol1106_ender_carrega()
      IF NOT pol1106_insere_lote_ender() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF STATUS = 0 THEN
      IF NOT pol1106_atualiza_lote() THEN
         RETURN FALSE
      END IF
   ELSE
      IF STATUS = 100 THEN
         CALL pol1106_lote_carrega()
         IF NOT pol1106_insere_lote() THEN
            RETURN FALSE
         END IF
      ELSE
         let p_cod_erro = STATUS
   ...
   IF NOT pol1106_atualiza_estoque() THEN
      RETURN FALSE
   END IF
...
#---------------------------------------#
FUNCTION pol1106_le_ctr_grade(p_cod_item)
#---------------------------------------#
...
#------------------------------------#
FUNCTION pol1106_atualiza_lote_ender()
#------------------------------------#
...
#-------------------------------#
FUNCTION pol1106_ender_carrega()
#-------------------------------#
...
#-----------------------------------#
FUNCTION pol1106_insere_lote_ender()
#-----------------------------------#
...
#-------------------------------#
FUNCTION pol1106_atualiza_lote()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol1106_atualiza_estoque()
#---------------------------------#
...
#---------------------------#
   ...
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol11062") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol11062 AT 5,1 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
   ...
   INPUT ARRAY pr_erros WITHOUT DEFAULTS FROM sr_erros.*
      ...
      ON KEY (control-v)
         IF pr_erros[p_index].num_programa IS NOT NULL THEN
            ...
            IF pol1106_edita_programa() THEN
...
#--------------------------------#
FUNCTION pol1106_edita_programa()
#--------------------------------#

   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol11063") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol11063 AT 7,3 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)
      
   DISPLAY p_cod_empresa to cod_empresa
   
   CALL pol1106_aceita_dados() RETURNING p_status
   RETURN TRUE
   
   IF pol1106_prende_registro() THEN
      
      IF pol1106_aceita_dados() THEN
...
#----------------------------------#
 FUNCTION pol1106_prende_registro()
#----------------------------------#
