   ...
   FOREACH cq_hist INTO p_aponta.*,p_rowid
      ...
      INSERT INTO apont_hist_man912
       VALUES(p_chav_seq,1,
              ...
              p_aponta.cod_mov,
              p_aponta.arq_orig,
              NULL,"pol0456","pol0456",p_cod_empresa)
...
#--------------------------------#
   ...
   {   SELECT COUNT(chav_seq)
      ...
      IF sqlca.sqlcode <> 0 THEN
         LET m_msg = 'OPERACAO NÃO CADASTRADA NA TAB OPER_EGA_MAN912 - POL0456'
         CALL pol0456_insere_erro()
         CONTINUE FOREACH
      END IF
      ...
      IF p_aponta.tip_mov <> 'P' AND 
         ...
         p_aponta.tip_mov <> '*' THEN
          LET m_msg = 'TIPO DE MOVIMENTO INVÁLIDO - POL0456: ', p_aponta.tip_mov
          CALL pol0456_insere_erro()
          CONTINUE FOREACH
      END IF
      ...
      IF p_aponta.tip_mov = "R" THEN
         IF NOT pol0456_tira_das_boas() THEN
            RETURN FALSE
         ELSE
            LET p_tex_observ = NULL
      ...
      IF l_qtd_tot_apont > m_qtd_item THEN
         ...
         IF NOT pol0456_le_man() THEN
            LET m_msg = 'ERRO AO LER DADOS DA TAB ITEM_MAN - POL0456'
            CALL pol0456_insere_erro()
            CONTINUE FOREACH
         END IF
...
#-----------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'OPERACAO ',p_cod_operac CLIPPED, 
                  ' NÃO CADASTRADA NA TAB OPERACAO - POL0456'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET m_msg = 'OPERACAO FINAL NÃO CADASTRADA - ORD_OPER - POL0456'
      RETURN FALSE
   END IF
...
#--------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ORDEM DE PRODUÇÃO NÃO EXISTE - POL0456'
      CALL pol0456_insere_erro()
      RETURN FALSE
   END IF
   ...
   ELSE
      ...
      IF p_envia_hist = 'N' THEN
         LET m_msg = 'ORDEM DE PRODUÇÃO NÃO ESTA EM PRODUCAO - POL0456'
         CALL pol0456_insere_erro()
      END IF
      RETURN FALSE
   END IF
...
#-------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ITEM: ', p_aponta.cod_item, ' NÃO CADASTRADO - POL0456'
      RETURN FALSE
   END IF
...
#----------------------------------#
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_aponta.cod_maquina,
                  ' NÃO CADASTRADA - MAQ_EGA_MAN912 - POL0456'
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_cod_maquina,' NÃO CADASTRADA - RECURSO - POL0456'
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      ...
      IF STATUS <> 0 THEN
         LET m_msg = 'MATRIC.OPERADOR:', p_aponta.mat_operador,
                     ' NÃO CADASTRADA - FUNCIONARIO - POL0456'
         RETURN FALSE
      ELSE
         ...
         IF STATUS <> 0 THEN
            LET m_msg = 'OPERADOR PADRAO:', p_aponta.mat_operador,
                        ' NÃO CADASTRADO - FUNCIONARIO - POL0456'
            RETURN FALSE
         END IF
      END IF
   END IF
...
#---------------------------------#
FUNCTION pol0456_troca_sit_estoq()
#---------------------------------#
   ...
   IF STATUS <> 0 THEN
      LET m_msg = 'ERRO ',STATUS,' ATUALIZANDO ESTOQUE-POL0456'
      CALL pol0456_insere_erro()
      LET p_sem_estoque = TRUE
      RETURN FALSE
   END IF
   ...
   IF p_saldo_disp < p_aponta.qtd_refugo THEN
      IF p_tip_peca = 'G' THEN
         LET m_msg = 'PECA GEMEA ',p_aponta.cod_item CLIPPED,
                     ' S/ ESTOQ-TRASNF.BOA P/REFUGO-POL0456'
      ELSE         
