...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: POL1292                                                 #
# OBJETIVO: RELATORIO DE ENTRADA DE APARAS E CANUDOS POR DATA       #
...
GLOBALS
    DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
           ...
           p_data_process       LIKE nf_sup.dat_emis_nf
...
#----------------------#
 FUNCTION pol1292_menu()
#----------------------#

    CALL log006_exibe_teclas("01",p_versao)
    INITIALIZE p_nom_tela TO NULL
    CALL log130_procura_caminho("pol1292") RETURNING p_nom_tela
    LET p_nom_tela = p_nom_tela CLIPPED
    OPEN WINDOW w_pol1292 AT 2,2 WITH FORM p_nom_tela
    ...
    MENU "OPCAO"
        COMMAND "Informar" "Informar Parâmetros p/ consulta"
            CALL pol1292_informar() RETURNING p_status
            IF p_status THEN
        ...
        COMMAND KEY ("!")
         ...
         COMMAND "Fim" "Retorna ao menu anterior."
             EXIT MENU
    END MENU
    CLOSE WINDOW w_pol1292

END FUNCTION

#--------------------------#
FUNCTION pol1292_informar()
#--------------------------#
...
#--------------------------#
FUNCTION pol1292_listagem()
#--------------------------#
    
    IF NOT pol1292_escolhe_saida() THEN
        RETURN
    END IF

    IF NOT pol1292_le_den_empresa() THEN
        RETURN
    END IF
    ...
    LET p_sql = " SELECT tf.num_aviso_rec,",
                "        nf.num_nf, nf.dat_emis_nf, nf.cod_fornecedor, ",
    ...
    IF p_tela.data_inicio_periodo IS NOT NULL THEN
        LET p_sql = p_sql CLIPPED, ' '," AND nf.dat_emis_nf >=  '",p_tela.data_inicio_periodo USING 'dd/mm/yyyy',"' "
    END IF
    
    IF p_tela.data_fim_periodo IS NOT NULL THEN
        LET p_sql = p_sql CLIPPED, ' '," AND nf.dat_emis_nf <=  '",p_tela.data_fim_periodo USING 'dd/mm/yyyy',"' "
    END IF
    
    LET p_sql = p_sql CLIPPED, " ORDER BY tf.cod_empresa, nf.dat_emis_nf, tf.num_placa, nf.cod_fornecedor"
    ...
    LET p_data_process = EXTEND(p_data, YEAR TO MONTH)
    ...
    FOREACH cq_impressao
       INTO p_lista.num_aviso_rec, 
        ...
        OUTPUT TO REPORT pol1292_relat(p_lista.dat_emis_nf)
        
        IF p_data_process = p_lista.dat_emis_nf THEN
        ...
        ELSE
            LET p_data_process = p_lista.dat_emis_nf
...
#-------------------------------#
FUNCTION pol1292_escolhe_saida()
#-------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         ...
         LET p_caminho = p_caminho CLIPPED, "pol1292.tmp"
         START REPORT pol1292_relat TO p_caminho
      ELSE
         START REPORT pol1292_relat TO p_nom_arquivo
      END IF
   END IF
...
#--------------------------------#
FUNCTION pol1292_le_den_empresa()
#--------------------------------#
...
#-----------------------------------#
REPORT pol1292_relat(p_data)
#-----------------------------------#

   DEFINE p_data LIKE nf_sup.dat_emis_nf
   ...
   FORMAT
      ...
      PAGE HEADER
         ...
         PRINT COLUMN 002, "pol1292",
               COLUMN 013, "RELATORIO DE ENTRADA DE APARAS E CANUDOS : ",p_data USING "dd/mm/yyyy",
         ...
         PRINT COLUMN 002, "--------------------------------------------------------------------------------------------------------------------"
         PRINT COLUMN 002, "  FORNECEDOR      N.FISCAL     MOTORISTA       PLACA    PESO NF   PESO BAL  DIFER     PRC.UNIT   TOTAL      ITEM      "
         PRINT
      BEFORE GROUP OF p_data
         SKIP TO TOP OF PAGE
         
      ON EVERY ROW
         PRINT COLUMN 002, p_lista.nom_fornecedor[1,15],
               COLUMN 019, p_lista.num_nf USING "##########",
               COLUMN 032, p_lista.nom_transpor[1,15], 
               COLUMN 048, p_lista.num_placa,
               COLUMN 057, p_lista.peso_nota USING "########&",
               COLUMN 067, p_lista.peso_balanca USING "########&",
               COLUMN 077, p_lista.peso_diferenca USING "########&",
               COLUMN 087, p_lista.pre_unit USING "###,##&.&&",
               COLUMN 098, p_lista.pre_total USING "###,##&.&&",
               COLUMN 109, p_lista.cod_item
               
      AFTER GROUP OF p_data
         PRINT COLUMN 002, "--------------------------------------------------------------------------------------------------------------------"
         PRINT COLUMN 002, "SUB - TOTAL",
               COLUMN 020, "ENTRADA",
               COLUMN 057, p_peso_total_dia USING "########&",
               COLUMN 067, p_peso_balanca_dia USING "########&",
               COLUMN 077, p_peso_difer_dia USING "########&",
               COLUMN 098, p_valor_total_dia USING "###,##&.&&"
         PRINT
         
      ON LAST ROW
         PRINT COLUMN 002, "--------------------------------------------------------------------------------------------------------------------"
         PRINT COLUMN 002, "TOTAL GERAL DESTE RELATORIO",
               COLUMN 057, p_peso_total_geral USING "########&",
               COLUMN 067, p_peso_balanca_geral USING "########&",
               COLUMN 077, p_peso_difer_geral USING "########&",
               COLUMN 098, p_valor_total_geral USING "###,##&.&&"
         PRINT
