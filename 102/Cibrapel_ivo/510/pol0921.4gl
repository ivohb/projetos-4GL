...
GLOBALS
  DEFINE p_cod_empresa          LIKE empresa.cod_empresa,
         ...
         p_cod_emp_g            LIKE empresa.cod_empresa,
...
MAIN
  ...
  LET p_versao = "POL0921-10.02.00" #Favor nao alterar esta linha (SUPORTE)
...
#---------------------------#
  ...
  CALL pol0921_entrada_parametros()
  CALL pol0921_processa_or()
  CALL pol0921_processa_sd()
  CALL pol0921_notas()
...
#------------------------------------#
 DECLARE cq_lp CURSOR FOR
 SELECT * 
   FROM ped_del
 FOREACH cq_lp INTO p_ped_del.*
   DELETE FROM ped_at_885
    WHERE cod_empresa =  p_ped_del.cod_empresa
      AND num_pedido  =  p_ped_del.num_pedido
 END FOREACH   

END FUNCTION

#------------------------------------#
 FUNCTION pol0921_entrada_parametros()
#------------------------------------#
 DEFINE l_dat_ini CHAR(10),
        l_dia     CHAR(02),
        l_mes     CHAR(02),
        l_ano     CHAR(04)
...
#-----------------------------#
 FUNCTION pol0921_processa_or()
#-----------------------------#

  DISPLAY 'PROCESSANDO PEDIDO ORIG'  AT 7,10

  DECLARE cq_del_po CURSOR FOR
    SELECT * 
      FROM ped_at_885
   FOREACH  cq_del_po INTO p_ped_at_885.*
     DELETE FROM ped_itens_orig_885 
      WHERE cod_empresa  IN (p_cod_emp1, p_cod_emp2, p_cod_emp3, p_cod_emp4, p_cod_emp5, p_cod_empg1, p_cod_empg2, p_cod_empg3, p_cod_empg4, p_cod_empg5) 
       ...
       SELECT UNIQUE a.* 
        ...
        WHERE a.cod_empresa  = p_cod_emp_g
          AND a.num_pedido   = p_ped_at_885.num_pedido
...
#-----------------------------#
 FUNCTION pol0921_processa_sd()
#-----------------------------#

  DISPLAY 'PROCESSANDO PEDIDOS SALDO' AT 7,10

  DECLARE cq_del_ps CURSOR FOR 
    SELECT * 
      FROM ped_at_885
   FOREACH  cq_del_ps INTO p_ped_at_885.*
    ...
    INSERT INTO ped_del VALUES (p_ped_at_885.cod_empresa, p_ped_at_885.num_pedido)
