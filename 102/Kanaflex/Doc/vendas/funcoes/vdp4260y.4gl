...
DEFINE l_cod_repres       LIKE pedido_comis.cod_repres_3,
  ...
  INITIALIZE mr_pedidos1.parametro_texto TO NULL
       ...
       LET sql_stmt = "SELECT pedidos.cod_empresa,     pedidos.num_pedido,        ",
                            ...
                            " FROM pedidos,"
       IF  l_cod_repres IS NULL OR #OS338410
           l_cod_repres = " "   THEN
           LET sql_stmt = sql_stmt CLIPPED," [OUTER] "
       END IF

       LET sql_stmt = sql_stmt CLIPPED," pedido_comis,"
       IF  l_forma_pgto IS NULL OR
           l_forma_pgto = " " THEN
           LET sql_stmt = sql_stmt CLIPPED,' [OUTER] '
       END IF

       LET sql_stmt = sql_stmt CLIPPED," ped_compl_pedido"
       IF  mr_pedidos1.parametro_texto <> ' '      AND
           mr_pedidos1.parametro_texto IS NOT NULL THEN
           LET sql_stmt = sql_stmt CLIPPED," ,ped_info_compl"
       END IF

       LET sql_stmt = sql_stmt CLIPPED," WHERE ", where_clause CLIPPED,
                             " AND ped_compl_pedido.empresa  = pedidos.cod_empresa",
                             " and ped_compl_pedido.pedido   = pedidos.num_pedido",
                             " and pedido_comis.cod_empresa  = pedidos.cod_empresa",
