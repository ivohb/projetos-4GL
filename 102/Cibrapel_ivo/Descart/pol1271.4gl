...
#--------------------------#
   ...
   CALL log006_exibe_teclas("01",p_versao)
	...
	# Eliminando solicitacoes nao faturadas - Agere
...
#---------------------------#
   ...
   FOREACH cq_montag_item INTO
      ...
      IF STATUS <> 0 THEN 
         CALL log003_err_sql('lendo','roma_item_885:cq_montag_item') 
         RETURN FALSE
      END IF
      ...
      LET p_peso_romaneiob = p_peso_romaneiob + mr_ordem_montag_item.pes_total_item
...
#--------------------------#
   ...
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol12711 AT 05,05 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
...
#------------------------------#
   ...
   IF l_trans_solic_fatura IS NOT NULL THEN
		  DELETE FROM fat_solic_fatura 
		   WHERE trans_solic_fatura = l_trans_solic_fatura AND ord_montag = p_num_om
...
#----------------------------#
   ...
   IF p_criticou THEN
      IF pol1271_carrega_criticas() THEN
         CALL pol1271_criticas_exibe()
      END IF
