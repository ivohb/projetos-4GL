...
GLOBALS
   ...
   DEFINE p_num_pedido         LIKE nf_item.num_pedido,    
          ...
          p_val_frete_unit     LIKE frete_unit_kana.val_frete_unit,
...
#-------------------------------#
   ...
   LET p_sql = 
       ...
       "WHERE a.cod_empresa   = '",p_tela.cod_emp_dupl,"' ",
        ...
        "a.val_bruto, a.val_liquido, a.pct_comis_1, a.num_docum_origem, a.ies_serie_fat ",               
         ...
         "AND b.num_docum NOT IN ",
              "(SELECT c.num_docum FROM docum_comis_pagas c ",
                "WHERE c.cod_empresa   = b.cod_empresa ",
                  ...
                  "AND c.num_seq_docum = b.num_seq_docum) ",
        "GROUP BY a.cod_empresa, a.cod_cliente, a.num_pedido, a.num_docum, ",
        "a.val_bruto, a.val_liquido, a.pct_comis_2, a.num_docum_origem, a.ies_serie_fat ",               
        "ORDER BY a.cod_empresa, a.cod_cliente, a.num_pedido "
         ...
         "AND b.num_docum NOT IN ",
              "(SELECT c.num_docum FROM docum_comis_pagas c ",
                "WHERE c.cod_empresa   = b.cod_empresa ",
                  ...
                  "AND c.num_seq_docum = b.num_seq_docum) ",
        "GROUP BY a.cod_empresa, a.cod_cliente, a.num_pedido, a.num_docum, ",
        "a.val_bruto, a.val_liquido, a.pct_comis_3, a.num_docum_origem, a.ies_serie_fat ",               
        "ORDER BY a.cod_empresa, a.cod_cliente, a.num_pedido "
...
#--------------------------#
   ...
   FOREACH cq_pgto INTO                                                                   
      ...
      LET p_cod_emp_nf = p_docum.num_docum[1,2]  # Ivo: 04/01/2011
      ...
      LET p_num_nff = p_docum.num_docum_origem
...
#----------------------------------#
   ...
   DEFINE l_serie CHAR(02)
	 ...
	 IF p_ies_serie_fat  = "SU" THEN 
   		LET l_serie = '2'
   ELSE
   		LET l_serie = p_ies_serie_fat
   END IF 
   ...
   SELECT tip_frete,                        #Will - 25/10/10
    ...
    WHERE empresa           = p_cod_emp_nf  #Will - 25/10/10
      AND nota_fiscal       = p_num_nff     #Will - 25/10/10
      AND serie_nota_fiscal = l_serie       #Will - 25/10/10
      AND tip_nota_fiscal   IN ("FATPRDSV" , "FATSERV")      #Ivo
    UNION
   SELECT tip_frete,                        #Will - 25/10/10
          val_nota_fiscal,                  #Will - 25/10/10
          empresa,                           #Will - 25/10/10
