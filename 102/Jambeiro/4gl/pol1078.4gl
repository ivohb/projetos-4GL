...
# PROGRAMA: pol1078                                                         #
# OBJETIVO: IMPRESSAO DAS NOTAS FISCAIS FATURA - SERVICO - JAMBEIRO         #
#						EMPRESA 7																												#
# AUTOR...: PAULO                                                           #
# DATA....: 21/12/2010                                                      #
# ALTERADO:                                                                 #
...
GLOBALS
   ...
   DEFINE p_nff       
          RECORD
             ...
             val_extenso1        CHAR(100),
             val_extenso2        CHAR(100),
...
MAIN
   ...
   DEFER INTERRUPT
   CALL log140_procura_caminho("pol1078.iem") RETURNING comando
   OPTIONS
      HELP    FILE comando
   ...
   IF p_status = 0 THEN
      CALL pol1078_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol1078_controle()
#-------------------------#
   CALL log006_exibe_teclas("01", p_versao)

   CALL log130_procura_caminho("pol1078") RETURNING comando    
   OPEN WINDOW w_pol1078 AT 2,3 WITH FORM comando
   ...
   MENU "OPCAO"
      COMMAND "Informar"   "Informar parametros "
         HELP 0009
         MESSAGE ""
         CALL pol1078_inicializa_campos()
         IF log005_seguranca(p_user,"VDP","pol1078","CO") THEN
            IF pol1078_entrada_parametros() THEN
               NEXT OPTION "Listar"
            END IF
         END IF
      COMMAND "Listar"  "Lista as Notas Fiscais Fatura"
         HELP 1053
         IF log005_seguranca(p_user,"VDP","pol1078","CO") THEN
            IF pol1078_imprime_nff() THEN
               IF  pol1078_verifica_param_exportacao() = TRUE THEN 
               END IF
               NEXT OPTION "Fim"
            END IF
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol1078
END FUNCTION

#-----------------------------------#
FUNCTION pol1078_entrada_parametros()
#-----------------------------------#

   CALL log006_exibe_teclas("01 02 08", p_versao)
   CURRENT WINDOW IS w_pol1078
...
#----------------------------------#
FUNCTION pol1078_inicializa_campos()
#----------------------------------#
...
#------------------------------------------#
FUNCTION pol1078_verifica_param_exportacao()
#------------------------------------------#
...
#----------------------------#    
   ...
   IF log028_saida_relat(13,29) IS NOT NULL THEN 
      ...
      IF p_ies_impressao = "S" THEN 
         IF g_ies_ambiente = "W" THEN
            ...
            LET p_caminho = p_caminho CLIPPED, 'pol1078.tmp' 
            START REPORT pol1078_relat TO p_caminho 
         ELSE 
            START REPORT pol1078_relat TO PIPE p_nom_arquivo
         END IF 
      ELSE
         START REPORT pol1078_relat TO p_nom_arquivo
      END IF
   ELSE
      RETURN TRUE
   END IF  

   CALL pol1078_cria_tabela_temporaria()
   CURRENT WINDOW IS w_pol1078

   CALL pol1078_busca_dados_empresa()
   ...
   FOREACH cq_fat_nf_mestre INTO p_fat_nf_mestre_serv.*
      ...
      CALL pol1078_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol1078_busca_cof_compl()

      CALL pol1078_busca_nome_pais()
      LET p_nff.den_pais = p_paises.den_pais              

      CALL pol1078_busca_dados_duplicatas()

      #--- busca extenso do valor total da nota
      CALL log038_extenso(p_fat_nf_mestre_serv.val_nota_fiscal,100,100,1,1)
            RETURNING p_nff.val_extenso1, p_nff.val_extenso2,
                      p_nff.val_extenso3, p_nff.val_extenso4

      CALL pol1078_carrega_end_cobranca()
      ...
      CALL pol1078_carrega_corpo_nff()  {le os itens pertencentes a nf}

      LET p_nff.val_tot_nff = p_fat_nf_mestre_serv.val_nota_fiscal

      CALL pol1078_carrega_tabela_temporaria() {corpo todo da nota}
      CALL pol1078_carrega_historico()
      ...
      LET p_nff.den_cnd_pgto = pol1078_den_cnd_pgto()
      LET p_ies_lista = TRUE

      CALL pol1078_calcula_total_de_paginas()

      CALL pol1078_monta_relat()
...
#---------------------------------------#
FUNCTION pol1078_cria_tabela_temporaria()
#---------------------------------------#
...
#----------------------------#
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      OUTPUT TO REPORT pol1078_relat(p_wnotalev.*)
...
#-----------------------------------------#
 FUNCTION pol1078_busca_dados_duplicatas()
#-----------------------------------------#
...
#-------------------------------------#
FUNCTION pol1078_carrega_end_cobranca()
#-------------------------------------#
...
#----------------------------------#
FUNCTION pol1078_carrega_historico()
#----------------------------------#
		...
		FOREACH cq_texto INTO  l_texto
			IF LENGTH(l_texto) > 0 THEN 
			   ...
			   FOR p_ind = 1 TO 5
                ...
                CALL pol1078_insert_array(pr_txt[p_ind].texto)
             END IF
         END FOR
			END IF 
		END FOREACH  
...
#----------------------------------#
FUNCTION pol1078_carrega_corpo_nff()
#----------------------------------#
...
#--------------------------------------#
FUNCTION pol1078_verifica_ctr_unid_med()
#--------------------------------------#
...
#------------------------------------------#
FUNCTION pol1078_carrega_tabela_temporaria()
#------------------------------------------#
...
#  osvaldo
   ...
   LET p_des_texto = " "
   CALL pol1078_insert_array(p_des_texto)
...
#-----------------------------------------#
FUNCTION pol1078_calcula_total_de_paginas()
#-----------------------------------------#
   ...
   { 42 = numero de linhas do corpo da nota fiscal }

   IF p_num_linhas IS NOT NULL AND 
      p_num_linhas > 0 THEN 
      LET p_tot_paginas = (p_num_linhas - (p_num_linhas MOD 42 )) / 42
      IF (p_num_linhas MOD 42 ) > 0 THEN 
...
#------------------------------------#
FUNCTION pol1078_busca_cof_compl()
#------------------------------------#
...
#------------------------------------#
FUNCTION pol1078_busca_dados_empresa()            
#------------------------------------#
...
#---------------------------------------------------#
FUNCTION pol1078_busca_dados_clientes(p_cod_cliente)
#---------------------------------------------------#
...
#--------------------------------#
FUNCTION pol1078_busca_nome_pais()                   
#--------------------------------#
...
#------------------------------------------------#
FUNCTION pol1078_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#----------------------------------------#
FUNCTION pol1078_insert_array(p_des_texto)
#----------------------------------------#

   DEFINE p_des_texto CHAR(120)

   LET p_num_seq = p_num_seq + 1
	 LET p_qtd_lin_obs = p_num_seq
...
#------------------------------#
REPORT pol1078_relat(p_wnotalev)
#------------------------------#
   ...
   PAGE HEADER
      ...
      PRINT COLUMN 001, p_8lpp, p_comprime
            #COLUMN 129, p_nff.num_nff USING "&&&&&&"
  
      LET p_data = p_nff.dat_emissao USING "dd/mm/yyyy"

      SKIP 4 LINES           
      PRINT COLUMN 104, p_data[1,2],
            COLUMN 111, p_data[4,5],
            COLUMN 117, p_data[7,10]
      
      SKIP 4 LINES
      
      IF p_nff.dat_vencto_sd2 > 0 THEN
         PRINT COLUMN 001, p_nff.num_nff            USING "&&&&&&",
               COLUMN 015, p_nff.dat_vencto_sd1     USING "dd/mm/yyyy",   
               COLUMN 033, p_nff.val_duplic1        USING "###,##&.&&", 
               COLUMN 052, p_nff.num_nff            USING "&&&&&&", 
               COLUMN 066, p_nff.dat_vencto_sd2     USING "dd/mm/yyyy",   
               COLUMN 084, p_nff.val_duplic2        USING "###,##&.&&"
         
         IF p_nff.dat_vencto_sd4 > 0 THEN
            PRINT COLUMN 001, p_nff.num_nff            USING "&&&&&&",
                  COLUMN 015, p_nff.dat_vencto_sd3     USING "dd/mm/yyyy",   
                  COLUMN 033, p_nff.val_duplic3        USING "###,##&.&&", 
                  COLUMN 052, p_nff.num_nff            USING "&&&&&&", 
                  COLUMN 066, p_nff.dat_vencto_sd4     USING "dd/mm/yyyy",
                  COLUMN 084, p_nff.val_duplic4        USING "###,##&.&&"
         ELSE
            IF p_nff.dat_vencto_sd3 > 0 THEN
               PRINT COLUMN 001, p_nff.num_nff            USING "&&&&&&",
                     COLUMN 015, p_nff.dat_vencto_sd3     USING "dd/mm/yyyy",   
                     COLUMN 033, p_nff.val_duplic3        USING "###,##&.&&"
            ELSE
         ...
         END IF
      ELSE
         IF p_nff.dat_vencto_sd1 > 0 THEN
            PRINT COLUMN 001, p_nff.num_nff            USING "&&&&&&",
                  COLUMN 015, p_nff.dat_vencto_sd1     USING "dd/mm/yyyy",   
                  COLUMN 033, p_nff.val_duplic1        USING "###,##&.&&"
            PRINT
      ...
      {PRINT COLUMN 051, p_nff.den_nat_oper[1,24],
            COLUMN 076, p_nff.cod_fiscal      USING "&&&&",
            COLUMN 082, p_nff.ins_estadual_trib}
      
      SKIP 4 LINES      
      PRINT COLUMN 025, p_nff.nom_destinatario
      PRINT COLUMN 020, p_nff.end_destinatario,
            COLUMN 108, p_nff.cod_cep
      SKIP 1 LINES      
      PRINT COLUMN 020, p_nff.den_cidade, 
            COLUMN 101, p_nff.cod_uni_feder
      SKIP 1 LINES      
      PRINT COLUMN 025, p_nff.den_cidade
      PRINT COLUMN 020, p_nff.num_cgc_cpf,
            COLUMN 095, p_nff.ins_estadual

      SKIP 2 LINES

      PRINT COLUMN 025, p_nff.val_extenso1
      PRINT COLUMN 025, p_nff.val_extenso2
   ...
   ON EVERY ROW
      ...
      CASE
         WHEN p_wnotalev.ies_tip_info = 1             
            PRINT COLUMN 001, p_wnotalev.qtd_item USING "####&.&&",
                  COLUMN 013, p_wnotalev.cod_unid_med,
                  COLUMN 024, p_wnotalev.den_item,                        
                  COLUMN 094, p_wnotalev.pre_unit       USING "##,##&.&&",
                  COLUMN 113, p_wnotalev.val_liq_item   USING "#,###,##&.&&"
                  
            LET p_linhas_print = p_linhas_print + 1 

         WHEN p_wnotalev.ies_tip_info = 2
            PRINT COLUMN 024, p_wnotalev.den_item 
            LET p_linhas_print = p_linhas_print + 1

         WHEN p_wnotalev.ies_tip_info = 3
            IF p_wnotalev.des_texto is null or p_wnotalev.des_texto = ' ' THEN 
            Else
              PRINT COLUMN 024, p_wnotalev.des_texto
            	LET p_linhas_print = p_linhas_print + 1
            END IF	

         WHEN p_wnotalev.ies_tip_info = 4
            WHILE TRUE
               IF p_linhas_print < 40 THEN 
                  PRINT
                  LET p_linhas_print = p_linhas_print + 1        
               ELSE 
                  EXIT WHILE
      ...
      IF p_linhas_print = 40  THEN { nr. de linhas do corpo da nota }
         IF p_num_pagina = p_tot_paginas THEN 
            PRINT COLUMN 113, p_nff.val_tot_servico USING "###,###,##&.&&"
            LET p_linhas_print = 0
            LET p_num_pagina = 0
         ELSE
            LET p_linhas_print = 40 - p_linhas_print         
            SKIP p_linhas_print LINES
            PRINT
            PRINT
            PRINT COLUMN 113, "**************" 
            LET p_linhas_print = 0
