...
GLOBALS
   DEFINE p_nf_mestre         RECORD LIKE nf_mestre.*,
          ...
          p_fat_numero_885    RECORD LIKE fat_numero_885.*,
   ...
   DEFINE p_nf_popup ARRAY[100] OF RECORD 
...
MAIN
   LET p_versao = "POL0712-05.10.10" #Favor nao alterar esta linha (SUPORTE)
   ...
   IF p_status = 0 THEN 
      ...
      CALL pol0712_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0712_controle()
#--------------------------#
   ...
   CALL log130_procura_caminho("pol0712") RETURNING p_nom_tela 
   OPEN WINDOW w_pol0712 AT 2,2 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   MENU "OPCAO"
{      COMMAND "Informar" "Informa periodo de faturamento"
         HELP 2010
         MESSAGE ""
         IF pol0712_entrada_dados() THEN
            NEXT OPTION "Processar"
         ELSE 
            NEXT OPTION "Informar"
         END IF }  
      COMMAND "Processar" "Executa Baixa de estaque"
         HELP 2011
         MESSAGE ""
         IF log005_seguranca(p_user,"VDP","pol0712","MO") THEN 
            CALL pol0712_processa()
            CALL log085_transacao("COMMIT")
            LET p_ies_processou = TRUE
            MESSAGE "BAIXAS PROCESSADAS COM SUCESSO ...!!!"
            ATTRIBUTE (REVERSE)  
            NEXT OPTION "Fim"
         END IF
      COMMAND KEY ("!")
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         END IF
   END MENU
   CLOSE WINDOW w_pol0712

END FUNCTION

#-------------------------------#
 FUNCTION pol0712_entrada_dados()
#-------------------------------#
 
   CALL log006_exibe_teclas("01 02 07",p_versao)
   CURRENT WINDOW IS w_pol0712

   LET INT_FLAG = FALSE  
   INPUT p_tela.dat_inicio,
         p_tela.dat_fim WITHOUT DEFAULTS  
    FROM dat_inicio,
         dat_fim

      AFTER FIELD dat_inicio
      IF p_tela.dat_inicio IS NULL THEN
         ERROR "O Campo data inicio nao pode ser Nulo"
...
#---------------------------------#
 FUNCTION pol0712_carrega_tabelas()
#---------------------------------#
   ...
   FOREACH c_nf_item INTO p_nf_item.num_pedido,
      ...
      WHERE a.cod_empresa = p_empresas_885.cod_emp_gerencial
        ...
        AND b.num_om        = p_nf_item.num_om
 
      IF SQLCA.SQLCODE <> 0 THEN
         LET t_nf_item[p_i].qtd_total = 0 
      ELSE
         IF p_nf_mestre.cod_cliente = '033767500000102' THEN 
            LET t_ped_itens[p_i].pre_unit = t_ped_itens[p_i].pre_unit * 1.15
         END IF    
...
#--------------------------#
 DEFINE p_val_liquido  DEC(17,2)
   ...
   IF pol0712_checa_par() THEN
      ERROR "Empresa para Copia sem Parametros Cadastrados" 
      RETURN 
   END IF
   
   DECLARE cq_mst CURSOR FOR
   SELECT * 
     FROM nf_mestre
    WHERE cod_empresa = p_cod_empresa 
      AND ies_situacao = "N"
      AND dat_emissao >= TODAY
      AND dat_emissao <= TODAY  
   FOREACH cq_mst INTO p_nf_mestre.*

##      DISPLAY "NOTA : "  AT  8,6
##      DISPLAY p_nf_mestre.num_nff   AT  8,15
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         LET p_houve_erro = TRUE 
         CALL log003_err_sql("SELECT","FAT_NUMERO_885")
         CALL log085_transacao("ROLLBACK")
      END IF
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         LET p_houve_erro = TRUE 
         CALL log003_err_sql("UPDATE","FAT_NUMERO_885")
         CALL log085_transacao("ROLLBACK")
      END IF
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         LET p_houve_erro = TRUE 
         CALL log003_err_sql("INSERT","REF_NOTA_885")
         CALL log085_transacao("ROLLBACK")
      END IF
...
#-----------------------#
FUNCTION pol0712_popup()
#-----------------------#

   CASE
      WHEN INFIELD(num_nff)
         CALL pol0712_popup_notas() RETURNING p_num_nff 
         CALL log006_exibe_teclas("01 02 03 07", p_versao)
         CURRENT WINDOW IS w_pol0712
         DISPLAY p_num_nff TO p_nf_mestre.num_nff
...
#----------------------------#
FUNCTION pol0712_popup_notas()
#----------------------------#

   DEFINE p_index, s_index SMALLINT,
          t_num_nff      LIKE nf_mestre.num_nff,
          t_nom_cliente  LIKE clientes.nom_cliente  

   INITIALIZE p_nom_tela, p_nf_popup TO NULL
   CALL log130_procura_caminho("pol07121") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol07121 AT 7,19 WITH FORM p_nom_tela
   ...
   FOREACH cq_nfs INTO t_num_nff,t_nom_cliente
   
      SELECT * 
        FROM nf_mestre 
       WHERE cod_empresa =  p_empresas_885.cod_emp_gerencial
         AND num_nff     =  t_num_nff
         
      IF SQLCA.sqlcode <> 0 THEN 
         LET p_nf_popup[p_index].num_nff = t_num_nff
         LET p_nf_popup[p_index].nom_cliente = t_nom_cliente
   ...
   DISPLAY ARRAY p_nf_popup TO s_nf_popup.*

      LET p_index = ARR_CURR()
      LET s_index = SCR_LINE() 
      
   CLOSE WINDOW w_pol07121
   
   IF INT_FLAG = 0 THEN
      RETURN p_nf_popup[p_index].num_nff
   ELSE
      LET INT_FLAG = 0
      RETURN "",""
   END IF
