...
#--------------------------------#
   ...
   LET p_msg = func018_le_ped_indus(m_cod_emp_pv, m_cod_emp_op, 0)
   ...
   FOREACH cq_ins_it INTO lr_r_item.*
      ...
      SELECT num_sequencia
       ...
       WHERE cod_empresa = m_cod_emp_op
         ...
         AND cod_item = lr_r_item.coditem

      IF STATUS <> 0 THEN
         CALL log003_err_sql('SELECT','ped_itens.cq_ins_it')
         RETURN FALSE
      END IF
      
      UPDATE roma_item_885 
         SET numpedido = l_pedido,
             numseqitem = l_num_seq
       WHERE codempresa = m_cod_emp_op
	       AND numsequencia = lr_r_item.numsequencia
	       AND numromaneio = lr_r_item.numromaneio
	       AND numseqpai = lr_r_item.numseqpai
	
      IF STATUS <> 0 THEN
         CALL log003_err_sql('UPDATE','ped_itens.cq_ins_it')
         RETURN FALSE
      END IF
...
#-------------------------------#
  RETURN "$Archive: /Logix/Fontes_Doc/Customizacao/10R2/gps_logist_e_gerenc_de_riscos_ltda/financeiro/solicitacao de faturameto/programas/pol1271.4gl $|$Revision: 61 $|$Date: 09/09/2019 12:60 $|$Modtime: 22/05/11 14:00 $" 
 END FUNCTION
