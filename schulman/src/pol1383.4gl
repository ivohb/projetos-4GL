...
#---------------------------------------#
    ...
    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_browse)
    CALL _ADVPL_set_property(l_tabcolumn,"HEADER","Bloq ped")
    CALL _ADVPL_set_property(l_tabcolumn,"COLUMN_WIDTH",80)
    CALL _ADVPL_set_property(l_tabcolumn,"VARIABLE","bloqueia_pedido")

    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_browse)
    CALL _ADVPL_set_property(l_tabcolumn,"HEADER","Bloq fat")
    CALL _ADVPL_set_property(l_tabcolumn,"COLUMN_WIDTH",80)
    ...
    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_browse)
    CALL _ADVPL_set_property(l_tabcolumn,"HEADER","Area/linha")
    ...
    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_browse)
    CALL _ADVPL_set_property(l_tabcolumn,"HEADER","Preço Uunit")
    CALL _ADVPL_set_property(l_tabcolumn,"COLUMN_WIDTH",120)
...
#-----------------------------#
   ...
   SELECT DISTINCT 
     ...
     INTO mr_cabec.cod_empresa,
          mr_cabec.num_Lista,
          mr_cabec.den_lista,
          mr_cabec.dat_val_ini,	   
          mr_cabec.dat_val_fim,	   
          mr_cabec.bloqueia_pedido, 
          mr_cabec.bloqueia_fatur  
     FROM lista_temp_schulman
   ...
   FOREACH cq_exib INTO 
      ma_itens[l_ind].cod_empresa,	         
      ma_itens[l_ind].num_Lista,	           
      ma_itens[l_ind].dat_val_ini,	         
      ma_itens[l_ind].dat_val_fim,	         
      ma_itens[l_ind].bloqueia_pedido,	     
      ma_itens[l_ind].bloqueia_faturamento, 
      ma_itens[l_ind].cod_moeda,	           
      ma_itens[l_ind].unid_medida,	         
      ma_itens[l_ind].cod_cliente,	         
      ma_itens[l_ind].area_e_linha,	       
      ma_itens[l_ind].cod_item,	           
      ma_itens[l_ind].preco_unit,
      ma_itens[l_ind].preco_minimo
...
#---------------------------#
   ...
   IF m_ies_lista THEN
      LET m_msg = 'A lista exibida na tela já existe no Logix.\n',
                  'Excluí-la do Logix e gerar novamente ?'
      IF NOT LOG_question(m_msg) THEN
         RETURN FALSE
      END IF
   END IF
