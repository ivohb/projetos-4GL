...
DATABASE logix
   ...
   DEFINE 
		  ...
		  p_obf_nf_eletr       RECORD LIKE obf_nf_eletr.*
...
#-------------------------------#
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql('Cirando','chave_tmp_7662')
      RETURN FALSE
   END IF  
...
#---------------------------#
  ...
  #END IF
	...
	UPDATE fat_nota_fiscal_texto_item 
	   SET descricao_texto = UPPER(descricao_texto)
   WHERE empresa = p_cod_empresa 
  ...
  END IF
	...
	IF NOT pol0989_insere_duplicata() THEN 
		RETURN FALSE
	END IF 
#--Manuel 11-03-2015 daqui 
	
	
	{IF NOT pol0989_insere_obf_nf_eletr() THEN  #Maria pediu para inibir
		RETURN FALSE
	END IF} 
	
#--Manuel 11-03-2015  até aqui 
...
#-----------------------------#
 ...
 END RECORD
    ...
    LET p_fat_item.val_acre_merc      = p_valor_ipi_item                              
    ...
    SELECT campo_inteiro 
      INTO p_fat_item.cest
      FROM obf_parametro_item 
     WHERE empresa = p_cod_empresa
       AND item = p_fat_item.item 
       AND campo = 'cest'
    
    IF STATUS <> 0 THEN
       LET p_fat_item.cest = NULL
    END IF
    ...
    IF NOT pol0989_gra_obf_fiscal() THEN
       RETURN FALSE
    END IF
		
		LET p_val_acres = p_val_acres + p_valor_ipi_item
...
#------------------------------#
   ...
   IF STATUS <> 0 THEN
      LET p_cod_status = STATUS
      LET p_msg = 'ERRO ', p_cod_status, 'LENDO TIPO DO ITEM NA TABELA VDP_PARAMETRO_ITEM'
			CALL pol0989_imprime_erros(p_msg)
			LET p_erro = TRUE 
      RETURN FALSE
   END IF
...
#-------------------------------#
   ...
   FOREACH cq_chave INTO p_chave
      ...
      WHEN 'CARTEIRA' 
         LET p_query  = p_query CLIPPED, " AND carteira = '",p_pedido.cod_tip_carteira,"' "
         LET p_matriz[5] = 'N'
   ...
   IF p_cheve_ok THEN
      ...
      FOREACH cq_obf_cfg INTO p_trans_config
         ...
         IF STATUS <> 0 THEN 
            CALL log003_err_sql('insert','tributo_tmp')
            RETURN FALSE
         END IF
...
#---------------------------------#
   ...
   DEFINE l_val_base_trib LIKE fat_nf_item_fisc.val_unit
   ...
   IF STATUS <> 0 THEN
      LET l_val_base_trib   = p_arquivo.total_val_item 
   ELSE
      SELECT aliquota
        INTO p_aliquota_ipi
      ...
      END IF
	    LET l_val_base_trib = p_arquivo.total_val_item / (1+(p_aliquota_ipi / 100))
   END IF
   ...
   FOREACH cq_trib_tmp INTO p_tributo_benef, p_trans_config
      ...
      IF p_tributo_benef = 'ICMS'   THEN
	       ...
	       IF p_controle_fci IS NULL OR p_controle_fci = ' ' OR p_controle_fci = 'N' THEN #ivo 24/09/2013
	       ELSE
            CALL pol0989_grava_fci() RETURNING p_status       #ivo 24/09/2013
         END IF                                               #ivo 24/09/2013
      ELSE	
         LET p_val_base_trib = l_val_base_trib
	    END IF
...
#--------------------------#
   ...
   INSERT INTO fat_nf_item_fisc(
      ...
      iden_processo,
      qtd_base_calc) 
      VALUES(p_item_fisc.empresa,                
             p_item_fisc.trans_nota_fiscal,      
             p_item_fisc.seq_item_nf,            
             p_item_fisc.tributo_benef,          
             p_item_fisc.trans_config,           
             p_item_fisc.bc_trib_mercadoria,     
             p_item_fisc.bc_tributo_frete,       
             p_item_fisc.bc_trib_calculado,      
             p_item_fisc.bc_tributo_tot,         
             p_item_fisc.val_trib_merc,          
             p_item_fisc.val_tributo_frete,      
             p_item_fisc.val_trib_calculado,     
             p_item_fisc.val_tributo_tot,        
             ...
             p_item_fisc.cotacao_moeda_upf,      
             p_item_fisc.simples_nacional,       
             p_item_fisc.iden_processo,          
             p_item_fisc.qtd_base_calc)          
...
#-------------------------------#
FUNCTION pol0989_gra_obf_fiscal()
#-------------------------------#

   DECLARE cq_item_fisc CURSOR FOR
    SELECT 
      empresa,           
      trans_nota_fiscal, 
      seq_item_nf,       
    ...
    FROM fat_nf_item_fisc
   WHERE empresa = p_cod_empresa
     AND trans_nota_fiscal = p_fat_item.trans_nota_fiscal
     AND seq_item_nf = p_fat_item.seq_item_nf
   
   FOREACH cq_item_fisc INTO
             p_item_fisc.empresa,                
             p_item_fisc.trans_nota_fiscal,      
             p_item_fisc.seq_item_nf,            
             p_item_fisc.tributo_benef,          
      ...
      IF STATUS <> 0 THEN
         LET p_cod_status = STATUS
         LET p_msg = 'ERRO ', p_cod_status, 'LENDO DADOS DA TABELA FAT_NF_ITEM_FISC'
         CALL pol0989_imprime_erros(p_msg)
	       LET p_erro = TRUE 
         RETURN FALSE
      END IF
         
      IF NOT pol0989_ins_obf_fiscal() THEN
         RETURN FALSE
      END IF
...
#-------------------------------#
FUNCTION pol0989_ins_obf_fiscal()
#-------------------------------#
                       
   DEFINE p_obf_fiscal         RECORD
          empresa              char(2),                 
          ...
          dia                  decimal(2,0),            
          conta_contab         char(23),                
          tip_incentiv         smallint,                
          ...
          #cod_item_nf_eletr    CHAR(60)
          #des_item_nf_eletr    CHAR(255)
          #aliquota_qtd         DECIMAL
          #qtd_base_calc        DECIMAL
          #cod_contrib          CHAR(2)
          #natureza_receita     CHAR(3)
          #cod_atividade        CHAR(8)
          #cod_ajuste           CHAR(10)
          #cod_apuracao         DECIMAL
   END RECORD

   LET p_obf_fiscal.empresa            = p_item_fisc.empresa 
   LET p_obf_fiscal.empresa_origem     = p_item_fisc.empresa
   LET p_obf_fiscal.trans_nota_fiscal  = p_item_fisc.trans_nota_fiscal
   LET p_obf_fiscal.tip_nota_fiscal    = p_fat_mestre.tip_nota_fiscal
   LET p_obf_fiscal.serie_nota_fiscal  = p_fat_mestre.serie_nota_fiscal
   LET p_obf_fiscal.subserie_nf        = p_fat_mestre.subserie_nf
   LET p_obf_fiscal.espc_nota_fiscal   = p_fat_mestre.espc_nota_fiscal
   LET p_obf_fiscal.nota_fiscal        = p_fat_mestre.nota_fiscal
   LET p_obf_fiscal.modelo_nota_fiscal = p_fat_mestre.modelo_nota_fiscal
   LET p_obf_fiscal.origem_nota_fiscal = p_fat_mestre.origem_nota_fiscal
   LET p_obf_fiscal.sit_nota_fiscal    = p_fat_mestre.sit_nota_fiscal
   LET p_obf_fiscal.cliente            = p_fat_mestre.cliente
   LET p_obf_fiscal.remetent           = p_fat_mestre.remetent
   LET p_obf_fiscal.zona_franca        = p_fat_mestre.zona_franca
   LET p_obf_fiscal.finalidade         = p_fat_mestre.finalidade
   LET p_obf_fiscal.transportadora     = p_fat_mestre.transportadora
   LET p_obf_fiscal.tip_frete          = p_fat_mestre.tip_frete
   LET p_obf_fiscal.dat_hor_emissao    = p_fat_mestre.dat_hor_emissao
   LET p_obf_fiscal.dia                = DAY(p_fat_mestre.dat_hor_emissao)
   LET p_obf_fiscal.conta_contab       = ' '
   LET p_obf_fiscal.tip_incentiv       = 0
   LET p_obf_fiscal.val_nota_fiscal    = p_fat_mestre.val_nota_fiscal  
   LET p_obf_fiscal.seq_item_nf        = p_item_fisc.seq_item_nf
   LET p_obf_fiscal.tip_item           = p_fat_item.tip_item
   LET p_obf_fiscal.item               = p_fat_item.item
   LET p_obf_fiscal.unid_medida        = p_fat_item.unid_medida
   LET p_obf_fiscal.qtd_item           = p_fat_item.qtd_item
   LET p_obf_fiscal.natureza_operacao  = p_fat_mestre.natureza_operacao   
   LET p_obf_fiscal.classif_fisc       = p_fat_item.classif_fisc
   LET p_obf_fiscal.item_prod_servico  = p_fat_item.item_prod_servico
   LET p_obf_fiscal.preco_unit_bruto   = p_fat_item.preco_unit_bruto
   LET p_obf_fiscal.preco_unit_liquido = p_fat_item.preco_unit_liquido 
   LET p_obf_fiscal.val_desc_item      = p_fat_item.val_desc_item
   LET p_obf_fiscal.val_desc_merc      = p_fat_item.val_desc_merc
   LET p_obf_fiscal.val_frete_cliente  = p_fat_item.val_frete_cliente
   LET p_obf_fiscal.val_seguro_cliente = p_fat_item.val_seguro_cliente 
   LET p_obf_fiscal.val_liquido_item   = p_fat_item.val_liquido_item
   LET p_obf_fiscal.val_merc_item      = p_fat_item.val_merc_item   
   LET p_obf_fiscal.val_contab_item    = p_fat_item.val_contab_item 
   LET p_obf_fiscal.tributo_benef      = p_item_fisc.tributo_benef
   LET p_obf_fiscal.bc_tributo_tot     = p_item_fisc.bc_tributo_tot
   LET p_obf_fiscal.val_tributo_tot    = p_item_fisc.val_tributo_tot
   LET p_obf_fiscal.val_ntribut_tot    = 0
   
   IF p_item_fisc.incide = 'S' THEN
      LET p_obf_fiscal.incide = 1
   ELSE
      IF p_item_fisc.incide = 'N' THEN
         LET p_obf_fiscal.incide = 3
      ELSE
         LET p_obf_fiscal.incide             = p_item_fisc.incide
      END IF
   END IF
   
   IF p_obf_fiscal.incide = 1 THEN
   ELSE
      LET p_obf_fiscal.val_ntribut_tot = p_obf_fiscal.bc_tributo_tot
      LET p_obf_fiscal.val_tributo_tot = 0
      LET p_obf_fiscal.bc_tributo_tot = 0
   END IF
   
   LET p_obf_fiscal.tributacao         = p_item_fisc.tributacao
   LET p_obf_fiscal.cod_fiscal         = p_item_fisc.cod_fiscal
   LET p_obf_fiscal.dipam_b            = p_item_fisc.dipam_b
   LET p_obf_fiscal.aliquota           = p_item_fisc.aliquota 
   LET p_obf_fiscal.pct_red_bas_calc   = p_item_fisc.pct_red_bas_calc  
   LET p_obf_fiscal.estado             = p_cod_uni_feder
                                                           
   INSERT INTO obf_fiscal_saida (
     empresa,                     
     empresa_origem,              
     trans_nota_fiscal,           
     tip_nota_fiscal,             
     ...
     aliquota,                    
     pct_red_bas_calc,            
     estado) VALUES (p_obf_fiscal.*)                 
        
   IF STATUS <> 0 THEN
      LET p_cod_status = STATUS
      LET p_msg = 'ERRO ', p_cod_status, 'INSERINDO DADOS NA TABELA OBF_FISCAL_SAIDA'
      CALL pol0989_imprime_erros(p_msg)
	    LET p_erro = TRUE 
      RETURN FALSE
   END IF
...
#--------------------------------#
   ...
   MESSAGE 'Atualizando fat_nf_mestre!'
	...
	SELECT SUM(peso_unit * qtd_item), 
	       SUM(val_merc_item),
	       SUM(val_contab_item)  #ivo 06/12/2016
    INTO p_tot_peso,
         p_val_liqui,
         p_val_bruto
    FROM fat_nf_item
   ...
   UPDATE fat_nf_mestre
      SET peso_bruto      	= p_tot_peso,
          ...
          val_mercadoria  	= p_val_bruto,     #ivo 06/12/2016
          val_duplicata   	= p_val_bruto,
          val_nota_fiscal 	= p_val_bruto,
		      val_acre_merc     = p_val_acres      #ivo 06/12/2016		           
    WHERE empresa = p_cod_empresa
      AND trans_nota_fiscal = p_trans_nf
   ...
   UPDATE obf_fiscal_saida
      SET val_nota_fiscal 	= p_val_bruto
    WHERE empresa = p_cod_empresa
      AND trans_nota_fiscal = p_trans_nf

   IF STATUS <> 0 THEN
      LET p_cod_status = STATUS
      LET p_msg = 'ERRO ', p_cod_status, 'ATUALIZANDO DADOS NA TABELA OBF_FISCAL_SAIDA'
      CALL pol0989_imprime_erros(p_msg)
      LET p_erro = TRUE 
      RETURN FALSE
   END IF
...
#-------------------------------------#
FUNCTION pol0989_insere_obf_nf_eletr()#
#-------------------------------------#

  INITIALIZE 	 p_obf_nf_eletr TO NULL
  
  LET  p_obf_nf_eletr.empresa           	= p_cod_empresa
  LET  p_obf_nf_eletr.trans_nota_fiscal 	= p_trans_nf
  LET  p_obf_nf_eletr.aviso_recebto     	= 0
  LET  p_obf_nf_eletr.origem_nota_fiscal    = 'S'
  LET  p_obf_nf_eletr.nota_fiscal       	= p_fat_mestre.nota_fiscal
  LET  p_obf_nf_eletr.tip_nota_fiscal   	= p_fat_mestre.tip_nota_fiscal
  LET  p_obf_nf_eletr.serie_nota_fiscal  	= p_fat_mestre.serie_nota_fiscal
  LET  p_obf_nf_eletr.subserie_nf           = p_fat_mestre.subserie_nf
  LET  p_obf_nf_eletr.sit_atual_nf_eletr    = 'N'
  LET  p_obf_nf_eletr.status_envio_nfe   	= '1'

	INSERT INTO obf_nf_eletr
	 VALUES(p_obf_nf_eletr.*)	        
	 
	IF SQLCA.SQLCODE <> 0 THEN 
      LET p_cod_status = STATUS
      LET p_msg = 'ERRO ', p_cod_status, 'INSERINDO DADOS DA TABELA OBF_NF_ELETR'
      CALL pol0989_imprime_erros(p_msg)
      LET p_erro = TRUE 
      RETURN FALSE
	END IF 
...
#  LET  p_nf_integr.dat_hr_intg_creceb	=  
  LET  p_nf_integr.status_integr_obf	= 'I'	 
  LET  p_nf_integr.dat_hor_integr_obf	= p_fat_mestre.dat_hor_emissao	 
...
#-------------------------#
   ...
   DEFINE p_cod_txt INTEGER
   
   LET p_cod_txt = p_item_fisc.hist_fiscal
   
   IF p_cod_txt IS NULL OR p_cod_txt = 0 THEN
      RETURN TRUE
   END IF
   ...
   SELECT   
    ...
    INTO p_cod_hist,  
      ...
      p_tex_hist_4
    FROM fiscal_hist  
   WHERE fiscal_hist.cod_hist = p_cod_txt  
   ...
   LET p_descricao_texto = NULL

   IF p_tex_hist_1 IS NULL OR p_tex_hist_1 = ' ' THEN
    ELSE
      CALL pol0989_tira_menor(p_tex_hist_1) 
   END IF

   IF p_tex_hist_2 IS NULL OR p_tex_hist_2 = ' ' THEN
   ELSE
      CALL pol0989_tira_menor(p_tex_hist_2) 
   END IF
         
   IF p_tex_hist_3 IS NULL OR p_tex_hist_3 = ' ' THEN
   ELSE
      CALL pol0989_tira_menor(p_tex_hist_3) 
   END IF

   IF p_tex_hist_4 IS NULL OR p_tex_hist_4 = ' ' THEN
   ELSE
      CALL pol0989_tira_menor(p_tex_hist_4) 
   END IF

   IF p_descricao_texto IS NOT NULL THEN          
      CALL pol0989_ins_texto_fci() RETURNING p_status
   END IF
...
#-----------------------------------#
FUNCTION pol0989_tira_menor(p_texto)#
#-----------------------------------#
   ...
   FOR m_ind = 1 TO LENGTH(m_texto)
       LET m_carac = m_texto[m_ind,m_ind]
       IF m_carac = '<' THEN  
          EXIT FOR
       END IF
   END FOR
   ...
   LET p_texto = m_texto[1,m_ind]
   
   IF p_descricao_texto IS NULL THEN
      LET p_descricao_texto = p_texto CLIPPED
   ELSE
      LET p_descricao_texto = p_descricao_texto CLIPPED, ' ', p_texto CLIPPED
   END IF

END FUNCTION

#-------------------------------#
FUNCTION pol0989_ins_texto_fci()#
#-------------------------------#
   
   LET p_descricao_texto = p_descricao_texto CLIPPED, ' ', p_num_controle_fci
