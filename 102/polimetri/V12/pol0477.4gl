...
#--------------------------#
   ...
   MENU "OPCAO"
      ...
      COMMAND KEY ("B") "liBerar" 'Libera a integração de apontamentos'
         ...
         END IF
      COMMAND "Ordem" 'Exporta ordens para o EGA'
         MESSAGE ""
         CALL log120_procura_caminho("pol0401") RETURNING comando
         LET comando = comando 
...
#---------------------------#
   ...
   IF STATUS = 100 THEN
      INSERT INTO proc_apont_man912 VALUES('N', CURRENT HOUR TO SECOND, p_cod_empresa)
      IF STATUS <> 0 THEN
...
#-------------------------------#
   ...
   DEFINE p_prog_impot CHAR(10)
   
   SELECT prog_import_op
     INTO p_prog_impot
   ...
   CALL log120_procura_caminho(p_prog_impot) RETURNING comando
