...
# PROGRAMA: pol0338                                                         #
# MODULOS.: pol0338  - LOG0010 - LOG0040 - LOG0050 - LOG0060                #
#           LOG0280  - LOG0380 - LOG1300 - LOG1400                          #
# OBJETIVO: IMPRESSAO DAS NOTAS ITAESBRA - SAIDA 					   			          #
...
GLOBALS
   DEFINE p_cod_empresa            LIKE empresa.cod_empresa,
          ...
          comando                  CHAR(80),      
          i                        SMALLINT,
           p_num_duns              LIKE duns_itaesbra.num_duns,
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol0338_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol0338_controle()
#-------------------------#

   CALL log006_exibe_teclas("01", p_versao)
   CALL log130_procura_caminho("pol0338") RETURNING comando    
   OPEN WINDOW w_pol0338 AT 2,2 WITH FORM comando
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   DISPLAY p_cod_empresa TO empresa

   CALL log085_transacao("BEGIN") 

   IF NOT pol0338_cria_tabs_tmp() THEN
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informar Parametros"
         HELP 0001
         IF pol0338_entrada_parametros() THEN
            LET p_ies_cons = TRUE
            NEXT OPTION "Listar"
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0338

END FUNCTION

#-----------------------------------#
FUNCTION pol0338_entrada_parametros()
#-----------------------------------#

   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol0338
...
#----------------------------#    
   ...
   IF log028_saida_relat(16,41) IS NOT NULL THEN 
      ...
      IF p_ies_impressao = "S" THEN 
         IF g_ies_ambiente = "U" THEN
            START REPORT pol0338_relat TO PIPE p_nom_arquivo
         ELSE 
            CALL log150_procura_caminho ('LST') RETURNING p_caminho
            LET p_caminho = p_caminho CLIPPED, 'pol0338.tmp' 
            START REPORT pol0338_relat TO p_caminho 
         END IF 
      ELSE
         START REPORT pol0338_relat TO p_nom_arquivo
      END IF
   ELSE
      RETURN TRUE
   END IF
   ...
   CALL pol0338_busca_dados_empresa()
   ...
   IF (p_reimpressao = "S") OR    (p_reimpressao = "R") THEN
         IF p_reimpressao = "S" THEN
         		LET p_reimpressao = "R"
         END IF 
           LET sql_stmt_1 = "SELECT * FROM fat_nf_mestre ",
      ...
      PREPARE cq_cursor FROM sql_stmt_1  
      DECLARE cq_fat_nf_mestre CURSOR WITH HOLD FOR cq_cursor

   FOREACH cq_fat_nf_mestre INTO p_fat_nf_mestre.*
   
   		LET p_agrupa_item = FALSE 
      ...
      CALL log085_transacao("BEGIN") 
      IF NOT pol0338_deleta_tabs_tmp() THEN
      ...
      FOREACH cq_codf INTO p_cod_fiscal
         ...
         IF p_nff.cod_fiscal IS NULL THEN
            ...
            LET p_nff.den_nat_oper = pol0338_den_nat_oper()
      ...
      IF SQLCA.sqlcode = 0 THEN
         LET p_agrupa_item = TRUE 
      END IF
      
      IF p_nat_operacao.ies_subst_tribut = "S" THEN
         CALL pol0338_busca_dados_subst_trib_uf()
      ...
      CALL pol0338_busca_dados_clientes()
      ...
      CALL pol0338_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol0338_busca_cof_compl()

      #--- busca nome do pais
      CALL pol0338_busca_nome_pais()
      LET p_nff.den_pais = p_paises.den_pais              

      #--- busca dados das duplicatas
      CALL pol0338_busca_dados_duplicatas()
			
			#especifico da Itaesbra não exibir nota fiscal por extenso
      ...
      CALL pol0338_carrega_corpo_nff()

      CALL pol0338_grava_corpo_nota()

      IF p_nat_operacao.ies_tip_controle = "2" THEN
         #CALL pol0338_obtem_peso_item()
      END IF
      ...
      CALL pol0338_grava_dados_end_entrega()

      #--- carrega endereco de cobranca
      CALL pol0338_carrega_end_cobranca()

      #--- trata zona franca
      IF p_fat_nf_mestre.zona_franca = 'S' THEN
         CALL pol0338_le_icms('ICMS_ZF')
         CALL pol0338_le_param_zf()
      ELSE
         CALL pol0338_le_icms('ICMS')
      END IF
      ...
      #CALL pol0338_retorno_terceiro()

      #--- busca dados da transportadora
      CALL pol0338_busca_dados_transport(p_fat_nf_mestre.transportadora)
      CALL pol0338_busca_dados_cidades(p_transport.cod_cidade)
      ...
      LET p_nff.nom_guerra      = pol0338_representante()
      ...
      CALL pol0338_den_cnd_pgto()
      
      #--- grava dados consignados
      #CALL pol0338_grava_dados_consig()
      
    #  IF NOT p_ret_terc THEN
    #     CALL pol0338_checa_nf_contra_ordem()
    #  END IF
      CALL pol0338_grava_historico_nf()

     # CALL pol0338_carrega_historico_fiscal()
      
      CALL pol0338_monta_relat()
...
#------------------------------------------#
FUNCTION pol0338_grava_historico_nf()
#------------------------------------------#

   DEFINE i SMALLINT,
   				l_nf_refer LIKE fat_nf_mestre.nota_fiscal,
   ...
   { imprime texto da nota, se este existir  } 
      ...
      FOREACH cq_texto_nf INTO p_des_texto
         IF p_des_texto IS NOT NULL OR
            p_des_texto <> " " THEN 
            CALL pol0338_insere_texto(p_des_texto,3)
         END IF
      END FOREACH               
   

   DECLARE cq_nf_ref CURSOR FOR
		SELECT A.NOTA_FISCAL_REFER, B.NOTA_FISCAL
		FROM FAT_NF_REFER_ITEM A, FAT_NF_MESTRE B
		WHERE A.EMPRESA = p_cod_empresa
		AND ( TRANS_NF_REFER = p_fat_nf_mestre.trans_nota_fiscal
		OR  A.TRANS_NOTA_FISCAL =p_fat_nf_mestre.trans_nota_fiscal)
		AND A.EMPRESA = B.EMPRESA
		AND A.TRANS_NOTA_FISCAL = B.TRANS_NOTA_FISCAL

   LET p_prim = 1

   FOREACH cq_nf_ref INTO l_nf_refer, l_nota_fiscal
      ...
      IF l_nf_refer IS NOT NULL THEN 
         LET p_des_texto = "N.F. DE VENDA Nro. ", l_nota_fiscal 
             USING "&&&&&&", " DE ", p_fat_nf_mestre.dat_hor_emissao USING "dd/mm/yyyy"
         CALL pol0338_insere_texto(p_des_texto,3)
      END IF
      IF l_nota_fiscal IS NOT NULL THEN 
         LET p_des_texto = "N.F. DE REMESSA Nro. ", l_nf_refer 
             USING "&&&&&&"
         CALL pol0338_insere_texto(p_des_texto,3)
      END IF
...
#-------------------------------#
FUNCTION pol0338_junta_volumes()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol0338_cria_tabs_tmp()
#-------------------------------#
   ...
   END IF
    ...
    CREATE TEMP TABLE wnotalev_aux
     (
      num_seq            SMALLINT,
      ies_tip_info       SMALLINT,
      cod_item           CHAR(15),
...
#---------------------------------#
FUNCTION pol0338_deleta_tabs_tmp()
#---------------------------------#
...
#----------------------------------#
FUNCTION pol0338_retorno_terceiro()
#----------------------------------#
   ...
   IF p_agrupa_item THEN 
	   ...
	   SELECT ies_tip_info,
	    ...
	    GROUP BY ies_tip_info, 
	             cod_item,
	             den_item,
	             cod_cla_fisc, 						# foi comentado essa linha a pedido do cliente 
	             cod_origem, 							# que queria que agrupasse o item se ele tivesse
	             cod_tributacao,					#	o perço unitario o cod do item igual
	             cod_unid_med,
	             pre_unit,
	             pct_icm,
	             pct_ipi 
	...
	#      LET p_wnotalev.cod_item_cli = g_cod_item_cliente
	      ...
	      FOREACH cq_den_item2 INTO p_den_item
	         IF p_den_item2 IS NULL THEN 
	            LET p_den_item2 = p_den_item CLIPPED
	         ELSE
	            LET p_den_item2 = p_den_item2 CLIPPED, ' / ', p_den_item CLIPPED
	         END IF
	      ...
	      IF p_den_item2 IS NOT NULL THEN
	         LET p_num_seq = p_num_seq + 1
	         LET p_wnotalev.num_seq = p_num_seq
...
#---- FIM DO AGRUPAMENTO ----#      	       
   ...
   CALL pol0338_calcula_total_de_paginas()
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      OUTPUT TO REPORT pol0338_relat(p_wnotalev.num_nff)
...
#---------------------------------------#
FUNCTION pol0338_busca_dados_duplicatas()
#---------------------------------------#
...
#-------------------------------------#
FUNCTION pol0338_carrega_end_cobranca()
#-------------------------------------#
...
#-----------------------------------#
FUNCTION pol0338_carrega_corpo_nff()
#-----------------------------------#
   ...
   FOREACH cq_wfat_item_rt INTO p_fat_nf_item.*
      ...
      CALL pol0338_verifica_texto_ped_it()
      IF p_texto_item_ped IS NOT NULL THEN
         LET pa_corpo_nff[p_ind].den_item2 = p_texto_item_ped
      END IF
      
       IF p_nff.num_pedido_cli IS NOT NULL OR
            p_nff.num_pedido_cli <> " " OR
            LENGTH(p_nff.num_pedido_cli) <> 0 THEN  
            IF pa_corpo_nff[p_ind].den_item2 IS NULL THEN
               LET pa_corpo_nff[p_ind].den_item2 = "OC:",p_nff.num_pedido_cli CLIPPED
            ELSE
               LET pa_corpo_nff[p_ind].den_item2 = pa_corpo_nff[p_ind].den_item2 CLIPPED,
                   " OC:", p_nff.num_pedido_cli CLIPPED
            END IF
         END IF
      ...
      END IF
 {   
 #  osvaldo especufico da itaesbra para esse cliente
         IF p_nff.cod_cliente = "1600" OR 
            p_nff.cod_cliente = "1601" THEN							precisa verificar
            SELECT prz_entrega
               INTO p_ped_itens.prz_entrega
            FROM ped_itens       
            WHERE cod_empresa = p_cod_empresa
              AND num_pedido = p_fat_nf_item.pedido
              AND num_sequencia = p_wfat_item.seq_item_nf
            LET pa_corpo_nff[p_ind].den_item2 = 
                pa_corpo_nff[p_ind].den_item2 CLIPPED, " Dt. Entr. ", 
...
#--------------------------------------#
FUNCTION pol0338_verifica_ctr_unid_med()
#--------------------------------------#
...
{
#-----------------------------------------#
FUNCTION pol0338_carrega_historico_fiscal()
#-----------------------------------------#  
   ...
   FOREACH cq_whist INTO p_fat_texto, p_sequencia_texto
      ...
      CALL pol0338_insere_texto(p_fat_texto,3)
...
#------------------------------------#
FUNCTION pol0338_le_icms(p_cod_nenef)
#------------------------------------#
...
#-----------------------------#  
   ...
   IF p_valor_pis > 0 THEN
      LET p_pct_pis = pol0338_le_pct('PIS_REC')
      LET p_des_texto = "PIS: ", p_pct_pis, " % = R$ ", p_valor_pis USING "##,###,##&.&&"
   END IF

   IF p_valor_cofins > 0 THEN
      LET p_pct_cofins = pol0338_le_pct('COFINS_REC')
   ...
   LET p_pct_icms = pol0338_le_pct('ICMS')
...
#---------------------------------#
FUNCTION pol0338_le_pct(p_tributo)
#---------------------------------#
...
#-----------------------------------#
FUNCTION pol0338_grava_corpo_nota()
#-----------------------------------#
   ...
   FOR i = 1 TO 999
      ...
      CALL pol0338_insere_item(pa_corpo_nff[i].den_item1, 1)
      CALL pol0338_insere_item(pa_corpo_nff[i].den_item2, 2)
...
#--------------------------------------------------#
FUNCTION pol0338_insere_item(p_den_item,p_tip_info)
#--------------------------------------------------#
...
#---------------------------------------------------#
FUNCTION pol0338_insere_texto(p_den_texto,p_tip_info)
#---------------------------------------------------#
...
#---------------------------------#
FUNCTION pol0338_obtem_peso_item()
#---------------------------------#
...
#-----------------------------------------#
FUNCTION pol0338_calcula_total_de_paginas()
#-----------------------------------------#
...
#------------------------------------------#
FUNCTION pol0338_busca_dados_subst_trib_uf()
#------------------------------------------#
...
#----------------------------------#
FUNCTION pol0338_busca_cof_compl()
#----------------------------------#
...
#-------------------------------------#
FUNCTION pol0338_busca_dados_empresa()            
#-------------------------------------#
...
#-------------------------------#
FUNCTION pol0338_representante()
#-------------------------------#
...
#--------------------------------------------------#
FUNCTION pol0338_busca_dados_clientes()
#--------------------------------------------------#
   ...
   IF STATUS <> 0 THEN
      INITIALIZE p_num_duns TO NULL
   END IF

END FUNCTION

#--------------------------------#
FUNCTION pol0338_busca_nome_pais()                   
#--------------------------------#
   INITIALIZE p_paises.* ,
              p_uni_feder.*    TO NULL 
...
#----------------------------------------------------#
FUNCTION pol0338_busca_dados_transport(p_cod_transpor)
#----------------------------------------------------#
...
#------------------------------------------------#
FUNCTION pol0338_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#-----------------------------------#
FUNCTION pol0338_busca_dados_pedido()
#-----------------------------------#  
...
#-----------------------------------------------#
FUNCTION pol0338_grava_dados_consig()
#-----------------------------------------------#
...
#----------------------------------------#
FUNCTION pol0338_grava_dados_end_entrega()
#----------------------------------------#
...
{  
#----------------------------------------#
FUNCTION pol0338_checa_nf_contra_ordem()
#----------------------------------------#
   ...
   IF STATUS = 0 THEN
      ...
      CALL pol0338_insere_texto(p_des_texto,4)
   ELSE 
      IF STATUS = 100 then
...
#---------------------------------------#
FUNCTION pol0338_verifica_texto_ped_it()
#---------------------------------------#
...
#-----------------------------#
REPORT pol0338_relat(p_num_nff)
#-----------------------------#
   ...
   FORMAT
      ...
      SKIP 2 LINES
      PRINT COLUMN 84, "X"
      PRINT COLUMN 105, p_nff.num_nff USING "&&&&&&"
      SKIP 3 LINES
			
			IF p_nff.cod_fiscal1 IS NOT NULL THEN
	      PRINT COLUMN 032, p_nff.den_nat_oper[1,24],
               COLUMN 057, p_nff.cod_fiscal         USING "&&&&",
               COLUMN 061, "/",
               COLUMN 062, p_nff.cod_fiscal1        USING "&&&&",
               COLUMN 067, p_nff.ins_estadual_trib  
      ELSE
         PRINT COLUMN 032, p_nff.den_nat_oper[1,24],
               COLUMN 057, p_nff.cod_fiscal         USING "&&&&",
               COLUMN 067, p_nff.ins_estadual_trib
     ...
     # SKIP 2 LINES
     PRINT COLUMN 066, p_num_duns,
            COLUMN 104, p_nff.cod_cliente
      IF p_pedidos.num_pedido_repres IS NOT NULL THEN
         PRINT COLUMN 005, p_comprime, p_textos.texto1[1,45], p_descomprime,
               COLUMN 053, p_nff.nom_destinatario[1,28], " ",
                           "PLANTA: ",p_pedidos.num_pedido_repres CLIPPED,"/",
                           p_tipo_venda.den_tip_venda CLIPPED;
      ELSE
         PRINT COLUMN 005, p_comprime, p_textos.texto1[1,45], p_descomprime,
               COLUMN 053, p_nff.nom_destinatario;
      END IF
            
     PRINT COLUMN 104, p_nff.num_cgc_cpf,
            COLUMN 125, p_nff.dat_emissao USING "DD/MM/YYYY"
      PRINT COLUMN 010, p_comprime, p_textos.texto2[1,40], p_descomprime
      PRINT COLUMN 007, p_comprime, p_textos.texto3[1,43], p_descomprime,
            COLUMN 034, p_nff.end_destinatario,
            COLUMN 097, p_nff.den_bairro[1,13],
            COLUMN 111, p_nff.cod_cep 
            #COLUMN 145, TODAY USING "DD/MM/YYYY"
      PRINT COLUMN 015, p_comprime, p_textos.texto4[1,35], p_descomprime
      PRINT COLUMN 010, p_comprime, p_textos.texto5[1,35], p_descomprime,
            COLUMN 049, p_nff.den_cidade[1,24],
            COLUMN 084, p_nff.num_telefone[1,12],
            COLUMN 098, p_nff.cod_uni_feder,
            COLUMN 102, p_nff.ins_estadual
      #     COLUMN 104, TIME
      PRINT 
      PRINT 

     PRINT COLUMN 032, p_nff.end_cob_cli CLIPPED, " ", 
                        p_nff.den_cidade_cob CLIPPED, " ", 
                        p_nff.cod_uni_feder_cobr
      PRINT 
      PRINT COLUMN 042, p_nff.num_dig_duplic1    USING "&&&&&&",
            COLUMN 052, p_nff.val_duplic1    		 USING "####,###,##&.&&",
            COLUMN 068, p_nff.dat_vencto_sd1		 USING "DD/MM/YY",
            COLUMN 078, p_nff.num_dig_duplic2    USING "&&&&&&",
            COLUMN 090, p_nff.val_duplic2		     USING "####,###,##&.&&",
            COLUMN 107, p_nff.dat_vencto_sd2		 USING "DD/MM/YY"
   ...
   ON EVERY ROW
    ...
    CASE 
      WHEN p_wnotalev.ies_tip_info = 1             
            PRINT COLUMN 002, p_wnotalev.cod_item, 
                  COLUMN 030, p_wnotalev.den_item[1,45],
                  COLUMN 089, p_wnotalev.cod_cla_fisc,
                  COLUMN 102, p_wnotalev.cod_origem USING "&",    
                  COLUMN 103, p_wnotalev.cod_tributacao USING "&&",
                  COLUMN 109, p_wnotalev.cod_unid_med,
                  COLUMN 112, p_wnotalev.qtd_item USING "####,##&.&&&",
                  COLUMN 125, p_wnotalev.pre_unit USING "###,###,##&.&&&&&&";
         IF p_nff.cod_uni_feder = "AM" THEN
            PRINT COLUMN 148, p_wnotalev.val_liq_item USING "##,###,###,##&.&&",
                  COLUMN 166, p_fat_nf_mestre.pct_icm USING "##&.&&"
         ELSE  
            PRINT COLUMN 148, p_wnotalev.val_liq_item USING "##,###,###,##&.&&",
                  COLUMN 166, p_wnotalev.pct_icm USING "##&.&&",
                  COLUMN 175, p_wnotalev.pct_ipi USING "&&.&",
                  COLUMN 182, p_wnotalev.val_ipi USING "#,###,###,##&.&&" 
         END IF
         ...
         WHEN p_wnotalev.ies_tip_info = 3
            PRINT COLUMN 002, p_wnotalev.des_texto 
            LET p_linhas_print = p_linhas_print + 1
...
#---------------------------------------------------------------------------
      IF p_linhas_print = 23 THEN { nr. de linhas do corpo da nota }
         ...
         END IF
         IF p_num_pagina = p_tot_paginas THEN 
            PRINT COLUMN 022, p_des_folha 
            PRINT p_descomprime
            PRINT COLUMN 001, p_nff.val_tot_base_icm    USING "##,###,##&.&&",
                  COLUMN 015, p_nff.val_tot_icm         USING "###,###,##&.&&",
                  COLUMN 031, p_nff.val_tot_base_ret    USING "###,###,##&.&&",
                  COLUMN 047, p_nff.val_tot_icm_ret     USING "###,###,##&.&&",
                  COLUMN 064, p_nff.val_tot_mercadoria  USING "###,###,##&.&&"
            SKIP 1 LINES
            PRINT COLUMN 001, p_nff.val_frete_cli       USING "##,###,##&.&&", 
                  COLUMN 015, p_nff.val_seguro_cli      USING "###,###,##&.&&",
                  COLUMN 031, p_nff.val_out_despesas    USING "###,###,##&.&&",
                  COLUMN 047, p_nff.val_tot_ipi         USING "###,###,##&.&&",
                  COLUMN 064, p_nff.val_tot_nff         USING "###,###,##&.&&"
            SKIP 2 LINES
            PRINT COLUMN 001, p_nff.nom_transpor,                  
                  COLUMN 047, p_nff.ies_frete           USING "&",
                  COLUMN 049, p_nff.num_placa,
                  COLUMN 058, p_nff.cod_uni_feder_trans,
                  COLUMN 061, p_nff.num_cgc_trans
            SKIP 1 LINES
            PRINT COLUMN 001, p_nff.end_transpor,    
                  COLUMN 040, p_nff.den_cidade_trans[1,17],   
                  COLUMN 058, p_nff.cod_uni_feder_trans,
                  COLUMN 061, p_nff.ins_estadual_trans   
            SKIP 1 LINES
            PRINT COLUMN 005, p_qtd_volumes      		 USING "####&",
                  COLUMN 011, p_des_especie[1,10],
                  COLUMN 040, p_nff.num_nff          USING "&&&&&&",
                  COLUMN 052, p_nff.pes_tot_bruto    USING "#,###,##&.&&&",
                  COLUMN 065, p_nff.pes_tot_liquido  USING "#,###,##&.&&&"
         ...
         END IF
         SKIP 3 LINES    
         PRINT COLUMN 004, p_nff.num_nff USING "&&&&&&" 
         SKIP 5 LINES    
         LET p_linhas_print = 0
      END IF
