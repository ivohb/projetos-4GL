...
# PROGRAMA: pol1199                                                 #
# MODULOS.: pol1199 - LOG0010 - LOG0030 - LOG0040 - LOG0050         #
...
GLOBALS
   DEFINE p_cod_empresa         LIKE empresa.cod_empresa,
          ...
          p_y_saldo    		      DECIMAL(15,3),
		      ...
		      p_pct_refugo          LIKE item_edi_vw_5054.pct_refugo,
		      p_num_ped_vw          LIKE item_edi_vw_5054.num_ped_vw,
		      p_cod_item_vw         LIKE item_edi_vw_5054.cod_item_vw,
		      p_contato             LIKE item_edi_vw_5054.contato,
...
END GLOBALS
   ...
   DEFINE p_prog RECORD LIKE prog_entrega_5054.*
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "pol1199-10.02.03"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol1199.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol1199_controle()
   END IF

END MAIN

#--------------------------#
 FUNCTION pol1199_controle()
#--------------------------#
   ...
   CALL log130_procura_caminho("pol1199") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1199 AT 2,2 WITH FORM p_nom_tela 
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informa Parâmetros para Gerar Programação."
         MESSAGE ""
         IF pol1199_informa_dados() THEN
            IF pol1199_mostra_fornecedor() THEN
   ...
   CLOSE WINDOW w_pol1199

END FUNCTION

#-----------------------#
 FUNCTION pol1199_sobre()
#-----------------------#
...
#-------------------------------#
 FUNCTION pol1199_informa_dados()
#-------------------------------#
...
#----------------------------------#
 FUNCTION pol1199_mostra_fornecedor() 
#----------------------------------#
...
#--------------------------#
   ...
   IF NOT pol1199_cria_temporaria() THEN 
      RETURN FALSE
   END IF 
   
   IF NOT pol1199_carrega_pedidos() THEN 
      RETURN FALSE
   END IF
 
   IF NOT pol1199_separa_componentes() THEN 
      RETURN FALSE
   END IF 
...
#---------------------------------#
 FUNCTION pol1199_cria_temporaria()
#---------------------------------#
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql("CREATE", "w_edi_volksvagen")
	    RETURN FALSE
   END IF

   DROP TABLE prog_entrega_5054
   CREATE TABLE prog_entrega_5054 (
       pedido                 DECIMAL(6,0),
       cod_fornecedor         CHAR(15),
       cod_item               CHAR(15),
	     num_sequencia          DECIMAL(5,0),
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql("CREATE","prog_entrega_5054")
      RETURN FALSE
   END IF
...
#---------------------------------# 
 FUNCTION pol1199_carrega_pedidos()
#---------------------------------# 
   ...
   DEFINE sql_stmt                CHAR(2000),
          ...
          l_versao                INTEGER,
		      ...
		      l_prz_entrega           DATE,
		      l_cod_item              CHAR(15),
		      l_cod_item_vw           CHAR(30),
		      l_num_ped_vw            CHAR(12)
   ...
   FOREACH cq_ocs INTO 
      ...
      l_cod_fornecedor, l_cod_item_vw, l_num_ped_vw,
      l_num_sequencia, l_prz_entrega, l_saldo
      ...
      IF l_saldo > 0 THEN
         ...
         IF STATUS <> 0 THEN
            CALL log003_err_sql("INSERT","prog_entrega_5054")
           RETURN FALSE
         END IF
      END IF
...
#-------------------------------------#
 FUNCTION pol1199_separa_componentes()
#-------------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         ...
         LET p_caminho = p_caminho CLIPPED, "pol1199.tmp"
         START REPORT pol1199_detalha TO p_caminho
      ELSE
         START REPORT pol1199_detalha TO p_nom_arquivo
      END IF
   END IF
   
   DECLARE cq_compon CURSOR FOR
    SELECT *
    FROM prog_entrega_5054
   ...
   FOREACH cq_compon  INTO p_prog.*
      ...
      MESSAGE "Pedido:", p_prog.pedido
	    ...
	    IF NOT pol1199_busca_dados_item(p_prog.cod_item) THEN
       	 RETURN FALSE
      END IF
      ...
      INSERT INTO w_edi_volksvagen 
       ...
       OUTPUT TO REPORT pol1199_detalha() 
	   
   END FOREACH

    FINISH REPORT pol1199_detalha
...
#---------------------------------------------#
 FUNCTION pol1199_busca_dados_item(l_cod_item)
#---------------------------------------------#
   ...
   SELECT pct_refugo, 
    ...
    IF STATUS <> 0 THEN
	   CALL log003_err_sql('Lendo','item_edi_vw_5054')
       RETURN FALSE
    ELSE
       RETURN TRUE
    END IF
...
#------------------------# 
	...
	PAGE HEADER
         ...
         PRINT COLUMN 001, "pol1199",
               COLUMN 041, "DETALHAMENTO DO ARQUIVO PARA VW",
               COLUMN 101, "DATA: ", TODAY USING "DD/MM/YYYY", " - ", TIME
               
     		 PRINT COLUMN 001, "------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
         PRINT COLUMN 001, "PEDIDO|NUM OC|   COD. ITEM   |  COD. FORNEC   |  SALDO   | PEDIDO VW |       COD. ITEM VW           |  CONTATO  | PRZ. ENTREGA" 
         PRINT COLUMN 001, "------|------|---------------|----------------|----------|-----------|------------------------------|-----------|--------------|----------------------------------------"

	ON EVERY ROW

         PRINT COLUMN 001, p_num_pc USING '&&&&&&', 
               COLUMN 007,'|',
               COLUMN 008, p_prog.pedido USING '&&&&&&', 
               COLUMN 014,'|',
               COLUMN 015, p_prog.cod_item, 
               COLUMN 030,'|',
               COLUMN 031, p_prog.cod_fornecedor, 
               COLUMN 047,'|',
               COLUMN 048, p_prog.saldo USING '#####&.&&&', 
               COLUMN 058,'|',
             	 COLUMN 059, p_prog.num_ped_vw USING '&&&&&&&&&&&', 
               COLUMN 070,'|',
             	 COLUMN 071, p_prog.cod_item_vw, 
               COLUMN 101,'|',
             	 COLUMN 102, p_contato,
               COLUMN 113,'|',
             	 COLUMN 114, p_prog.prz_entrega,
               COLUMN 128,'|',
               COLUMN 129, p_den_item40
...
#------------------------------#
    ...
    FOREACH cq_fornec  INTO p_cod_fornecedor
	  ...
	  IF NOT pol1199_grava_arquivo() THEN
       		RETURN FALSE
      END IF
...
#--------------------------------#
 FUNCTION pol1199_grava_arquivo()
#--------------------------------#
   ...
   LET p_nom_arquivo = p_nom_arquivo CLIPPED,l_num_cnpj CLIPPED,'_RND00107_',
                       l_cnpj_vw CLIPPED,'_',l_dat_hora CLIPPED,'.txt'
     
   START REPORT pol1199_relat_arq  TO p_nom_arquivo

   IF  NOT pol1199_emite_arquivo_edi() THEN 
       RETURN FALSE 
   END IF  	   

   FINISH REPORT pol1199_relat_arq     
...
#-------------------------------------#
 FUNCTION pol1199_emite_arquivo_edi()
#-------------------------------------#
  ...
  #---PE3v0 M G PE1v0 02 CRONOGRAMA DE ENTREGA         
   ...
   DEFINE l_num_reg            INTEGER,
          ...
          l_cod_fornecedor     LIKE fornecedor.cod_fornecedor, 
          l_cod_item_vw        LIKE item_edi_vw_5054.cod_item_vw, 
          l_num_ped_vw         LIKE item_edi_vw_5054.num_ped_vw,
          l_contato            LIKE item_edi_vw_5054.contato,
          l_dat_atual          CHAR(6),
          l_hor_atual          CHAR(8),
          l_capa               SMALLINT,  
          ...
          l_inteiro_7          INTEGER, 
          l_decimal_7          INTEGER,
  		    l_cod_uni_med        LIKE item_edi_vw_5054.cod_uni_med,
  ...
  DEFINE l_item_edi_vw_5054  RECORD LIKE item_edi_vw_5054.*
 
   LET l_capa  = TRUE
   LET m_count = 0
   LET l_num_reg = 0
   INITIALIZE z_cod_item   TO NULL
   INITIALIZE l_item_edi_vw_5054.* TO NULL
   ...
   FOREACH cq_edi INTO z_cod_item
      ...
      SELECT den_empresa, num_cgc
        INTO lr_arq_edi.nom_tms, l_cnpj_vw
        FROM empresa
       WHERE cod_empresa = p_cod_empresa
      ...
      LET lr_arq_edi.ident_ger_mov    = l_dat_atual, 
                                        ...
                                        l_hor_atual[4,5],
                                        l_hor_atual[7,8]              
      LET lr_arq_edi.ident_tms_comun  = l_cnpj_vw[2,3], 
                                        l_cnpj_vw[5,7],
                                        l_cnpj_vw[9,11],
                                        l_cnpj_vw[13,16],
                                        l_cnpj_vw[18,19]
      ...
      IF l_capa THEN 
         ...
         OUTPUT TO REPORT pol1199_relat_arq(0,lr_arq_edi.*)
         LET l_capa = FALSE
      END IF
   ...
   #--- a seguir, preparação e envio dos dados do item
      ...
      END IF
	    ...
	    SELECT *
	      INTO l_item_edi_vw_5054.*
	      FROM item_edi_vw_5054
       WHERE cod_empresa   = p_cod_empresa
         AND cod_item      = z_cod_item
			 
  	  IF STATUS <> 0 THEN
    	   CALL log003_err_sql("SELECT","item_edi_vw_5054")
	 	     RETURN FALSE
      END IF	
      ...
      LET lr_arq_edi.cod_item_cli     = l_item_edi_vw_5054.cod_item_vw
      LET lr_arq_edi.cod_item_forn    = z_cod_item 
      LET lr_arq_edi.num_ped_comp     = l_item_edi_vw_5054.num_ped_vw 
      LET lr_arq_edi.cod_loc_dest     = '011'  
      LET lr_arq_edi.ident_para_cont  = l_item_edi_vw_5054.contato 
      LET lr_arq_edi.cod_unid_med     = l_item_edi_vw_5054.cod_uni_med
      LET lr_arq_edi.qtd_casas_dec    = '0' 
      LET lr_arq_edi.cod_tip_fornto   = 'P' 
	  
	    LET l_num_reg = l_num_reg + 1
      OUTPUT TO REPORT pol1199_relat_arq(1,lr_arq_edi.*)
   ...
   #--- a seguir, preparação e envio dos dados da última entrega
      ...
      LET lr_arq_edi.dat_lib_prod     = '0000'
	    ...
	    LET l_num_reg = l_num_reg + 1
      OUTPUT TO REPORT pol1199_relat_arq(2,lr_arq_edi.*)
      ...
      FOREACH cq_prz_entrega INTO ma_edi[l_ind].saldo, ma_edi[l_ind].prz_entrega
	    ...
	    WHILE x_ind < l_ind
         ...
         LET lr_arq_edi.espaco_pe3                = '      '    
	          ...
	          IF ma_edi[x_ind].saldo > 0 THEN
               ...
               LET lr_arq_edi.hor_ent_item_1     = '00'    
               LET l_qtd_item_1 = ma_edi[x_ind].saldo * (1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
               ...
               LET lr_arq_edi.hor_ent_item_2     = '00'    
               LET l_qtd_item_2 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
	       ...
	       LET x_ind = x_ind + 1 
               ...
               LET lr_arq_edi.hor_ent_item_3     = '00'    
               LET l_qtd_item_3 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
	       ...
	       LET x_ind = x_ind + 1 
               ...
               LET lr_arq_edi.hor_ent_item_4     = '00'    
               LET l_qtd_item_4 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
	       ...
	       LET x_ind = x_ind + 1 
               ...
               LET lr_arq_edi.hor_ent_item_5     = '00'    
               LET l_qtd_item_5 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
	       ...
	       LET x_ind = x_ind + 1 
               ...
               LET lr_arq_edi.hor_ent_item_6     = '00'    
               LET l_qtd_item_6 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
	       ...
	       LET x_ind = x_ind + 1 
               ...
               LET lr_arq_edi.hor_ent_item_7     = '00'    
               LET l_qtd_item_7 = ma_edi[x_ind].saldo *(1 + (l_item_edi_vw_5054.pct_refugo / 100))
         ...
         END IF
 	       ...
 	       LET l_num_reg = l_num_reg + 1
			   OUTPUT TO REPORT pol1199_relat_arq(3,lr_arq_edi.*)
   ...
   IF l_num_reg > 0   THEN 
      ...
      OUTPUT TO REPORT pol1199_relat_arq(4,lr_arq_edi.*)
   END IF	  
...
#-------------------------------------------#
 REPORT pol1199_relat_arq(l_tipo, lr_arq_edi)
#-------------------------------------------#
...
#-------------------------------#  
 FUNCTION pol1199_imprime_relat()
#-------------------------------#  

   IF NOT pol1199_escolhe_saida() THEN
   		RETURN 
   END IF
   

    START REPORT pol1199_relat TO p_nom_arquivo


    CALL pol1199_emite_relatorio()
 
    FINISH REPORT pol1199_relat
...
#------------------------------#
FUNCTION pol1199_escolhe_saida()
#------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         CALL log150_procura_caminho("LST") RETURNING p_caminho
         LET p_caminho = p_caminho CLIPPED, "pol1199.tmp"
         START REPORT pol1199_relat TO p_caminho
      ELSE
         START REPORT pol1199_relat TO p_nom_arquivo
      END IF
   END IF
...
#---------------------------------#
 FUNCTION pol1199_emite_relatorio()
#---------------------------------#
   ...
   SELECT den_empresa
    ...
    ORDER BY cod_fornecedor, cod_item, prz_entrega 
     ...
     FOREACH cq_relat_2 INTO 
         ...
         OUTPUT TO REPORT pol1199_relat(lr_relat.*)
