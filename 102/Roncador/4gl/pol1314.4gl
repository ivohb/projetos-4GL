...
#----------------------#
    ...
    CALL _ADVPL_set_property(m_dialog,"TITLE","RATEIO POR TIPO DE DESPESA")
...
#----------------------------------------#
    ...
    CALL _ADVPL_set_property(l_label,"TEXT","Cód fornecedor:")    
...
#------------------------------#
   ...
   IF mr_cabec.cod_fornecedor IS NULL THEN
      LET m_msg = 'Já existe grade para a empresa e tipo de despesa informados.'
      SELECT 1
   ...
   ELSE
      LET m_msg = 'Já existe grade para a empresa/tip despesa/fornecedor informados.'
      SELECT 1
        FROM rateio_tip_desp_orig912
       WHERE empresa_orig = mr_cabec.cod_emp_orig
         AND cod_tip_desp = mr_cabec.cod_tip_desp
   ...
   IF m_count > 0 THEN
      LET m_msg = 'O tipo de despesa informado já está cadstrado para outra empresa.'
      RETURN TRUE
   END IF}
...
#------------------------#
   ...
   IF m_qtd_aprov > 0 THEN
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",
             "Grade com aprovações não podem ser modificada.")
      RETURN FALSE
   END IF
...
#------------------------#
   ...
   IF m_qtd_aprov > 0 THEN
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",
             "Grade com aprovações não podem ser excluída.")
      RETURN FALSE
   END IF
...
#-------------------------#
   ...
   IF NOT LOG_question("Confirma a aprovação do rateio?") THEN
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",
             "Operação cancelada.")
      RETURN FALSE
   END IF
   ...
   CALL log0030_mensagem('Aprovação efetuada \n com sucesso.','info')
...
#----------------------------------#
   ...
   IF STATUS = 100 THEN
      LET l_erro = 'Você não está autorizado a apovar grade da empresa ',mr_cabec.cod_emp_orig
   ELSE
      IF STATUS <> 0 THEN
   ...
   IF STATUS = 0 THEN
      LET l_erro = 'Você já aprovou essa grade na data ', l_dat
   ELSE
      IF STATUS <> 100 THEN
         CALL log003_err_sql('SELECT','aprovador_912')
   ...
   FOREACH cq_aprov INTO l_user
     ...
     SELECT 1
      ...
      ELSE
         IF STATUS <> 0 THEN
            CALL log003_err_sql('SELECT','rateio_aprovado_912')
            LET m_msg = 'Erro ',STATUS,' lendo aprovações da tabela rateio_aprovado_912'
            RETURN FALSE
         END IF
      END IF 
...
#-----------------------------#
   ...
   ELSE
      LET m_msg = 'Para criar nova versão, \n selecione uma grade ativa.'
      CALL log0030_mensagem(m_msg,'info')
      RETURN FALSE
   END IF
   ...
   IF m_count > 0 THEN
      LET m_msg = 'Já esxiste uma nova versão em manutenção.\n',
                  'Sua replicação não é permitida.'
      CALL log0030_mensagem(m_msg,'info')
      RETURN FALSE
   END IF
        
   IF NOT LOG_question("Confirma a geração da nova versão?") THEN
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",
             "Operação cancelada.")
      RETURN FALSE
   END IF
   ...
   CALL log0030_mensagem('Nova versão criada \n com sucesso.','info')
