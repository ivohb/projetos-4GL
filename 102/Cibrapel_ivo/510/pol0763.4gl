#-------------------------------------------------------------------#
# PROGRAMA: pol0763                                                 #
# OBJETIVO: RELATORIO DE COMISSOES GERENTES - OFICIAL               #
#-------------------------------------------------------------------#
DATABASE logix
...
GLOBALS
   ...
   DEFINE p_cod_empresa       LIKE empresa.cod_empresa,
          ...
          p_val_bruto         DECIMAL(15,2), 
          p_val_base_ir       DECIMAL(15,2),
          p_val_garantia      DECIMAL(15,2), 
          p_val_liq           DECIMAL(15,2), 
          p_val_ir            DECIMAL(15,2),
          p_dat_exp_gar       DATE,
          p_versao            CHAR(18),
          p_rec_erro,
          p_ped_erro,
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "POL0763-10.02.00"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0763.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol0763_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0763_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0763") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0763 AT 3,03 WITH FORM p_nom_tela 
   ...
   MENU "OPCAO"
      ...
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0763","IN") THEN
         IF pol0763_entrada_dados() THEN
            LET p_ies_cons = TRUE
      ...
      LET p_achou = FALSE
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0763","MO") THEN
         IF p_ies_cons = TRUE THEN 
           IF pol0763_calcula_valores()  THEN 
            IF log028_saida_relat(16,40) IS NOT NULL THEN
               MESSAGE " Processando a extracao do relatorio ... " 
                  ATTRIBUTE(REVERSE)
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT pol0763_relat TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'pol0763.tmp'
                     START REPORT pol0763_relat  TO p_caminho
                  END IF
               ELSE
                  START REPORT pol0763_relat TO p_nom_arquivo
               END IF
               CALL pol0763_emite_relatorio()   
               IF p_count = 0 THEN
      ...
      EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0763

END FUNCTION

#----------------------------------------#
 FUNCTION pol0763_entrada_dados()
#----------------------------------------#
...
#---------------------------------#
 FUNCTION pol0763_calcula_valores()
#---------------------------------#
DEFINE l_so_gar   CHAR(01)
...
WHENEVER ERROR STOP
 ...
 SELECT count(*)
   ...
   FROM lanc_com_885 a,
        ger_com_885 b                
  WHERE a.cod_repres  = b.cod_gerente
...
IF p_ies_calc_pag = 'S' THEN
   DECLARE cq_ger CURSOR FOR
   SELECT cod_gerente,val_gar_ofi,dat_exp_gar 
     FROM ger_com_885
   ORDER BY cod_gerente 
   FOREACH cq_ger INTO p_cod_gerente,p_val_garantia,p_dat_exp_gar 

     DELETE FROM lanc_com_885                
      WHERE cod_empresa  = p_cod_empresa
        AND dat_pagto    = p_tela.dat_pagto
        AND cod_repres   = p_cod_gerente
        AND ies_origem   IN ("P","I")

     LET l_so_gar = 'S'

     IF p_dat_exp_gar < TODAY THEN
        LET p_val_garantia = 0
     END IF 
     ...
     LET t_tot_rep.cod_repres  = p_cod_gerente
    ...
    IF p_val_garantia > t_tot_rep.val_bruto THEN 
       LET t_tot_rep.val_bruto = p_val_garantia
       IF l_so_gar = 'S' THEN 
          LET p_lanc_com_885.cod_empresa  = p_cod_empresa
          LET p_lanc_com_885.num_docum    = '0'
          LET p_lanc_com_885.num_nff      = 0
          LET p_lanc_com_885.cod_repres   = p_cod_gerente
...
#---------------------------------#
 FUNCTION pol0763_emite_relatorio()
#---------------------------------#
DEFINE l_cod_rep   LIKE representante.cod_repres
   ...
   FOREACH  cq_reprel INTO l_cod_rep
      ...
      FOREACH cq_relat INTO p_lanc_com_885.*
         ...
         OUTPUT TO REPORT pol0763_relat(p_relat.*,p_nom_repres,p_num_cgc_cpf) 
...
#-------------------------------------------------------#
 REPORT pol0763_relat(p_relat,p_nom_repres,p_num_cgc_cpf)
#-------------------------------------------------------#
   ...
   FORMAT
      ...
      PAGE HEADER  
         ...
         PRINT COLUMN 001, "----------------------------------------",
                           ...
                           "------------"
         SKIP 1 LINE
         PRINT COLUMN 001, "GERENTE : ", p_relat.cod_repres, " - ",
                           p_nom_repres,                          
