...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: pol1165                                                 #
# OBJETIVO: UNIDADE FUNCIONAL POR USUÁRIO, P/ APROVAÇÃO DE DOCTOS.  #
# AUTOR...: IVO H BARBOSA                                           #
# DATA....: 18/09/12                                                #
#-------------------------------------------------------------------#
...
GLOBALS
   ...
   DEFINE p_nom_usuario        CHAR(08),
          p_nom_usuarioa       CHAR(08),
...
MAIN
   ...
   LET parametro.num_programa = 'POL1165'
...
#----------------------#
 FUNCTION pol1165_menu()
#----------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol1165") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1165 AT 2,2 WITH FORM p_nom_tela
   ...
   MENU "OPCAO"
      COMMAND "Incluir" "Inclui dados na tabela."
         CALL pol1165_inclusao() RETURNING p_status
         IF p_status THEN
      ...
      COMMAND "Seguinte" "Exibe o próximo item encontrado na consulta."
         IF p_ies_cons THEN
            CALL pol1165_paginacao("S")
         ELSE
            ERROR "Não existe nenhuma consulta ativa !!!"
         END IF 
      COMMAND "Anterior" "Exibe o item anterior encontrado na consulta."
         IF p_ies_cons THEN
            CALL pol1165_paginacao("A")
         ELSE
            ERROR "Não existe nenhuma consulta ativa !!!"
         END IF 
      COMMAND "Modificar" "Modifica dados da tabela."
         IF p_ies_cons THEN
            CALL pol1165_modificacao() RETURNING p_status  
            IF p_status THEN
               DISPLAY p_nom_usuario TO cod_usuario
      ...
      COMMAND "Excluir" "Exclui dados da tabela."
         IF p_ies_cons THEN
            CALL pol1165_exclusao() RETURNING p_status
            IF p_status THEN
      ...
      COMMAND "Fim"       "Retorna ao menu anterior."
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol1165

END FUNCTION

#-----------------------#
 FUNCTION pol1165_sobre()
#-----------------------#
...
#--------------------------#
   ...
   IF pol1165_edita_dados() THEN      
      IF pol1165_edita_unid('I') THEN      
         IF pol1165_grava_dados() THEN                                                     
            RETURN TRUE                                                                    
         END IF                                                                      
      END IF
   END IF
...
#-----------------------------#
 FUNCTION pol1165_edita_dados()
#-----------------------------#
   
   LET INT_FLAG = FALSE
   
   INPUT p_nom_usuario WITHOUT DEFAULTS
    FROM nom_usuario
      ...
      IF STATUS = 100 THEN
         LET p_msg = "Usuário não cadastrado !!!"
      ...
      IF STATUS <> 0 THEN
         CALL log003_err_sql('lendo','unid_aprov_265')
         RETURN FALSE
      END IF 
  
      IF p_count > 0 THEN
         LET p_msg = "Já existe unid funcional p/ o usuário - Use modificar"
         CALL log0030_mensagem(p_msg,'exclamation')
         NEXT FIELD nom_usuario
      END IF
...
#-----------------------#
   ...
   CASE
      ...
      WHEN INFIELD(cod_uni_funcio)
         LET p_filtro = " 1=1 and dat_validade_fim > '",p_dat_atu,"' "
         CALL log009_popup(8,10,"Unidade Funcional","uni_funcional",
              "cod_uni_funcio","den_uni_funcio","","S",p_filtro) 
              RETURNING p_codigo
         CALL log006_exibe_teclas("01 02 07", p_versao)
...
#-------------------------------------#
 FUNCTION pol1165_edita_unid(p_funcao)
#-------------------------------------#     
   ...
   ELSE
      ...
      IF p_funcao = 'I' THEN
         ...
         DISPLAY p_cod_empresa TO cod_empresa
      ELSE
        CALL pol1165_carrega_itens() RETURNING p_status
      END IF
      RETURN FALSE
   END IF
...
#-----------------------------#
   ...
   FOR p_ind = 1 TO ARR_COUNT()
       IF pr_unid[p_ind].cod_uni_funcio IS NOT NULL THEN
          INSERT INTO uni_temp_265 
           VALUES(pr_unid[p_ind].cod_uni_funcio)
   ...
   DELETE FROM unid_aprov_265
    WHERE nom_usuario = p_nom_usuario
      AND cod_empresa = p_cod_empresa
      
   IF STATUS <> 0 THEN
      CALL log003_err_sql("Deletando", "unid_aprov_265")
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   END IF 
   ...
   FOR p_ind = 1 TO ARR_COUNT()
       IF pr_unid[p_ind].cod_uni_funcio IS NOT NULL THEN
		       ...
		       INSERT INTO unid_aprov_265
		       VALUES (p_cod_empresa,
		               p_nom_usuario,
		               pr_unid[p_ind].cod_uni_funcio)
		
		       IF STATUS <> 0 THEN 
		          CALL log003_err_sql("Incluindo", "unid_aprov_265")
		          CALL log085_transacao("ROLLBACK")
		          RETURN FALSE
		       END IF
		       LET p_incluiu = TRUE
...
#---------------------------#
   ...
   FOREACH cq_del INTO p_cod_unidade
      ...
      LET parametro.texto = 'EXCLUSAO DA UNIDADE FUNCIONAL ', 
            p_cod_unidade CLIPPED, ' DO USUARIO ', p_nom_usuario 
      IF NOT pol1161_grava_auadit(parametro) THEN
         RETURN FALSE
      END IF
...
#--------------------------#
   ...
   IF INT_FLAG THEN
      IF p_ies_cons THEN 
         ...
         CALL pol1165_exibe_dados() RETURNING p_status
      END IF    
      RETURN FALSE 
   END IF

   LET sql_stmt = "SELECT DISTINCT nom_usuario ",
                  "  FROM unid_aprov_265 ",
...
#------------------------------#
   ...
   IF NOT pol1165_carrega_itens() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
 FUNCTION pol1165_carrega_itens()
#---------------------------------#
   ...
   DECLARE cq_array CURSOR FOR
    SELECT DISTINCT
     ...
     WHERE a.cod_empresa = p_cod_empresa 
       AND a.nom_usuario = p_nom_usuario
...
#-----------------------------------#
 FUNCTION pol1165_paginacao(p_funcao)
#-----------------------------------#
   ...
   WHILE TRUE
      ...
      IF STATUS = 0 THEN
         ...
         IF p_nom_usuario = p_nom_usuarioa THEN
            CONTINUE WHILE
         END IF
...
#----------------------------------#
 FUNCTION pol1165_prende_registro()
#----------------------------------#
...
#-----------------------------#
   ...
   IF pol1165_prende_registro() THEN
      IF pol1165_edita_unid('M') THEN
         IF pol1165_grava_dados() THEN
            LET p_retorno = TRUE
         END IF
      END IF
...
#--------------------------#
   ...
   IF pol1165_prende_registro() THEN
     ...
     FOR p_ind = 1 TO ARR_COUNT()
       IF pr_unid[p_ind].cod_uni_funcio IS NOT NULL THEN
          LET parametro.texto = 'EXCLUSAO DA UNIDADE FUNCIONAL ', 
              pr_unid[p_ind].cod_uni_funcio CLIPPED, ' DO USUARIO ', p_nom_usuario 
          IF NOT pol1161_grava_auadit(parametro) THEN
             RETURN FALSE
          END IF
       END IF
     END FOR
      ...
      IF STATUS = 0 THEN               
         ...
         LET p_retorno = TRUE 
      ELSE
         CALL log003_err_sql("Excluindo","unid_aprov_265")
      END IF
      CLOSE cq_prende
   END IF
...
{
#--------------------------#
 FUNCTION pol1165_listagem()
#--------------------------#     

   IF NOT pol1165_escolhe_saida() THEN
   		RETURN 
   END IF
      
   IF NOT pol1165_le_den_empresa() THEN
      RETURN
   END IF   
   ...
   SELECT cod_usuario,
          ...
          estado
     FROM unid_aprov_265
 ORDER BY cod_usuario, cod_uni_funcio                          
   ...
   OUTPUT TO REPORT pol1165_relat(p_nom_usuario) 

      LET p_count = 1
      
   END FOREACH

   FINISH REPORT pol1165_relat   
...
#-------------------------------#
 FUNCTION pol1165_escolhe_saida()
#-------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         ...
         LET p_caminho = p_caminho CLIPPED, "pol1165.tmp"
         START REPORT pol1165_relat TO p_caminho
      ELSE
         START REPORT pol1165_relat TO p_nom_arquivo
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol1165_le_den_empresa()
#--------------------------------#
...
#--------------------------------#
 REPORT pol1165_relat(p_nom_usuario)
#--------------------------------#
    
   DEFINE p_nom_usuario LIKE bancos.cod_usuario
   ...
   FORMAT
      ...
      BEFORE GROUP OF p_nom_usuario
         ...
         PRINT
         PRINT COLUMN 003, "Banco: ", p_nom_usuario, " - ", p_nom_funcionario
         PRINT
