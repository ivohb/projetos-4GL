...
#---parâmetros recebidos com visibilidade local
  ...
  END RECORD
   ...
   IF p_ies_ctr_lote = 'N' AND p_item.num_lote IS NOT NULL THEN
      LET p_msg = 'IT:,',p_item.cod_item CLIPPED,
                  ' USO DE LOTE EM ITEM QUE NAO CONTROLA LOTE'
      RETURN FALSE
   END IF
   ...
   IF p_ies_ctr_lote = 'S' THEN
      IF p_item.num_lote IS NULL  OR p_item.num_lote = ' ' THEN
         LET p_msg = 'IT:,',p_item.cod_item CLIPPED,
                     ' LOTE NULO PARA ITEM QUE CONTROLA LOTE'
         RETURN FALSE
      END IF
   END IF
