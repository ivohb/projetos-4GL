#-------------------------------------------------------------------#
# PROGRAMA: POL0755                                                 #
# OBJETIVO: RELATORIO DE COMISSOES FUNCIONARIOS - OFICIAL           #
#-------------------------------------------------------------------#
DATABASE logix
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "POL0755-10.02.00"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0755.iem") RETURNING p_nom_help
   ...
   IF p_status = 0  THEN
      CALL pol0755_controle()
   END IF
END MAIN

#--------------------------#
 FUNCTION pol0755_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0755") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0755 AT 3,03 WITH FORM p_nom_tela 
   ...
   MENU "OPCAO"
      ...
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0755","IN") THEN
         IF pol0755_entrada_dados() THEN
            LET p_ies_cons = TRUE
      ...
      LET p_achou = FALSE
      MESSAGE ""
      IF log005_seguranca(p_user,"VDP","pol0755","MO") THEN
         IF p_ies_cons = TRUE THEN 
           IF pol0755_calcula_valores()  THEN 
            IF log028_saida_relat(16,40) IS NOT NULL THEN
               MESSAGE " Processando a extracao do relatorio ... " 
                  ATTRIBUTE(REVERSE)
               IF p_ies_impressao = "S" THEN
                  IF g_ies_ambiente = "U" THEN
                     START REPORT pol0755_relat TO PIPE p_nom_arquivo
                  ELSE
                     CALL log150_procura_caminho ('LST') RETURNING p_caminho
                     LET p_caminho = p_caminho CLIPPED, 'pol0755.tmp'
                     START REPORT pol0755_relat  TO p_caminho
                  END IF
               ELSE
                  START REPORT pol0755_relat TO p_nom_arquivo
               END IF
               CALL pol0755_emite_relatorio()   
               IF p_count = 0 THEN
      ...
      EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0755

END FUNCTION

#----------------------------------------#
 FUNCTION pol0755_entrada_dados()
#----------------------------------------#
...
#---------------------------------#
 FUNCTION pol0755_calcula_valores()
#---------------------------------#
...
#---------------------------------#
 FUNCTION pol0755_emite_relatorio()
#---------------------------------#
DEFINE l_cod_rep   LIKE representante.cod_repres
   ...
   FOREACH  cq_reprel INTO l_cod_rep, p_cod_gerente         
      ...
      FOREACH cq_relat INTO p_lanc_com_885.*
         ...
         OUTPUT TO REPORT pol0755_relat(p_relat.*,p_nom_repres,p_num_cgc_cpf,p_nom_gerente) 
...
#----------------------------------------------------------------------#
 REPORT pol0755_relat(p_relat,p_nom_repres,p_num_cgc_cpf,p_nom_gerente)
#----------------------------------------------------------------------#
   ...
   FORMAT
      ...
      AFTER GROUP OF p_relat.ies_origem
         ...
         PRINT COLUMN 080, group sum(p_relat.val_base_com) USING "##,###,##&.&&",
