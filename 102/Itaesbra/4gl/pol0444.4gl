...
# SISTEMA.: SUPRIMENTOS                                                 #
# PROGRAMA: pol0444                                                     #
# MODULOS.: pol0444-LOG0010-LOG0030-LOG0040-LOG0050-LOG0060             #
#           LOG0090-LOG0280-LOG1200-LOG1300-LOG1400-LOG1500             #
# OBJETIVO: INSPEÇÃO DE ENTRADA                                         #
...
MAIN
   ...
   DEFER INTERRUPT
   LET p_versao = "pol0444-10.02.18"
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0444.iem") RETURNING p_nom_help
   LET p_nom_help = p_nom_help CLIPPED

   {CALL log001_acessa_usuario("ESPEC999","")
      RETURNING p_status, p_cod_empresa, p_user
   IF p_status = 0  THEN
      CALL pol0444_controle()
   END IF}
...
#--------------------------#
 FUNCTION pol0444_controle()
#--------------------------#
   
   CALL log085_transacao("BEGIN")
   WHENEVER ERROR CONTINUE

   IF pol0444_processa() THEN
...
#--------------------------#
   ...
   FOREACH cq_zz2 INTO 
      ...
      IF SQLCA.SQLCODE = NOTFOUND THEN 
         ...
         IF NOT pol0444_atualiza_zz2() THEN
            RETURN FALSE
         ELSE
            CONTINUE FOREACH
         END IF
 ...
 #  Alterado pelo Manuel em 29-06-2012 pois não estava usando o local de inspeção corrretamente 
 ...
 #        END IF     
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         CALL log003_err_sql("ATUALIZAÇÃO","ESTOQUE")
         RETURN FALSE
      END IF
      ...
      IF p_zz2.zz2_tpreg <> 'OP' THEN      
         ...
         IF SQLCA.SQLCODE <> 0 THEN                                                      
            CALL log003_err_sql("ATUALIZAÇÃO","AVISO_REC")                               
            RETURN FALSE                                                                 
         END IF                                                                          
      END IF
      
      IF NOT pol0444_mov_estoque() THEN
         RETURN FALSE
      END IF
...
#-----------------------------#
   ...
   IF p_estoque_lote.qtd_saldo >= 1 THEN
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         CALL log003_err_sql("AUTUALIZAÇÃO","ESTOQUE_LOTE")
         RETURN FALSE
      END IF
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         CALL log003_err_sql("AUTUALIZAÇÃO","ESTOQUE_LOTE_ENDER")
         RETURN FALSE
      END IF
   ELSE
   ...
   IF p_zz2.zz2_qtdlib > 0 THEN
      ...
      IF SQLCA.SQLCODE = NOTFOUND THEN 
         ...
         IF NOT pol0444_insere_lote() THEN
            RETURN FALSE
         END IF
      ELSE
         IF NOT pol0444_atualiza_lote() THEN
            RETURN FALSE
         END IF
      END IF
   END IF
...
#-----------------------------#
   ...
   IF SQLCA.SQLCODE <> 0 THEN 
      CALL log003_err_sql("INCLUSÃO","ESTOQUE_LOTE")
      RETURN FALSE
   END IF
   ...
   IF SQLCA.SQLCODE <> 0 THEN 
      CALL log003_err_sql("INCLUSÃO","ESTOQUE_LOTE_ENDER")
      RETURN FALSE
   END IF
      
   LET p_endereco =   p_estoque_lote_ender.endereco 
      
   IF NOT pol0444_insere_trans() THEN
      RETURN FALSE
   END IF
...
#-------------------------------#
FUNCTION pol0444_atualiza_lote()
#-------------------------------#
...
#    LET  p_count = p_count + 1
    ...
    LET  p_estoque_trans.num_prog           = "POL0444"
    ...
    INSERT INTO estoque_trans VALUES (p_estoque_trans.*)
     ...
     LET p_estoque_trans_end.num_prog = "POL0444"
     ...
     IF SQLCA.SQLCODE <> 0 THEN 
        CALL log003_err_sql("INSERÇÃO","est_trans_area_lin")
        RETURN FALSE
     END IF
    ...
    LET p_estoque_auditoria.num_programa = 'POL0444'
      ...
      SELECT num_transac                                              
       ...
       WHERE cod_empresa   = p_cod_empresa                            
        ...
        IF SQLCA.SQLCODE = 0 THEN                                     
           ...
           IF SQLCA.SQLCODE <> 0 THEN                                 
              CALL log003_err_sql("INSERÇÃO","sup_mov_orig_dest")     
              RETURN FALSE                                            
           END IF                                                     
        END IF                                                        
...
#-----------------------------#
  ...
  # EXECUTE VAR_QUERY
   ...
   IF SQLCA.SQLCODE <> 0 THEN 
      CALL log003_err_sql("ATUALIZAÇÃO","ZZ2")
      RETURN FALSE
   END IF
...
29/08/2012 - Considerar o lote no local do AR, caso não esteja no local de inspeção ou estoque
04/09/2012 - Considerar na pesquisa do lote os locais DEVOLUCAO e  RETRABALHO, caso não
             encontre nos locais tradicionais
17/01/2013 - ler as tabelas protheus, a partir de uma instância parametrizada na tebla min_par_modulo
