...
# PROGRAMA: pol0971                                                            #
# OBJETIVO: IMPORTAÇÃO DO EGA x LOGIX                                          #
# DATA....: 18/09/2009                                                         #
#                         														                         #
#------------------------------------------------------------------------------#
  DATABASE logix
...
#---------------------------#
   ...
   #---acerta op_lote para OP's não mais liberadas e/ou sem saldo---#   
   ...
   IF STATUS <> 0 THEN 
      CALL log003_err_sql("DELEÇÃO","rovapont_erro_man912")
      #CALL log085_transacao("ROLLBACK")
      ROLLBACK WORK
      RETURN 
   END IF
...
#---------------------------------#
   ...
   IF STATUS <> 0 THEN 
      CALL log003_err_sql("DELEÇÃO","rovman_apont_454:D")
      RETURN FALSE
   END IF
   ...
   FOREACH cq_hist INTO p_aponta.*,p_rowid
      ...
      IF STATUS <> 0 THEN 
         CALL log003_err_sql("INCLUSÃO","rovapont_hist_man912")
         RETURN FALSE
      END IF
...
#--------------------------------#
   ...
   DECLARE cq_importa CURSOR WITH HOLD FOR 					#Expecifico da rovemar, o cod da operação no ega eo numero da 
   																									#operação no logix ,mas o numero da sequencia no logix
   ...
   FOREACH cq_importa INTO p_aponta.*
     ...
     END IF
      ...
      IF sqlca.sqlcode <> 0 THEN
         ...
         LET m_msg = 'OPERACAO NÃO CADASTRADA NA TAB rovoper_ega_man912 - pol0971'
         CALL pol0971_insere_erro()
         CONTINUE FOREACH
      END IF
      ...
      IF p_aponta.tip_mov <> 'P' AND 
         ...
         p_aponta.tip_mov <> '*' THEN
          LET p_num_op = p_aponta.num_op
          LET m_msg = 'TIPO DE MOVIMENTO INVÁLIDO - pol0971: ', p_aponta.tip_mov
          CALL pol0971_insere_erro()
          CONTINUE FOREACH
      END IF
...
#--------------------------------#
FUNCTION pol0971_consiste_gemeas()				# verifica a existencia de pças gemeas
#--------------------------------#				# se houver peças gemeas ele apont a peça mais de uma vez
...
#-----------------------------------#					#
   ...
   SELECT *
    ...
    WHERE cod_empresa = p_cod_empresa
      AND cod_operac  = p_cod_operac
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'OPERACAO CORRESPONDENTE NÃO CADASTRADA NA TAB OPERACAO - pol0971'
      RETURN FALSE
   END IF
   ...
   IF STATUS <> 0 THEN
      LET m_msg = 'OPERACAO FINAL NÃO CADASTRADA - ORD_OPER - pol0971'
      RETURN FALSE
   END IF
...
#--------------------------------#
 FUNCTION pol0971_consiste_ordem()				#consiste o numero da op, se naõ estiver cadastrado ou
#--------------------------------#				#a op do ega nao bater com a do logix nao aponta
   ...
   IF sqlca.sqlcode <> 0 THEN
   		LET p_num_ordem = p_aponta.num_op
      LET m_msg = 'ORDEM DE PRODUÇÃO NÃO EXISTE - pol0971'
      CALL pol0971_insere_erro()
      RETURN FALSE
   END IF
   ...
   ELSE
      ...
      IF p_envia_hist = 'N' THEN
      	 LET p_num_ordem = p_aponta.num_op
         LET m_msg = 'ORDEM DE PRODUÇÃO NÃO ESTA LIBERADA - pol0971'
         CALL pol0971_insere_erro()
      END IF
      RETURN FALSE
   END IF
...
#-------------------------------#							#Se nao estiver nao aponta!!
   SELECT cod_local_estoq
    ...
    WHERE cod_empresa = p_cod_empresa
      AND cod_item    = p_aponta.cod_item
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'ITEM: ', p_aponta.cod_item, ' NÃO CADASTRADO - pol0971'
      RETURN FALSE
   END IF
...
#----------------------------------#
 FUNCTION pol0971_consiste_maquina()				#Faz a consistencia da maquina do logix e do ega se não 
#----------------------------------#				#estiver cadastrada não aponta.
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_aponta.cod_maquina,
                  ' NÃO CADASTRADA - rovmaq_ega_man912 - pol0971'
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      LET m_msg = 'MAQUINA:',p_cod_maquina,' NÃO CADASTRADA - RECURSO - pol0971'
      RETURN FALSE
   END IF
   ...
   IF sqlca.sqlcode <> 0 THEN
      ...
      IF STATUS <> 0 THEN
         LET m_msg = 'MATRIC.OPERADOR:', p_aponta.mat_operador,
                     ' NÃO CADASTRADA - FUNCIONARIO - pol0971'
         RETURN FALSE
      ELSE
         ...
         IF STATUS <> 0 THEN
            LET m_msg = 'OPERADOR PADRAO:', p_aponta.mat_operador,
                        ' NÃO CADASTRADO - FUNCIONARIO - pol0971'
            RETURN FALSE
         END IF
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol0971_consiste_movto()						#Função que faz a comparação do movmento ega com o movmento 
...
END RECORD
   ...
   IF STATUS <> 0 THEN
      CALL log003_err_sql("DELEÇÃO","rovman_apont_erro_912")
      RETURN
   END IF
	 ...
	 FOREACH cq_apont INTO 	p_w_apont_prod.cod_empresa,
	    ...
	    END IF 
	 	...
	 	END IF 
		  ...
		  END FOREACH
			IF STATUS <> 0 THEN
	      CALL log003_err_sql("INCLUSÃO","rovapont_erro_man912")
	    ELSE 
      END IF 
