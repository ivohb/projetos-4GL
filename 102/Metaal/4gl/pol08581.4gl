...
# PROGRAMA: pol08581                                                         #
# MODULOS.: pol08581  - LOG0010 - LOG0040 - LOG0050 - LOG0060                #
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol08581_controle()
   END IF
END MAIN

#-------------------------#
FUNCTION pol08581_controle()
#-------------------------#

   CALL log006_exibe_teclas("01", p_versao)
   CALL log130_procura_caminho("pol08581") RETURNING comando    
   OPEN WINDOW w_pol08581 AT 2,2 WITH FORM comando
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   DISPLAY p_cod_empresa TO empresa

   CALL log085_transacao("BEGIN") 

   IF NOT pol08581_cria_tabs_tmp() THEN
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informar Parametros"
         HELP 0001
         IF pol08581_entrada_parametros() THEN
            LET p_ies_cons = TRUE
            NEXT OPTION "Listar"
         END IF
      ...
      COMMAND "Fim" "Retorna ao Menu Anterior"
         ...
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol08581

END FUNCTION

#-----------------------------------#
FUNCTION pol08581_entrada_parametros()
#-----------------------------------#

   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol08581
...
#----------------------------#    
   ...
   IF log028_saida_relat(16,41) IS NOT NULL THEN 
      ...
      IF p_ies_impressao = "S" THEN 
         IF g_ies_ambiente = "U" THEN
            START REPORT pol08581_relat TO PIPE p_nom_arquivo
         ELSE 
            CALL log150_procura_caminho ('LST') RETURNING p_caminho
            LET p_caminho = p_caminho CLIPPED, 'pol08581.tmp' 
            START REPORT pol08581_relat TO p_caminho 
         END IF 
      ELSE
         START REPORT pol08581_relat TO p_nom_arquivo
      END IF
   ELSE
      RETURN TRUE
   END IF
   ...
   CALL pol08581_busca_dados_empresa()
   ...
   FOREACH cq_fat_nf_mestre INTO p_fat_nf_mestre.*
      ...
      CALL log085_transacao("BEGIN") 
      IF NOT pol08581_deleta_tabs_tmp() THEN
      ...
      FOREACH cq_codf INTO p_cod_fiscal
         ...
         IF p_nff.cod_fiscal IS NULL THEN
            ...
            LET p_nff.den_nat_oper = pol08581_den_nat_oper()
      ...
      IF p_nat_operacao.ies_subst_tribut = "S" THEN
         CALL pol08581_busca_dados_subst_trib_uf()
      ...
      CALL pol08581_busca_dados_clientes()
      ...
      CALL pol08581_busca_dados_cidades(p_clientes.cod_cidade)
      ...
      CALL pol08581_busca_cof_compl()

      #--- busca nome do pais
      CALL pol08581_busca_nome_pais()
      LET p_nff.den_pais = p_paises.den_pais              

      #--- busca dados das duplicatas
      CALL pol08581_busca_dados_duplicatas()
      ...
      CALL pol08581_carrega_corpo_nff()

      CALL pol08581_grava_corpo_nota()

      IF p_nat_operacao.ies_tip_controle = "2" THEN
         #CALL pol08581_obtem_peso_item()
      END IF
      ...
      CALL pol08581_grava_dados_end_entrega()

      #--- carrega endereco de cobranca
      CALL pol08581_carrega_end_cobranca()

      #--- trata zona franca
      IF p_fat_nf_mestre.zona_franca = 'S' THEN
         CALL pol08581_le_icms('ICMS_ZF')
         CALL pol08581_le_param_zf()
      ELSE
         CALL pol08581_le_icms('ICMS')
      END IF
      ...
      CALL pol08581_retorno_terceiro()

      #--- busca dados da transportadora
      CALL pol08581_busca_dados_transport(p_fat_nf_mestre.transportadora)
      CALL pol08581_busca_dados_cidades(p_transport.cod_cidade)
      ...
      LET p_nff.nom_guerra      = pol08581_representante()
      ...
      CALL pol08581_den_cnd_pgto()
      
      #--- grava dados consignados
      #CALL pol08581_grava_dados_consig()
      
      IF NOT p_ret_terc THEN
         CALL pol08581_checa_nf_contra_ordem()
      END IF

      CALL pol08581_carrega_historico_fiscal()
      
      CALL pol08581_monta_relat()
...
#-------------------------------#
FUNCTION pol08581_junta_volumes()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol08581_cria_tabs_tmp()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol08581_deleta_tabs_tmp()
#---------------------------------#
...
#----------------------------------#
FUNCTION pol08581_retorno_terceiro()
#----------------------------------#
   ...
   FOREACH cq_ret_terc INTO p_num_nf,
      ...
      IF NOT p_ret_terc THEN
         ...
         CALL pol08581_insere_texto(p_des_texto,3)
...
#----------------------------#
   ...
   CALL pol08581_calcula_total_de_paginas()
   ...
   FOREACH cq_wnotalev INTO p_wnotalev.*
      ...
      OUTPUT TO REPORT pol08581_relat(p_wnotalev.num_nff)
...
#---------------------------------------#
FUNCTION pol08581_busca_dados_duplicatas()
#---------------------------------------#
...
#-------------------------------------#
FUNCTION pol08581_carrega_end_cobranca()
#-------------------------------------#
...
#-----------------------------------#
FUNCTION pol08581_carrega_corpo_nff()
#-----------------------------------#
   ...
   FOREACH cq_wfat_item_rt INTO p_fat_nf_item.*
      ...
      CALL pol08581_busca_dados_pedido()
      LET pa_corpo_nff[p_ind].num_pedido_cli = p_nff.num_pedido_cli

      CALL pol08581_item_cliente()
      ...
      CALL pol08581_verifica_texto_ped_it()
...
#-----------------------------#
FUNCTION pol08581_item_cliente()
#-----------------------------#
...
#--------------------------------------#
FUNCTION pol08581_verifica_ctr_unid_med()
#--------------------------------------#
...
#-----------------------------------------#
FUNCTION pol08581_carrega_historico_fiscal()
#-----------------------------------------#  
   ...
   FOREACH cq_whist INTO p_fat_texto, p_sequencia_texto
      ...
      CALL pol08581_insere_texto(p_fat_texto,4)
...
#------------------------------------#
FUNCTION pol08581_le_icms(p_cod_nenef)
#------------------------------------#
...
#-----------------------------#  
   ...
   IF p_valor_pis > 0 THEN
      LET p_pct_pis = pol08581_le_pct('PIS_REC')
      LET p_des_texto = "PIS: ", p_pct_pis, " % = R$ ", p_valor_pis USING "##,###,##&.&&"
   END IF

   IF p_valor_cofins > 0 THEN
      LET p_pct_cofins = pol08581_le_pct('COFINS_REC')
   ...
   LET p_pct_icms = pol08581_le_pct('ICMS')
...
#---------------------------------#
FUNCTION pol08581_le_pct(p_tributo)
#---------------------------------#
...
#-----------------------------------#
FUNCTION pol08581_grava_corpo_nota()
#-----------------------------------#
   ...
   FOR i = 1 TO 999
      ...
      CALL pol08581_insere_item(pa_corpo_nff[i].den_item1, 1)
      CALL pol08581_insere_item(pa_corpo_nff[i].den_item2, 2)
...
#--------------------------------------------------#
FUNCTION pol08581_insere_item(p_den_item,p_tip_info)
#--------------------------------------------------#
...
#---------------------------------------------------#
FUNCTION pol08581_insere_texto(p_den_texto,p_tip_info)
#---------------------------------------------------#
...
#---------------------------------#
FUNCTION pol08581_obtem_peso_item()
#---------------------------------#
...
#-----------------------------------------#
FUNCTION pol08581_calcula_total_de_paginas()
#-----------------------------------------#
...
#------------------------------------------#
FUNCTION pol08581_busca_dados_subst_trib_uf()
#------------------------------------------#
...
#-----------------------------#
FUNCTION pol08581_den_nat_oper()
#-----------------------------#
...
#----------------------------------#
FUNCTION pol08581_busca_cof_compl()
#----------------------------------#
...
#-------------------------------------#
FUNCTION pol08581_busca_dados_empresa()            
#-------------------------------------#
...
#-------------------------------#
FUNCTION pol08581_representante()
#-------------------------------#
...
#-----------------------------#
FUNCTION pol08581_den_cnd_pgto()
#-----------------------------#
...
#--------------------------------------------------#
FUNCTION pol08581_busca_dados_clientes()
#--------------------------------------------------#
...
#--------------------------------#
FUNCTION pol08581_busca_nome_pais()                   
#--------------------------------#
   INITIALIZE p_paises.* ,
              p_uni_feder.*    TO NULL 
...
#----------------------------------------------------#
FUNCTION pol08581_busca_dados_transport(p_cod_transpor)
#----------------------------------------------------#
...
#------------------------------------------------#
FUNCTION pol08581_busca_dados_cidades(p_cod_cidade)
#------------------------------------------------#
...
#-----------------------------------#
FUNCTION pol08581_busca_dados_pedido()
#-----------------------------------#  
...
#-----------------------------------------------#
FUNCTION pol08581_grava_dados_consig()
#-----------------------------------------------#
...
#----------------------------------------#
FUNCTION pol08581_grava_dados_end_entrega()
#----------------------------------------#
...
#----------------------------------------#
FUNCTION pol08581_checa_nf_contra_ordem()
#----------------------------------------#
   ...
   IF STATUS = 0 THEN
      ...
      CALL pol08581_insere_texto(p_des_texto,4)
   ELSE 
      IF STATUS = 100 then
...
#---------------------------------------#
FUNCTION pol08581_verifica_texto_ped_it()
#---------------------------------------#
...
#-----------------------------#
REPORT pol08581_relat(p_num_nff)
#-----------------------------#
