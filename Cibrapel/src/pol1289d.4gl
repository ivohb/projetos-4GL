...
#--------------------------------------#
    ...
    INPUT p_nf_tab.num_aviso_rec,
        ...
        AFTER FIELD num_aviso_rec
            ...
            SELECT ns.num_nf,
             ...
             WHERE ns.cod_empresa = p_cod_empresa
               AND ns.num_aviso_rec = p_nf_tab.num_aviso_rec
            ...
            SELECT qtd_recebida, cod_unid_med_nf
              FROM aviso_rec ar
             WHERE ar.cod_empresa = p_cod_empresa
               AND ar.num_aviso_rec = p_nf_tab.num_aviso_rec
            
            FOREACH cq_qtd INTO p_qtd_recebida, p_cod_unid                            
            ...
            END IF
                        ...
                        LET p_nf_tab.peso_balanca = p_peso_balanca
       ...
       END IF
            ...
            ELSE
                SELECT cl.nom_cliente
                 ...
                 WHERE ft.cod_fornecedor = p_cod_fornecedor
                   AND ft.cod_transpor   = p_nf_tab.cod_transpor
                ...
                ELSE
                    ...
                    ELSE
                    	IF p_peso_balanca > p_tara_minima THEN
                    	    LET p_nf_tab.peso_pagar = p_peso_balanca
                    	ELSE
        ...
        AFTER FIELD tabela
            IF p_nf_tab.tabela IS NULL THEN
                ERROR "Campo com preenchimento obrigatório !!!"
                NEXT FIELD tabela
            ELSE
                SELECT versao,
                       ...
                       destino,
                       val_tonelada
                  INTO p_nf_tab.versao,
                       p_origem,
                       p_destino,
                       p_nf_tab.val_tonelada
                  FROM tab_frete_885
                 WHERE tabela = p_nf_tab.tabela
                   AND versao_atual = 'S'
                ...
                ELSE
                    ...
                    LET p_nf_tab.val_frete = p_nf_tab.val_tonelada * p_nf_tab.peso_pagar
   ...
   CASE
      WHEN INFIELD(cod_transpor)
         ...
         IF p_codigo IS NOT NULL THEN
            ...
            DISPLAY p_nf_tab.cod_transpor TO cod_transpor
         END IF
      ...
      WHEN INFIELD(tabela)
         ...
         CURRENT WINDOW IS w_pol1289
         IF p_codigo IS NOT NULL THEN
            LET p_nf_tab.tabela       = p_codigo CLIPPED
            LET p_nf_tab.versao       = p_versao_tabela CLIPPED
            LET p_nf_tab.val_tonelada = p_val_tonelada  CLIPPED
...
#-----------------------#
    ...
    UPDATE nf_x_tab_frete_885
       SET cod_transpor = p_nf_tab.cod_transpor,
           num_placa    = p_nf_tab.num_placa,
           tabela       = p_nf_tab.tabela,
           versao       = p_nf_tab.versao,
           val_tonelada = p_nf_tab.val_tonelada,
           peso_balanca = p_nf_tab.peso_balanca,
           peso_pagar   = p_nf_tab.peso_pagar,
           val_frete    = p_nf_tab.val_frete
     WHERE num_aviso_rec = p_nf_tab.num_aviso_rec
       AND cod_empresa = p_cod_empresa
...
#------------------------------#
    ...
    ELSE
    	...
    	LET p_nf_tab.val_frete = p_nf_tab.val_tonelada * p_nf_tab.peso_balanca
...
#--------------------------#
    ...
    FOREACH cq_impressao
       INTO p_nf_tab.*
        ...
        OUTPUT TO REPORT pol1289_relat(p_nf_tab.num_aviso_rec)
...
#-----------------------------------#
   ...
   FORMAT
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 002, p_nf_tab.num_aviso_rec USING "#########",
               COLUMN 017, '     ', 
               COLUMN 023, p_nf_tab.cod_transpor,
               COLUMN 034, p_nf_tab.num_placa,
               COLUMN 044, p_nf_tab.tabela,
               COLUMN 050, p_nf_tab.versao,
               COLUMN 057, p_nf_tab.val_tonelada USING '###,##&.&&',
               COLUMN 066, p_nf_tab.peso_balanca USING '###,##&.&&',
               COLUMN 074, p_nf_tab.peso_pagar USING '###,##&.&&',
               COLUMN 090, p_nf_tab.val_frete USING '###,##&.&&'
