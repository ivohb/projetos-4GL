...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: pol0980                                                 #
# OBJETIVO: LISTAGEM DE BAIXAS NO ESTOQUE                           #
...
MAIN
   ...
   IF p_status = 0 THEN
      CALL pol0980_controle()
   END IF
END MAIN

#---------------------------#
 FUNCTION pol0980_controle()
#--------------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol0980") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0980 AT 2,2 WITH FORM p_nom_tela
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   IF NOT pol0980_le_empresa() THEN
      RETURN
   END IF
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informa parâmetros para listagem"
         CALL pol0980_informar() RETURNING p_status
         IF p_status THEN
            LET p_ies_cons = TRUE 
      ...
      COMMAND "Listar" "Listagem dos parâmetros informados"
         IF p_ies_cons = TRUE THEN 
            CALL pol0980_listagem() RETURNING p_status
            IF p_status THEN
               LET p_ies_cons = FALSE
            ELSE
               ERROR 'Operação cancelada !!!'
      ...
      COMMAND "Fim"       "Retorna ao menu anterior"
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol0980

END FUNCTION

#----------------------------#
FUNCTION pol0980_le_empresa()
#----------------------------#
...
#--------------------------#
 FUNCTION pol0980_listagem()
#--------------------------#
   
   IF NOT pol0980_escolhe_saida() THEN
   		RETURN FALSE  
   END IF
      
   IF NOT pol0980_le_den_empresa() THEN
      RETURN FALSE 
   END IF   
   ...
   FOREACH cq_leitura INTO 
      ...
      OUTPUT TO REPORT pol0980_relat(p_tela.cod_item) 
   
      LET p_count = 1
      
   END FOREACH 
         
   FINISH REPORT pol0980_relat   
...
#-------------------------------#
 FUNCTION pol0980_escolhe_saida()
#-------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         ...
         LET p_caminho = p_caminho CLIPPED, "pol0980.tmp"
         START REPORT pol0980_relat TO p_caminho
      ELSE
         START REPORT pol0980_relat TO p_nom_arquivo
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol0980_le_den_empresa()
#--------------------------------#
...
#-------------------------------#
 REPORT pol0980_relat(p_cod_item)
#-------------------------------#
