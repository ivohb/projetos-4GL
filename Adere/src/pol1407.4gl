...
#----------------------#
   ...
   MENU "OPCAO"
      COMMAND "Informar" "Informar pedido para o processamento"
         ...
         IF p_status THEN
            ERROR 'Operaçãoo efetuada com sucesso !!!'
         ELSE
            ERROR 'Operação cancelada !!!'
         END IF 
      COMMAND "Processar" "Processa a unificação"
         LET m_erro = NULL
         CALL pol1407_controle() 
         IF m_erro IS NULL THEN
            LET m_erro = 'Operação efetuada com sucesso.'
         END IF
         ERROR m_erro
         LET p_ies_cons = FALSE
...
#--------------------------#
   ...
   INPUT p_tela.num_pedido
      ...
      IF p_tela.num_pedido IS NOT NULL THEN 
         ...
         IF m_count = 0 THEN
            ERROR "Pedido sem romaneio ou não exsiste !!!"
...
#--------------------------#
   ...
   END IF
    ...
    FOREACH cq_oms INTO l_num_pedido
      ...
      IF STATUS = 100 THEN
         LET m_erro = 'Pedido não existe ou está cancelado'
      ...
      IF STATUS = 100 THEN
         LET m_erro = 'Carteira do pedido não consta dos parâmetros'
         IF NOT pol1407_insere_erro(l_num_pedido, m_erro) THEN
            RETURN 
         END IF
         CONTINUE FOREACH
...
#-------------------------------#
  ...
  RETURN "$Archive: /Logix/Fontes_Doc/Customizacao/10R2/gps_logist_e_gerenc_de_riscos_ltda/financeiro/solicitacao de faturameto/programas/pol1407.4gl $|$Revision: 9 $|$Date: 08/02/2021 14:43 $|$Modtime: 29/01/2021 13:31 $" 
