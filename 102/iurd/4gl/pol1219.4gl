...
# SISTEMA.: LOGIX                                                   #
# PROGRAMA: pol1219                                                 #
# OBJETIVO: CADASTRO DE UNIDADE FUNCIONALADM ISENTA DE SUBSTITUIÇÃO #
# AUTOR...: IVO H BARBOSA                                           #
# DATA....: 18/09/12                                                #
#-------------------------------------------------------------------#
...
MAIN
   ...
   LET parametro.num_programa = 'POL1219'
...
#----------------------#
 FUNCTION pol1219_menu()
#----------------------#

   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL 
   CALL log130_procura_caminho("pol1219") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol1219 AT 2,2 WITH FORM p_nom_tela
   ...
   MENU "OPCAO"
      COMMAND "Incluir" "Inclui dados na tabela."
         CALL pol1219_inclusao() RETURNING p_status
         IF p_status THEN
      ...
      COMMAND "Seguinte" "Exibe o próximo item encontrado na consulta."
         IF p_ies_cons THEN
            CALL pol1219_paginacao("S")
         ELSE
            ERROR "Não existe nenhuma consulta ativa !!!"
         END IF 
      COMMAND "Anterior" "Exibe o item anterior encontrado na consulta."
         IF p_ies_cons THEN
            CALL pol1219_paginacao("A")
         ELSE
            ERROR "Não existe nenhuma consulta ativa !!!"
         END IF 
      COMMAND "Modificar" "Modifica dados da tabela."
         IF p_ies_cons THEN
            CALL pol1219_modificacao() RETURNING p_status  
            IF p_status THEN
      ...
      COMMAND "Excluir" "Exclui dados da tabela."
         IF p_ies_cons THEN
            CALL pol1219_exclusao() RETURNING p_status
            IF p_status THEN
      ...
      COMMAND "Fim"       "Retorna ao menu anterior."
         EXIT MENU
   END MENU
   CLOSE WINDOW w_pol1219

END FUNCTION

#-----------------------#
 FUNCTION pol1219_sobre()
#-----------------------#
...
#--------------------------#
   ...
   IF pol1219_edita_dados() THEN      
      IF pol1219_edita_unid('I') THEN      
         IF pol1219_grava_dados() THEN                                                     
            RETURN TRUE                                                                    
         END IF                                                                      
      END IF
   END IF
   ...
   INPUT p_empresa WITHOUT DEFAULTS
    FROM empresa
      ...
      SELECT COUNT(empresa)
        INTO p_count
        FROM unid_adm_isenta_265
       WHERE empresa = p_empresa
       
      IF STATUS <> 0 THEN
         CALL log003_err_sql('lendo','unid_adm_isenta_265')
         RETURN FALSE
      END IF 
...
#-------------------------#
   ...
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol1219a") RETURNING p_nom_tela
   LET p_nom_tela = p_nom_tela CLIPPED
   OPEN WINDOW w_pol1219a AT 5,20 WITH FORM p_nom_tela
...
#-------------------------------------#
 FUNCTION pol1219_edita_unid(p_funcao)
#-------------------------------------#     
   ...
   ELSE
      ...
      IF p_funcao = 'I' THEN
         ...
         DISPLAY p_cod_empresa TO cod_empresa
      ELSE
        CALL pol1219_carrega_itens() RETURNING p_status
      END IF
      RETURN FALSE
   END IF
...
#-----------------------------#
   ...
   DELETE FROM unid_adm_isenta_265
    WHERE empresa = p_empresa
    
   IF STATUS <> 0 THEN
      CALL log003_err_sql("Deletando", "unid_adm_isenta_265")
      CALL log085_transacao("ROLLBACK")
      RETURN FALSE
   END IF 
   ...
   FOR p_ind = 1 TO ARR_COUNT()
       IF pr_unid[p_ind].cod_uni_funcio IS NOT NULL THEN
		       ...
		       INSERT INTO unid_adm_isenta_265
		       VALUES (p_empresa,
		               pr_unid[p_ind].cod_uni_funcio)
		
		       IF STATUS <> 0 THEN 
		          CALL log003_err_sql("Incluindo", "unid_adm_isenta_265")
		          CALL log085_transacao("ROLLBACK")
		          RETURN FALSE
		       END IF
		       LET p_incluiu = TRUE
...
#--------------------------#
   ...
   IF INT_FLAG THEN
      IF p_ies_cons THEN 
         ...
         CALL pol1219_exibe_dados() RETURNING p_status
      END IF    
      RETURN FALSE 
   END IF
...
#------------------------------#
   ...
   IF NOT pol1219_carrega_itens() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
 FUNCTION pol1219_carrega_itens()
#---------------------------------#
   ...
   DECLARE cq_array CURSOR FOR
    SELECT DISTINCT
           a.cod_uni_funcio,
           b.den_uni_funcio
      FROM unid_adm_isenta_265 AS a
           LEFT OUTER JOIN uni_funcional AS b
              ON b.cod_empresa = a.empresa
             AND b.cod_uni_funcio = a.cod_uni_funcio
             AND b.dat_validade_fim > CURRENT YEAR TO SECOND
...
#-----------------------------------#
 FUNCTION pol1219_paginacao(p_funcao)
#-----------------------------------#
...
#----------------------------------#
 FUNCTION pol1219_prende_registro()
#----------------------------------#
...
#-----------------------------#
   ...
   IF pol1219_prende_registro() THEN
      IF pol1219_edita_unid('M') THEN
         IF pol1219_grava_dados() THEN
            LET p_retorno = TRUE
         END IF
      END IF
   ...
   IF pol1219_prende_registro() THEN
     ...
     END FOR
      ...
      IF STATUS = 0 THEN               
         ...
         LET p_retorno = TRUE 
      ELSE
         CALL log003_err_sql("Excluindo","unid_adm_isenta_265")
      END IF
      CLOSE cq_prende
   END IF
...
{
#--------------------------#
 FUNCTION pol1219_listagem()
#--------------------------#     

   IF NOT pol1219_escolhe_saida() THEN
   		RETURN 
   END IF
      
   IF NOT pol1219_le_den_empresa() THEN
      RETURN
   END IF   
 ...
 ORDER BY cod_empresa, cod_uni_funcio                          
   ...
   OUTPUT TO REPORT pol1219_relat(p_empresa) 

      LET p_count = 1
      
   END FOREACH

   FINISH REPORT pol1219_relat   
...
#-------------------------------#
 FUNCTION pol1219_escolhe_saida()
#-------------------------------#
   ...
   IF g_ies_ambiente = "W" THEN
      IF p_ies_impressao = "S" THEN
         ...
         LET p_caminho = p_caminho CLIPPED, "pol1219.tmp"
         START REPORT pol1219_relat TO p_caminho
      ELSE
         START REPORT pol1219_relat TO p_nom_arquivo
      END IF
   END IF
...
#--------------------------------#
 FUNCTION pol1219_le_den_empresa()
#--------------------------------#
...
#--------------------------------#
 REPORT pol1219_relat(p_empresa)
#--------------------------------#
