...
#---------------------------------#
   ...
   FOREACH cq_hist INTO p_aponta.*,p_rowid
     ...
     #Manuel - 31/08/2012 - Ega não manda o item
      select cod_item
        into p_cod_item
        from ordens
       WHERE cod_empresa = p_cod_empresa
      ...
      IF STATUS <> 0 THEN
         let p_cod_item = ''
      END IF #Manuel - 31/08/2012 - até aqui
...
#---------------------------#
   ...
   FOREACH cq_gra_gem INTO p_cod_peca_gemea, p_qtd_peca_gemea
      ...
      FOREACH cq_op_gem INTO 
         ...
         #LET p_num_ordem = p_aponta.num_op #se tiver erro, gerar o erro na OP da peça principal
...
## Manuel em 12/09/2012 Alterei o programa para pegar uma maquina válida por Empresa/arranjo   
         ...
         END IF
	 	  ...
	 	  IF p_qtd_transf_refug > 0 THEN
	 	     ...
	 	     ELSE
	 	        CALL log085_transacao("COMMIT")
	 	     END IF
	 	  END IF #Manuel  - 26/09/2012 até aqui
...
#      LET m_msg = 'Não foi possível ler o movimento de\n ',
