...
2-	Qual preço vamos usar na carga do Plano de vendas ?
}
...
#-----------------------------------------#
    ...
    LET l_tabcolumn = _ADVPL_create_component(NULL,"LTABLECOLUMNEX",m_brz_prog)
    CALL _ADVPL_set_property(l_tabcolumn,"HEADER","Preço logix")
    CALL _ADVPL_set_property(l_tabcolumn,"COLUMN_WIDTH",60)
    CALL _ADVPL_set_property(l_tabcolumn,"EDIT_COMPONENT","LNUMERICFIELD")
...
#--------------------------------#
   ...
   IF l_count > 0 THEN
      LET m_msg = 'Erro limpando tabela temporária w_prog_komatsu'
      CALL log0030_mensagem(m_msg,'info')
      RETURN
   END IF      
...
#--------------------------#
   ...
   IF STATUS = 100 THEN                               
      LET ma_prog[m_ind].mensagem = 'Plano não cadastrado no Logix'
      RETURN TRUE
   ELSE
      IF STATUS <> 0 THEN 
