...
#--------------------------#
  ...
  OPEN WINDOW w_pol0753 AT 2,1 WITH FORM p_nom_tela 
...
#--------------------------------------#
  ...
  LET p_repres_885.tip_perc = 'I'
  LET p_repres_885.tip_comis = 'R'
  LET p_repres_885.val_garantia = 0
  LET p_repres_885.dat_exp_gar = TODAY + 360
  LET p_repres_885.ies_exp = 'N'
...
#---------------------------------------#
  ...
  INPUT   BY NAME p_repres_885.* WITHOUT DEFAULTS  
    ...
    AFTER FIELD cod_lin_prod
       ...
       IF p_repres_885.cod_lin_prod IS NOT NULL THEN
          CALL pol0753_le_linha()
          IF p_den_estr_linprod IS NULL THEN
             ERROR 'Limha de produto inválida'
       ...
       END IF
         ...
         ELSE
            ERROR "Representante/linha ja cadastrado" 
            NEXT FIELD cod_repres 
         END IF  
...
#--------------------------#
FUNCTION pol0753_le_linha()#
#--------------------------#

   SELECT den_estr_linprod
     INTO p_den_estr_linprod
     FROM linha_prod
    WHERE cod_lin_prod = p_repres_885.cod_lin_prod
      AND cod_lin_recei = 0
      AND cod_seg_merc = 0
      AND cod_cla_uso = 0
  ...
  IF p_ies_cons THEN
     ...
     WHILE TRUE
        ...
        WHERE cod_repres  = p_repres_885.cod_repres
          AND cod_lin_prod = p_repres_885.cod_lin_prod
...
#-----------------------#
  ...
  CASE
    ...
    WHEN infield(cod_lin_prod)
         CALL log009_popup(6,25,"LINHA DE PRODUTO","linha_prod",
                          "cod_lin_prod","den_estr_linprod",
                          "","N"," 1=1 and cod_lin_recei = 0 and cod_seg_merc = 0 and cod_cla_uso = 0") 
                          RETURNING p_codigo
         CALL log006_exibe_teclas("01 02 03 07", p_versao)
         CURRENT WINDOW IS w_pol0753 
         IF   p_codigo IS NOT NULL THEN
              LET p_repres_885.cod_lin_prod  = p_codigo
