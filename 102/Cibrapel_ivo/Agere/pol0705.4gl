...
GLOBALS
   ...
   DEFINE p_de_para_turno_885,p_de_para_turno_885a  RECORD 
   									cod_empresa 	CHAR(01),
   									turno_simula	CHAR(01),
   									turno_logix     INTEGER
   							   END RECORD
...
#--------------------------#
   ...
   IF pol0705_entrada_dados("INCLUSAO") THEN
...
#---------------------------------------#
   ...
   INPUT BY NAME p_de_para_turno_885.* 
      ...
      ELSE
         ...
         IF STATUS = 0 THEN
            ERROR "Código do Turno já Cadastrado na Tabela de_para_turno_885!!!"
            NEXT FIELD turno_logix
         END IF
...
#--------------------------#
   ...
   CONSTRUCT BY NAME where_clause ON de_para_turno_885.turno_logix
...
#-----------------------------#
   ...
   IF pol0705_cursor_for_update() THEN
      LET p_de_para_turno_885a.* = p_de_para_turno_885.*
      IF pol0705_entrada_dados("MODIFICACAO") THEN
         UPDATE de_para_turno_885
            SET turno_simula = p_de_para_turno_885.turno_simula
              WHERE cod_empresa = p_cod_empresa
                AND turno_logix = p_de_para_turno_885.turno_logix
         IF STATUS = 0 THEN
            LET p_retorno = TRUE
...
#-------------------------------#
    ...
    DEFINE pr_empresa       ARRAY[3000]
     OF RECORD
         turno_logix        INTEGER, #LIKE de_para_turno_885.turno_logix,
