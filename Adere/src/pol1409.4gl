...
#--------------------------#
   ...
   FOREACH cq_unif INTO 
      ...
      IF ma_unif[l_ind].romaneio IS NULL OR ma_unif[l_ind].romaneio = 0 THEN
         LET ma_unif[l_ind].ies_situa = NULL
         LET ma_unif[l_ind].num_nff = NULL
      ELSE
         SELECT ies_sit_om,                                             
                num_nff                                                 
           INTO ma_unif[l_ind].ies_situa,                               
                ma_unif[l_ind].num_nff                                  
           FROM ordem_montag_mest                                       
          WHERE cod_empresa = p_cod_empresa                             
            AND num_om = ma_unif[l_ind].romaneio                        
                                                                        
         IF STATUS <> 0 THEN                                            
            CALL log003_err_sql('SELECT','ordem_montag_mest:cq_unif')   
            EXIT FOREACH                                                
         END IF                                                         
      END IF
