#--------------------------------------------------------------------#
# SISTEMA.: SUPRIMENTOS                                              #
# OBJETIVO: GERAÇÃO DA NFE NA EMPRESA GERENCIAL                      #
# CLIENTE.: CIBRAPEL                                                 #
# DATA....: 07/12/07                                                 #
# ALTERAÇÃO MOTIVO                                                   #
# 24/11/08  GRAVAR ESTOQUE_TRANS.DAT_MOVTO=AVISO_REC.DAT_INCLUSAO_SEQ#
# 30/07/09  Gerar somente um movimento na gerencial, mesmo que a quan-
#           tidade contada seja maior que a qtd recebida             #
# 18/09/09  update estoque_trans.dat_movto = dat_entrada_nf          #
#--------------------------------------------------------------------#
...
GLOBALS
  ...
  DEFINE p_cod_empresa        LIKE empresa.cod_empresa,
         p_cod_emp_atu        LIKE empresa.cod_empresa,
       	 ...
       	 p_dat_inclusao_seq   LIKE aviso_rec.dat_inclusao_seq,
...
MAIN
   ...
   DEFER INTERRUPT 
   LET p_versao = "pol0665-10.02.00"
   
   INITIALIZE p_nom_help TO NULL  
   CALL log140_procura_caminho("pol0665.iem") RETURNING p_nom_help
   ...
   IF p_status = 0 THEN
      CALL pol0665_controle()
   END IF

END MAIN

#--------------------------#
 FUNCTION pol0665_controle()
#--------------------------#
   
   CALL log006_exibe_teclas("01",p_versao)
   INITIALIZE p_nom_tela TO NULL
   CALL log130_procura_caminho("pol0665") RETURNING p_nom_tela
   LET  p_nom_tela = p_nom_tela CLIPPED 
   OPEN WINDOW w_pol0665 AT 3,2 WITH FORM p_nom_tela 
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST, FORM LINE FIRST)

   DISPLAY p_cod_empresa TO cod_empresa

   IF p_num_ar IS NULL THEN
      PROMPT 'Informe o AR: ' FOR p_num_ar
   END IF
   
   CALL pol0665_processa()
   
#   CLOSE WINDOW w_pol0665

END FUNCTION

#-------------------------#
FUNCTION pol0665_processa()
#-------------------------#
   ...
   ELSE
      ...
      ELSE
         ...
         IF STATUS <> 0 THEN
            ...
            RETURN FALSE
         END IF
         LET p_cod_emp_dest = p_cod_empresa
   ...
   IF STATUS = 0 THEN
      CALL log0030_mensagem('Processao já efetuado.','exclamation')
      RETURN
   END IF

   IF NOT pol0665_cria_tab() THEN
      RETURN
   END IF
   ...
   FOREACH cq_cc INTO 
      ...
      IF p_ies_liberacao_cont <> 'S' THEN
         CALL log0030_mensagem('Favor efetuar a contagem previamente !!!','exclamation')
         RETURN
      END IF
         
      IF p_ies_liberacao_insp <> 'S' THEN
         CALL log0030_mensagem('Favor efetuar a inspeção previamente !!!','exclamation')
         RETURN
      END IF
   ...
   FOREACH cq_ct INTO 
      ...
      IF NOT pol0665_le_item(p_cod_item) THEN
         RETURN FALSE
      END IF
      ...
      DELETE FROM movtos_tmp_885
      DELETE FROM tmp_est_tr_885

      INSERT INTO tmp_est_tr_885
      SELECT cod_empresa,
             num_transac,
             dat_movto,
             qtd_movto,
   ...
   IF NOT pol0665_processa_entradas() THEN
      CALL log085_transacao("ROLLBACK")
      CALL log0030_mensagem('Processo de Cópia Cancelado','exclamation')
   ELSE
      CALL log085_transacao("COMMIT")
      CALL log0030_mensagem('Processo de Cópia Efetuado c/ Sucesso','exclamation')
   END IF
   
END FUNCTION

#--------------------------#
FUNCTION pol0665_cria_tab()
#--------------------------#
   ...
   DROP TABLE ar_tmp_885

   IF STATUS = 0 OR STATUS -206 THEN 
 
      CREATE  TABLE ar_tmp_885(
         num_seq  DECIMAL(3,0),
         qtd_lote DECIMAL(10,3),
         val_lote DECIMAL(12,2)
       );
   ...
   DROP TABLE tmp_est_tr_885

   IF STATUS = 0 OR STATUS -206 THEN 

      CREATE  TABLE tmp_est_tr_885(
             cod_empresa        CHAR(02), 
             num_transac        INTEGER,
      ...
      IF SQLCA.SQLCODE <> 0 THEN 
         CALL log003_err_sql("CRIACAO","tmp_est_tr_885")
         RETURN FALSE
      END IF
   ELSE
      RETURN FALSE
...
#----------------------------------#
FUNCTION pol0665_le_item(p_cod_item)
#----------------------------------#
...
#-----------------------------------#
FUNCTION pol0665_processa_entradas()
#-----------------------------------#
   ...
   IF p_count > 0 THEN
      IF NOT pol0665_processa_aparas() THEN
         RETURN FALSE
      END IF
   END IF
   ...
   IF p_count > 0 THEN
      IF NOT pol0665_processa_nao_aparas() THEN
         RETURN FALSE
      END IF
   END IF

   IF NOT pol0665_update_na_zero() THEN
      RETURN FALSE
   END IF

   IF NOT pol0665_insere_ar_proces() THEN
      RETURN FALSE
   END IF
...
#---------------------------------#
FUNCTION pol0665_processa_aparas()
#---------------------------------#

   IF NOT pol0665_informa_dados() THEN
      RETURN FALSE
   END IF
   ...
   FOREACH cq_apara INTO 
       ...
       IF NOT pol0665_le_ordem_sup() THEN
          RETURN FALSE
       END IF
       ...
       IF NOT pol0665_insere_estoque() THEN
          RETURN FALSE
       END IF
   ...
   IF p_val_nota_ofic < p_val_nota_ger THEN
      IF NOT pol0665_gera_ad() THEN
         RETURN FALSE
      END IF
   END IF 
   
   IF p_val_nota_ofic > p_val_nota_ger THEN
      IF NOT pol0665_gera_adiantamento() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------#
   ...
   IF NOT pol0665_insere_nf_sup() THEN
      RETURN FALSE
   END IF
...
#------------------------------#
FUNCTION pol0665_informa_dados()
#------------------------------#

   IF NOT pol0665_exib_cabec() THEN
      RETURN FALSE
   END IF
   ...
   INPUT ARRAY pr_itens 
      ...
      BEFORE ROW
         ...
         IF pr_itens[p_index].cod_item IS NOT NULL THEN
            IF NOT pol0665_le_item(pr_itens[p_index].cod_item) THEN
               RETURN FALSE
            END IF
      ...
      AFTER FIELD qtd_contagem
         IF pr_itens[p_index].cod_item IS NOT NULL THEN
            ...
            IF NOT pol0665_le_cotacao(pr_itens[p_index].cod_item) THEN
               RETURN FALSE
            END IF
...
#-------------------------------------#
FUNCTION pol0665_le_cotacao(p_cod_item)
#-------------------------------------#
...
#---------------------------#
   ...
   LET p_audit_ar.num_prog       = 'pol0665'
...
#--------------------------------#
FUNCTION pol0665_insere_estoque()
#--------------------------------#
   ...
   IF p_gerar = 'E' THEN
      ...
      END IF
   ELSE
      IF NOT pol0665_insere_trans() THEN
         RETURN FALSE
      END IF
   END IF
...
#------------------------------#
   ...
   LET  p_estoque_trans.num_prog           = "pol0665"
...
#-------------------------------#
FUNCTION pol0665_insere_nf_sup()
#-------------------------------#
...
#-----------------------------------#
FUNCTION pol0665_gera_adiantamento()
#-----------------------------------#
...
#-------------------------#
   ...
   IF NOT pol0665_le_par_ad() THEN 
      RETURN FALSE
   END IF

   IF NOT pol0665_insere_ad() THEN
      RETURN FALSE
   END IF

   IF NOT pol0665_insere_lanc() THEN
      RETURN FALSE
   END IF

   IF NOT pol0665_grava_aps() THEN
      RETURN FALSE
   END IF
...
#---------------------------#
FUNCTION pol0665_insere_ad()
#---------------------------#

   IF NOT pol0665_calc_dat_vencto() THEN
      RETURN FALSE
   END IF
   ...
   LET p_msg = 'POL0665 - INCLUSAO DA AD No. ', p_msg CLIPPED
...
#---------------------------------#
FUNCTION pol0665_calc_dat_vencto()
#---------------------------------#
...
#---------------------------#
FUNCTION pol0665_insere_ap()
#---------------------------#

    IF NOT pol0665_le_par_ap() THEN 
       RETURN FALSE
    END IF
   ...
   LET p_msg = 'POL0665 - INCLUSAO DA AP No. ', p_msg CLIPPED
   LET p_hora = CURRENT HOUR TO SECOND
...
#------------------------------------#
FUNCTION pol0665_processa_nao_aparas()
#------------------------------------#
...
#---------------------------#
   ...
   IF NOT pol0665_insere_tmp() THEN
      RETURN FALSE
   END IF
...
#----------------------------------#
FUNCTION pol0665_insere_ar_proces()
#----------------------------------#
...
#--------------------------------#
FUNCTION pol0665_update_na_zero()
#--------------------------------#
