...
#--------------------------#   
   ...
   DELETE from ap_proces_912   where 
        ap_proces_912.num_ap IS NOT NULL
       AND ap_proces_912.num_ap > 0
       and ap_proces_912.num_ap not in 
    (SELECT ap.num_ap FROM ap
          WHERE ap.cod_empresa = ap_proces_912.cod_empresa)
   ...
   DELETE FROM gi_ad_erro_912
    WHERE den_erro LIKE '%cq_pgto%'
    
   IF NOT pol1328_pgto_ap() THEN
      CALL pol1328_grava_erro() RETURN l_ret
   END IF

   DELETE FROM gi_ad_erro_912
    WHERE den_erro LIKE '%cq_exclui%'

   IF NOT pol1328_exclusao() THEN
      CALL pol1328_grava_erro() RETURN l_ret
   END IF
...
#----------------------------------#
   ...
   IF STATUS = 0 THEN
      RETURN TRUE
   ELSE
      LET m_msg = 'ERRO: ',STATUS USING '<<<<<<<<', ' TRAVANDO OBRIGAÇÃO PARA UPDATE'
      RETURN FALSE
   END IF
   ...
   FOREACH cq_nota INTO mr_nota.*
      ...
      IF m_num_seq = 0 THEN
         ...
         IF mr_nota.ies_gera_nota MATCHES '[AS]' THEN
            ...
            IF STATUS = 0 THEN
               LET m_msg = 'Já existe uma NF de numero ', mr_nota.num_nf
               LET m_msg = m_msg CLIPPED, ' serie ', mr_nota.ser_nf, ' sub serie ', mr_nota.ssr_nf
                LET m_msg = m_msg CLIPPED, ' fornecedor ', mr_nota.cod_fornecedor CLIPPED
...
#-----------------------------------#
    ...
    LET lr_cap.seql_lanc_cap = m_seql_lanc_cap
...
#-----------------------------#
   ...
   LET mr_reten_irrf_pg.val_base_calc = mr_ad_mestre.val_tot_nf 
...
#-----------------------------#
   ...
   IF STATUS <> 0 THEN                                                           
      LET m_erro = STATUS USING '<<<<<'                                          
      LET m_msg = 'Erro de status: ',m_erro                                      
      LET m_msg = m_msg CLIPPED, ' inserindo tipo de valor na tab ap_valores.'   
      RETURN FALSE                                                               
   END IF   }                                                                     
