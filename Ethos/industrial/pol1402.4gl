...
#----------------------------#      
   ...
   IF STATUS = 0 THEN
      LET m_msg = 'Item já cadastrado no POL1402'
      CALL _ADVPL_set_property(m_statusbar,"ERROR_TEXT",m_msg)
      RETURN FALSE
   ELSE
      IF STATUS <> 100 THEN
...
#----------------------#
    ...
    IF m_construct IS NULL THEN
       ...
       CALL _ADVPL_set_property(m_construct,"ADD_VIRTUAL_COLUMN","item_bloqueio_547","cod_item","Código",1 {CHAR},15,0)
       CALL _ADVPL_set_property(m_construct,"ADD_VIRTUAL_COLUMN","item_bloqueio_547","den_item","Descrição",1 {CHAR},76,0)
    END IF
   ...
   FOREACH cq_query INTO ma_item[l_ind].*
      ...
      IF l_ind > 5000 THEN
         LET m_msg = 'Limite de linhas da grade ultrapassou\n',
                     'Serão exibidos apenas 5000 registros.'
