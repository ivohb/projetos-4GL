...
#MODULARES
   ...
   DEFINE m_qtd_itens_por_pag     SMALLINT, #Qtde de itens por página do demonstrativo (default 18)
          ...
          m_qtd_itens_demonst_pdf SMALLINT  #Qtde de itens que serão impressos no demonstrativo,
                                            #utilizado na função cre1055_carrega_demonstrativo_detalhado (Default 41)
...
MAIN
   ...
   LET p_versao = 'CRE1055-10.02.00' #Favor nao alterar esta linha (SUPORTE)
   LET p_cod_programa = "CRE1055"

   WHENEVER ERROR CONTINUE
      CALL log1400_isolation()
      LET m_caminho = log140_procura_caminho('cre1055.iem')
   ...
   IF p_status = 0 THEN
      CALL cre1055_acha_parcre()
      CALL cre1055_controle()
   END IF
END MAIN

#-----------------------------#
 FUNCTION cre1055_acha_parcre()
#-----------------------------#
 ...
 CALL cre1055_verifica_data_corte_logix_10()

 END FUNCTION

#-----------------------------------------------#
 FUNCTION cre1055_verifica_data_corte_logix_10()
#-----------------------------------------------#
...
#--------------------------#
   ...
   LET comando = log130_procura_caminho("cre10552")

   OPEN WINDOW w_cre10552 AT 2,2 WITH FORM comando
   ...
   CALL cre1055_consulta_sistema()

   CURRENT WINDOW IS w_cre10552

   MENU "OPÇÃO"
      COMMAND KEY ("I") "Informar" "Informar parâmetros para processar a atualização de documentos."
         MESSAGE " "

         IF log005_seguranca(p_user,'CRECEBER','CRE1055','IN') THEN
            IF  cre1055_informa_selecao_atualizacao() THEN
                IF log0040_confirm(12,20,' Processar atualização de documentos ? ') THEN
                    CALL cre1055_processa_selecao_atualizacao()
                END IF
            END IF
         END IF
      ...
      COMMAND KEY ("R") "processar_Reimpressão" "Processar Reimpressão do(s) bloqueto(s)."
         ...
         IF log005_seguranca(p_user, 'CRECEBER', 'cre1055', 'IN') THEN
            IF cre1055_informa_reimpressao() THEN
               #Variavel Global utilizada nas funções crexxx_calcula_barras()
               LET m_reimpressao = TRUE
               LET m_deleta_caminho_aux = "caminho temporario"
               CALL cre1055_processa_reimpressao()
            END IF
            CLOSE WINDOW w_cre10551
            NEXT OPTION "Fim"
         END IF

      COMMAND 'Listar_erros'
         IF m_tem_erros_temp THEN
            CALL cre1055_lista_erros()
         ELSE
...
########################
#----------------------------------------------#
 FUNCTION cre1055_informa_selecao_atualizacao()
#----------------------------------------------#
   CALL log006_exibe_teclas('01',p_versao)
   CURRENT WINDOW IS w_cre10552
   ...
   INPUT BY NAME mr_selecao.* WITHOUT DEFAULTS
      ...
      AFTER FIELD todos_tip_docum
         IF mr_selecao.todos_tip_docum = 'N' THEN
            ...
            ELSE
               IF cre1055_verifica_tip_docum() = FALSE THEN
...
#-------------------------------------#
 FUNCTION cre1055_verifica_tip_docum()
#-------------------------------------#
...
#-----------------------------------------------#
 FUNCTION cre1055_processa_selecao_atualizacao()
#-----------------------------------------------#
   ...
   IF g_todos_sistemas = "N" THEN
      ...
      IF l_count > 0 THEN
         LET sql_stmt  = sql_stmt CLIPPED, ", cre_compl_docum, cre_popup_sist_781"#t_sistema "
         LET sql_where = sql_where CLIPPED,
                         " AND cre_compl_docum.empresa = docum.cod_empresa ",

                         " AND cre_compl_docum.docum   = docum.num_docum ",
                         " AND cre_compl_docum.tip_docum = docum.ies_tip_docum ",
                         " AND cre_compl_docum.sistema_gerador = cre_popup_sist_781.sistema ",#t_sistema.sistema_gerador "
   ...
   FOREACH cq_cre9300 INTO lr_cre9300.empresa,
      ...
      IF NOT cre1055_verifica_emite_boleto(lr_cre9300.portador, 'B') THEN
         CONTINUE FOREACH
      END IF
      #---fim----OS 719260  #
...
#-----------------------------------#
 FUNCTION cre1055_informa_impressao()
#-----------------------------------#
   ...
   CALL log006_exibe_teclas("01",p_versao)
   LET comando = log130_procura_caminho("cre1055")

   OPEN WINDOW w_cre1055 AT 2,2 WITH FORM comando
      ATTRIBUTE(BORDER,MESSAGE LINE LAST,PROMPT LINE LAST)

   LET m_total_ordena = 0

   IF cre1055_sist_gerad_ordena_impressao() = FALSE THEN
      RETURN
   END IF
   ...
   IF mr_selecao.todos_portadores = 'N' THEN
      CALL cre1055_carrega_portadores()
   END IF
   ...
   INPUT BY NAME mr_tela1.* WITHOUT DEFAULTS
      ...
      AFTER FIELD empresa
         IF mr_tela1.empresa IS NOT NULL THEN
            IF cre1055_verifica_empresa(mr_tela1.empresa) = FALSE THEN
      ...
      AFTER FIELD tip_portador
         ...
         IF mr_tela1.portador IS NOT NULL THEN
            IF mr_tela1.tip_portador NOT MATCHES '[BCRE]' THEN
               ...
               NEXT FIELD portador
            ELSE
               CALL cre1055_verifica_portador(mr_tela1.empresa, mr_tela1.portador, mr_tela1.tip_portador)
               RETURNING l_status
            ...
            #--inicio--OS 719260 Jefe #
            #Potador parametrizado para não emitir boleto.Ver cre0340.
            IF NOT cre1055_verifica_emite_boleto(mr_tela1.portador, mr_tela1.tip_portador) THEN
...
#------------------------------------#
 FUNCTION cre1055_carrega_portadores()
#------------------------------------#
...
#-------------------------------------#
 FUNCTION cre1055_informa_reimpressao()
#-------------------------------------#
   ...
   CALL log130_procura_caminho("cre10551") RETURNING comando
   LET comando = log130_procura_caminho("cre10551")

   OPEN WINDOW w_cre10551 AT 2,2 WITH FORM comando
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)

   DISPLAY mr_tela2.*

   CALL cre1055_cria_temp()
   ...
   INPUT BY NAME mr_tela2.* WITHOUT DEFAULTS
      ...
      AFTER FIELD empresa
         IF mr_tela2.empresa IS NOT NULL THEN
            IF cre1055_verifica_empresa(mr_tela2.empresa) = FALSE THEN
               ERROR "Empresa não cadastrada."
               NEXT FIELD empresa
            END IF
         END IF
      ...
      AFTER FIELD ies_num_docum
         IF mr_tela2.ies_num_docum = "N" THEN
            IF cre1055_popup_docum() = FALSE THEN
               CALL log006_exibe_teclas("01",p_versao)
               CURRENT WINDOW IS w_cre10551
      ...
      AFTER FIELD tip_portador
         ...
         IF m_tem_dados = FALSE THEN
            ...
            ELSE
               ...
               CALL cre1055_verifica_portador(mr_tela2.empresa, mr_tela2.portador,mr_tela2.tip_portador)
               RETURNING l_status
               IF NOT l_status THEN
                  LET INT_FLAG = 1
                  EXIT INPUT
      ...
      AFTER FIELD mes_competencia
         ...
         ELSE
            IF NOT cre1055_trata_mes_competencia() THEN
...
#-------------------------------------------#
 FUNCTION cre1055_verifica_empresa(l_empresa)
#-------------------------------------------#
...
#----------------------------------------------------------------------------#
 FUNCTION cre1055_verifica_portador(l_cod_empresa, l_portador, l_tip_portador)
#----------------------------------------------------------------------------#
...
#---------------------------------------------------#
 FUNCTION cre1055_verifica_desconto_base()
#---------------------------------------------------#
 ...
 WHENEVER ERROR CONTINUE
    SELECT natureza_operacao
      INTO l_nat_oper
      FROM cre_compl_docum
     WHERE empresa    = mr_docum.cod_empresa #497350 p_cod_empresa
        AND docum     = mr_relat.num_docum
        AND tip_docum = mr_docum.ies_tip_docum
 WHENEVER ERROR STOP

 IF sqlca.sqlcode <> 0 THEN
    CALL log003_err_sql('LEITURA','cre_compl_docum')
    RETURN FALSE
 ELSE
...
#------------------------------------#
 FUNCTION cre1055_acha_par_bloqueto() #frank aqui carrega os valores da função para representante
#------------------------------------#
...
#############fim do controle para portador representante
  ...
  IF l_existe THEN
     ...
     IF NOT cre1055_valida_portador_representante() THEN
        RETURN FALSE
     END IF
...
#------------------------------------------------#
 FUNCTION cre1055_valida_portador_representante()
#------------------------------------------------#
...
#------------------------------------#
 FUNCTION cre1055_processa_impressao()
#------------------------------------#
 DEFINE l_cont                      SMALLINT                            ,
        l_laser                     SMALLINT                            ,
        l_selecao                   SMALLINT                            ,
 ...
 IF NOT cre1055_cria_temp_processamento() THEN
    ERROR "Processamento cancelado."
    RETURN
 END IF

 IF NOT cre1055_carrega_textos_retencao() THEN
    ERROR "Processamento cancelado."
    RETURN
 END IF
 ...
 LET l_sql_stmt  = " INSERT INTO t_impressao        ",
                   " SELECT d.cod_empresa         , ",      {2}
                          ...
                          " o.val_desc_comercial, ",        {58}
                          " o.val_cons_pesquisa , ",        {59}
                          " o.val_outro_desc    , ",        {60}
                          " o.val_desc_pamcard  , ",        {61}
                          " o.val_credito       , ",        {62}
                          " o.val_debito        , ",        {63}
                          " o.val_acumulado_ant , ",        {64}
                          " o.val_honorar       , ",        {65}
                          " o.pct_irrf          , ",        {66}
                          " o.val_irrf          , ",        {67}
                          " o.tip_portador_pagto, ",        {68}
                          " o.dat_bilhetag      , ",        {69}
                          " o.endereco_cobranca , ",        {70}
                          " o.compl_end_cobranca, ",        {71}
                          " o.bairro_cobranca   , ",        {72}
                          " o.cidade_cobranca   , ",        {73}
                          " o.estado_cobranca   , ",        {74}
                          " o.cep_cobranca      , ",        {75}
                          " o.sucursal          , ",        {76}
                          " o.moeda_fatura      , ",        {77}
                          ...
                          " o.estado_etiq       , ",        {89}
                          " o.cep_etiq          , ",        {90}
                          " o.caixa_postal_etiq , ",        {91}
                          " o.obs_endereco_etiq , ",        {92}
                          " o.tip_contrato      , ",        {93}
                          " o.meio_envio        , ",        {94}
                          " o.roteiro           , ",        {95}
                          " o.dirigente_cliente , ",        {96}
                          " o.geren_cta_cliente , ",        {97}
                          " o.dirigente_negocio , ",        {98}
                          " o.geren_cta_negocio , ",        {99}
                          " o.ramo_ativ         , ",        {100}
                          " o.corretor          , ",        {101}
                          " o.contrato_agrupado , ",        {102}
                          " o.empresa_item      , ",        {103}
                          " o.item_preco_minimo , ",        {104}
                          " o.qtd_preco_minimo  , ",        {105}
                          " o.val_tot_pre_minimo  ",        {106}
                     " FROM docum_banco b, docum d, cre_compl_docum o "
 ...
 LET l_sql_stmt = l_sql_stmt CLIPPED,
                    " WHERE b.num_titulo_banco   IS NULL ",
                      ...
                      " AND o.docum               = d.num_docum ",
                      " AND o.tip_docum           = d.ies_tip_docum ",
 ...
 FOR l_ind = 1 TO 3
    ...
    WHENEVER ERROR CONTINUE
    DECLARE cq_cre1055_001 CURSOR WITH HOLD FOR var_query_002
    WHENEVER ERROR STOP
    IF sqlca.sqlcode <> 0 THEN
       CALL log003_err_sql("DECLARE","CQ_CRE1055_001")
       RETURN
    END IF

    WHENEVER ERROR CONTINUE
    FOREACH cq_cre1055_001 INTO mr_docum.*,
                                mr_cre_compl_docum.*
    WHENEVER ERROR STOP
       IF sqlca.sqlcode <> 0 THEN
          CALL log003_err_sql("foreach","cq_cre1055_001")
          RETURN
       END IF
 ...
 FOREACH c_lista_bloqueto1 INTO mr_docum.*,
     ...
     IF  cre1055_acha_par_bloqueto() = FALSE THEN
         LET l_laser = FALSE
         EXIT FOREACH
     END IF

     IF NOT cre1055_verifica_parametro_geracao_pdf() THEN
        {Mensagem de parâmetros não cadastrados já mostradas dentro da função.}
        EXIT FOREACH
     END IF

     CALL cre1055_impr_ext_bloq()
 ...
 CALL cre1055_data_extenso_carta()
 ...
 FOREACH c_lista_bloqueto3 INTO mr_docum.*,
    ...
    CALL cre1055_processa_dados()

    IF m_erro_docum THEN
       CONTINUE FOREACH
    END IF

    CALL cre1055_processa_laser()
    CALL cre1055_retencao_381()
    CALL cre1055_impr_ext_bloq()
    ...
    IF m_emitiu = TRUE THEN
       IF l_roteiro_anterior = m_roteiro_especifico THEN
          IF mr_docum.cod_cliente <> m_cod_cliente_ant THEN
             ...
             CALL cre1055_emite_carta_envio(l_filial_cobranca_anterior ,
    ...
    #impressao cre1055_emite_carta_envio() quando o FOREACH ja passou pro
    #registro da proxima carta. Por isso no final quando ja acabou o FOREACH, ele
    #chama a funcao cre1055_emite_carta_envio novamente. Fora do FOREACH ele
    ...
    #de impressao da carta (cre1055_emite_carta_envio)
    LET m_cod_cliente_ant = mr_docum.cod_cliente

    IF cre1055_insere_informacao_carta() = FALSE THEN
       CONTINUE FOREACH
    END IF

    IF cre1055_verifica_cnpj_item_igual_documento(mr_cre_compl_docum.empresa   ,
    ...
    OR mr_cre_compl_docum.contrato_agrupado = "N" THEN
       CALL cre1055_calcula_qtd_itens_demonstrativo(TRUE)
    ELSE
       CALL cre1055_calcula_qtd_itens_demonstrativo(FALSE)
    END IF
    ...
    AND l_verifica_cnpj_igual                = FALSE THEN
       ...
       CALL cre1055_calcula_qtd_pag_total(mr_cre_compl_docum.empresa   ,
                                          mr_cre_compl_docum.docum     ,
       ...
       WHENEVER ERROR CONTINUE
        DECLARE cq_contrato_agrupado_reimp CURSOR FOR
         SELECT DISTINCT(cnpj_cli_agrupado)
           FROM cre_itcompl_docum
          WHERE cre_itcompl_docum.empresa   = mr_cre_compl_docum.empresa
            AND cre_itcompl_docum.docum     = mr_cre_compl_docum.docum
            AND cre_itcompl_docum.tip_docum = mr_cre_compl_docum.tip_docum
            AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
            AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
       WHENEVER ERROR STOP
       IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
       ...
       WHENEVER ERROR STOP
          ...
          CALL cre1055_carrega_demonstrativo_detalhado(l_cnpj_cli_agrupado)
    ...
    IF m_tem_demonst_agrupado = FALSE THEN
       CALL cre1055_calcula_qtd_pag_so_bloqueto(mr_cre_compl_docum.empresa   ,
                                                mr_cre_compl_docum.docum     ,
                                                mr_cre_compl_docum.tip_docum )
    END IF
    ...
    IF m_imprime_bloqueto = "D" THEN
       CALL cre1055_carrega_demonstrativo_resumo()
    END IF

    IF NOT cre1055_imprime_bloqueto_resumo() THEN
       CALL log0030_mensagem("Erro ao imprimir Bloqueto em PDF.","exclamation")
    ELSE
       CALL cre1055_grava_cre_docum_compl(mr_cre_compl_docum.empresa  ,
                                       mr_cre_compl_docum.docum    ,
                                       mr_cre_compl_docum.tip_docum)
    END IF
    ...
    IF cre1055_update_par_bloq() = FALSE THEN
       LET m_houve_erro = TRUE
       CALL log085_transacao("ROLLBACK")
       EXIT FOREACH
    END IF

    IF cre1055_processa_docum_banco() = FALSE THEN
       LET m_houve_erro = TRUE
       CALL log085_transacao("ROLLBACK")
       EXIT FOREACH
    END IF
 ...
 IF m_emitiu = TRUE THEN
    ...
    ELSE
       IF m_total_ordena <> 0 THEN
          ...
          #CALL cre1055_caminho_carta(m_caminho_diretorio_pdf, "PDF")
          #-----
          ...
          #CALL cre1055_caminho_carta(m_caminho_carta_envio, "TXT")
          #-----
       END IF
    END IF
 END IF
 ...
 IF m_tem_cartas = FALSE  THEN
    CALL cre1055_imprime_boletos_sem_carta()
 END IF
...
#------------------------------#
FUNCTION cre1055_impr_ext_bloq()
#------------------------------#
 DEFINE l_sistema LIKE cre_compl_docum.sistema_gerador
   ...
   ELSE
       WHENEVER ERROR CONTINUE
          SELECT cre_compl_docum.sistema_gerador
            INTO l_sistema
            FROM cre_compl_docum, cre_popup_sist_781 #t_sistema
           WHERE cre_compl_docum.empresa         = mr_docum.cod_empresa
             AND cre_compl_docum.docum           = mr_docum.num_docum
             AND cre_compl_docum.tip_docum       = mr_docum.ies_tip_docum
             #AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
             #AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
             AND cre_compl_docum.sistema_gerador = cre_popup_sist_781.sistema #Os 451380
             AND cre_popup_sist_781.usuario      = p_user
             AND cre_popup_sist_781.sit_sistema  = 'S'
             AND cre_popup_sist_781.programa     = p_cod_programa
...
#-------------------------------#
 FUNCTION cre1055_processa_laser()
#-------------------------------#
 ...
 INITIALIZE l_pct_csll, l_pct_pis, l_pct_cofins,
  ...
  #---------------------------# Fim
   ...
   ELSE
      LET mr_relat_imp.unid_feder = cre1055_sigla_unidade_federacao(mr_cre_compl_docum.estado_cobranca)
   END IF
   #OS455477
   ...
   LET l_taxa_multa = cre1055_busca_multa()
...
#    CALL cre1055_consistencias(l_pct_csll, l_pct_pis, l_pct_cofins)
   ...
   #aqui desativado pq passou a receber 7
...
#----------------------------------------------#
 FUNCTION cre1055_carrega_demonstrativo_resumo()
#----------------------------------------------#
  ...
  CALL cre1055_verifica_classif_premio_minimo()
  CALL cre1055_total_itens_por_demonstrativo()
  ...
  WHENEVER ERROR CONTINUE
   DECLARE cq_cre_itcompl_docum CURSOR FOR
    SELECT DISTINCT(cre_itcompl_docum.praca),
           cre_classif_item.classif_item,
           cre_classif_item.tip_consulta,
           SUM(cre_itcompl_docum.qtd_item),
           SUM(cre_itcompl_docum.val_tot_item),
           cre_itcompl_docum.item
      FROM cre_itcompl_docum, cre_classif_item
     WHERE cre_itcompl_docum.empresa    = mr_docum.cod_empresa
       AND cre_itcompl_docum.docum      = mr_docum.num_docum
       AND cre_itcompl_docum.tip_docum  = mr_docum.ies_tip_docum
       AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
       AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
       AND cre_classif_item.empresa     = cre_itcompl_docum.empresa
       AND cre_classif_item.item        = cre_itcompl_docum.item
     GROUP BY cre_itcompl_docum.praca,
              cre_classif_item.classif_item,
              cre_classif_item.tip_consulta,
              cre_itcompl_docum.item
     ORDER BY cre_itcompl_docum.praca,
              cre_classif_item.classif_item,
              cre_classif_item.tip_consulta
  WHENEVER ERROR STOP
  IF sqlca.sqlcode <> 0 THEN
     CALL log003_err_sql('SELECAO','CRE_ITCOMPL_DOCUM1')
     RETURN
  END IF

  WHENEVER ERROR CONTINUE
   FOREACH cq_cre_itcompl_docum
      INTO l_item_praca, l_item_classif_item, l_tip_consulta, l_item_qtd_tot_item, l_item_val_tot_item, l_item
  WHENEVER ERROR STOP
     IF sqlca.sqlcode <> 0 THEN
        CALL log003_err_sql('FOREACH','CQ_CRE_ITCOMPL_DOCUM')
        RETURN
     END IF
     ...
     IF l_praca_ant <> l_item_praca THEN
        ...
        LET ma_demonst_item[l_ind].praca = cre1055_get_den_praca(l_item_praca)
  ...
  IF m_verifica_tip_consulta_pdf = TRUE THEN
     ...
     END IF
        ...
        IF l_praca_ant <> l_item_praca THEN
           ...
           LET l_den_praca = cre1055_get_den_praca(l_item_praca)
  ...
  OR l_achou_pm    = TRUE THEN
     ...
     WHENEVER ERROR STOP
     IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
        CALL log003_err_sql("SELECT","cre_compl_docum")
     END IF
     ...
     #::: Valores de INSS
     CALL cre1055_busca_valores_de_inss( mr_docum.cod_empresa   ,
                                         mr_docum.num_docum     ,
  ...
  LET mr_demonst.val_end_4_cob      = 'CEP ', mr_relat.cod_cep CLIPPED, '   ', cre1055_contato_etiqueta() CLIPPED
  #-----

 END FUNCTION

#-----------------------------------#
 FUNCTION cre1055_contato_etiqueta()
#-----------------------------------#
...
#---------------------------------#
 FUNCTION cre1055_busca_historico()
#---------------------------------#
...
#--------------------------------------#
 FUNCTION cre1055_processa_reimpressao()
#--------------------------------------#
 ...
 LET l_sql_stmt =  " d.cod_empresa         ,",          {2}
                 " d.num_docum           ,",            {3}
                 " d.ies_tip_docum       ,",            {4}
                 " d.dat_emis            ,",            {5}
                 " d.dat_vencto_c_desc   ,",            {6}
                 ...
                 " d.num_lote_lanc_cont  ,",            {52}
                 " d.dat_atualiz         ,",            {53}
                 " b.num_titulo_banco    ,",            {54}
                 " o.empresa             ,",            {55}
                 " o.docum               ,",            {56}
                 " o.tip_docum           ,",            {57}
                 " o.sistema_gerador     ,",            {58}
                 " o.val_desc_comercial  ,",            {59}
                 " o.val_cons_pesquisa   ,",            {60}
                 " o.val_outro_desc      ,",            {61}
                 " o.val_desc_pamcard    ,",            {62}
                 " o.val_credito         ,",            {63}
                 " o.val_debito          ,",            {64}
                 " o.val_acumulado_ant   ,",            {65}
                 " o.val_honorar         ,",            {66}
                 " o.pct_irrf            ,",            {67}
                 " o.val_irrf            ,",            {68}
                 " o.tip_portador_pagto  ,",            {69}
                 " o.dat_bilhetag        ,",            {70}
                 " o.endereco_cobranca   ,",            {71}
                 " o.compl_end_cobranca  ,",            {72}
                 " o.bairro_cobranca     ,",            {73}
                 " o.cidade_cobranca     ,",            {74}
                 " o.estado_cobranca     ,",            {75}
                 " o.cep_cobranca        ,",            {76}
                 " o.sucursal            ,",            {77}
                 " o.moeda_fatura        ,",            {78}
                 " o.filial_cobranca     ,",            {79}
                 " o.filial_admin        ,",            {80}
                 " o.tip_nota_fiscal     ,",            {81}
                 " o.tip_cli_contrato    ,",            {82}
                 " o.grupo_economico     ,",            {83}
                 " o.natureza_operacao   ,",            {84}
                 " o.cond_pagto          ,",            {85}
                 " o.endereco_etiq       ,",            {86}
                 " o.compl_end_etiq      ,",            {87}
                 " o.bairro_etiq         ,",            {88}
                 ...
                 " o.estado_etiq         ,",            {90}
                 " o.cep_etiq            ,",            {91}
                 " o.caixa_postal_etiq   ,",            {92}
                 " o.obs_endereco_etiq   ,",            {93}
                 " o.tip_contrato        ,",            {94}
                 " o.meio_envio          ,",            {95}
                 " o.roteiro             ,",            {96}
                 " o.dirigente_cliente   ,",            {97}
                 " o.geren_cta_cliente   ,",            {98}
                 " o.dirigente_negocio   ,",            {99}
                 " o.geren_cta_negocio   ,",            {100}
                 " o.ramo_ativ           ,",            {101}
                 " o.corretor            ,",            {102}
                 " o.contrato_agrupado   ,",            {103}
                 " o.empresa_item        ,",            {104}
                 " o.item_preco_minimo   ,",            {105}
                 " o.qtd_preco_minimo    ,",            {106}
                 " o.val_tot_pre_minimo   ",            {107}
            " FROM docum_banco b, docum d, cre_compl_docum o "
 ...
 FOREACH c_lista_bloqueto2 INTO l_selecao, mr_docum.*, l_num_titulo_banco, lr_cre_compl_docum.*
    ...
    #ESTES VALORES CHEGAVAM ZERADOS NA FUNÇÃO "cre1055_carrega_demonstrativo_resumo()"
    ...
    IF cre1055_verifica_cnpj_item_igual_documento(lr_cre_compl_docum.empresa   ,
    ...
    AND l_verifica_cnpj_igual                = FALSE  THEN
       ...
       CALL cre1055_calcula_qtd_pag_total(lr_cre_compl_docum.empresa   ,
                                          lr_cre_compl_docum.docum     ,
       ...
       WHENEVER ERROR CONTINUE
        DECLARE cq_contrato_agrupado CURSOR FOR
         SELECT DISTINCT(cnpj_cli_agrupado)
           FROM cre_itcompl_docum
          WHERE cre_itcompl_docum.empresa   = lr_cre_compl_docum.empresa
            AND cre_itcompl_docum.docum     = lr_cre_compl_docum.docum
            AND cre_itcompl_docum.tip_docum = lr_cre_compl_docum.tip_docum
            AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
            AND   YEAR(cre_itcompl_docum.dat_servico) =  YEAR(mr_docum.dat_competencia))
       WHENEVER ERROR STOP
       IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
    ...
    IF m_tem_demonst_agrupado = FALSE THEN
       CALL cre1055_calcula_qtd_pag_so_bloqueto(lr_cre_compl_docum.empresa   ,
                                                lr_cre_compl_docum.docum     ,
                                                lr_cre_compl_docum.tip_docum )
    END IF
    ...
    ELSE
       ...
       CALL cre1055_grava_cre_docum_compl(lr_cre_compl_docum.empresa   ,
                                          lr_cre_compl_docum.docum     ,
                                          lr_cre_compl_docum.tip_docum )
    END IF
...
#---------------------------------------------------------------#
 FUNCTION cre1055_verifica_cnpj_item_igual_documento(l_empresa  ,
                                                     l_docum    ,
                                                     l_tip_docum)
#---------------------------------------------------------------#
 ...
 WHENEVER ERROR CONTINUE
   SELECT COUNT(cnpj_cli_agrupado)
     INTO l_count
     FROM cre_itcompl_docum
    WHERE cre_itcompl_docum.empresa   = l_empresa
      AND cre_itcompl_docum.docum     = l_docum
      AND cre_itcompl_docum.tip_docum = l_tip_docum
      AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
      AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
 WHENEVER ERROR STOP
 IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
 ...
 WHENEVER ERROR CONTINUE
   SELECT COUNT(cnpj_cli_agrupado)
    ...
    WHERE cre_itcompl_docum.empresa           = l_empresa
      ...
      AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
      AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
      AND cre_itcompl_docum.cnpj_cli_agrupado = mr_relat.num_cgc_cpf
 WHENEVER ERROR STOP
 IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
    CALL log003_err_sql("SELECT","CRE_ITCOMPL_DOCUM")
 END IF
...
#--------------------------------#
 FUNCTION cre1055_processa_dados()
#--------------------------------#
 ...
 IF mr_docum.dat_emis < m_data_corte_contabilidade_L10 THEN
    ...
    END IF
 ELSE
    IF NOT cre1055_busca_dados_logix_10() THEN
 ...
 CALL cre1055_busca_endereco_cliente()
...
#-----------------------------------------#
 FUNCTION cre1055_busca_endereco_cliente()
#-----------------------------------------#
 ...
 IF sqlca.sqlcode <> 0 THEN
    IF sqlca.sqlcode = 100 THEN
       ...
       CALL cre1055_grava_temp_erros()
    ELSE
       CALL log003_err_sql('SELEÇÃO','CRE_COMPL_DOCUM')
...
#------------------------------------------#
 FUNCTION cre1055_cria_temp_processamento()
#------------------------------------------#
...
#-----------------------------------#
 FUNCTION cre1055_grava_temp_erros()
#-----------------------------------#
 ...
 IF log0280_saida_relat(18,35) IS NOT NULL THEN
    ...
    IF g_ies_ambiente = "W" THEN
       IF p_ies_impressao = "S" THEN
          ...
          LET m_caminho = m_caminho CLIPPED, "cre1055.tmp"
          START REPORT cre1055_relat_erros TO m_caminho
       ELSE
          START REPORT cre1055_relat_erros TO p_nom_arquivo
       END IF
    ELSE
       IF p_ies_impressao = "S" THEN
          START REPORT cre1055_relat_erros TO PIPE p_nom_arquivo
       ELSE
          START REPORT cre1055_relat_erros TO p_nom_arquivo
       END IF
    END IF
    ...
    END IF
       ...
       OUTPUT TO REPORT cre1055_relat_erros ( lr_relat_erros.* )

    END FOREACH

    FREE cq_relat_erros

    FINISH REPORT cre1055_relat_erros
...
#--------------------------------------#
 FUNCTION cre1055_processa_docum_banco()
#--------------------------------------#
...
#----------------------------------#
 FUNCTION cre1055_update_par_bloq()
#----------------------------------#
...
#------------------------------------------#
 REPORT cre1055_relat_erros(lr_relat_erros)
#------------------------------------------#
...
#--------------------------------#
 ...
 IF l_parametro_texto = 'faturamento' OR l_parametro_texto = 'hibrido_fat' THEN
    LET l_val_csll   = cre1055_get_cre_docum_compl_valor('val_csll')
    LET l_val_pis    = cre1055_get_cre_docum_compl_valor('val_pis')
    LET l_val_cofins = cre1055_get_cre_docum_compl_valor('val_cofins')
 END IF

 LET l_pct_csll            = cre1055_get_cre_docum_compl_valor('pct_csll')
 LET l_pct_pis             = cre1055_get_cre_docum_compl_valor('pct_pis')
 LET l_pct_cofins          = cre1055_get_cre_docum_compl_valor('pct_cofins')
 LET m_val_csll_retencao   = cre1055_get_cre_docum_compl_valor('val_csll')
 LET m_val_pis_retencao    = cre1055_get_cre_docum_compl_valor('val_pis')
 LET m_val_cofins_retencao = cre1055_get_cre_docum_compl_valor('val_cofins')
 LET m_pct_irrf            = cre1055_get_cre_docum_compl_valor('pct_irrf')
 LET m_val_irrf            = cre1055_get_cre_docum_compl_valor('val_irrf')
 LET m_val_iss_retencao    = cre1055_get_cre_docum_compl_valor('val_iss_retencao')
 LET m_pct_iss             = cre1055_get_cre_docum_compl_valor('pct_iss')
 ...
 LET l_val_base = cre1055_get_cre_docum_compl_valor('val_base')
 ...
 IF m_val_iss_retencao > 0 AND m_pct_iss > 0 THEN #497350
    ...
    IF NOT cre1055_verifica_desconto_base() THEN
...
#-----------------------------------#
 FUNCTION cre1055_get_cre_docum_compl_valor(l_param)
#-----------------------------------#
...
#-----------------------------------#
 FUNCTION cre1055_consulta_sistema()
#-----------------------------------#

 #IF cre1055_cria_temp_sistema() = FALSE THEN
 #   RETURN
 #END IF
 ...
 ELSE
    ...
    #CALL cre1055_carrega_sistema()
    #IF cre1055_informa_sistema() = FALSE THEN
    #   LET g_todos_sistemas = 'S'
    #END IF
...
#----------------------------------#
 FUNCTION cre1055_carrega_sistema()
#----------------------------------#
...
#--------------------------------------------------#
 FUNCTION cre1055_sigla_unidade_federacao(l_estado)
#--------------------------------------------------#
...
#-------------------------------------------------#
 FUNCTION cre1055_verifica_parametro_geracao_pdf()
#-------------------------------------------------#
  ...
  CALL cre1055_busca_parametro_complementar(l_empresa,l_portador,"diretorio_781")
  ...
  IF m_diretorio_pdf IS NULL OR m_diretorio_pdf = " " THEN
     LET l_mensagem = cre1055_mensagem_parametro_nao_informado(l_empresa,l_portador,"diretorio_781")
     CALL log0030_mensagem(l_mensagem,"exclamation")
     RETURN FALSE
  END IF
CALL cre1055_busca_parametro_complementar(l_empresa,l_portador,"mont_nom_arquivo_781")
...
#---------------------------------------------------------------------------#
 FUNCTION cre1055_busca_parametro_complementar(l_empresa,l_portador,l_campo)
#---------------------------------------------------------------------------#
  DEFINE l_empresa      LIKE empresa.cod_empresa,
  ...
  IF sqlca.sqlcode <> 0 THEN
     IF sqlca.sqlcode = 100 THEN
        LET l_mensagem = cre1055_mensagem_parametro_nao_informado(l_empresa,l_portador,l_campo)
        CALL log0030_mensagem(l_mensagem,"exclamation")
     ELSE
...
#-------------------------------------------------------------------------------#
 FUNCTION cre1055_mensagem_parametro_nao_informado(l_empresa,l_portador,l_campo)
#-------------------------------------------------------------------------------#
  DEFINE l_empresa      LIKE empresa.cod_empresa,
         l_portador     LIKE par_bloqueto_laser.cod_portador,
         l_campo        LIKE vdp_par_blqt_compl.campo
...
#---------------------------------------#
 FUNCTION cre1055_imprime_bloqueto_resumo()
#---------------------------------------#
  ...
  FOR l_num_pag = m_qtd_pag_demonst TO 1 step -1
    ...
    LET ma_config[m_ind_pdf].linha = 'valor_docum = ', cre1055_troca_ponto_por_virgula(mr_relat.val_docum, 2)
    ...
    LET m_ind_pdf = m_ind_pdf + 1
    LET l_caminho_pdf = m_diretorio_pdf CLIPPED,cre1055_processa_nome_arquivo()
    ...
    IF m_imprime_bloqueto = 'D' THEN
       ...
       IF m_tem_demonst_agrupado = FALSE THEN
          FOR l_ind_item = 1 TO m_qtd_itens_por_pag
             ...
             AND l_ind_item_aux = 1    THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 0, ma_tela_2[l_ind_item_aux].praca)
             ELSE
                CALL cre1055_preenche_celula_dem(l_ind_item, 0, ma_demonst_item[l_ind_item_aux].praca)
             END IF
             ...
             AND l_ind_item_aux      = 1 THEN
                ...
                CALL cre1055_preenche_celula_dem(l_ind_item, 1, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_sem_desconto, FALSE))
                CALL cre1055_preenche_celula_dem(l_ind_item, 2, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_com_desconto, FALSE))
                #-----
             ELSE
                IF ma_tip_consulta[l_ind_item_aux].qtd_sem_desconto > 0
                OR ma_tip_consulta[l_ind_item_aux].qtd_com_desconto > 0 THEN
                   CALL cre1055_preenche_celula_dem(l_ind_item, 1, cre1055_mostra_valor_demonst({ma_tip_consulta[l_ind_item_aux].qtd_sem_desconto}" ", FALSE))
                   CALL cre1055_preenche_celula_dem(l_ind_item, 2, cre1055_mostra_valor_demonst({ma_tip_consulta[l_ind_item_aux].qtd_com_desconto}" ", FALSE))
                ELSE
                   CALL cre1055_preenche_celula_dem(l_ind_item, 1, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_sem_desconto, FALSE))
                   CALL cre1055_preenche_celula_dem(l_ind_item, 2, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_com_desconto, FALSE))
                END IF
             ...
             AND m_classif_item     = "CONSULTAS"
             AND l_ind_item_aux     = 1 THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 3, cre1055_mostra_valor_demonst(ma_tela_2[l_ind_item_aux].qtd_preco_minimo, FALSE))
             ELSE
                IF ma_tip_consulta[l_ind_item_aux].qtd_sem_desconto > 0
                OR ma_tip_consulta[l_ind_item_aux].qtd_com_desconto > 0 THEN
                   CALL cre1055_preenche_celula_dem(l_ind_item, 3, cre1055_mostra_valor_demonst(ma_tip_consulta[l_ind_item_aux].qtd_sem_desconto +
                                                                                                ma_tip_consulta[l_ind_item_aux].qtd_com_desconto, FALSE))
                ELSE
                   CALL cre1055_preenche_celula_dem(l_ind_item, 3, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_sem_desconto +
                                                                                                ma_demonst_item[l_ind_item_aux].qtd_com_desconto, FALSE))
                END IF
             END IF

             #SE TIVER PREMIO MINIMO, QUANDO O INDICE FOR "1" ESTA CAMPO VAI ESTA = '0'
             CALL cre1055_preenche_celula_dem(l_ind_item, 4, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_carreteiro, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 5, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_agregado, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 6, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_frota, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 7, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_out_func, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 8, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_repesquisa, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 9, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_recad_agr, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 10, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_recad_fro, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 11, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_recad_out_f, FALSE))
             #-----

             IF  m_tem_premio_minimo = TRUE
             AND m_classif_item      = "PESQUISAS"
             AND l_ind_item_aux      = 1 THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 12, cre1055_mostra_valor_demonst(ma_tela_2[l_ind_item_aux].qtd_preco_minimo, FALSE))
             ELSE
                CALL cre1055_preenche_celula_dem(l_ind_item, 12, cre1055_mostra_valor_demonst(ma_demonst_item[l_ind_item_aux].qtd_carreteiro  +
       ...
       IF m_tem_demonst_agrupado = TRUE THEN
          FOR l_ind_item = 1 TO m_qtd_itens_por_pag
             ...
             AND l_ind_item_aux      = 1 THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 0, ma_tela_2[l_ind_item_aux].praca)
             ELSE
                CALL cre1055_preenche_celula_dem(l_ind_item, 0, ma_total_demonst_p_bloqueto[l_ind_item_aux].praca)
             END IF
             #-----

             #SE TIVER PREMIO MINIMO, QUANDO O INDICE FOR "1" ESTA CAMPO VAI ESTA = '0'
             CALL cre1055_preenche_celula_dem(l_ind_item, 1, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_sem_desconto, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 2, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_com_desconto, FALSE))
             #-----
             ...
             AND l_ind_item_aux      = 1 THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 3, cre1055_mostra_valor_demonst(ma_tela_2[l_ind_item_aux].qtd_preco_minimo, FALSE))
             ELSE
                CALL cre1055_preenche_celula_dem(l_ind_item, 3, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_sem_desconto +
                                                                                             ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_com_desconto, FALSE))
             END IF

             #SE TIVER PREMIO MINIMO, QUANDO O INDICE FOR "1" ESTA CAMPO VAI ESTA = '0'
             CALL cre1055_preenche_celula_dem(l_ind_item, 4, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_carreteiro, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 5, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_agregado, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 6, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_frota, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 7, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_out_func, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 8, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_repesquisa, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 9, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_recad_agr, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 10, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_recad_fro, FALSE))
             CALL cre1055_preenche_celula_dem(l_ind_item, 11, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_recad_out_f, FALSE))
             #-----
             ...
             AND l_ind_item_aux      = 1 THEN
                CALL cre1055_preenche_celula_dem(l_ind_item, 12, cre1055_mostra_valor_demonst(ma_tela_2[l_ind_item_aux].qtd_preco_minimo, FALSE))
             ELSE
                CALL cre1055_preenche_celula_dem(l_ind_item, 12, cre1055_mostra_valor_demonst(ma_total_demonst_p_bloqueto[l_ind_item_aux].qtd_carreteiro  +
       ...
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 0, 'TOTAIS')
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 0, 'VALOR  TOTAL')
       ...
       IF l_num_pag = m_qtd_pag_demonst THEN
          ...
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 1, cre1055_mostra_valor_demonst((mr_demonst_totais.qtd_sem_desconto),FALSE))

          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 1, cre1055_troca_ponto_por_virgula((mr_demonst_totais.val_sem_desconto),2))

          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 2, cre1055_mostra_valor_demonst((mr_demonst_totais.qtd_com_desconto),FALSE))

          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 2, cre1055_troca_ponto_por_virgula((mr_demonst_totais.val_com_desconto),2))

          IF  m_tem_premio_minimo = TRUE
          AND m_classif_item      = "CONSULTAS" THEN
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 3, cre1055_mostra_valor_demonst(mr_cre_compl_docum.qtd_preco_minimo,FALSE))
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 3, cre1055_troca_ponto_por_virgula(mr_cre_compl_docum.val_tot_pre_minimo,2))
          ELSE
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 3, cre1055_mostra_valor_demonst((mr_demonst_totais.qtd_sem_desconto +
                                                                                                        mr_demonst_totais.qtd_com_desconto),FALSE))

             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 3, cre1055_troca_ponto_por_virgula((mr_demonst_totais.val_sem_desconto +
                                                                                                           mr_demonst_totais.val_com_desconto),2))
          END IF

          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 4, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_carreteiro,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 4, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_carreteiro,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 5, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_agregado,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 5, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_agregado,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 6, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_frota,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 6, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_frota,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 7, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_out_func,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 7, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_out_func,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 8, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_repesquisa,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 8, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_repesquisa,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 9, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_agr,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 9, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_agr,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 10, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_fro,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 10, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_fro,2))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 11, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_out_f,FALSE))
          CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 11, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_out_f,2))

          IF  m_tem_premio_minimo = TRUE
          AND m_classif_item      = "PESQUISAS" THEN
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 12, cre1055_mostra_valor_demonst(mr_cre_compl_docum.qtd_preco_minimo,FALSE))
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 12, cre1055_troca_ponto_por_virgula(mr_cre_compl_docum.val_tot_pre_minimo,2))
          ELSE
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 1, 12, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_carreteiro +
             ...
             CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag + 2, 12, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_carreteiro +
                                                                                                           mr_demonst_totais.val_agregado   +
                                                                                                           mr_demonst_totais.val_frota      +
                                                                                                           mr_demonst_totais.val_out_func   +
                                                                                                           mr_demonst_totais.val_repesquisa +
       ...
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_extra = ', cre1055_mostra_valor_bloqueto((mr_tip_consulta.val_sem_desconto + mr_tip_consulta.val_com_desconto), l_num_pag, FALSE)
       ...
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_cons_pesq = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_cons_pesq, l_num_pag, FALSE)
       ...
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_outros_desc = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_outros_desc, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_desconto = (-)DESCONTO'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_desconto = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_desconto, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_val_acom = (+) VLR ACUMULADO ANT'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_val_acom = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_val_acom, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_iss = ISS'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_iss_1 = (', cre1055_mostra_valor_bloqueto(mr_demonst.val_iss_1, l_num_pag, TRUE) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_iss_2 = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_iss_2, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_pis = PIS'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_pis_1 = (', cre1055_mostra_valor_bloqueto(mr_demonst.val_pis_1, l_num_pag, TRUE) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_pis_2 = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_pis_2, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_confins = COFINS'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_confins_1 = (', cre1055_mostra_valor_bloqueto(mr_demonst.val_confins_1, l_num_pag, TRUE) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_confins_2 = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_confins_2, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_csll = CSLL'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_csll_1 = (', cre1055_mostra_valor_bloqueto(mr_demonst.val_csll_1, l_num_pag, TRUE) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_csll_2 = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_csll_2, l_num_pag, FALSE)
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_sub_total = SUB TOTAL'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_sub_total = ', cre1055_mostra_valor_bloqueto( mr_demonst.val_sub_total ,
                                                                                           l_num_pag                ,
                                                                                           FALSE                    )
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_irrf = IRRF'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_irrf_1 = (', cre1055_mostra_valor_bloqueto(mr_demonst.val_irrf_1, l_num_pag, TRUE) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_irrf_2 = ', cre1055_mostra_valor_bloqueto(mr_demonst.val_irrf_2, l_num_pag, FALSE)

       #::: Valor de INSS
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_inss = INSS'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_inss_1 = (', cre1055_mostra_valor_bloqueto( mr_demonst.val_inss_1, l_num_pag, TRUE ) CLIPPED, '%)'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_inss_2 = ', cre1055_mostra_valor_bloqueto( mr_demonst.val_inss_2, l_num_pag, FALSE )
       #:::

       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'txt_total_fat = TOTAL DESTA FATURA'
       LET m_ind_pdf = m_ind_pdf + 1
       LET ma_config[m_ind_pdf].linha = 'val_total_fat = ', cre1055_mostra_valor_bloqueto( mr_demonst.val_total_fat ,
                                                                                           l_num_pag                ,
                                                                                           FALSE                    )
    ...
    CALL cre1055_grava_sequencia_carta( l_diretorio_config ,
                                        l_caminho_pdf      ,
                                        "B"                )
...
#--------------------------------#
 FUNCTION cre1055_preenche_celula_dem(l_x, l_y, l_valor)
#--------------------------------#
  DEFINE l_x INTEGER,
         l_y INTEGER,
         l_valor CHAR(40)
...
#--------------------------------#
 FUNCTION cre1055_mostra_valor_demonst(l_valor, l_ies_total)
#--------------------------------#
...
#--------------------------------#
 FUNCTION cre1055_mostra_valor_bloqueto(l_valor, l_num_pag, l_ies_perc)
#--------------------------------#
  {Parâmetros:
  ...
  ELSE
     IF (l_num_pag = m_qtd_pag_demonst) OR (l_ies_perc = true) THEN
        RETURN cre1055_troca_ponto_por_virgula(l_valor,2) #Fixo 2 decimais para valor
     ELSE
        RETURN '******'
     END IF
  END IF
...
#--------------------------------#
 FUNCTION cre1055_troca_ponto_por_virgula(l_valor, l_qtd_decimais)
#--------------------------------#
...
#----------------------------------------#
 FUNCTION cre1055_processa_nome_arquivo()
#----------------------------------------#
  ...
  FOR l_cont=1 TO l_tamanho_nome
     ...
     ELSE
        ...
        OR l_cont = l_tamanho_nome THEN
           ...
           CALL cre1055_trim(l_palavra) RETURNING l_palavra
...
#--------------------------------#
 FUNCTION cre1055_trim(l_palavra)
#--------------------------------#
...
#-------------------------------------------------#
 FUNCTION cre1055_calcula_qtd_itens_demonstrativo(l_parametro)
#-------------------------------------------------#
 ...
 #DESTA FORMA É REALIZADO O SELECT COUNT(DISTINCT cre_itcompl_docum.cnpj_cli_agrupado)

 #SE O PARAMETRO ESTIVER TRUE, SERÁ IMPRESSO SOMENTE O DEMONSTRATIVO RESUMO.
 #SELECT COUNT(DISTINCT cre_itcompl_docum.praca)
 ...
 IF l_parametro = FALSE THEN
    WHENEVER ERROR CONTINUE
       SELECT COUNT(DISTINCT cre_itcompl_docum.cnpj_cli_agrupado)
    ...
    WHENEVER ERROR STOP
    IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> NOTFOUND THEN
       CALL log003_err_sql('SELEÇÃO','CRE_ITCOMPL_DOCUM2')
       LET m_qtd_pag_demonst = 1
       RETURN
    END IF
 ELSE
...
#--------------------------------#
 FUNCTION cre1055_get_den_praca(l_item_praca)
#--------------------------------#
  DEFINE l_item_praca LIKE cre_itcompl_docum.praca
...
#---------------------------------------------#
 FUNCTION cre1055_sist_gerad_ordena_impressao()
#---------------------------------------------#
...
#------------------------------------------#
 FUNCTION cre1055_insere_informacao_carta()
#------------------------------------------#
 ...
 IF NOT cre1055_sistema_gerador_imprime_carta(mr_docum.cod_empresa) THEN
 ...
 LET l_mes_extenso = cre1055_mes_extenso(mr_docum.dat_competencia)
 ...
 IF mr_docum.ies_tip_docum_orig = 'NF' THEN
    ...
    ELSE
       ...
       CALL cre1055_busca_nf_eletronica( mr_docum.cod_empresa   ,
                                         l_nota_fiscal          ,
...
#----------------------------------------------------------#
 FUNCTION cre1055_busca_nf_eletronica( l_cod_empresa      ,
                                       l_num_docum_origem ,
                                       l_ies_serie_fat    )
#----------------------------------------------------------#
...
#-------------------------------------#
 FUNCTION cre1055_data_extenso_carta()
#-------------------------------------#
 DEFINE l_den_munic     LIKE empresa.den_munic,
        l_mes_extenso   CHAR(15)

 IF NOT cre1055_sistema_gerador_imprime_carta(" ") THEN
    RETURN
 END IF
 ...
 LET l_mes_extenso = cre1055_mes_extenso(mr_par_cre.dat_proces_doc)
...
#------------------------------------#
 FUNCTION cre1055_mes_extenso(l_data)
#------------------------------------#
 DEFINE l_data          DATE,
        l_mes_extenso   CHAR(15)
...
#-----------------------------------------------------#
 FUNCTION cre1055_emite_carta_envio(l_filial_cobranca ,
                                    l_meio_envio      ,
                                    l_roteiro         )
#-----------------------------------------------------#
 ...
 START REPORT cre1055_carta_envio TO m_caminho_carta_2_via_aux

 LET l_qtd_registro = 4 #Linha da cidade e data + 2 espaços em branco

 CALL cre1055_busca_nome_cliente()
 ...
 OUTPUT TO REPORT cre1055_carta_envio('CABECALHO_CARTA', lr_carta.*)
 ...
 END IF
    ...
    OUTPUT TO REPORT cre1055_carta_envio('CABECALHO_TEXTO', lr_carta.*)
 ...
 OUTPUT TO REPORT cre1055_carta_envio('CABECALHO_DOCTO', lr_carta.*)

 CALL cre1055_atualiza_tabela_t_carta()

 CALL cre1055_monta_sql_t_carta()
 ...
 END IF
    ...
    CALL cre1055_imprime_boletos_gravados( lr_carta.empresa ,
                                           lr_carta.docum   )

    OUTPUT TO REPORT cre1055_carta_envio('CORPO_CARTA', lr_carta.*)
 ...
 FINISH REPORT cre1055_carta_envio
...
#-------------------------------------------------#
 REPORT cre1055_carta_envio(l_parametro, lr_carta)
#-------------------------------------------------#
...
#---------------------------------------------------------#
 FUNCTION cre1055_busca_cre_imp_item_781(l_valor          ,
                                         l_empresa        ,
                                         l_docum          ,
                                         l_tip_docum      ,
                                         l_item           ,
...
#--------------------------------------------------------------------#
 FUNCTION cre1055_carrega_demonstrativo_detalhado(l_cnpj_cli_agrupado)
#--------------------------------------------------------------------#
 ...
 WHENEVER ERROR CONTINUE
   SELECT COUNT(cre_itcompl_docum.praca)
     INTO l_qtd_registros
     FROM cre_itcompl_docum, cre_classif_item
    WHERE cre_itcompl_docum.empresa           = mr_docum.cod_empresa
      AND cre_itcompl_docum.docum             = mr_docum.num_docum
      AND cre_itcompl_docum.tip_docum         = mr_docum.ies_tip_docum
      AND cre_itcompl_docum.cnpj_cli_agrupado = l_cnpj_cli_agrupado
      AND (MONTH(cre_itcompl_docum.dat_servico) = MONTH(mr_docum.dat_competencia)
      AND  YEAR(cre_itcompl_docum.dat_servico)  = YEAR(mr_docum.dat_competencia))
      AND cre_classif_item.empresa            = cre_itcompl_docum.empresa
      AND cre_classif_item.item               = cre_itcompl_docum.item
 ...
 WHENEVER ERROR CONTINUE
  DECLARE cq_cre_itcompl_docum1 CURSOR FOR
   SELECT DISTINCT(cre_itcompl_docum.praca)  ,
          cre_classif_item.classif_item      ,
          cre_classif_item.tip_consulta      ,
          SUM(cre_itcompl_docum.qtd_item)    ,
          SUM(cre_itcompl_docum.val_tot_item),
          cre_itcompl_docum.empresa          ,
          cre_itcompl_docum.docum            ,
          cre_itcompl_docum.tip_docum        ,
          cre_itcompl_docum.item             ,
          cre_itcompl_docum.sequencia_docum  ,
          cre_itcompl_docum.dat_servico
     FROM cre_itcompl_docum, cre_classif_item
    WHERE cre_itcompl_docum.empresa           = mr_docum.cod_empresa
      AND cre_itcompl_docum.docum             = mr_docum.num_docum
      AND cre_itcompl_docum.tip_docum         = mr_docum.ies_tip_docum
    ...
    GROUP BY cre_itcompl_docum.praca            ,
             ...
             cre_classif_item.classif_item      ,
             cre_classif_item.tip_consulta      ,
             cre_itcompl_docum.qtd_item         ,
             cre_itcompl_docum.val_tot_item     ,
             cre_itcompl_docum.empresa          ,
             cre_itcompl_docum.docum            ,
             cre_itcompl_docum.tip_docum        ,
             cre_itcompl_docum.sequencia_docum  ,
             cre_itcompl_docum.dat_servico
    ORDER BY cre_itcompl_docum.praca, cre_itcompl_docum.item
 WHENEVER ERROR STOP
 IF sqlca.sqlcode <> 0 AND sqlca.sqlcode <> 100 THEN
    CALL log003_err_sql("DECLARE","CQ_CRE_ITCOMPL_DOCUM1")
 END IF


 WHENEVER ERROR CONTINUE
  FOREACH cq_cre_itcompl_docum1 INTO l_item_praca       ,
                                     l_item_classif_item,
                                     l_tip_consulta     ,
                                     l_item_qtd_tot_item,
                                     l_item_val_tot_item,
 ...
 WHENEVER ERROR STOP
 IF sqlca.sqlcode <> 0 THEN
    CALL log003_err_sql('FOREACH','CQ_CRE_ITCOMPL_DOCUM1')
    RETURN
 END IF
    ...
    LET mr_demonst.val_iss_1          = m_pct_iss
    LET mr_demonst.val_iss_2          = cre1055_busca_cre_imp_item_781('val_iss_retencao', l_empresa, l_docum, l_tip_docum, l_item, l_sequencia_docum) #m_val_iss_retencao
    LET mr_demonst.val_pis_1          = m_pct_pis
    LET mr_demonst.val_pis_2          = cre1055_busca_cre_imp_item_781('val_pis-retencao', l_empresa, l_docum, l_tip_docum, l_item, l_sequencia_docum) #m_val_pis_retencao
    LET mr_demonst.val_confins_1      = m_pct_cofins
    LET mr_demonst.val_confins_2      = cre1055_busca_cre_imp_item_781('val_cofins-retencao', l_empresa, l_docum, l_tip_docum, l_item, l_sequencia_docum) #m_val_cofins_retencao
    LET mr_demonst.val_csll_1         = m_pct_csll
    LET mr_demonst.val_csll_2         = cre1055_busca_cre_imp_item_781('val_csll-retencao', l_empresa, l_docum, l_tip_docum, l_item, l_sequencia_docum) #m_val_csll_retencao
    LET mr_demonst.val_irrf_1         = m_pct_irrf
    LET mr_demonst.val_irrf_2         = cre1055_busca_cre_imp_item_781('val_irrf-retencao', l_empresa, l_docum, l_tip_docum, l_item, l_sequencia_docum) #m_val_irrf
  ...
  IF cre1055_imprime_demonstrativo_detalhado() = FALSE THEN
...
#-------------------------------------------------#
 FUNCTION cre1055_imprime_demonstrativo_detalhado()
#-------------------------------------------------#
  DEFINE l_ind2             SMALLINT,
         l_ind_item         SMALLINT,
         l_ind_item_aux     INTEGER,
  ...
  FOR l_num_pag = m_qtd_pag_demonst_pdf TO 1 step -1
    ...
    #Totais do demonstrativo
    CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 0, 'TOTAIS')
    CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 0, 'VALOR  TOTAL')

    #Só imprime o total na última pagina
    IF l_num_pag = m_qtd_pag_demonst_pdf THEN
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 1, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_sem_desconto,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 1, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_sem_desconto,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 2, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_com_desconto,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 2, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_com_desconto,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 3, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_sem_desconto +
                                                                                               mr_demonst_totais.qtd_com_desconto,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 3, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_sem_desconto +
                                                                                               mr_demonst_totais.val_com_desconto,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 4, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_carreteiro,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 4, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_carreteiro,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 5, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_agregado,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 5, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_agregado,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 6, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_frota,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 6, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_frota,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 7, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_out_func,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 7, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_out_func,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 8, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_repesquisa,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 8, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_repesquisa,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 9, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_agr,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 9, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_agr,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 10, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_fro,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 10, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_fro,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 11, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_recad_out_f,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 11, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_recad_out_f,2))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 1, 12, cre1055_mostra_valor_demonst(mr_demonst_totais.qtd_carreteiro +
                                                                                                     mr_demonst_totais.qtd_agregado   +
                                                                                                     mr_demonst_totais.qtd_frota      +
                                                                                                     mr_demonst_totais.qtd_out_func   +
                                                                                                     mr_demonst_totais.qtd_repesquisa +
                                                                                                     ...
                                                                                                     mr_demonst_totais.qtd_recad_fro  +
                                                                                                     mr_demonst_totais.qtd_recad_out_f,FALSE))
       CALL cre1055_preenche_celula_dem(m_qtd_itens_por_pag_pdf + 2, 12, cre1055_troca_ponto_por_virgula(mr_demonst_totais.val_carreteiro +
                                                                                                     mr_demonst_totais.val_agregado   +
                                                                                                     mr_demonst_totais.val_frota      +
                                                                                                     mr_demonst_totais.val_out_func   +
                                                                                                     mr_demonst_totais.val_repesquisa +
    ...
    LET ma_config[m_ind_pdf].linha = 'txt_extra = DEBITO'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_extra = ', cre1055_mostra_valor_demonstrativo_pdf((mr_tip_consulta.val_sem_desconto + mr_tip_consulta.val_com_desconto), l_num_pag, FALSE)
    ...
    LET ma_config[m_ind_pdf].linha = 'txt_cons_pesq = TOTAL CONS+PESQ'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_cons_pesq = ', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_cons_pesq, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_outros_desc = (-)OUTROS DESC'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_outros_desc = ', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_outros_desc, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_desconto = (-)DESCONTO'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_desconto = ', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_desconto, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_val_acom = (+) VLR ACUMULADO ANT'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_val_acom = ', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_val_acom, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_iss = ISS'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_iss_1 = (', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_iss_1, l_num_pag, TRUE) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_iss_2 = ', cre1055_mostra_valor_demonstrativo_pdf(m_val_iss_2, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_pis = PIS'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_pis_1 = (', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_pis_1, l_num_pag, TRUE) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_pis_2 = ', cre1055_mostra_valor_demonstrativo_pdf(m_val_pis_2, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_confins = COFINS'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_confins_1 = (', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_confins_1, l_num_pag, TRUE) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_confins_2 = ', cre1055_mostra_valor_demonstrativo_pdf(m_val_confins_2, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_csll = CSLL'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_csll_1 = (', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_csll_1, l_num_pag, TRUE) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_csll_2 = ', cre1055_mostra_valor_demonstrativo_pdf(m_val_csll_2, l_num_pag, FALSE)
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_sub_total = SUB TOTAL'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_sub_total = ', cre1055_mostra_valor_demonstrativo_pdf( mr_demonst.val_sub_total ,
                                                                                                 l_num_pag                ,
                                                                                                 FALSE                    )
    ...
    LET ma_config[m_ind_pdf].linha = 'txt_irrf = IRRF'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_irrf_1 = (', cre1055_mostra_valor_demonstrativo_pdf(mr_demonst.val_irrf_1, l_num_pag, TRUE) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_irrf_2 = ', cre1055_mostra_valor_demonstrativo_pdf(m_val_irrf_2, l_num_pag, FALSE)

    #::: Valor de INSS
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_inss = INSS'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_inss_1 = (', cre1055_mostra_valor_demonstrativo_pdf( mr_demonst.val_inss_1, l_num_pag, TRUE ) CLIPPED, '%)'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_inss_2 = ', cre1055_mostra_valor_demonstrativo_pdf( mr_demonst.val_inss_2, l_num_pag, FALSE )
    #:::

    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'txt_total_fat = TOTAL DESTA FATURA'
    LET m_ind_pdf = m_ind_pdf + 1
    LET ma_config[m_ind_pdf].linha = 'val_total_fat = ', cre1055_mostra_valor_demonstrativo_pdf( mr_demonst.val_total_fat ,
                                                                                                 l_num_pag                ,
                                                                                                 FALSE                    )
...
#----------------------------------------------------------#
 FUNCTION cre1055_mostra_valor_demonstrativo_pdf(l_valor   ,
                                                 l_num_pag ,
                                                 l_ies_perc)
#----------------------------------------------------------#
  {Parâmetros:
...
#---------------------------------------------------#
 FUNCTION cre1055_calcula_qtd_pag_total(l_empresa   ,
                                        l_docum     ,
                                        l_tip_docum )
#---------------------------------------------------#
 DEFINE l_empresa   LIKE cre_compl_docum.empresa  ,
...
#---------------------------------------------------------#
 FUNCTION cre1055_calcula_qtd_pag_so_bloqueto(l_empresa   ,
                                              l_docum     ,
                                              l_tip_docum )
#---------------------------------------------------------#
 DEFINE l_empresa   LIKE cre_compl_docum.empresa  ,
 ...
 WHENEVER ERROR STOP
 IF sqlca.sqlcode <> 0 THEN
    CALL log003_err_sql('SELECAO','CRE_ITCOMPL_DOCUM3')
 END IF
...
#-----------------------------------------------#
 FUNCTION cre1055_total_itens_por_demonstrativo()
#-----------------------------------------------#
 ...
 #528048 ->
 LET sql_stmt = " SELECT SUM(cre_itcompl_docum.qtd_item) ",
                "   FROM cre_itcompl_docum, cre_classif_item ",
                "  WHERE cre_itcompl_docum.empresa     = '",mr_docum.cod_empresa,"' ",
                "    AND cre_itcompl_docum.docum       = '",mr_docum.num_docum,"' ",
                "    AND cre_itcompl_docum.tip_docum   = '",mr_docum.ies_tip_docum,"' ",
                "    AND cre_itcompl_docum.item       <> '",mr_cre_compl_docum.item_preco_minimo,"' ",
                "    AND cre_itcompl_docum.item        = cre_classif_item.item ",
                "    AND cre_classif_item.empresa      = cre_itcompl_docum.empresa "
...
#------------------------------------------------#
 FUNCTION cre1055_verifica_classif_premio_minimo()
#------------------------------------------------#
...
#---------------------------------------------------#
 FUNCTION cre1055_grava_cre_docum_compl(l_empresa  ,
                                        l_docum    ,
                                        l_tip_docum)
#---------------------------------------------------#
...
#------------------------------------------#
 FUNCTION cre1055_carrega_textos_retencao()
#------------------------------------------#
{USO INTERNO
...
#---------------------------------------------------------#
 FUNCTION cre1055_busca_valores_de_inss( l_cod_empresa   ,
                                         l_num_docum     ,
                                         l_ies_tip_docum )
#---------------------------------------------------------#
 DEFINE l_cod_empresa   LIKE docum.cod_empresa
...
#-------------------------------------#
 FUNCTION cre1055_busca_nome_cliente()
#-------------------------------------#
 DEFINE l_nom_cliente LIKE clientes.nom_cliente
...
#------------------------------------------------------------------#
 FUNCTION cre1055_verifica_emite_boleto(l_portador, l_tip_portador)
#------------------------------------------------------------------#
...
#----------------------------------------#
 FUNCTION cre1055_trata_mes_competencia()
#----------------------------------------#
...
#-------------------------------#
  ...
  CALL log130_procura_caminho("cre10556") RETURNING l_comando
    OPEN WINDOW w_cre10556 AT 04,12  WITH FORM l_comando
 ...
 CURRENT WINDOW IS w_cre10556
  ...
  INPUT ARRAY la_num_docum WITHOUT DEFAULTS FROM s_num_docum.*
    ...
    AFTER FIELD portador
         ...
         CALL cre1055_verifica_portador(l_empresa_d, l_portador, "B")
               RETURNING l_status
               IF NOT l_status THEN
                  NEXT FIELD portador
               END IF
     ...
     FOR l_cont = 1 TO 300
        ...
        END IF
         ...
         AND la_num_docum[l_cont].portador      IS NOT NULL THEN
            ...
            CALL cre1055_grava_temp_num_docum(l_empresa_d, l_numero_docum, l_tipo_docum,
                                              l_portador)
         END IF
     END FOR
...
#--------------------------------------------------------------------------------#
 FUNCTION cre1055_grava_temp_num_docum(l_empresa_d, l_numero_docum, l_tipo_docum,
                                       l_portador)
#--------------------------------------------------------------------------------#
...
#------------------------------------#
 FUNCTION cre1055_monta_sql_t_carta()
#------------------------------------#
...
#------------------------------------------#
 FUNCTION cre1055_atualiza_tabela_t_carta()
#------------------------------------------#
...
# por último as NFs com valor nulo ou zero
 ...
 FOR l_ind = 1 TO 3 #aqui
    ...
    LET l_sql_stmt = " SELECT docum              ,",
       ...
       CALL cre1055_processa_atualizacao_temporaria( l_sql_stmt )
...
#--------------------------------------------------------------#
 FUNCTION cre1055_processa_atualizacao_temporaria( l_sql_stmt )
#--------------------------------------------------------------#
 DEFINE l_sql_stmt CHAR(2000)
 DEFINE lr_carta RECORD
        docum               CHAR(14)     ,
...
#-------------------------------------------------------------#
 FUNCTION cre1055_grava_sequencia_carta( l_diretorio_config  ,
                                         l_caminho_impressao ,
                                         l_tipo              )
#-------------------------------------------------------------#
...
#--------------------------------------------------------#
 FUNCTION cre1055_imprime_boletos_gravados( l_empresa   ,
                                            l_documento )
#--------------------------------------------------------#
...
#--------------------------------------------#
 FUNCTION cre1055_imprime_boletos_sem_carta()
#--------------------------------------------#
 DEFINE l_empresa      LIKE docum.cod_empresa
 DEFINE l_documento    LIKE docum.num_docum
 DEFINE l_caminho      CHAR(200)
...
#----------------------------------------------#
 FUNCTION cre1055_busca_dados_logix_10()
#----------------------------------------------#
...
#---------------------------------------------------------#
 FUNCTION cre1055_sistema_gerador_imprime_carta(l_empresa)
#---------------------------------------------------------#
...
#-------------------------------#
 FUNCTION cre1055_version_info()
#-------------------------------#

 RETURN "$Archive: /Logix/Fontes_Doc/Customizacao/10R2/_clientes_sem_guarda/gps_logist_e_gerenc_de_riscos_ltda/financeiro/contas_receber/programas/cre1055.4gl $|$Revision: 30 $|$Date: 06/11/14 12:15 $|$Modtime: 23/05/11 14:07 $" #Informações do controle de versão do SourceSafe - Não remover esta linha (FRAMEWORK)
