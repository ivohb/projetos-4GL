...
MAIN
LET p_versao = "POL0802-05.10.15" #Favor nao alterar esta linha (SUPORTE)
          ...
          ELSE
             ...
             ELSE
                ...
                IF pol0802_verifica_om_cancelada() = FALSE THEN
                   ...
                   NEXT FIELD num_om
                END IF
                IF pol0802_verifica_om_itens_val_zerado() = FALSE THEN
                   ERROR " OM possui item com preço zerado "
                   NEXT FIELD num_om
                END IF
             END IF
          END IF    
...
#-----------------------------------------------#
 FUNCTION pol0802_verifica_om_itens_val_zerado()
#-----------------------------------------------#
DEFINE l_num_ped         LIKE ped_itens.num_pedido,
       l_num_seq         LIKE ped_itens.num_sequencia,
       l_pre_unit        LIKE ped_itens.pre_unit 

  DECLARE cq_om_zer CURSOR FOR
    SELECT num_pedido, num_sequencia
      FROM ordem_montag_item 
     WHERE ordem_montag_item.cod_empresa = p_cod_empresa   
       AND ordem_montag_item.num_om = t1_nf_solicit[pa_curr].num_om
       
  FOREACH  cq_om_zer INTO  l_num_ped, l_num_seq
     SELECT pre_unit
       INTO l_pre_unit
       FROM ped_itens 
      WHERE cod_empresa    =  p_cod_empresa
        AND num_pedido     =  l_num_ped
...
IF pol0802_consiste_controle_ef() THEN 
   ...
   LET p_nf_solicit.ies_situacao  = "C"
	...
	IF l_trans_solic_fatura IS NOT NULL THEN
		...
		DELETE FROM fat_solic_fatura WHERE trans_solic_fatura = l_trans_solic_fatura
		DELETE FROM fat_solic_embal	 WHERE trans_solic_fatura = l_trans_solic_fatura
	END IF
   ...
   FOR pa_curr = 1 TO 200
       IF t1_nf_solicit[pa_curr].num_om IS NOT NULL THEN
          ...
          END IF
			...
			LET lr_fat_solic_fatura.controle			= p_nf_solicit.num_controle
			LET lr_fat_solic_fatura.cond_pagto			= p_nf_solicit.cod_cnd_pgto
			LET lr_fat_solic_fatura.qtd_dia_acre_dupl	= p_nf_solicit.qtd_dias_acr_dupl
			LET lr_fat_solic_fatura.texto_1				= p_nf_solicit.num_texto_1
			LET lr_fat_solic_fatura.texto_2				= p_nf_solicit.num_texto_2
			LET lr_fat_solic_fatura.texto_3				= p_nf_solicit.num_texto_3
			...
			LET lr_fat_solic_fatura.estado_placa_veic	= p_uf_placa
...
#-----------------------------------------#
 DEFINE where_clause, sql_stmt  CHAR(2500)
 LET p_nf_solicitr.* = p_nf_solicit.*
 ...
 LET sql_stmt = "SELECT UNIQUE num_solicit ",
                ...
                " nf_solicit.nom_usuario = """, p_user , """ AND ",
         		" EXISTS (SELECT 1 FROM fat_solic_mestre ",
                "     WHERE empresa = nf_solicit.cod_empresa ",
                "     AND usuario = nf_solicit.nom_usuario ",
                "     AND solicitacao_fatura = nf_solicit.num_solicit) AND ",
 ...
 ELSE LET p_ies_cons = TRUE
      SELECT UNIQUE nf_solicit.dat_refer, nf_solicit.cod_via_transporte, nf_solicit.cod_transpor, nf_solicit.num_placa, nf_solicit.cod_entrega
...
#-----------------------------------------#
 ...
 SELECT MAX(parametro_texto[1,2])
   INTO p_uf_placa
  ...
  WHERE empresa = p_cod_empresa 
    ...
    AND usuario = p_user
    AND campo = 'uf_placa'  
    AND parametro_texto IS NOT NULL 
    AND parametro_texto <> ' '   
...
#-----------------------------------------#
  ...
  CALL log006_exibe_teclas("01 02 07", p_vers