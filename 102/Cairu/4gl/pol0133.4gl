...
# PROGRAMA: POL0133                                               #
# MODULOS.: POL0133 - LOG0010 - LOG0030 - LOG0040 - LOG0050       #
...
GLOBALS
  ...
  DEFINE p_valor_item_ipi        decimal(17,2),
         p_valor_item_icm        decimal(17,2),
         p_valor_item_pis        decimal(17,2),
         p_valor_item_comis      decimal(17,2),
       	 p_ind3                  INTEGER,
       	 p_padesc                INTEGER,
  ...
  DEFINE p_num_pedido            LIKE pedidos.num_pedido,
   ...
   DEFINE p_cod_nat_oper       LIKE fat_nf_mestre.natureza_operacao,
          p_cod_cnd_pgto       LIKE fat_nf_mestre.cond_pagto,
          p_cod_cidade         LIKE clientes.cod_cidade,
          m_cod_cidade         LIKE clientes.cod_cidade,
  ...
  DEFINE p_ctr_meta               RECORD LIKE ctr_meta.*,
         ...
         p_pct_icm_contrib        decimal(5,2),
         p_pct_icm_ncontrib       decimal(5,2),
         p_pct_icm_consumo        decimal(5,2),
...
MAIN
  ...
  IF   p_status = 0
  THEN CALL pol0133_controle()
  END IF
END MAIN

#--------------------------#
 FUNCTION pol0133_controle()
#--------------------------#

  CALL log006_exibe_teclas("01 02", p_versao)
  CALL log130_procura_caminho("pol0133") RETURNING p_nom_tela
  OPEN WINDOW w_pol0133 AT 2,02 WITH FORM p_nom_tela    
       ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
       
  display p_cod_empresa to cod_empres
  
  LET p_status = FALSE
  
  CALL  pol0133_cria_t_mestre()
  
  MENU "OPCAO"
    COMMAND "Incluir" "Inclui Pedido "
      MESSAGE ""
      CALL pol0133_inclusao_pedido()
    ...
    COMMAND "Fim"        "Retorna ao Menu Anterior"
      ...
      EXIT MENU
  END MENU
  CLOSE WINDOW w_pol0133 
END FUNCTION

#----------------------------------------------------------------------#
 FUNCTION  pol0133_insert_t_mestre(p_num_pedido,      m_cod_nat_oper,
...
#----------------------------------------------------------------------#
 ...
 INSERT INTO t_mestre VALUES (p_num_pedido,    m_cod_nat_oper,    p_cod_cnd_pgto,
                              p_pct_desc_adic, p_pct_desc_financ, p_cod_moeda );
 END FUNCTION

#-------------------------------#
 FUNCTION pol0133_cria_t_mestre()
#-------------------------------#
...
#---------------------------------#
 FUNCTION pol0133_inclusao_pedido()
#---------------------------------#
 ...
 SELECT par_ies
   INTO p_consiste_ped_cli
   FROM par_vdp_pad
  WHERE cod_empresa = p_cod_empresa
    AND cod_parametro = "consiste_pedido_cli"
 IF p_consiste_ped_cli IS NULL OR
    p_consiste_ped_cli = " "   THEN 
    LET p_consiste_ped_cli = "N"
 END IF 
 ...
 WHILE p_flag = 1
    CASE
      WHEN p_tela = 1
           CALL pol0133_entrada_dados_mestr("INCLUSAO")   RETURNING p_status
      WHEN p_tela = 2
           CALL pol0133_entrada_dados_ent_obs("INCLUSAO") RETURNING p_status
      WHEN p_tela = 3
           CALL pol0133_entrada_dados_item("INCLUSAO")    RETURNING p_status
      WHEN p_tela = 4
           CALL pol0133_total("INCLUSAO")                 RETURNING p_status
    END CASE
 END WHILE
  
 IF   p_status = 0 THEN 
      CALL pol0133_efetiva_inclusao()
      CURRENT WINDOW IS w_pol0133
      IF   p_erro = FALSE THEN 
           let p_msg = 'Pedido OK. Inclusão efetuada em carteira!'
      ELSE 
           let p_msg = 'Pedido consistido. Inclusão efetuada em bath!'
      END IF
      #CURRENT WINDOW IS w_pol0133
      #CALL pol0133_exibe_dados()
      #CALL log006_exibe_teclas("01 02", p_versao)
      PROMPT p_msg FOR p_ok
 ELSE 
...
#---------------------------------------------#
 FUNCTION pol0133_entrada_dados_mestr(p_funcao)
#---------------------------------------------#
 ...
 LET p_pedido_dig_mest.ies_finalidade   = 1
 ...
 INPUT BY NAME p_pedido_dig_mest.cod_empresa,
       ...
       before input
          if p_pedido_dig_mest.cod_cliente is not null then
             call pol0133_verifica_cliente() RETURNING p_status
          end if
       ...
       AFTER  FIELD dat_prazo_entrega
              ...
              THEN ERROR " Data de entrega nao pode ser Nula "
                   NEXT FIELD dat_prazo_entrega
              END IF
              IF   pol0133_confirma_prazo_entrega() = FALSE
       ...
       AFTER  FIELD cod_cliente
              ...
              IF pol0133_verifica_cliente() = TRUE
                 THEN IF pol0133_busca_repres() = FALSE
              ...
              let p_pedido_dig_mest.pct_desc_adic = pol0133_calc_dif_icms()
              DISPLAY p_pedido_dig_mest.pct_desc_adic TO pct_desc_adic
              
              IF   pol0133_verifica_credito_cliente() = FALSE
              ...
              IF p_pedido_dig_mest.cod_cliente IS NOT NULL THEN
                 ...
                 IF sqlca.sqlcode = 0 THEN
                    ...
                    SELECT nom_reduzido into p_nom_trans
                      FROM clientes 
       ...
       AFTER  FIELD cod_repres
              IF pol0133_verifica_repres(p_pedido_dig_mest.cod_repres) = FALSE  THEN 
                 ERROR "Representante nao cadastrado "
                 NEXT FIELD cod_repres
              ELSE
# O.S.50780-Ju   IF pol0133_verifica_repres_canal() = FALSE THEN
...
#                 END IF
       ...
       AFTER  FIELD cod_repres_adic
              IF   pol0133_verifica_repres(p_pedido_dig_mest.cod_repres_adic) = FALSE
...
#             END IF
              IF   p_pedido_dig_mest.num_list_preco IS NULL OR
                   p_pedido_dig_mest.num_list_preco = 0
              THEN
              ELSE IF   pol0133_verifica_lista_preco() 
                   THEN ERROR " Lista de preco nao cadastrada. "
                        NEXT FIELD num_list_preco
       ...
       AFTER  FIELD pct_desc_adic
              ...
              THEN ERROR "Pct. desconto adicional maior que limite "
                   LET p_erro = TRUE
              END IF
              IF   p_pedido_dig_mest.pct_desc_adic > 0 then
                   LET p_desc_mest = 100 - p_pedido_dig_mest.pct_desc_adic
              ...
              LET p_desc_tela1  =  p_desc_unico_mest 
       ...
       AFTER  FIELD cod_cnd_pgto
	             IF   pol0133_verifica_cnd_pagamento()
              THEN NEXT FIELD cod_cnd_pgto
              END IF
              CALL pol0133_apaga_zoom()

       AFTER FIELD num_pedido_cli
             IF p_pedido_dig_mest.num_pedido_cli is not null THEN
                IF p_consiste_ped_cli = "S" THEN 
                   IF pol0133_verifica_pedido_cli() THEN 
                      ERROR p_msg
       ...
       AFTER FIELD num_pedido_repres
             IF   p_par_vdp.num_prx_pedido <> 0  
             THEN IF pol0133_verifica_pedido_repres()
       ...
       AFTER  FIELD cod_transpor
              IF   p_pedido_dig_mest.cod_transpor IS NOT NULL AND
                   p_pedido_dig_mest.cod_transpor <> " " THEN
                   IF pol0133_verifica_transportadora(p_pedido_dig_mest.cod_transpor) = FALSE THEN
                   ...
                   SELECT nom_reduzido
                    ...
                    WHERE cod_cliente = p_pedido_dig_mest.cod_transpor
                   IF STATUS = 0 THEN 
                      DISPLAY p_den_trans to den_transp
                   END IF
              END IF
              CALL pol0133_apaga_zoom()
       ...
       AFTER  FIELD cod_consig  
              IF   p_pedido_dig_mest.cod_consig IS NOT NULL AND
                   p_pedido_dig_mest.cod_consig <> " "
              THEN IF   pol0133_verifica_transportadora(p_pedido_dig_mest.cod_consig) = FALSE
       ...
       AFTER FIELD ies_finalidade 
              IF pol0133_verifica_finalidade() = FALSE
       ...
       AFTER  FIELD cod_moeda
              IF   pol0133_verifica_moeda() = FALSE
       ...
       AFTER  FIELD cod_tip_carteira
              IF   pol0133_verifica_carteira() = FALSE
       ...
       AFTER  FIELD ies_incl_txt
              ...
              THEN IF   ies_incl_txt = "S"  THEN
###aquiooooooo          IF   vdp243_digita_texto(p_pedido_dig_mest.num_pedido, "0") = FALSE
####                    THEN LET ies_incl_txt = "N"
####  ate aquiooooo     END IF
                        CALL log006_exibe_teclas("01 02 07", p_versao)
                        CURRENT WINDOW IS w_pol0133
                   END IF
              END IF
...
#------------------------------#
FUNCTION pol0133_calc_dif_icms()
#------------------------------#

   define p_icm_trunc decimal(3,1),
          p_icm_calc  decimal(17,7)
          
   if p_cod_uni_feder = 'PR' OR
      p_cod_uni_feder = 'RS' OR
      p_cod_uni_feder = 'RJ' OR
      p_cod_uni_feder = 'MG' OR
      p_cod_uni_feder = 'SC' THEN
      LET p_icms_cli = 88/100
      LET p_pct_icm = 12
   else
      LET p_icms_cli = 93/100
      LET p_pct_icm = 7
   end if

   let p_icm_trunc = 0
   
   if p_cod_uni_feder = 'SP' then
      RETURN(p_icm_trunc)
   end if

   let p_icms_emp = 82/100
   
   let p_icm_calc = 100 - ((p_icms_emp / p_icms_cli) * 100)
   let p_icm_trunc = p_icm_calc 
  
   if p_icm_trunc > p_icm_calc then
      let p_icm_trunc = p_icm_trunc - (1/10)
   end if
   
   # Alterado por Manuel em 10-01-2013 a pedido do Montanini para deixar o desconto condicional = 4%
...
#-------------------------------#
 FUNCTION pol0133_mostra_zoom()
#-------------------------------#
 DISPLAY "( Zoom )" AT 3,68
 END FUNCTION

#------------------------------#
 FUNCTION pol0133_apaga_zoom()
#------------------------------#
 DISPLAY "--------" AT 3,68
 END FUNCTION

#------------------------------------------#
 FUNCTION pol0133_confirma_prazo_entrega()
#------------------------------------------#
...
#-------------------------------------------------#
 FUNCTION pol0133_entrada_dados_ent_obs(p_funcao)
#-------------------------------------------------#
 DEFINE p_funcao                CHAR(12)
  
 CALL log006_exibe_teclas("01 02 07", p_versao)
 CALL log130_procura_caminho("pol01331") RETURNING p_nom_tela
 OPEN WINDOW w_pol01331 AT 2,02 WITH FORM p_nom_tela     
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 CURRENT WINDOW IS w_pol01331
 #DISPLAY FORM f_pol01331 
 ...
 INPUT p_num_sequencia,
  ...
  FROM num_sequencia,
    ...
    AFTER  FIELD num_sequencia
	          ...
	          THEN 
	          ELSE IF   pol0133_verifica_endeco_entrega()
...
#--------------------------------------#
 FUNCTION pol0133_verifica_lista_preco()
#--------------------------------------#
  SELECT *
...
#---------------------------------------------------------#
 FUNCTION pol0133_verifica_transportadora(p_cod_transpor)
#---------------------------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0133_verifica_endeco_entrega()
#-----------------------------------------#
...
#----------------------------------------------#
 FUNCTION pol0133_entrada_dados_item(p_funcao)
#----------------------------------------------#
 DEFINE p_funcao          CHAR(12),  
 ...
 CALL log130_procura_caminho("pol01332") RETURNING p_nom_tela
 OPEN WINDOW w_pol01332 AT 2,02 WITH FORM p_nom_tela    
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 CURRENT WINDOW IS w_pol01332
 #DISPLAY FORM f_pol01332 
 ...
 INPUT ARRAY t_pedido_dig_item WITHOUT DEFAULTS FROM s_pedido_dig_item.*
       ...
       BEFORE FIELD cod_item
              CALL pol0133_mostra_zoom()
              CALL pol0133_mostra_estoque(t_pedido_dig_item[pa_curr].cod_item)
              DISPLAY "                "  AT 5,20
              DISPLAY "Desconto pedido:"  at 5,3
              DISPLAY p_desc_tela1     at 5,20   
              DISPLAY "Desconto item  :"  at 5,30
              DISPLAY p_desc_unico_mest at 5,47    
       ...
       AFTER  FIELD cod_item
              ...
              IF   t_pedido_dig_item[pa_curr].cod_item IS  NULL then    
                   EXIT INPUT
              ELSE
                   CALL pol0133_verifica_item() 
              ...
              IF p_pedido_dig_mest.ies_comissao = "S" AND p_pedido_dig_mest.pct_comissao = 0  then 
                 CALL vdp088_consiste_perc_comissao(p_cod_empresa,
                   ...
                   IF  p_achou = FALSE then
                       ERROR " Percentual de comissao nao encontrado."
                       NEXT FIELD cod_item
                   END IF
       ...
       AFTER  FIELD  qtd_pecas_solic
              ...
              THEN IF   Pol0133_Entrada_Ped_Itens_Rem() = FALSE
    ...
    END IF
       ...
       AFTER  FIELD pct_desc_adic 
              ...
              IF   t_pedido_dig_item[pa_curr].pct_desc_adic IS NULL then  
                   ERROR " Percentual de desconto invalido "
                   NEXT FIELD pct_desc_adic 
              ELSE
                   LET p_desc_adic_m_i = 100 - p_pedido_dig_mest.pct_desc_adic
              ...
              DISPLAY p_desc_tot_geral  AT 7,72

              CALL pol0133_verifica_valor_pedido() 
{
    ...
    LET p_pre_unit_liq      = p_pre_unit_liq -
     ...
     IF p_ies_emite_dupl_nat = "S" AND
        p_par_vdp.par_vdp_txt[39,39] = "S" then
          call pol0133_le_icm_ipi()
...
################### PREVISAO - PRODUCAO - INICIO  ###################
    IF p_par_vdp.par_vdp_txt[14,14] = "S" THEN
       IF pol0133_verifica_prevprod()= FALSE
          THEN NEXT FIELD qtd_pecas_solic
       END IF
    END IF
...
###aquioooo      IF vdp243_digita_texto(p_pedido_dig_item.num_pedido,
###                 pa_curr) = FALSE 
###              THEN 
###                 LET t_pedido_dig_item[pa_curr].ies_incl_txt = "N"
###                 SLEEP 2
###ateaquioooo   END IF
       ...
       ON KEY (control-w, f1)
              CALL pol0133_help_itens()
    
       ON KEY (control-z)
              CALL pol0133_popup(3)
...
#-------------------------------------------------------#
 FUNCTION pol0133_calcula_pre_unit(p_pre_unit,p_pct_desc)
#-------------------------------------------------------#
...
#-----------------------------------#
 FUNCTION pol0133_verifica_prevprod()
#-----------------------------------#
 ...
 INITIALIZE p_prev_producao.* TO NULL
{
 ...
 ELSE IF (p_prev_producao.qtd_pedido + p_qtd_saldo) >
      ...
      LET p_prev_producao.qtd_pedido = p_prev_producao.qtd_pedido +
                                       p_qtd_saldo
      CALL pol0133_atualiza_prevprod()
      LET p_audit_logix.cod_empresa = p_cod_empresa
...
#-----------------------------------------------#
 FUNCTION pol0133_verifica_repres(p_cod_repres)
#-----------------------------------------------#
...
#--------------------------------------#
FUNCTION pol0133_verifica_repres_canal()
#--------------------------------------#
...
#----------------------------------#
 FUNCTION pol0133_atualiza_prevprod()
#----------------------------------#
...
#----------------------------------#
 FUNCTION pol0133_insere_prevprod()
#----------------------------------#
...
#----------------------------------#
 FUNCTION pol0133_busca_num_pedido()
#----------------------------------#
...
#-----------------------------------#
 FUNCTION pol0133_verifica_pedido()
#-----------------------------------#
 SELECT * FROM pedidos
...
#-------------------------------------------#
 FUNCTION pol0133_verifica_natureza_operacao()
#-------------------------------------------#
...
#------------------------------------#
 FUNCTION pol0133_verifica_cliente()
#------------------------------------#
 ...
 SELECT clientes.nom_cliente,  clientes.end_cliente,
        ...
        num_cgc_cpf, ins_estadual,cod_consig
 ...
 DISPLAY p_cod_consig    TO cod_consig   
...
#--------------------------------------#
 FUNCTION pol0133_verifica_fiscal_par()
#--------------------------------------#

 define p_estado char(02),
        p_icms   dec(5,2)
...
#-------------------------------------#
 FUNCTION pol0133_consiste_nat_oper()
#-------------------------------------#
    CASE
...
#------------------------------------------#
 FUNCTION pol0133_verifica_cnd_pagamento()
#------------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0133_verifica_pedido_cli()
#-------------------------------------#
   define p_pedido integer
   
   SELECT pedidos.num_pedido INTO p_pedido
     FROM pedidos
    WHERE pedidos.num_pedido_cli = p_pedido_dig_mest.num_pedido_cli
      AND pedidos.cod_cliente    = p_pedido_dig_mest.cod_cliente
      AND pedidos.cod_empresa       = p_cod_empresa
      AND pedidos.ies_sit_pedido   <> "9"
  IF sqlca.sqlcode = NOTFOUND THEN 
  ELSE
     LET p_msg = 'PEDIDO DO CLIENTE JÁ POSSUI PEDIDO EM CARTEIRA ', p_pedido
     RETURN TRUE
  END IF
  SELECT pedido_dig_mest.num_pedido INTO p_pedido
     FROM pedido_dig_mest 
    WHERE pedido_dig_mest.num_pedido_cli = p_pedido_dig_mest.num_pedido_cli
      AND pedido_dig_mest.cod_cliente    = p_pedido_dig_mest.cod_cliente
      AND pedido_dig_mest.cod_empresa    = p_cod_empresa
  IF   sqlca.sqlcode = NOTFOUND  THEN
  ELSE 
     LET p_msg = 'PEDIDO DO CLIENTE JA JÁ POSSUI PEDIDO EM BATCH ', p_pedido
     RETURN TRUE
  END IF
  SELECT pedidos_hist.num_pedido INTO p_pedido
    FROM pedidos_hist
   WHERE pedidos_hist.num_pedido_cli  = p_pedido_dig_mest.num_pedido_cli
     AND pedidos_hist.cod_cliente     = p_pedido_dig_mest.cod_repres
     AND pedidos_hist.cod_empresa     = p_cod_empresa
     AND pedidos_hist.ies_sit_pedido   <> "9"
  IF sqlca.sqlcode = NOTFOUND   THEN 
     RETURN FALSE
  ELSE 
     LET p_msg = 'PEDIDO DO CLIENTE JA JÁ POSSUI PEDIDO EM HISTORICO ', p_pedido
     RETURN TRUE
  END IF
END FUNCTION

#---------------------------------------#
 FUNCTION pol0133_verifica_pedido_repres()
#---------------------------------------#
...
#------------------------------------#
 FUNCTION pol0133_verifica_tipo_venda()
#------------------------------------#
...
#----------------------------------#
 FUNCTION pol0133_verifica_moeda()
#----------------------------------#
...
#--------------------------------#
 FUNCTION pol0133_verifica_cidade()
#--------------------------------#
...
#---------------------------------#
 FUNCTION pol0133_verifica_item()
#---------------------------------#
 DEFINE      p_ies_situacao    LIKE item.ies_situacao,
  ...
  let p_cod_nat_oper = p_pedido_dig_mest.cod_nat_oper
  
  IF pol0133_existe_nat_oper_refer() THEN
     let p_cod_nat_oper = m_cod_nat_oper_ref
  end if
  
  let p_cod_cliente = p_pedido_dig_mest.cod_cliente
  LET p_cod_tip_carteira = p_pedido_dig_mest.cod_tip_carteira
  LET p_cod_item = t_pedido_dig_item[pa_curr].cod_item
  ...
  #let p_status = pol0133_le_param_fisc(t_pedido_dig_item[pa_curr].cod_item) 

  LET p_dat_atu = TODAY
  CALL pol0133_cria_ctrl_acesso() RETURNING p_status
 
  let p_status = vdpr99_consiste_fiscal('', 
                       p_cod_empresa, 
                       p_dat_atu, 
                       p_cod_nat_oper, 
                       p_cod_cliente, 
  ...
  if not p_status then 
     CALL log0030_mensagem('Parâmetros fiscais não encontrados!','excla')
     RETURN p_status, p_qtd_padr_embal
  END IF
  ...
  IF   p_pedido_dig_mest.num_list_preco = 0 OR
       ...
       p_pedido_dig_mest.num_list_preco = "    "
  THEN
  ELSE CALL pol0133_busca_preco_lista(p_cod_item, p_cod_lin_prod,
       ...
       THEN IF p_pre_unit_tab > 0
            ...
            ELSE CALL pol0133_calcula_pre_unit(p_pre_unit_bruto,
                                              p_desc_bruto_tab)
...
#-----------------------------------------#
 FUNCTION pol0133_verifica_valor_pedido()
#-----------------------------------------#
  ...
  IF p_pedido_dig_mest.cod_moeda > 0 THEN 
     ...
     IF sqlca.sqlcode = 0 THEN
     ELSE 
        OPEN WINDOW w_pol01337 AT 10,10 WITH 5 ROWS, 30 COLUMNS
...
#    p_pedido_dig_mest.ies_aceite_finan = "N" THEN 
     FOR p_dep = 1 to pa_curr
         ...
         CALL pol0133_calcula_pre_unit(p_pre_unit_liq,
                                      p_pct_desc_m)
         ...
         IF (p_valor_pedido + p_val_dup_aberto + p_val_ped_carteira) > 
             ...
             OPEN WINDOW w_pol01338 AT 10,30 WITH 6 ROWS, 40 COLUMNS
                  ATTRIBUTE(BORDER, PROMPT LINE LAST)
...
#---------------------------------#
 FUNCTION pol0133_efetiva_inclusao()
#---------------------------------#
 CALL pol0133_inclui_mestre()
 CALL pol0133_insert_ped_list()
 IF   p_pedido_dig_ent.end_entrega IS NOT NULL
 THEN CALL pol0133_inclui_end_entr()
 END IF
 IF   p_pedido_dig_obs.tex_observ_1 IS NOT NULL
 THEN CALL pol0133_inclui_observ()
 END IF
 CALL pol0133_inclui_itens()
 CALL vdp106_inclui_texto()  
#CALL pol0133_inclui_itens_bnf()
 CALL pol0133_inclui_ped_itens_desc()
 END FUNCTION

#--------------------------------#
 FUNCTION pol0133_popup(p_status)
#--------------------------------#
DEFINE  p_cod_transpor    LIKE transport.cod_transpor,
 ...
 CASE
    ...
    WHEN infield(pct_desc_adic) AND p_status = 1
         CALL pol0133_controle_peditdesc(p_cod_empresa,
                                          p_pedido_dig_mest.num_pedido,
                                          0 )
         CALL log006_exibe_teclas("01 02 03 07", p_versao)
         CURRENT WINDOW IS w_pol0133
...
#------------------------------#
 FUNCTION pol0133_inclui_mestre()
#------------------------------#
 ...
 THEN IF   p_consist_cred = TRUE 
      ...
      THEN IF   p_par_vdp.par_vdp_txt[22,22] = "S"
           THEN CALL pol0133_insert_t_mestre(p_pedidos.num_pedido,
...
#--------------------------------#
 FUNCTION pol0133_insert_ped_list()
#--------------------------------#
 IF   p_erro = FALSE
...
#--------------------------------#
 FUNCTION pol0133_inclui_end_entr()
#---------------------------------#
...
#------------------------------#
 FUNCTION pol0133_inclui_observ()
#------------------------------#
...
#
         ...
         THEN CALL vdp596_pesquisa_ctr_meta(p_cod_empresa,
              ...
              THEN IF   p_pedido_dig_mest.ies_aceite_comer = "S" OR
                   ...
                   THEN
                   ELSE IF   pol0133_verifica_meta_venda()
                        THEN 
                        END IF
                   END IF
              END IF
...
#END FOR
       ...
       END IF
       IF   p_ies_tip_controle = "2"
       THEN CALL pol0133_insert_ped_itens_rem()
       END IF 
...
#-----------------------------------#
#FUNCTION pol0133_inclui_itens_bnf()
#-----------------------------------#
#FOR pa_curr = 1 TO  99  
...
#--------------------------------#
 FUNCTION pol0133_incl_ped_of_pcp()
#--------------------------------#
...
#---------------------------------#
 FUNCTION pol0133_total(p_funcao)
#---------------------------------#
 DEFINE p_funcao                CHAR(12),
        p_pct_desc_m            LIKE ped_itens.pct_desc_adic,
        p_pct_desc_i            LIKE ped_itens.pct_desc_adic,
 ...
 CALL log006_exibe_teclas("01 02 07", p_versao)
 CALL log130_procura_caminho("pol01333") RETURNING p_nom_tela
 OPEN WINDOW w_pol01333 AT 2,02 WITH FORM p_nom_tela    
      ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
 CURRENT WINDOW IS w_pol01333
 #DISPLAY FORM f_pol01333 
 DISPLAY p_cod_empresa TO cod_empresa
 DISPLAY p_pedido_dig_mest.num_pedido  TO num_pedido
 ...
 FOR p_count = 1 TO 500
    ...
    THEN LET p_totalc.quantidade = p_totalc.quantidade +
         ...
         CALL vdp784_busca_desc_adic_unico(p_cod_empresa,
                                           ...
                                           p_pedido_dig_mest.pct_desc_adic) 
             RETURNING p_pct_desc_m
         CALL pol0133_calcula_pre_unit(t_pedido_dig_item[p_count].pre_unit,
                                      p_pct_desc_m)
             RETURNING p_pre_unit_liq
         CALL vdp784_busca_desc_adic_unico(p_cod_empresa,
                                           p_pedido_dig_mest.num_pedido,
...
#---------------------------------------------------------------#
 FUNCTION pol0133_busca_preco_lista(p_cod_item, p_cod_lin_prod,
                                  p_cod_lin_recei, p_cod_seg_merc,
                                  p_cod_cla_uso)
#---------------------------------------------------------------#
 DEFINE      p_cod_item        LIKE item_vdp.cod_item,
...
#----------------------------------------------#
 FUNCTION pol0133_verifica_finalidade_tipo_cli()
#----------------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0133_verifica_finalidade()
#-------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0133_verifica_credito_cliente()
#-----------------------------------------#
...
SELECT qtd_dias_atr_dupl,
 ...
 THEN 
      CURRENT WINDOW IS w_pol01331
      OPEN WINDOW w_pol01331 AT 10,10 WITH 5 ROWS, 30 COLUMNS
           ATTRIBUTE(BORDER, PROMPT LINE LAST)
...
#-------------------------------------#
 FUNCTION pol0133_verifica_meta_venda()
#-------------------------------------#
...
#--------------------------------------------#
#FUNCTION pol0133_mostra_estoque(p_cod_item)
#--------------------------------------------#
...
#-----------------------------------------------#
 FUNCTION pol0133_mostra_estoque(p_cod_item_est)
#-----------------------------------------------#
...
#------------------------------------------------------------------#
FUNCTION pol0133_controle_peditdesc(p_cod_emp, p_num_ped, p_num_seq)
#------------------------------------------------------------------#
   ...
   CALL log130_procura_caminho("pol01334") RETURNING p_nom_tela
   OPEN WINDOW w_pol01334 AT 2,2  WITH FORM p_nom_tela
        ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
   CURRENT WINDOW IS w_pol01334
   CALL log006_exibe_teclas("01 02 07", p_versao)
   CURRENT WINDOW IS w_pol01334
   ...
   CLOSE WINDOW w_pol01334
   LET int_flag = 0
END FUNCTION

#---------------------------------------#
 FUNCTION pol0133_inclui_ped_itens_desc()
#---------------------------------------#
...
#------------------------------------------------------#
 FUNCTION pol0133_busca_desc_adic_unico(p_num_sequencia,
				       p_desc)
#------------------------------------------------------#
...
#-------------------------------------#
 FUNCTION pol0133_verifica_carteira()
#-------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0133_existe_nat_oper_refer()
#----------------------------------------#
...
#------------------------------------#
 FUNCTION pol0133_existe_fiscal_par()
#------------------------------------#
...
#---------------------------------------#
 FUNCTION pol0133_insert_ped_itens_rem()
#---------------------------------------#
...
#--------------------------------------#
FUNCTION pol0133_entrada_ped_itens_rem()
#--------------------------------------#
  ...
  CALL log006_exibe_teclas("01 02 07", p_versao)
  CALL log130_procura_caminho("pol01335") RETURNING p_comando
  OPEN WINDOW w_pol01335 AT 2,2 WITH FORM p_comando
       ATTRIBUTE(BORDER, MESSAGE LINE LAST, PROMPT LINE LAST)
  CURRENT WINDOW IS w_pol01335
  ...
  CALL pol0133_verifica_motivo_remessa()      RETURNING p_status
  CALL pol0133_verifica_area_negocio()        RETURNING p_status
  CALL pol0133_verifica_linha_negocio()       RETURNING p_status
  ...
  INPUT BY NAME p_ped_itens_rem.* WITHOUT DEFAULTS
    ...
    AFTER  FIELD cod_motivo_remessa
           IF   pol0133_verifica_motivo_remessa() = FALSE
    ...
    AFTER  FIELD cod_area_negocio
           ...
           END IF
           IF   pol0133_verifica_area_negocio() = FALSE
    ...
    AFTER  FIELD cod_lin_negocio
           ...
           END IF
           IF   pol0133_verifica_linha_negocio() = FALSE
           THEN ERROR " Linha de Negocio nao cadastrada "
                NEXT FIELD cod_lin_negocio   
           END IF
           IF   pol0133_verifica_area_lin_negocio() = FALSE
...
####aquioooo    CALL con088_verifica_cod_conta(p_cod_empresa,
...
###ateaquiooo   END IF       
    ...
    AFTER  FIELD num_pedido_compra  
           ...
           THEN IF   pol0133_verifica_pedido_compra() = FALSE
                THEN NEXT FIELD num_pedido_compra
                END IF
           END IF 
    
    ON KEY (control-w, f1)
           CALL pol0133_help_rem()
    
    ON KEY (control-z)
           CALL pol0133_popup_rem()
  END INPUT
  
  CLOSE WINDOW w_pol01335
...
#----------------------------#
 FUNCTION pol0133_popup_rem()
#----------------------------#
  DEFINE p_mot_rem      LIKE ped_itens_rem.cod_motivo_remessa
  
  CASE
    WHEN infield(cod_motivo_remessa)
###aquiooLET p_mot_rem = sup260_popup_motivo_remessa(p_cod_empresa)
         CALL log006_exibe_teclas("01 02 07", p_versao)
         CURRENT WINDOW IS w_pol01335
...
#------------------------------------------#
 FUNCTION pol0133_verifica_motivo_remessa()
#------------------------------------------#
...
#----------------------------------------#
 FUNCTION pol0133_verifica_area_negocio()
#----------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0133_verifica_linha_negocio()
#-----------------------------------------#
...
#--------------------------------------------#
 FUNCTION pol0133_verifica_area_lin_negocio()
#--------------------------------------------#
...
#-----------------------------------------#
 FUNCTION pol0133_verifica_pedido_compra()
#-----------------------------------------#
...
#------------------------------------#
FUNCTION pol0133_le_param_fisc(p_item)
#------------------------------------#
   ...
   LET p_ies_tipo = 'S' 
   LET p_ies_finalidade = p_pedido_dig_mest.ies_finalidade
   ...
   IF p_count = 0 THEN
      LET p_msg = 'Não há tributos parametrizados para:\n',
                  'Empresa:  ', p_cod_empresa, '/n',
                  'Operação: ', p_cod_nat_oper,'/n'
      CALL log0030_mensagem(p_msg,'excla')
      RETURN FALSE
   END IF
   ...
   if not pol0133_cria_temporaria() then
      RETURN false
   End if
   ...
   FOREACH  cq_tributos INTO
      ...
      DECLARE cq_acesso CURSOR FOR
       SELECT sequencia_acesso, num_ctr_acesso
         FROM obf_ctr_acesso
        WHERE empresa         = p_cod_empresa
      ...
      FOREACH cq_acesso INTO p_seq_acesso
         ...
         CALL pol0133_pega_chave()

         IF NOT pol0133_checa_tributo() THEN
            RETURN FALSE
         END IF
...
#-------------------------------#
FUNCTION pol0133_checa_tributo()
#-------------------------------#
   ...
   LET p_matriz = 'SSSSSSSSSSSSSSSSSSSSSSS'
   ...
   LET p_query = 
       ...
       " AND (('",p_dat_atu,"' BETWEEN VALID_CONFIG_INI AND VALID_CONFIG_FINAL) OR (VALID_CONFIG_INI IS NULL AND VALID_CONFIG_FINAL IS NULL)) "
   ...
   IF p_cheve_ok THEN
      ...
      DECLARE cq_obf_cfg CURSOR FOR var_query2
...
#-------------------------------#
FUNCTION pol0133_le_obf_regiao()
#-------------------------------#
...
#-------------------------------#
FUNCTION pol0133_le_obf_cl_fisc()
#-------------------------------#
...
#---------------------------------#
FUNCTION pol0133_le_obf_fisc_item()
#---------------------------------#
...
#---------------------------------#
FUNCTION pol0133_le_obf_fisc_cli()
#---------------------------------#
...
#----------------------------------#
 FUNCTION pol0133_cria_ctrl_acesso()
#----------------------------------#

 DROP TABLE T_CTRL_ACESSO

 CREATE TEMP TABLE T_CTRL_ACESSO (
    ...
    DATA_VALIDADE DATE, 
    TRANS_CONFIG INTEGER NOT NULL)  WITH NO LOG  
...
#----------------------------------#
 FUNCTION pol0133_cria_temporaria()
#----------------------------------#
...
#----------------------------#
   ...
   if status = 0 then
      call le_obf_config()
      IF p_cod_incide = 1 then
         let p_pct_ipi = p_aliquota
      End if
   End if
...
#-------------------------------#
FUNCTION pol0133_le_obf_config()
#-------------------------------#
