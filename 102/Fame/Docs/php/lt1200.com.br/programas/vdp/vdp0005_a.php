...
if($tipo=='K')
 ...
 while (is_array($mat))
 {
  ...
  if($rep_ant<>$rep_atu)
  {
   ...
   $titulo=$situacao."->  Comissôes  ".$mes_ref." / ".$ano_ref." Rep.:".round($rep_atu)."-".$raz_atu;
   $pdf->AddPage();
  } 
  $pdf->ln();
 ...
 */
  ...
  {
   $busca_cota=" select cota from lt1200_cota_mes
                            where  mes_ref='".$mes_ref."'
                              and ano_ref='".$ano_ref."'
                              and cod_repres='".$rep_ant."' ";

   $res_cota = $cconnect("lt1200",$ifx_user,$ifx_senha);
   $result_cota = $cquery($busca_cota,$res_cota);
   $mat_cota=$cfetch_row($result_cota);
   $cota=$mat_cota["cota"];
  }else{
   $cota=$mat["cota"];
  }
 ...
 //Verifico se é do tipo telemarketing
  if(trim($mat["tipo"])=="K")
  	{
  	$tlmk="S";
  	}
  ...
  if($tipo_atu<>$tipo_ant)
  {
   if($tipo_ant=="G")
   {
    $desc_tipo="Total Lançamentos de notas e Titulos a Creditar";
   }elseif($tipo_ant=="A"){
    $desc_tipo="Total de Outros Lançamentos";
   }   
   $pdf->ln();
  ...
  if($rep_ant<>$rep_atu)
  {
   ...
   if($tipo=="F" or  $tipo=="S")
   {
    ...
    $pdf->cell(35,5,"Percentual alcançado:",'LRTB',0,'C');
    if($cota==0)
    {
     $pct_alcancado=0;
    }else{
    ...
    // Caso seja supervisor do TLMK	
      if($pct_alc >= 70 && $pct_alc < 100)
      	...
      	if($cod_nivel_2_ant == $rep_ant)
      		...
      		{
      		$pct_alcancado=50;	// extinção da função de supervisor
      		}	
      	}
      elseif($pct_alc >= 100)
      	{
      	if($cod_nivel_2_ant == $rep_ant)
      		...
      		{
      		$pct_alcancado=100;	// extinção da função de supervisor
      		}	
      	}
      else
      	{
     ...
     $pdf->cell(35,5,"Cobertura:",'LRTB',0,'C');
     $pdf->cell(25,5,number_format($pct_sal,2,",","."),'LRTB',0,'R');
     $pct_alcancado=$pct_sal;
  ...
  if($tipo=="K")
   {
    ...
    $pct_consulta = 0;
    /*
    // pct máximo de 160% de cobertura
    if($pct_alc > 160)
    	$pct_consulta = 160;
    else 
   ...
   if($tipo=="R" or $tipo=="A" or $tipo=="C")
   {
    ...
    $pdf->ln();
    $pdf->cell(225,5,"Valores Total da Comissão:",'0',0,'R');
    ...
    if($ind=="S")
    {
     $pdf->cell(225,5,"Indenização:",'0',0,'R');
     $val_ind=($sal_alcancado/12);
     $pdf->cell(50,5,number_format($val_ind,2,",","."),'LRTB',0,'R');
     $pdf->ln();
     $pdf->cell(225,5,"Comissão+Indenização:",'0',0,'R');
   ...
   if($gestor!="S" && trim($tlmk) != "S" && $rep_ant != "127" && $rep_ant != "198") // PROVISÓRIO (criar um campo no cadastro de repres para indicar se tem bonus ou não)
   	{
   	if($pct_alc >= 100 && trim($supervisor) == "S")
	   {
	   	//(22/07/2009) - Marcelo Peres - Inclusão da tabela gradual de bônus
	    /*
	     $bonus=0;
	   	 ...
	   	 */
	   	 $pdf->SetFont('Arial','B',8);
	     $pdf->cell(100,5,'Bônus','LRTB',0,'C');
	     //$pdf->cell(50,5,number_format($bonus,2,",","."),'LRTB',0,'R');
	     $pdf->cell(50,5,number_format(($salario*0.1),2,",","."),'LRTB',0,'R');
	     //$val_extra=$val_extra+$bonus;
	     $val_extra=$val_extra+($salario*0.1);
	     $pdf->ln();	
	   }
