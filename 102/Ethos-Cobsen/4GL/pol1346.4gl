 ...
 DEFINE l_qtd_refug, l_qtd_boas, 
   ...
   FOREACH cq_estrut INTO m_num_ordem, m_cod_item, 
      ...
      IF m_ies_situa = '4' THEN   
         {LET l_tot_apon = l_qtd_boas + l_qtd_refug + l_qtd_sucata
         IF l_tot_apon = 0 THEN
            IF NOT pol1346_abre_op(m_num_ordem) THEN
               IF NOT pol1346_guarda_erro() THEN
                  RETURN FALSE
               END IF
               EXIT FOREACH
...
#-----------------------------#
   ...
   IF STATUS <> 0 THEN
      LET m_texto = STATUS
      LET m_msg = 'Erro ', m_texto CLIPPED, ' alterando status da OP para 3 - tab ordens'
      RETURN FALSE
   END IF         
   
   UPDATE necessidades SET ies_situa = '3'
    WHERE cod_empresa = p_cod_empresa
      AND num_ordem = l_op

   IF STATUS <> 0 THEN
      LET m_texto = STATUS
      LET m_msg = 'Erro ', m_texto CLIPPED, ' alterando status da OP para 3 - tab necessidades'
      RETURN FALSE
   END IF         
...
#------------------------------#
   ...
   SELECT COUNT(*) INTO m_count
    ...
    WHERE cod_empresa = p_cod_empresa 
      AND num_ordem = m_num_ordem
      AND cod_item_compon[1,6] <> '10-510'
      
   IF STATUS <> 0 THEN
      LET m_texto = STATUS
      LET m_msg = 'Erro ', m_texto CLIPPED, ' lendo ord_compon.count'
      RETURN FALSE
   END IF
   ...
   FOREACH cq_loc_baixa INTO m_num_ordem, m_cod_item, 
      ...
      FOREACH cq_strut_op INTO m_num_neces, m_cod_compon, m_ies_tip_item
         ...
         IF m_ies_tip_item MATCHES "[FP]" THEN
            ...
            END IF
         ELSE
            IF m_cod_compon[1,6] = '10-510' THEN
               LET m_msg = 'item.cod_item =  ',m_cod_compon
               SELECT cod_local_estoq INTO m_cod_local
