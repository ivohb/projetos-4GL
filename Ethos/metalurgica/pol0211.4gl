...
#------------------------------#
  ...
  DELETE from pedidos_edi_pe4_547
...
{  SELECT cgc_entrega
   ...
   ELSE
      IF (qfptran_1.pedido_cmp[1,4] = 'QAPD' OR qfptran_1.pedido_cmp[1,4] = 'QHPD') THEN
...
#--------------------------------------------#
   ...
   DELETE
     FROM pedidos_edi_pe4_547
    WHERE cod_empresa    = p_cod_empresa
      AND num_pedido     = qfptran_1.pedido
   IF sqlca.sqlcode <> 0 THEN
      CALL log003_err_sql("DELETE","pedidos_edi_pe4_547")
      RETURN TRUE
   END IF 

   INSERT INTO pedidos_edi_pe4_547 VALUES (p_pedidos_edi_pe4.*)
   IF sqlca.sqlcode <> 0 AND  
      sqlca.sqlcode <> -1226 THEN 
      CALL log003_err_sql("INCLUSAO","pedidos_edi_pe4_547")
   END IF
